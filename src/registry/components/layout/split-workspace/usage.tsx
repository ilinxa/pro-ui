export default function SplitWorkspaceUsage() {
  return (
    <div className="max-w-none text-sm leading-relaxed text-foreground">
      <h3 className="mb-2 mt-0 text-base font-semibold">When to use</h3>
      <p className="text-muted-foreground">
        Reach for <code>SplitWorkspace</code> when one fixed layout never fits every
      ...
      <pre className="overflow-x-auto rounded-md border border-border bg-muted p-4 font-mono text-xs">
        <code>{`import { SplitWorkspace, type SplitWorkspaceComponent } from "@/components/workspace"
  ...
  return <Chart width={width} height={height} highlight={isFocused} />
      ...
      <ul className="ml-5 list-disc space-y-1 text-muted-foreground">
        ...
        <li>
          ...
          {" "}<code>flex-1</code>, etc.) — SplitWorkspace fills its container.
        </li>
        <li>
