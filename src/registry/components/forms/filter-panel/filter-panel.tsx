...
import { FilterPanelFooter } from "./parts/filter-panel-footer";
import { cn } from "@/lib/utils";

const SCHEMA_INSTABILITY_THRESHOLD = 5;

export function FilterPanel<T = unknown>(props: FilterPanelProps<T>) {
  const {
    items,
    categories,
    values,
  ...
  useEffect(() => {
    ...
    if (
      ...
      tracker.warned = true;
      console.warn(
        "[filter-panel] `categories` prop is changing every render. " +
    ...
    (categoryId: string) => {
      ...
      if (!cat) {
        if (process.env.NODE_ENV !== "production") {
          console.warn(
            `[filter-panel] clear("${categoryId}") — unknown category id; no-op.`,
          );
        }
        return;
      }
  ...
  useImperativeHandle(
    ref,
    (): FilterPanelHandle => ({ clearAll, clear, isEmpty }),
