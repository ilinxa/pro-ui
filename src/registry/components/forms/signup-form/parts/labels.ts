import type { SignupLabels } from "../types";

/**
 * Default English labels for `<SignupForm>`. Consumers override
 * any subset via the `labels?: Partial<SignupLabels>` prop;
 * `mergeSignupLabels` shallow-merges the override over these
 * defaults at the top of the component.
 *
 * The shape mirrors json-form's `defaultJsonFormStrings` + `mergeJsonFormStrings`
 * pattern.
 */
export const defaultSignupLabels: SignupLabels = {
  // Field labels
  emailLabel: "Email",
 ...
 * the full, populated `SignupLabels` object so downstream parts
 * can read by key without optional-chaining.
 */
export function mergeSignupLabels(
  override?: Partial<SignupLabels>,
): SignupLabels {
  if (!override) return defaultSignupLabels;
  return { ...defaultSignupLabels, ...override };
}
