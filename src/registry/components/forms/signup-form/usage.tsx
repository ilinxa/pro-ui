export default function SignupFormUsage() {
  return (
    <div className="max-w-none text-sm leading-relaxed text-foreground">
      <h3 className="mb-2 mt-0 text-base font-semibold">When to use</h3>
      <p className="text-muted-foreground">
        Drop <code>&lt;SignupForm&gt;</code> when you need a
...
/>`}</code>
      ...
      <pre className="overflow-x-auto mt-2 rounded-md border border-border bg-muted/40 p-3 font-mono text-xs">
        <code>{`const [status, setStatus] = useState<SignupFormStatus>("idle");
...
/>`}</code>
  ...
  await api.signUp(payload.values);
      ...
      <dl className="space-y-2 text-muted-foreground">
        ...
        <dd>
          No. Hand-rolled on RHF v7 + zod v4 directly. Signup is a
