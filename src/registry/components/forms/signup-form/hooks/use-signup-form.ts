...
export interface UseSignupFormArgs {
  ...
  passwordPolicy: SignupFormProps["passwordPolicy"];
  consent: SignupFormProps["consent"];
  labels: SignupLabels;
...
export function useSignupForm(
  ...
  const [internalStatus, setInternalStatus] =
    useState<SignupFormStatus>("idle");
  ...
  const status: SignupFormStatus = isControlled
    ? controlledStatus!
    : internalStatus;
  ...
  const skip = useCallback(async () => {
    ...
    try {
      ...
      const step1Values: SignupStep1Values = {
        email: values.email,
