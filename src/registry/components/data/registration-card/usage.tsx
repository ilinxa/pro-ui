export default function RegistrationCardUsage() {
  return (
    <div className="max-w-none text-sm leading-relaxed text-foreground">
      <h3 className="mb-2 mt-0 text-base font-semibold">When to use</h3>
      <p className="text-muted-foreground">
        Reach for <code>RegistrationCard</code> for any signup / RSVP /
      ...
      <pre className="overflow-x-auto rounded-md border border-border bg-muted p-4 font-mono text-xs">
        <code>{`import { RegistrationCard } from "@/components/registration-card";
...
const stillOpen = events.filter(
  (e) =>
    ...
    deriveRegistrationStatus({
      ...
      <h3 className="mb-2 mt-6 text-base font-semibold">
        Composing with <code>event-card</code>&apos;s state
      </h3>
      <p className="text-muted-foreground">
        registration-card is architecturally{" "}
        <strong>independent</strong> of event timing — the host derives event
        status (using <code>getEventStatus</code> from{" "}
        <code>event-card</code>) and passes <code>closed: true</code>{" "}
        when registration should close. Two common composition patterns:
      </p>
      <pre className="overflow-x-auto rounded-md border border-border bg-muted p-4 font-mono text-xs">
        <code>{`import { getEventStatus } from "@/components/event-card";
...
/>`}</code>
      ...
      <p className="text-muted-foreground">
        ...
        compose with <code>share-bar</code>:
      </p>
      <pre className="overflow-x-auto rounded-md border border-border bg-muted p-4 font-mono text-xs">
        <code>{`<RegistrationCard
  capacity={200}
...
/>`}</code>
      ...
      <ul className="ml-5 list-disc space-y-1 text-muted-foreground">
        <li>
          ...
          <code>event-card</code>&apos;s grid variant which has a decorative
          CTA.)
        </li>
        <li>
