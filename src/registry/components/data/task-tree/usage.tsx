...
export default function TaskTreeUsage() {
  return (
    <div className="max-w-none space-y-6 text-sm leading-relaxed text-foreground">
      <Section title="When to use">
        <p>
          <code>TaskTree</code> is the lightweight sibling to{" "}
          <code>@ilinxa/task-card</code>. Same fixed{" "}
          <code>TaskItem</code> schema, but renders a thin two-line row (bold
        ...
        <p>
          ...
          <code>task-card</code>. For kanban boards, compose{" "}
          <code>@ilinxa/kanban-board</code> with{" "}
          <code>taskCardKanbanRenderer</code>.
        </p>
      </Section>

      <Section title="Quick start">
        <Code>{`import { TaskTree } from "@ilinxa/task-tree";
...
<TaskTree value={items} onChange={({ items }) => setItems(items)} />`}</Code>
      </Section>

      <Section title="With editor: TaskTreeWithEditor">
        <p>
          Pair the tree with a Dialog-mounted{" "}
          <code>TaskCard</code> in one line. Clicking a row opens the
          matching card editable; live-saves propagate back into the tree
          (Q-P1 auto-persistence).
        </p>
        <Code>{`import { TaskTreeWithEditor } from "@ilinxa/task-tree";
...
/>`}</Code>
        <p>
          ...
          editor surface) compose <code>{`<TaskTree>`}</code> + your own
          dialog using the <code>onItemClick</code> callback.
        </p>
      </Section>

      <Section title="Imperative handle (26 methods)">
        <Code>{`const ref = useRef<TaskTreeHandle>(null);
...
ref.current?.clearAllFilters();`}</Code>
      ...
      <Section title="Headless mode: useTaskTreeState">
        <p>
          The same engine that powers <code>{`<TaskTree>`}</code> is also a
        ...
        <Code>{`import { useTaskTreeState, TaskTree } from "@ilinxa/task-tree";
