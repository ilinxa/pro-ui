export default function TaskCardUsage() {
  return (
    <div className="max-w-none space-y-6 text-sm leading-relaxed text-foreground">
      <section>
        <h3 className="mb-2 mt-0 text-base font-semibold">When to use</h3>
        <p className="text-muted-foreground">
          Reach for <code>TaskCard</code> when you need a time-aware task
        ...
        <p className="mt-2 text-muted-foreground">
          ...
          <code>task-tree</code> procomp — shares the same{" "}
          <code>TaskItem</code> schema so drags + clipboard payload work
          cross-procomp. For kanban, use{" "}
          <code>kanban-board</code> + the renderer exported below.
        </p>
      </section>
    ...
    />
      ...
      <section>
        ...
        <p className="text-muted-foreground">
          ...
          <em>not</em> stored on <code>TaskItem</code>, so JSON I/O round-trips
      ...
      <section>
        ...
        <p className="text-muted-foreground">
          ...
          <code>application/x-ilinxa-task+json</code> with a{" "}
      ...
      <section>
        <h3 className="mb-2 text-base font-semibold">Composing with kanban-board</h3>
        <p className="text-muted-foreground">
          The package exports{" "}
          <code>taskCardKanbanRenderer</code> — a typed{" "}
          <code>KanbanCardRenderer&lt;TaskItem&gt;</code> with{" "}
  ...
  return (
    ...
    />
        ...
        <p className="mt-2 text-xs text-muted-foreground">
          <code>rendererId: &quot;task-card&quot;</code> on each kanban
          item must match{" "}
          <code>taskCardKanbanRenderer.id</code> — the board uses this to
      ...
      <section>
        ...
        <ul className="ml-5 list-disc space-y-1 text-muted-foreground">
          <li>
            <strong>flow-canvas</strong> (via{" "}
            <code>todo-card-tree-node</code> sibling adapter, shipping
            separately): exposes a NodeRenderer that wraps this card.
          </li>
          <li>
            <strong>task-tree</strong> (sibling procomp, shipping separately):
            lightweight tree-row alternative that shares the same{" "}
            <code>TaskItem</code> schema and DnD payload so drags cross-procomp.
          </li>
        </ul>
      </section>
