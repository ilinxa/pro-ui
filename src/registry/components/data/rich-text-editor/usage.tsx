export default function RichTextEditorUsage() {
  return (
    <div className="max-w-none text-sm leading-relaxed text-foreground">
      ...
      <p className="mt-2 text-muted-foreground">
        Two exports from one folder: <code>RichTextEditor</code> is a{" "}
        <code>&quot;use client&quot;</code> editor (~150KB gzip);{" "}
        <code>RichTextViewer</code> is server-renderable (~30KB gzip).
...
export function ArticleEditPage({ initial }: { initial: RichTextValue }) {
  ...
  return (
    ...
    />
      ...
      <pre className="overflow-x-auto rounded-md border border-border bg-muted p-4 font-mono text-xs">
        <code>{`import { RichTextViewer } from "@/components/rich-text-editor"
