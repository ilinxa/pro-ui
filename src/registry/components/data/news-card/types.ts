...
// See .claude/decisions/2026-05-28-post-card-v0.3.1-fcross-11-fs1-cleanup.md
import type { CommentMenuItem } from "../comment-thread/types";
 ...
 * {@link NewsCardProps.moderatorActions} slot which supplies the menu items.
 ...
 * {@link NewsCardProps.kebabActions} still bypasses everything.
 */
...
// ─────────────────────────────────────────────────────────────────────────────
// v0.3.0 — Extended NewsCardItem (31 new optional fields)
// ─────────────────────────────────────────────────────────────────────────────
...
export interface NewsCardItem {
   ...
   * When {@link NewsCardItem.authorEntity} is also set, the structured
   * version wins for rendering; this string is the fallback.
   */
  author?: string;
  /**
   * Publish date — ISO-8601 string or Date. Stays for soft-compat.
   * When {@link NewsCardItem.publishedAt} is set, it wins; this is the fallback.
   */
  date?: string | Date;
...
/**
 * Discriminator for the universal {@link NewsCardProps.canPerformAction}
 ...
 * {@link NewsCardProps.moderatorActions} returns ≥1 item, the kebab
...
/**
 * Editor + reader mutation handlers. Flattened onto {@link NewsCardProps}
 ...
 */
export interface NewsCardMutationHandlers {
  // editor-side (12)
 ...
 * pass `<EngagementBar>` into the `renderEngagementCounts` slot — the same
 * handlers below are pre-wired into the slot's `helpers` bag.
 */
export interface NewsCardEngagementHandlers {
  ...
  /** Alias for {@link onCommentCountClick} — kept for symmetry with post-card. */
...
export interface NewsCardLabels {
  ...
  // ─── v0.3.0 verified-byline label (1 extra for parallelism with post-card) ─
  /** aria-label on the verified-author check icon. Default "Verified author". */
  verifiedAuthorLabel?: string;
}

/**
 * Defaults — English. Hosts override via the {@link NewsCardProps.labels}
 * prop, which deep-merges into this object.
 */
export const DEFAULT_LABELS: Required<NewsCardLabels> = {
  // v0.2
  readMore: "Read More",
  minutesShort: "min",
 ...
 * category routes.
 */
export interface NewsCardClickHandlers {
...
export interface NewsCardProps
  extends NewsCardMutationHandlers,
    NewsCardEngagementHandlers,
    NewsCardClickHandlers {
  /** The item to render. */
  item: NewsCardItem;
  ...
  permissions?: NewsCardPermissions;
  /**
   ...
   */
  canPerformAction?: (
    action: NewsCardPermissionAction,
    item: NewsCardItem,
  ) => boolean | undefined;
  ...
  renderExcerpt?: (item: NewsCardItem) => ReactNode;
   ...
   * `<EngagementBar>` here for the news article detail page (see
  ...
  renderEngagementCounts?: (
    ...
    helpers: {
      handlers: {
        onLike?: NewsCardProps["onLike"];
        onComment?: NewsCardProps["onCommentCountClick"];
        onShare?: NewsCardProps["onShare"];
        onBookmark?: NewsCardProps["onBookmark"];
      };
    },
  ) => ReactNode;
  /**
   ...
   */
  kebabActions?: (item: NewsCardItem) => CommentMenuItem[];
  /**
   ...
   */
  moderatorActions?: (item: NewsCardItem) => CommentMenuItem[];
...
export interface ResolvedPartProps {
  ...
  labels: Required<NewsCardLabels>;
  LinkComponent: ElementType;
  ...
  resolvedPermissions: Required<NewsCardPermissions>;
  canModerate: boolean;
  ...
  renderBadges: NewsCardProps["renderBadges"];
  renderAuthor: NewsCardProps["renderAuthor"];
  renderExcerpt: NewsCardProps["renderExcerpt"];
  renderPaywallGate: NewsCardProps["renderPaywallGate"];
  renderSensitiveGate: NewsCardProps["renderSensitiveGate"];
  renderQuoted: NewsCardProps["renderQuoted"];
  renderEngagementCounts: NewsCardProps["renderEngagementCounts"];
  ...
  onAuthorClick: NewsCardProps["onAuthorClick"];
  onPublisherClick: NewsCardProps["onPublisherClick"];
  onCategoryClick: NewsCardProps["onCategoryClick"];
  onTopicClick: NewsCardProps["onTopicClick"];
  onTagClick: NewsCardProps["onTagClick"];
  onQuotedClick: NewsCardProps["onQuotedClick"];
}
