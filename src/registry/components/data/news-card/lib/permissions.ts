 ...
 */
const EDITOR_DEFAULTS: Required<NewsCardPermissions> = {
  // editor-side (12)
  canEdit: true,
  canDelete: true,
  canPublish: true,
 ...
 */
const READER_DEFAULTS: Required<NewsCardPermissions> = {
  // editor-side (12)
  canEdit: false,
  canDelete: false,
  canPublish: false,
 ...
 * @returns A fully-resolved `Required<NewsCardPermissions>` matrix
 */
export function resolveNewsCardPermissions(
  viewerMode: NewsViewerMode | undefined,
  permissions: NewsCardPermissions | undefined,
): Required<NewsCardPermissions> {
  ...
  const baseline: Required<NewsCardPermissions> =
...
/**
 * Map `NewsCardPermissionAction` discriminator → corresponding
 * `NewsCardPermissions` field name. Pure lookup.
 */
const ACTION_TO_FIELD: Record<
  NewsCardPermissionAction,
  keyof NewsCardPermissions
> = {
  edit: "canEdit",
  delete: "canDelete",
  publish: "canPublish",
 ...
 * to decide whether to include it. The pure {@link resolveNewsCardPermissions}
...
export function canPerformActionInternal(
  ...
  item: NewsCardItem,
  resolved: Required<NewsCardPermissions>,
  canPerformAction?: (
    action: NewsCardPermissionAction,
    item: NewsCardItem,
  ) => boolean | undefined,
