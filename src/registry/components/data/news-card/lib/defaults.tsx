...
import type {
  ...
  NewsCardItem,
  NewsCardMutationHandlers,
  NewsCardLabels,
  NewsCardPermissionAction,
  NewsCardPermissions,
  NewsViewerMode,
} from "../types";
 ...
 * `onReport` lives on {@link NewsCardMutationHandlers} already.
 */
...
interface DefaultNewsCardKebabArgs {
  item: NewsCardItem;
  ...
  mutationHandlers?: NewsCardMutationHandlers;
  moderatorActions?: (item: NewsCardItem) => CommentMenuItem[];
  labels?: NewsCardLabels;
}

/**
 * Build the canonical kebab item list for news-card.
 *
 ...
 *
 * **Resolution layering:** the helper calls {@link resolveNewsCardPermissions}
 ...
 */
export function defaultNewsCardKebabActions(
  args: DefaultNewsCardKebabArgs,
): CommentMenuItem[] {
  const {
    item,
    engagementHandlers = {},
  ...
  const resolvedLabels: Required<NewsCardLabels> = {
    ...DEFAULT_LABELS,
    ...labels,
  };
  ...
  const resolved = resolveNewsCardPermissions(viewerMode, permissions);
  const allowed = (action: NewsCardPermissionAction): boolean =>
...
/**
 * Strip `quotedArticle` from a `NewsCardItem` to prevent infinite recursion
 ...
 * mutate the input.
 */
export function stripQuotedRecursion(item: NewsCardItem): NewsCardItem {
