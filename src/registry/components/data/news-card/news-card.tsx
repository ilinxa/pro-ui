...
import { defaultNewsCardKebabActions } from "./lib/defaults";
import { resolveNewsCardPermissions } from "./lib/permissions";
...
/**
 * NewsCard — magazine-style content card with 5 visual variants
 ...
 * mini-card, light engagement counts (with slot-based engagement-bar
 ...
 * docs/procomps/news-card-procomp/news-card-procomp-description-v0.3.0.md
 * for the full A+ trait set.
 */
function NewsCardImpl(props: NewsCardProps) {
  const {
    item: itemProp,
    variant = "medium",
    href,
    ...
    // v0.3 — mutation handlers (flattened from NewsCardMutationHandlers)
    onEdit,
    onDelete,
    onPublish,
    onUnpublish,
  ...
  const [statefulItem, setStatefulItem] = useState<NewsCardItem>(itemProp);
  ...
  const kebabItems = useMemo(() => {
    ...
    return defaultNewsCardKebabActions({
      item: effectiveItem,
...
// Procomps that adapt to NewsCardItem must import these types from THIS
