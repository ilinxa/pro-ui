...
/* eslint-disable react-hooks/refs -- bodyScrollRef is bundled on the context value;
   ...
   (see blackboard). Ref reads here are confined to effects + ref= props. */
...
import type { GanttSnap, GanttTimeUnit, GanttViewport, TaskItem } from "../types";
import {
  MS,
  addUnit,
  isWeekend,
...
type HoverState = { item: TaskItem; leftPx: number; topPx: number };
...
export function GanttTimelineBody({ className }: { className?: string }) {
  ...
  function activate(item: TaskItem) {
