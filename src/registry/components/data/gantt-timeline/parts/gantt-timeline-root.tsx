...
>(function GanttTimelineRoot(props, ref) {
  ...
  const geometryFor = useCallback(
    (item: TaskItem) => computeGeometry(item, nowMs, toneFor(item, statusOptions)),
    [nowMs, statusOptions],
  );
  ...
  const summarySpanFor = useCallback(
    (item: TaskItem) => summarySpan(item),
    [],
  );
  ...
  /* ── cross-surface clipboard (v0.5.0) — copy/cut/paste TaskItems through the OS
  ...
  useEffect(() => {
    ...
    const onCopy = (e: ClipboardEvent) => {
      ...
      if (!item) return;
      writeTasksToClipboardEvent(e, [item], "gantt-timeline");
      e.preventDefault();
    };
