/**
 * Public + internal types for the gantt-timeline pro-component.
 *
 * v0.1.0 surface — read-only, fully-navigable project timeline over the
 * canonical `TaskItem[]`. Bars from effective start→end, WBS summary rows from
 * `children`, milestone diamonds, continuous-zoom two-tier axis, pan/swipe/zoom
 * canvas, filled status/urgency bars (ramp imported from task-card).
 *
 ...
 * drops in additively. See docs/procomps/gantt-timeline-procomp/.
 */
...
export type GanttColorResolver = (item: TaskItem) => GanttBarColor;

export type GanttTooltipRenderer = (item: TaskItem) => ReactNode;
...
export type GanttTimelineProps = {
  ...
  // CRUD + field events (shapes reused from task-card)
...
export type GanttTimelineHandle = {
  ...
  addTask(parentId: string | null, item?: Partial<TaskItem>): void;
...
export type GanttContextValue = {
  ...
  geometryFor: (item: TaskItem) => GanttBarGeometry;
  summarySpanFor: (item: TaskItem) => { startMs: number; endMs: number } | null;
  ...
  can: (action: GanttEditAction, item: TaskItem) => boolean;
  /**
   ...
   */
  canGroupMove: (item: TaskItem) => boolean;
