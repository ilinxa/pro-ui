 ...
 * task-card-shaped event, then `onChange(next)`. No internal copy of `data`
...
type Args = {
  ...
  onItemAdded?: (e: TaskItemAddedEvent) => void;
  onItemRemoved?: (e: TaskItemRemovedEvent) => void;
  onItemMoved?: (e: TaskItemMovedEvent) => void;
  onFieldEdited?: (e: TaskFieldEditedEvent) => void;
...
const ACTION_RULE: Record<GanttEditAction, keyof TaskPermissionRule> = {
  move: "drag",
  resize: "drag",
  delete: "remove",
  create: "addChildren",
...
export function useGanttEdit(args: Args) {
  ...
  const can = useCallback(
    (action: GanttEditAction, item: TaskItem): boolean => {
  ...
  const guard = useCallback(
    (action: GanttEditAction, id: string): TaskItem | null => {
      const item = index.get(id)?.item;
      if (!item) return null;
      if (can(action, item)) return item;
      const reason: TaskPermissionReason = item.locked
        ? "locked"
  ...
  const moveItemAction = useCallback(
    (id: string, newParentId: string | null, newIndex: number) => {
      ...
      // index is one too far. (task-tree semantics.)
  ...
  const moveSubtree = useCallback(
    (id: string, deltaMs: number) => {
      ...
      for (const leaf of leaves) {
        if (!can("move", leaf)) {
          const reason: TaskPermissionReason = leaf.locked
            ? "locked"
  ...
  const changeStatus = useCallback(
    (id: string, status: string) => {
      ...
      const apply = (items: TaskItem[]): TaskItem[] =>
        items.map((it) =>
          it.id === id
            ? { ...it, status }
            : it.children?.length
  ...
  const changePriority = useCallback(
    (id: string, priority: string) => {
      ...
      // "priority" is not a `TaskEditableField`, so no typed field event — the
  ...
  const copyItem = useCallback(
    (id: string) => {
      ...
      // unhandled rejection.
      copyTasksToClipboard([item], "gantt-timeline").catch(() => {
        if (process.env.NODE_ENV !== "production") {
          console.warn("[gantt-timeline] copyItem: clipboard write failed");
        }
      });
    },
    [index],
  ...
  const cutItem = useCallback(
    (id: string) => {
      ...
      copyTasksToClipboard([item], "gantt-timeline").then(
        () => deleteItem(id),
        () => {
          if (process.env.NODE_ENV !== "production") {
            console.warn(
              "[gantt-timeline] cutItem: clipboard write failed — cut aborted, item was NOT removed",
            );
          }
        },
      );
  ...
  const pasteTasks = useCallback(
    (items: TaskItem[], parentId: string | null, atIndex?: number) => {
  ...
  const applyEditedSubtree = useCallback(
    (nextItem: TaskItem) => {
      ...
      // intentionally excluded — it is not a `TaskEditableField`, so it
      // persists via `onChange` only (parity with `changePriority`).
      const FIELDS: TaskFieldEditedEvent["key"][] = [
        "name",
        "description",
        "status",
        "active",
