export default function KanbanBoardUsage() {
  return (
  ...
  return (
    ...
    />
      ...
      <p className="text-muted-foreground">
        ...
        all of its features intact. The demo wires <code>&lt;CardTree&gt;</code>{" "}
        from <code>@ilinxa/card-tree</code> — same pattern works for any rich
      ...
      <pre className="overflow-x-auto rounded-md border border-border bg-muted p-4 font-mono text-xs">
        <code>{`import { CardTree, type CardTreeJsonNode } from "@ilinxa/card-tree";
...
/>`}</code>
      ...
      <p className="mt-2 text-muted-foreground">
        ...
        <code>@ilinxa/task-card</code> exports a ready-to-use{" "}
        <code>taskCardKanbanRenderer</code> (typed as{" "}
        <code>KanbanCardRenderer&lt;TaskItem&gt;</code>,{" "}
        <code>dragHandle: &quot;header&quot;</code>) — no factory wrapper
        needed. Drop it directly into <code>renderers={"{[...]}"}</code> and
        give each item <code>rendererId: &quot;task-card&quot;</code> with
        a <code>TaskItem</code>-shaped <code>data</code> payload. See the
        task-card detail page for the live kanban demo + the full code
        recipe.
      </p>
