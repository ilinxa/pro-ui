export default function MediaLibraryUsage() {
  return (
    <div className="max-w-none text-sm leading-relaxed text-foreground">
      <h3 className="mb-2 mt-0 text-base font-semibold">When to use</h3>
      <p className="text-muted-foreground">
        Reach for <code>MediaLibrary</code> when you need a Google-Drive-style asset
        ...
        <code>video-player</code>) and a folder <code>file-tree</code> — you supply the
        data + the backend callbacks.
      </p>

      <h3 className="mb-2 mt-6 text-base font-semibold">Full (batteries-included)</h3>
      <pre className="overflow-x-auto rounded-md border border-border bg-muted p-4 font-mono text-xs">
        <code>{`import { MediaLibrary } from "@/components/media-library"
...
<MediaLibraryRoot nodes={tree} onMove={moveAssets} preview={false}>
  ...
  <MediaLibraryFileGrid />
      ...
      <pre className="overflow-x-auto rounded-md border border-border bg-muted p-4 font-mono text-xs">
        <code>{`import { FilePreview } from "@/components/media-library"
