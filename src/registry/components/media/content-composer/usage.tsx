export default function ContentComposerUsage() {
  return (
    <div className="max-w-none text-sm leading-relaxed text-foreground">
      <h3 className="mb-2 mt-0 text-base font-semibold">When to use</h3>
      <p className="text-muted-foreground">
        Reach for <code>ContentComposer</code> when you need a multi-step
        content-authoring surface in a CMS — one shell that composes structured
        metadata fields (<code>json-form</code>), a rich body (
        <code>rich-text-editor</code> / Plate or a plaintext fallback), and a
        captured/edited hero (<code>media-editor</code>). Each content type is
...
/>`}</code>
      ...
      <ul className="ml-5 list-disc space-y-1 text-muted-foreground">
        <li>
          ...
          <code>mediaCarouselSlot</code> → <code>carousel-composer</code> —
