...
import {
  ...
  type StoryComposerHandle,
  type StoryComposerLabels,
  type StoryComposerProps,
} from "./types";

/**
 * StoryComposer v0.2.0 — thin wrapper around `MediaEditor`.
 *
 ...
 * text-only canvas) now lives inside MediaEditor — landed in the R1–R4
 * backfill commits.
 *
 * Known v0.2.0 polish gaps (none break the type contract):
 *   - `labels` overrides pass through verbatim as the v0.1.5 flat shape;
 *     full MediaEditorLabels nested-shape mapping lands at C17.
...
export const StoryComposer = forwardRef<
  StoryComposerHandle,
  StoryComposerProps
>(function StoryComposer(props, ref) {
  ...
  const labels = useMemo<Required<StoryComposerLabels>>(
    ...
    (): StoryComposerHandle => ({
      ...
      exportBlob: async () => {
        if (!editorRef.current) {
          throw new Error("story-composer: editor not mounted");
        }
  ...
  return (
    <>
      <MediaEditor
        ...
        // the nested MediaEditorLabels surface.
        labels={labelOverrides as unknown as Partial<MediaEditorLabels>}
