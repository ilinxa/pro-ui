...
export const meta: ComponentMeta = {
  ...
  dependencies: {
    // Shadcn primitives directly imported by story-composer-01: just `button`
    // (composer-publish-bar + publishing-progress-overlay). `dialog` is no
    // longer imported here — the wrapper's MediaEditor01 mount owns the dialog
    // chrome, so dialog resolves transitively via the @ilinxa/media-editor-01
    // registry dependency. Other primitives (alert-dialog, popover, slider,
    // toggle-group) moved with their consumer parts to media-editor-01.
    shadcn: ["button"],
    npm: {
      "lucide-react": "^1.11.0",
