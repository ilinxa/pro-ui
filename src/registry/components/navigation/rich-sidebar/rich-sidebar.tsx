 ...
 */
export function RichSidebar(props: RichSidebarProps) {
  const {
    items,
    currentPath,
    isActive,
  ...
  const autoId = useId();
  const sidebarId = idProp ?? `rich-sidebar-${autoId}`;
  ...
  // When external state is provided, the hook (useRichSidebarState)
  ...
  const contextValue = useMemo<RichSidebarContextValue>(
    () => ({
  ...
  return (
    <RichSidebarContext.Provider value={contextValue}>
      ...
      <div
        ...
        data-component="rich-sidebar-mobile-wrapper"
        style={cssVars}
      >
        <Sheet
