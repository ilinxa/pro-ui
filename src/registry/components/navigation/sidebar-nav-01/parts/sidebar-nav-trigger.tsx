 ...
 *   2. Nearest SidebarNav01Context (works when trigger is a DESCENDANT
 *      of <SidebarNav01>; siblings can't read it because the context
 ...
 * A future SidebarNav01Provider wrapper will enable trigger state
...
export function SidebarNav01Trigger({
  ...
  const handleClick = () => {
    ...
    let resolved: SidebarNav01Handle | null = null;
    if (controls) {
      if (typeof controls === "object" && controls !== null && "current" in controls) {
        resolved = (controls as React.RefObject<SidebarNav01Handle | null>).current;
      } else {
        resolved = controls as SidebarNav01Handle;
      }
    }
    if (!resolved && ctx) {
      resolved = ctx.handle;
    ...
    if (resolved) {
      ...
      console.warn(
        "[sidebar-nav-01] <SidebarNav01Trigger> clicked with no <SidebarNav01> in context and no `controls` prop — no-op.",
      );
    }
  };
