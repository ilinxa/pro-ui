export default function TeamFeedbackLoopUsage() {
  return (
    <div className="max-w-none text-sm leading-relaxed text-foreground">
      <h3 className="mb-2 mt-0 text-base font-semibold">When to use</h3>
      <p className="text-muted-foreground">
        Reach for <code>TeamFeedbackLoop</code> to close the cooperative
      ...
      <pre className="overflow-x-auto rounded-md border border-border bg-muted p-4 font-mono text-xs">
        <code>{`import { TeamFeedbackLoop } from "@/components/team-feedback-loop"
...
ref.current?.celebrate({ kind: "task-complete", title: "Your team cleared the column" })`}</code>
      ...
      <p className="text-muted-foreground">
        If you also use <code>team-trophy-shelf</code>, route each event kind to
