...
export function resolveProgress(args: {
  ...
  if (value != null) {
    if (milestones?.length) {
      devWarn(
        "[team-progress-bar] both `value` and `milestones` supplied; `value` takes precedence.",
      );
    }
    return {
...
export function resolveLabelFormat(
  ...
  if (requested === "fraction" && total == null) {
    devWarn(
      "[team-progress-bar] labelFormat='fraction' requires `milestones`; falling back to 'percent'.",
    );
    return "percent";
  }
  return requested;
