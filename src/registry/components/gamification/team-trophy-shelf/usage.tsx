export default function TeamTrophyShelfUsage() {
  return (
    <div className="max-w-none text-sm leading-relaxed text-foreground">
      <h3 className="mb-2 mt-0 text-base font-semibold">When to use</h3>
      <p className="text-muted-foreground">
        Reach for <code>TeamTrophyShelf</code> when a team board needs a durable
  ...
  milestoneId?: string // the milestone that earned it (optional link)
      ...
      <pre className="overflow-x-auto rounded-md border border-border bg-muted p-4 font-mono text-xs">
        <code>{`import { TeamTrophyShelf } from "@/components/team-trophy-shelf"
...
/>`}</code>
      ...
      <pre className="overflow-x-auto rounded-md border border-border bg-muted p-4 font-mono text-xs">
        <code>{`import { TeamMilestoneBadge } from "@/components/team-trophy-shelf"
...
<TeamMilestoneBadge badge={badge} size="sm" />`}</code>
      ...
      <ul className="ml-5 list-disc space-y-1 text-muted-foreground">
        <li>
          ...
          events to a <code>team-feedback-loop</code> overlay, set{" "}
