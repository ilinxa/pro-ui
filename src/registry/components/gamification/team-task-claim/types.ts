/**
 * `team-task-claim` — public type surface.
 *
...
export type TaskClaimEvent = {
  type: "task-claim.interaction";
  teamId: string;
  taskId: string;
};

/** Layout density — fits a kanban card vs a card-tree row. */
export type TaskClaimDensity = "compact" | "comfortable";

/** Per-state copy overrides (D8 — one button, overridable). */
export interface TaskClaimLabels {
...
/** Props for the assembly `TeamTaskClaim`. */
export interface TeamTaskClaimProps {
  /**
