export default function TeamTaskClaimUsage() {
  return (
    <div className="max-w-none text-sm leading-relaxed text-foreground">
      <h3 className="mb-2 mt-0 text-base font-semibold">When to use</h3>
      <p className="text-muted-foreground">
        Drop <code>TeamTaskClaim</code> onto any team task card to give
      ...
      <pre className="overflow-x-auto rounded-md border border-border bg-muted p-4 font-mono text-xs">
        <code>{`import { TeamTaskClaim } from "@/components/team-task-claim"
...
<OpenForAnyoneToggle open={task.choice.openForAnyone} onOpenChange={setOpen} />`}</code>
      ...
      <ul className="ml-5 list-disc space-y-1 text-muted-foreground">
        ...
        <li>
          ...
          <code>task-claim.interaction</code> on meaningful interactions only
