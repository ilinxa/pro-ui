...
import { resolveTaskClaimState } from "./lib/state";
import type { TeamTaskClaimProps } from "./types";

/**
 * The single-unit assembly — a **logic-free fan-out**. It resolves the three-
 * state slice once (`resolveTaskClaimState`, exported) and renders the relevant
...
export function TeamTaskClaim({
  ...
  const state = resolveTaskClaimState(value);

  const emit = React.useCallback(() => {
    onEvent?.({ type: "task-claim.interaction", teamId, taskId: value.taskId });
