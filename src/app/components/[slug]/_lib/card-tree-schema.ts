...
/**
 * A `CardTreeJsonNode` is an open JSON object — scalar keys are fields, nested
 ...
 */
export type ValidateCardTreeResult =
  | { ok: true; value: CardTreeJsonNode }
  | { ok: false; error: string };

export function validateCardTreeNode(text: string): ValidateCardTreeResult {
  if (!text.trim())
    return { ok: false, error: "Write a CardTreeJsonNode to begin." };
  let parsed: unknown;
  try {
