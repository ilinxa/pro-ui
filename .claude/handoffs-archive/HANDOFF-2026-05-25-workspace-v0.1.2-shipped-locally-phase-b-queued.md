...
> **MOSTLY SUPERSEDED 2026-05-25 (same day).** Resume paths 1 AND 3 both executed and completed: path 1 (push + smoke + close F-01) in commit `3a6f707`; path 3 (v0.1.3 micro-patch closing F-03 via `@internal` JSDoc) in commits `48a503c` + `b262b0c`. **Path 2 (Phase B v0.2.0) is also partially advanced** — GATE 1 description addendum signed off 2026-05-25 in commit `3158a02` ([decision](decisions/2026-05-25-workspace-v0.2.0-gate-1-closed.md), [addendum doc](../docs/procomps/workspace-procomp/workspace-procomp-description-v0.2.0.md)); GATE 2 plan refresh + GATE 3 full-checklist + implementation remain. The current truth lives in [`STATUS.md`](STATUS.md) (top THREE banner rows). Rest of this file preserved for context.
>
...
> 2. Start Phase B (v0.2.0 — alpha → beta promotion). Full plan at [`C:/Users/AsiaData/.claude/plans/lets-create-a-comprehensive-proud-cloud.md`](file:///C:/Users/AsiaData/.claude/plans/lets-create-a-comprehensive-proud-cloud.md). Phase A is fully shipped; Phase B is untouched code BUT **GATE 1 description addendum is now signed off** (commit `3158a02`, 2026-05-25) — see [`docs/procomps/workspace-procomp/workspace-procomp-description-v0.2.0.md`](../docs/procomps/workspace-procomp/workspace-procomp-description-v0.2.0.md) for the 9 locked Q-Ps. Next: GATE 2 plan refresh (~1h, still no code), then implementation.
> 3. ~~v0.1.3 micro-patch closing the v0.1.2 spotcheck F-03 (`useResizeKeyboard` export — un-export OR document for the rare consumer reaching for it).~~ ✅ **Completed 2026-05-25 in commits `48a503c` + `b262b0c`** — `@internal` JSDoc + guide "Public API surface" note + migration block; no public API change; GATE 3 exempt. Decision: [`.claude/decisions/2026-05-25-workspace-v0.1.3-f-03-internal-jsdoc.md`](decisions/2026-05-25-workspace-v0.1.3-f-03-internal-jsdoc.md).
