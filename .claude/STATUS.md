...
- **✅ 2026-06-01 — story-composer-01 v0.1.5 patch SHIPPED (photo-preview latency + keyboard arrows + demo uploader)** ([decision](decisions/2026-06-01-story-composer-01-v0.1.5-photo-preview-and-arrow-keys.md)) — Runtime-only patch on top of the v0.1.0→v0.1.4 same-day arc. (1) Photo preview no longer flashes black between shutter and editor — Suspense fallback now renders `<img src={draft.url}>` instead of a black div; matching underlay `<img>` inside ComposerEditor bridges the gap until Konva's `useImage` + Stage init resolve (gated on `!image` so applied filters don't bleed-through). Deeper canvas-direct path deferred to v0.2. (2) Keyboard arrow pan direction inverted in `usePanZoom` — flipped to moving-the-image semantics so ArrowRight moves image right. No public-API change. (3) `demo.tsx` swapped all five tabs onto a shared `demoUploader` (fakes 1.5s success returning `URL.createObjectURL(blob)`) so the docs site round-trips Publish → Done without a real backend; `SAMPLE_UPLOAD_URL` (= `https://example.com/upload/story`, a non-POST endpoint) was making every Publish hit the working error path. Patch-bump, no GATE 3 required. Tsc + lint + validate:meta-deps all clean.
