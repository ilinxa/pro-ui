...
> Snapshot date: **2026-05-14** (flow-canvas-01 v0.1.4 patch — custom-json renderer port-handle bug fix; v0.2 perf-tier description signed off + pre-work artifacts shipped)
...
- **v0.1.4:** `flow-canvas-01` (custom-json renderer was missing `<PortsAt>` calls for declared ports — xyflow's edge layer failed `getEdgePosition` thousands of times per second and warning-spammed the console; ~6-line fix = 15-20× FPS improvement at the light-fixture cliff; surfaced during v0.2 perf-tier pre-work).
...
| `flow-canvas-01`        | Flow Canvas                | 0.1.4   | alpha  |
