...
|----|----------|-------------|------------|--------|
| F-cross-01 | ⚠️ High | Originally 6 missing-guide carriers: `detail-panel`, `entity-picker`, `filter-stack`, `force-graph`, `markdown-editor`, `properties-form`. **Updates:** force-graph removed in session 3 (→ 5 carriers). data-table flagged in session 4 with all-three-docs-missing — different shape (→ 5+1). markdown-editor confirmed in session 5 (F-02 of that review). Two original Tier 1 carriers remain for session 6: `properties-form` + `entity-picker`. Rollup batches all carriers into a documentation backlog at sweep close. | 2026-05-08 (pre-flight; affirmed s5) | **Open — 1/3 Tier 1 confirmed** |
...
| F-cross-06 | 🔸 Medium | **Sweep-wide `usage.tsx` import-path drift — CONFIRMED SYSTEMIC (6/6 Tier 1).** Kanban (F-04 s1), rich-card (F-04 s2), article-body-01 (F-02 s4), data-table (F-02 s4), workspace (F-03 s5), markdown-editor (F-04 s5) all use producer-side `@/registry/components/<category>/<slug>` instead of consumer-side `@/components/<slug>` (or `@ilinxa/<slug>`). 100% hit rate across reviewed components → strongly suggests every Tier 2 component has the same drift. **Sweep-wide single-commit fix at session 7 mid-checkpoint** is the right approach. Recommendation: grep-replace all 36 components' `usage.tsx` files in a single pass; verify by re-running the smoke harness. | 2026-05-08 (session 2; affirmed s4 + s5) | **Open — confirmed systemic 6/6** |
| F-cross-08 | ⚠️ High | **`process.env.NODE_ENV` use confirmed at 2/7 Tier 1.** Session 3 found 4 occurrences in flow-canvas-01 violating description §6 #10 ("no `process.env`"). Session 5 affirmed in markdown-editor (1 occurrence in `hooks/use-imperative-handle.ts:12` — F-01 of that review). The other 5 reviewed Tier 1 components (kanban, rich-card, article-body-01, data-table, workspace) are clean. Worth a sweep-wide grep across remaining 29 Tier 2 components to identify systemic violators. Either relax the criterion (allow `NODE_ENV` dev-warn gates) OR replace with a project-level `IS_DEV` constant. **Recommended: relax + document, since `process.env.NODE_ENV` is standard React-ecosystem practice and bundlers universally compile out the dev branches.** Resolution targeted for session 7 mid-checkpoint. | 2026-05-08 (session 3; affirmed s5) | **Open — 2/7 Tier 1 confirmed** |
| F-cross-07 | ⚠️ High | **Cross-component dep declaration drift — AFFIRMED 2/7 Tier 1 (wrong-major/non-standard-semver shape) + 1/7 phantom-dep shape.** Rich-card declares `@dnd-kit/sortable: ^11.x` (non-standard `^N.x`, wrong major — v11 doesn't exist; producer pkg has `^10.0.0`). Article-body-01 declares `lucide-react: ^0.x` (non-standard `^N.x`, wrong major — producer pkg has `^1.11.0`). Workspace + markdown-editor confirm clean for wrong-version drift (lucide-react `^1.11.0` matches producer in both; markdown-editor's 9 CodeMirror + `marked` versions all match producer). **NEW (session 5): markdown-editor F-03 surfaces a related-but-different shape** — `meta.npm` declares `radix-ui: ^1.4.3` that no markdown-editor source file imports (phantom over-declaration vs wrong-version). Treat as a sub-class within F-cross-07's family. A consumer installing rich-card + article-body-01 + markdown-editor gets two divergent ancient/nonexistent deps + an unused 1.4.3 meta-package. Library should enforce a project-level convention that inter-component dep pins match producer's `package.json` AND meta-declared deps reflect actual imports. **Recommended: `pnpm validate:meta-deps` CI check** that grep-walks each component's imports and cross-checks with `meta.ts.dependencies` AND with producer's `package.json` (wrong-version + phantom both caught). | 2026-05-08 (session 2; affirmed s4 + s5) | **Open — affirmed 2/7 (wrong-version) + 1/7 (phantom)** |
