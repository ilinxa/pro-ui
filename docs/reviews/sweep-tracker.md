...
| F-cross-08 | ⚠️ High | **`process.env.NODE_ENV` use confirmed at 2/5 Tier 1.** Session 3 found 4 occurrences in flow-canvas-01 violating description §6 #10 ("no `process.env`"). Session 5 affirmed in markdown-editor (1 occurrence in `hooks/use-imperative-handle.ts:12` — F-01 of that review). The other 3 reviewed components (kanban, rich-card, article-body-01, data-table — wait that's 4: kanban, rich-card, article-body, data-table, workspace are clean; 2 of 5 carry the violation). Worth a sweep-wide grep across remaining 31 Tier 2 components to identify systemic violators. Either relax the criterion (allow `NODE_ENV` dev-warn gates) OR replace with a project-level `IS_DEV` constant. **Recommended: relax + document, since `process.env.NODE_ENV` is standard React-ecosystem practice and bundlers universally compile out the dev branches.** Resolution targeted for session 7 mid-checkpoint. | 2026-05-08 (session 3; affirmed s5) | **Open — 2/5 Tier 1 confirmed** |
...
| F-cross-09 | ⚠️ High | **NEW (session 5): shadcn CLI smoke regression.** Fresh smoke run shows **19 pass / 1 expected-fail / 16 unexpected-fail** vs session 1's **31 / 1 / 5**. Failures consistently emit "try a previous version: `pnpm dlx shadcn@4.6.0 add @ilinxa/<slug>` --yes" — strong signal `shadcn@latest` resolved a newer CLI version since session 1 that broke namespaced-registry resolution. Affected (NEW failures vs session 1): `workspace`, `markdown-editor`, `properties-form`, `detail-panel`, `filter-stack`, `author-card-01`, `page-hero-news-01`, `share-bar-01`, `video-player-01`, `content-card-news-01`, `article-body-01` (timeout @120s), `article-meta-01` (timeout @120s). The 4 F-cross-05 carriers (`comment-thread-01`, `post-card-01`, `media-carousel-01`, `story-viewer-01`) still fail for their original bare-name-sibling-dep reason. **Recommendations:** (a) pin smoke harness to `pnpm dlx shadcn@4.6.0` until the CLI regression is understood; (b) investigate the new CLI's namespaced-registry resolution behavior (changelog diff between 4.6.0 and current latest); (c) decide whether to advise consumers to pin too. Bigger than any single component review. | 2026-05-08 (session 5) | **Open — needs CLI version diff + harness pin** |
...
| 5 | 2026-05-08 | `workspace` v0.1 → Pass with follow-ups (7 findings); `markdown-editor` v0.1 → Pass with follow-ups (7 findings) | F-cross-06 confirmed 6/6 systemic; F-cross-08 affirmed 2/5 (markdown-editor); F-cross-01 narrowed to 2/3 Tier 1 remaining (properties-form + entity-picker for session 6); **NEW F-cross-09 — shadcn CLI smoke regression (19/1/16 vs session 1's 31/1/5; 12 new fails)**; markdown-editor's phantom `radix-ui` dep flagged (F-03); workspace's adaptive-flatten silently discards content past cap (F-01 — Q12 contract violation); plan-vs-code "loud not silent" recurring theme. | fresh session 5 smoke: 19 pass / 1 expected-fail / 16 unexpected-fail (F-cross-09). Workspace + markdown-editor session 1 baseline preserved (9.8s + 10.8s). | Workspace: most ambitious component reviewed; planning docs reference-grade; renderer-registry pattern confirmed across kanban + flow-canvas-01 + workspace (3/4 host components). Markdown-editor: 28-file plan §8.1 lands exactly; CM6 substrate done right (echo guard, StateField runtime candidates, per-instance Marked, useDeferredValue); findings target ecosystem more than substrate. Tier 1 progress: 7/9. |
