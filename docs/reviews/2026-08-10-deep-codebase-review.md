...
**Post-ship smoke discoveries (2026-08-10 late, consumer harness):**
1. ⚠️ **shadcn CLI 4.6.0 corrupts the consumer's package.json on `add`** — version specs get shuffled across package names (`eslint` received react-hook-form's `^7.75.0`, `@types/react-dom` received cmdk's `^1.1.1`, 23 specs corrupted in the harness). Every subsequent `pnpm add` inside the CLI then fails BEFORE file writes, so **the smoke harness has been silently installing nothing since ~July** — July's "smoke CLEAN" results after the corruption point are unverified. Harness repaired (specs re-aligned to producer truth); real-consumer impact: any consumer running repeated `@ilinxa` adds may hit the same corruption — track upstream, consider pinning a newer CLI once verified.
2. ⚠️ **F-cross-13 path-b cohort quantified**: a deploy-parity manual-copy smoke of 38 components against the CURRENT base-nova (Base UI) primitives shows **~45 `asChild`-on-trigger sites (~20 components), 5 ToggleGroup single-value sites (file-manager view/icon controls — same class as the filter-stack fix), and 4 further `delayDuration` sites** that don't compile on today's Base-UI style surface. Pre-existing (predates P1; masked by discovery #1), affects fresh Base-UI consumers. Two carriers were fixed same-day (filter-stack 0.1.1 ToggleGroup+delayDuration, properties-form 0.1.2 delayDuration); **the remaining cohort is the "F-cross-13 path-b producer sweep" — now scheduled as the P1.5 follow-up before P2** (one mechanical pattern; buttonVariants/direct-trigger escape hatches documented in memory).
   *(Manual-copy method caveat: 32 `@/registry/...` module-not-found errors + downstream implicit-any errors in that run are method artifacts — the CLI rewrites those imports at install time — and consumer-side primitive-vintage mismatches (avatar/command/slider) are harness-state, not shipped bugs.)*
   **→ CLOSED 2026-08-11 by P1.5** ([decision](../../.claude/decisions/2026-08-11-p1-5-carrier-sweep.md)): full programmatic scan found the true cohort was ~55 sites / 23 components (review undercounted json-form, entity-picker, filter-bar-01, kanban-board-01, media-editor-01); all fixed — zero `asChild`/`delayDuration`/ToggleGroup carriers remain in shipped registry code. 5 fix agents + 3 adversarial verifiers; one fix-introduced regression (calendar touch long-press vs stopPropagation) caught and closed pre-ship.
