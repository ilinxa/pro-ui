# expandable-text — procomp plan

> Stage 2: how. The implementation contract.
>
> See [`expandable-text-procomp-description.md`](./expandable-text-procomp-description.md) for the what & why.
>
...
| Q-P7 | `content` accepts `ReactNode`? | **No — `string` only.** Measurement requires a stable text node. Rich content is `rich-text-editor`'s job. |
| Q-P8 | Component category? | **`data`** (matches sibling primitives `progress-timeline`, `info-list`, `schedule-list`). |
...
```ts
// src/registry/components/data/expandable-text/types.ts
...
```ts
// src/registry/components/data/expandable-text/hooks/use-line-clamp-detect.ts
...
export type {
  ...
  ExpandableTextToggleRenderProps,
} from "./types";
...
```
src/registry/components/data/expandable-text/
├── expandable-text.tsx              # 1 — root
├── hooks/
...
const ExpandableText = memo(ExpandableTextInner);
...
1. **Default (collapsed)** — `<ExpandableText content={LONG_EN} />`. Shows the toggle, demonstrates the basic interaction.
2. **Custom maxLines** — `<ExpandableText content={LONG_EN} maxLines={6} />`. Shows that maxLines is dynamic (toggle still appears since content > 6 lines on the demo width).
3. **Localized (TR)** — `<ExpandableText content={LONG_TR} labels={{ showMore: "Daha fazla göster", showLess: "Daha az göster" }} />`.
...
- **Anti-patterns** — passing `ReactNode` (don't; use `rich-text-editor`); expecting auto-collapse on click outside (we don't do that); animating expand height (we don't in v0.1)
...
export const meta: ComponentMeta = {
  ...
  related: ["rich-text-editor", "comment-thread", "post-card"],
};
```
...
- Any rich-text library (use `rich-text-editor` for that)
...
- [ ] `pnpm lint` clean (1 pre-existing card-tree warning OK; no new)
- [ ] `pnpm build` clean — `/components/expandable-text` prerendered (35th route)
...
- `pnpm new:component data/expandable-text` — scaffolds the sealed folder
...
- Author `expandable-text.tsx` per spec above
...
- Run `pnpm registry:build` — verify `public/r/expandable-text.json` + `public/r/expandable-text-fixtures.json` produced
- Update `.claude/STATUS.md` with the entry
- Author `expandable-text-procomp-guide.md` (consumer-facing notes — composes alongside the implementation)
...
- v0.3: optional rich-content escape hatch via `<ExpandableText><CustomChildren /></ExpandableText>` — but this likely lives in a separate `expandable-block-01` since measurement gets harder with arbitrary children
