# `team-trophy-shelf` — Pro-component Plan (Stage 2 / GATE 2)

> **Stage 2 of 3 · Status: DRAFT — pending GATE 2 sign-off**
> **Slug:** `team-trophy-shelf` · **Category:** `gamification` · **Tier:** pro-component · **Structure:** shadcn-style compound
> **Element / SDT need:** E2 (Team Milestone Badges) / **Competence + Relatedness**
> **Predecessor:** [`team-trophy-shelf-procomp-description.md`](./team-trophy-shelf-procomp-description.md) (GATE 1)
...
| `TeamTrophyShelf` | **A (assembly)** | `team-trophy-shelf.tsx` | no (composes B parts) | `Root` + `Header?` + (`Grid` | `Empty`) gated by `showHeader` / emptiness. **Contains no logic the parts don't.** Demo + screenshot use this. |
...
- **`BadgeAwardOverlay` enters the graph ONLY via `React.lazy(() => import("./parts/badge-award-overlay"))`**, referenced solely from `TeamTrophyShelfGrid`, and only *invoked* when `animateAward !== false` AND a newly-earned badge exists. A consumer importing only `TeamMilestoneBadge`, or mounting the shelf with `animateAward={false}`, **never loads the overlay chunk** → the burst code stays out of their bundle. **Verified at GATE 3** by inspecting the built lazy-chunk boundary (the compound rule's lazy-boundary check; the media-library / event-calendar precedent).
...
`TeamTrophyShelfRoot` is the single source of state + the diff + the telemetry firing. `TeamTrophyShelf` is a fixed child tree with `showHeader` + emptiness gating and **zero** state of its own — so a hand-assembled layout (description §6.3) gets identical behavior. A reviewer rejects any logic that lives in the assembly but not the parts.
...
- **No reducer needed** (small surface): the Root uses `useState` for `openedBadgeId` + `mounted`, `useRef` for `prevBadgesRef` + `firedViewRef`, and `useMemo` for `resolveSlots` / `earnedCount` / the `newlyEarned` set. The "reconcile via `useMemo`, not a prune effect" lesson (blackboard) applies — derive, don't store-then-sync.
...
Sealed folder under `src/registry/components/gamification/team-trophy-shelf/` (compound layout; `parts/` co-locates Tier-B wrappers + Tier-C cores per the house pattern):

| File | Contents |
|---|---|
| `team-trophy-shelf.tsx` | **Tier A** `TeamTrophyShelf` assembly — `"use client"`; `Root` + `showHeader`-gated `Header` + (`Grid` | `Empty`). The only public default-layout entry. No logic the parts lack. |
...
Flat exported names: `TeamTrophyShelf` · `TeamTrophyShelfRoot` · `TeamTrophyShelfGrid` · `TeamTrophyShelfHeader` · `TeamTrophyShelfEmpty` · `TeamMilestoneBadge` · `BadgeAwardOverlay` · `useTeamTrophyShelf`, plus the public types.
...
export type TeamTrophyShelfProps = {
   ...
   * D-16: a host routing badge/milestone events to `team-feedback-loop` (E6)
...
Per [`docs/component-guide.md §11.5`](../../component-guide.md#115-shipping-via-the-registry) + the `shadcn-registry-pro` skill. **Two items**, locked target convention: every file `type: "registry:component"`, `target: "components/team-trophy-shelf/<sub-path>"`. **Never ship `demo.tsx`, `usage.tsx`, or `meta.ts`.**

**Base item — `team-trophy-shelf`:**
- `team-trophy-shelf.tsx`, `index.ts`, `types.ts`
...
**Fixtures sibling — `team-trophy-shelf-fixtures`:**
- `dummy-data.ts` only; `registryDependencies: ["@ilinxa/team-trophy-shelf"]` (depends on the base, adds the fixture). Target `components/team-trophy-shelf/dummy-data.ts`.

Smoke (F-cross-11 path-b, recommended on first ship): `pnpm dlx shadcn add @ilinxa/team-trophy-shelf` into a tmp consumer → consumer-side `pnpm tsc --noEmit` clean. Because there's **no internal registry dep**, the cross-procomp rewriter traps (content-composer F-01 / gantt §3.3) do not apply here — but the **lazy `./parts/badge-award-overlay` relative import** must survive the rewriter (it's a relative dynamic `import()`, which the rewriter leaves untouched per the rewriter-import-rules) → verify the lazy chunk resolves in the consumer artifact.
...
| **D-g** | Slug / category | `gamification/team-trophy-shelf`. Category plumbing (D-01) lands before scaffold. **LOCKED.** |
...
1. `pnpm tsc --noEmit` clean · `pnpm lint` clean · `pnpm validate:meta-deps` clean (shadcn primitives + `lucide-react` declared AND imported; **no internal registry dep** to reconcile). **meta-deps gotcha:** in any file with side-effect imports, place the dep-declaring `from`-import FIRST; never reference a dep name only inside a comment (blackboard + content-composer lessons).
2. `pnpm build` succeeds.
3. Docs render at `/components/team-trophy-shelf`; all demo tabs interactive (the **Award reveal** tab must visibly play < 1s, non-blocking, and stop under `prefers-reduced-motion` / `animateAward={false}`).
4. `pnpm registry:build`; spot-check `public/r/team-trophy-shelf.json` + `team-trophy-shelf-fixtures.json` (targets follow `components/team-trophy-shelf/<sub>`; no demo/usage/meta shipped; **no `@ilinxa/*` registryDependency**).
...
- [ ] **User approved to proceed** → scaffold `gamification/team-trophy-shelf` (after the D-01 category plumbing) + implementation.
