# `team-trophy-shelf` — Pro-component Description (Stage 1 / GATE 1)

> **Stage:** 1 of 3 · **Status:** DRAFT — pending GATE 1 sign-off
> **Slug:** `team-trophy-shelf` · **Category:** `gamification` · **Tier:** pro-component (ships as a **shadcn-style compound** — see §0)
...
| **A — assembly** | `TeamTrophyShelf` | `Root` + the parts above, gated by `show*` toggles. Contains no logic the parts don't. Demo + screenshot use this. |
...
- **Not a progress bar.** Live completion % is `team-progress-bar` (E1); the shelf is the durable artifact gallery.
- **Not a notification/toast system.** The award animation is an in-place reveal on the shelf/token, not a global toast queue.
- **Not a quest timeline.** Milestone-as-narrative-chapters is `team-quest-log` (E5); the shelf references milestones but doesn't sequence them.
...
Non-targets: public profile pages, inter-team comparison views, per-individual achievement walls (all excluded by system §5.3 / D-08), live-rate displays (→ `team-progress-bar`).
...
```ts
// Declared in team-trophy-shelf/types.ts — NOT imported from another component.
interface Badge {
...
Illustrative. The canonical shape lands in `team-trophy-shelf/types.ts` at plan stage; defer to it on naming. Types are **declared here, not imported** (system D-03).
...
```tsx
import { TeamTrophyShelf } from "@/registry/components/gamification/team-trophy-shelf";
...
```tsx
import { TeamMilestoneBadge } from "@/registry/components/gamification/team-trophy-shelf";
...
} from "@/registry/components/gamification/team-trophy-shelf";
...
| **D-05** | **Ships as a shadcn-style compound** — `Root` + flat parts (`TeamTrophyShelfGrid`, …) + Tier-C `TeamMilestoneBadge` + `BadgeAwardOverlay` + a logic-free `<TeamTrophyShelf>` assembly. Flat exports (never `X.Root`); heavy animation dep `React.lazy`. | system D-05; [compound rule](../../../.claude/rules/compound-component-structure.md) |
...
| **D-g** | **Slug / category** | `gamification/team-trophy-shelf`. `-01` suffix per library convention; `gamification` is the new category (system D-01, one-off plumbing before GATE 2). |
...
- [ ] **User approved to proceed** → Stage 2 (`team-trophy-shelf-procomp-plan.md`, GATE 2).
