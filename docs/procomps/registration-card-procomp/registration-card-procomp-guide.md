# registration-card — procomp guide
...
- **Time-bound progress** (registration window over time) — use `progress-timeline` for a separate widget alongside

## Architectural decision: independent of event timing

`registration-card` is **architecturally independent** of `event-card`'s date-driven state machine — sealed-folder rule. The host derives event status (using `getEventStatus` from `event-card` if needed) and passes `closed: true` when registration should end.
...
- **`registration-card`** — is registration open right now? (`open` / `lastSpots` / `full` / `closed`) — uses `(capacity, registered, closed)`
