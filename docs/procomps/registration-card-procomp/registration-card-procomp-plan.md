# registration-card — procomp plan

> Stage 2: how. The implementation contract.
>
> See [`registration-card-procomp-description.md`](./registration-card-procomp-description.md) for the what & why.
...
```ts
// src/registry/components/data/registration-card/types.ts
...
export interface RegistrationCardLabels {
...
export const DEFAULT_REGISTRATION_CARD_LABELS: Required<RegistrationCardLabels> = {
...
```ts
// src/registry/components/data/registration-card/lib/registration-status.ts
...
```
src/registry/components/data/registration-card/
├── registration-card.tsx         # 1 — root
├── lib/
...
export const trLabels: RegistrationCardLabels = {
...
- `@/components/ui/progress` — already installed (event-card first user)
...
- **NOT** event-card — sealed-folder rule. Status concerns are independent.

## Composition pattern

Single-file root component (no `parts/` — small enough that splitting would over-engineer). Helper kernel in `lib/`. Same shape as progress-timeline.
...
- [ ] `pnpm build` clean — `/components/registration-card` prerendered
...
- [ ] Helper-only import works: `import { deriveRegistrationStatus } from "@/registry/components/data/registration-card"` typechecks without React imports

## Risks & alternatives

### Risk 1: Sealed-folder rule means `EVENT_STATUS_CONFIG` from event-card isn't reusable here

The kasder source uses event-card's status to derive the registration CTA. Pro-comp can't import across components. **Mitigation:** registration-card has its own simpler 4-state machine; host integrates the two at page level (passes `closed: getEventStatus(event) === "expired"` etc.). Documented in description. The two state machines have different concerns — event lifecycle vs registration window — and separating them is architecturally cleaner.

### Risk 2: Default share button might be too narrow for some consumers

The default ships ONE outline button with `Share2` icon + "Share" label. Consumers wanting Twitter / LinkedIn / WhatsApp / Email shares need `actions` slot with `share-bar`. **Mitigation:** documented + the actions slot is the explicit escape hatch; share-bar is one of the related components.
...
1. **Cross-import event-card's `EVENT_STATUS_CONFIG`** — rejected (sealed-folder rule).
2. **Merge into event-card as a "registration variant"** — rejected (different layout, different interaction model — separate component is honest).
...
- v0.2: built-in countdown timer for time-bound registrations (would compose `progress-timeline` internally? — sealed-folder forbids it; alternative: render-prop slot for countdown content)
