# signup-form — procomp plan

> Stage 2: how. Implementation contract.
>
> **Predecessor:** [`signup-form-procomp-description.md`](./signup-form-procomp-description.md), GATE 1 signed off 2026-05-22 with seven refinements folded back into the spec (envelope payload, off-screen honeypot, mutual-exclusion docs, `strengthCalculator` seam, fixtures-show-the-slot, prefers-reduced-motion, hand-roll lock).
>
> Batch: 2 of 2 in the CMS conversion-block batch. Sibling: [`pricing-table`](../pricing-table-procomp/) (in-flight; structure mirrored here).

## Substrate decisions (locked)

| Decision | Choice | Source |
|---|---|---|
| Slug | `signup-form` | Description, GATE 1 lock |
...
export interface SignupStep2Values extends SignupStep1Values {
  firstName?: string;
  lastName?: string;
  displayName?: string;
  phone?: string;
...
export type SignupSubmitPayload =
  | { stepCompleted: "single"; values: SignupStep2Values; isHoneypotTripped: boolean }
  | { stepCompleted: "step1"; values: SignupStep1Values; isHoneypotTripped: boolean }
  | { stepCompleted: "step2"; values: SignupStep2Values; isHoneypotTripped: boolean };
...
The default English labels live in `parts/labels.ts` and are merged with consumer overrides at the top of the component via `mergeSignupLabels()`.

## File-by-file plan

### 1. `signup-form.tsx` — root
...
- Uses `useSignupForm()` hook for the RHF + Zod glue.
...
- Exports `defaultSignupLabels: SignupLabels` (English defaults) + `mergeSignupLabels(override?: Partial<SignupLabels>): SignupLabels` helper (shallow merge over defaults).
...
- Return shape (explicit action surface, no opaque `transitions` bag):
  ```ts
  {
    ...
    status: SignupFormStatus;          // current (controlled or internal)
    step: "step1" | "step2";
...
- Signature: `useStrengthMeter(args: { calculator?: StrengthCalculator; labels: Pick<SignupLabels, "strengthWeak" | "strengthFair" | "strengthStrong" | "strengthExcellent" | "strengthLabel"> }): { score: 0|1|2|3|4; label: string }`.
...
- Reused by `parts/honeypot-field.tsx` + the submit handler in `hooks/use-signup-form.ts` (which reads the honeypot value via `form.getValues(HONEYPOT_RHF_KEY)` to set the `isHoneypotTripped` flag on the payload).
...
- Imported once at the root component (`import "./signup-form.css"`).
...
- Already shown above. Exported types: `SignupFormProps`, `OptionalFieldName`, `OAuthProvider`, `SignupFormStatus`, `SignupStep1Values`, `SignupStep2Values`, `SignupSubmitPayload`, `SignupLabels`.

### 25. `dummy-data.ts`

- Six fixtures:
  - `defaultSignupProps` — minimal single-step + email/password + consent (no OAuth, no optional fields).
  - `oauthSignupProps` — single-step + Google + GitHub OAuth + email/password + consent + `oauthIcons` example wiring lucide-react `<Mail />` (Google fallback) + `<Github />`.
  - `twoStepSignupProps` — two-step + firstName/lastName optional in step 2 + skippable.
  - `magicLinkSignupProps` — magic-link strategy + email-only + Apple + Google OAuth.
  - `denseSignupProps` — `density: "compact"` + minimal fields for sidebar.
  - `controlledSignupProps` — `status` + `onStatusChange` example (demonstrates mutual-exclusion contract).
...
- Top-of-page caption: "Six common signup shapes. Tabs above; submit any form to see the discriminated payload."
...
- `slug: "signup-form"`, `name: "Signup Form 01"`, `category: "forms"`, `status: "alpha"`, `version: "0.1.0"`, `createdAt: "2026-05-22"`, `updatedAt: "2026-05-22"`.
...
- `description` + `features` follow the json-form / pricing-table pattern (load-bearing intro + bullet list of capabilities).
- `tags`: `["signup", "form", "auth", "oauth", "magic-link", "rhf", "zod"]`.

### 29. `index.ts`

- Re-exports `SignupForm` (the root component) + all public types from `types.ts` + `defaultSignupLabels` + `mergeSignupLabels` + `defaultStrengthCalculator`.
...
- [ ] `pnpm registry:build` produces `public/r/signup-form.json` + `signup-form-fixtures.json` cleanly
- [ ] Docs site renders at `/components` + `/components/signup-form` without console errors
...
- [ ] Path-b smoke: install `@ilinxa/signup-form` into the local `e:/tmp/ilinxa-smoke-consumer/` harness; consumer-tsc clean; component renders
...
11 commits, one logical concern each. Each prefixed `feat(signup-form):` unless noted.

| # | Subject | Files |
|---|---|---|
| C1 | scaffold + types + meta + index + empty parts | `src/registry/components/forms/signup-form/**` (sealed folder via `pnpm new:component`); `src/registry/manifest.ts` (3-line entry add) |
| C2 | lib + hooks — Zod schema builder, strength calc, honeypot const, step state, status state | `lib/build-schema.ts`, `lib/honeypot.ts`, `lib/oauth-providers.ts`, `lib/strength-calculator.ts`, `hooks/use-signup-form.ts`, `hooks/use-strength-meter.ts`, `hooks/use-form-step.ts` |
...
| C4 | parts (shell + state surfaces) — step-indicator, step-shell, server-error, success-screen, submit-row, sign-in-link, oauth-row, strength-meter, labels, CSS | `parts/step-indicator.tsx`, `parts/step-shell.tsx`, `parts/server-error.tsx`, `parts/success-screen.tsx`, `parts/submit-row.tsx`, `parts/sign-in-link.tsx`, `parts/oauth-row.tsx`, `parts/strength-meter.tsx`, `parts/labels.ts`, `signup-form.css` |
| C5 | root component — orchestrator threading | `signup-form.tsx`, `index.ts` (final re-exports) |
...
| C10 | GATE 3 spotcheck — author review file | `docs/procomps/signup-form-procomp/reviews/2026-05-22-v0.1.0-spotcheck.md` |
| C11 | STATUS.md + component-versions.md + decision file + manifest + meta-deps audit + final commit + push | `.claude/STATUS.md`, `.claude/decisions/2026-05-22-signup-form-v0.1.0-first-ship.md`, `docs/component-versions.md` |

**Estimated total LOC** ≈ 1,400 (root 200 + parts ~450 + hooks ~250 + lib ~150 + CSS 30 + types 120 + dummy-data 100 + demo 130). Comparable to `pricing-table` (in-flight) and `newsletter-signup` (~700 LOC).

---

**GATE 2 ask:** confirm the file-by-file structure, the commit chain, the dependency set, and the verification checklist. Once signed off, I scaffold with `pnpm new:component forms/signup-form` and start C1. No code lands before GATE 2 sign-off.
