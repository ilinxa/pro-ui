# newsletter-signup — procomp plan

> Stage 2: how. The implementation contract.
>
> See [description](./newsletter-signup-procomp-description.md) for what & why.
>
> **Migration origin:** [`docs/migrations/newsletter-signup/`](../../migrations/newsletter-signup/) — see [`analysis.md`](../../migrations/newsletter-signup/analysis.md) for design DNA + 12 numbered rewrite items.
...
```
src/registry/components/marketing/newsletter-signup/
├── newsletter-signup.tsx     # 1
├── parts/
│   ├── inline-form.tsx        # 2
│   └── cta-only.tsx           # 3
├── types.ts                   # 4
...
- Owns the status state (auto-derived) — `useState<NewsletterSignupStatus>('idle')`.
...
```ts
export const NEWSLETTER_SIGNUP_DEMO_LABELS_TR = {
...
- [ ] `pnpm lint` clean.
- [ ] `pnpm build` clean — all routes prerendered including `/components/newsletter-signup`.
...
1. **Compound API** (`<NewsletterSignup.Header>`, `<NewsletterSignup.Body>`, `<NewsletterSignup.Action>`). Rejected — too rigid for the small variant set; the 2 variants barely justify a single-variant prop.
