# `split-workspace` — Pro-component Guide (Stage 3)

> **Stage:** 3 of 3 · **Shipped:** v0.1.0 (alpha) · **Date:** 2026-04-27
> **Slug:** `split-workspace` · **Category:** `layout`
> **Files:** [src/registry/components/layout/split-workspace/](../../../src/registry/components/layout/split-workspace/)
> **Detail page:** `/components/split-workspace` (after `pnpm dev`)

The consumer-facing reference. The description ([split-workspace-procomp-description.md](split-workspace-procomp-description.md)) explains *why*; the plan ([split-workspace-procomp-plan.md](split-workspace-procomp-plan.md)) explains *how we built it*; this doc explains *how to use it*.

---

## When to use Split Workspace
...
## When NOT to use Split Workspace
...
- Mobile-first apps. Split Workspace collapses to a card stack on phones — the gesture model assumes desktop. If your users are 90%+ mobile, this is overkill.
- Apps where you (the developer) actually *want* one canonical layout. Split Workspace is for "we don't know what users will want, so let them decide."
...
export function App() {
  return (
    <div className="h-screen w-full">
      <SplitWorkspace components={components} defaultComponentId="chart" />
    </div>
  );
}
```
...
Full prop reference lives in [`types.ts`](../../../src/registry/components/layout/split-workspace/types.ts).
...
> `useAreaContext` throws if called outside a Split Workspace area's `render()` subtree. Don't try to read it from a parent.
...
```tsx
function makeMetricCard(metricId: string): SplitWorkspaceComponent {
  return {
    id: `metric-${metricId}`,
...
```tsx
const presets: SplitWorkspacePreset[] = [
...
Split Workspace fills its container. If the container has no height, the workspace is 0px tall. Common fixes:
...
### `aria-label` defaults to "Split Workspace"

The root has `role="application"` (appropriate for keyboard-driven workspace UIs) and `aria-label="Split Workspace"` by default. Override per page:

```tsx
<SplitWorkspace components={...} defaultComponentId="..." aria-label="Sales dashboard" />
```
...
<SplitWorkspace ... onLayoutChange={persist} />
```
...
<SplitWorkspace
  components={components}
  defaultComponentId="chart"
  onError={(errors) => toast.error(`Split Workspace validation: ${errors.join("; ")}`)}
/>
```
...
return <div ref={ref}><SplitWorkspace ... /></div>;
```
...
<SplitWorkspace
  ...
  onError={(errors) => {
    ...
    toast.error(`Split Workspace validation: ${errors.join("; ")}`);
  }}
/>
```
...
```tsx
<SplitWorkspace components={...} defaultComponentId="..." cardStackItemHeight={420} />
```
...
<SplitWorkspace layout={layout} onLayoutChange={setLayout} ... />
```
...
- `useResizeKeyboard` in `hooks/use-keyboard-actions.ts` is now `@internal`-tagged. It was never re-exported from `index.ts`, but consumers deep-importing it should stop — its inputs (`leaves` / `dividers` / `renderedTree`) are derived state internal to `<SplitWorkspace>`. Closes spotcheck F-03.
...
- **Imperative ref API** — landing in v0.2.0 (`SplitWorkspaceHandle` mirroring `TaskTreeHandle`).
...
- **Description (what & why):** [split-workspace-procomp-description.md](split-workspace-procomp-description.md)
- **Plan (how it was built):** [split-workspace-procomp-plan.md](split-workspace-procomp-plan.md)
- **Source:** [src/registry/components/layout/split-workspace/](../../../src/registry/components/layout/split-workspace/)
- **Public API surface:** see `index.ts` in the source folder — `SplitWorkspace`, `useAreaContext`, plus 9 type exports (`AreaContext`, `AreaTree`, `AreaTreeLeaf`, `AreaTreeSplit`, `Breakpoint`, `ResponsiveValue`, `SplitOrientation`, `SplitWorkspaceComponent`, `SplitWorkspacePreset`, `SplitWorkspaceProps`). v0.1.2 added optional `SplitWorkspaceProps` fields `onError` + `cardStackItemHeight`; no new exported types or symbols. **Anything not re-exported by `index.ts` is internal and may change in any patch** — e.g. the `useResizeKeyboard` hook in `hooks/use-keyboard-actions.ts` is `@internal`-tagged in v0.1.3 and exists only to keep `split-workspace.tsx` lean; do not deep-import it.
- **Dummy registry used in the demo:** [src/registry/components/layout/split-workspace/demo.tsx](../../../src/registry/components/layout/split-workspace/demo.tsx) — Notes, Clock, Counter, Data Table. Demo exercises `cardStackItemHeight={420}` + `onError` callback rendered as an alert beneath the canvas + the new divider-keyboard hint in the inline copy.
