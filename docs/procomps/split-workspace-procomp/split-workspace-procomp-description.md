# `split-workspace` — Pro-component Description (Stage 1)
...
- **Split Workspace presets** — named saved layouts the user can switch between via tabs.
...
type SplitWorkspaceProps = {
  ...
  presets?: SplitWorkspacePreset[];         // optional named layouts shown as tabs
...
10. **Compiles and renders** at `/components/split-workspace` with no console warnings.
...
|---|---|---|---|
| Q1 | **Slug name** — `split-workspace` vs alternatives | **`split-workspace`** ✓ | Confirmed. Preset-tab UI labels can use "Layouts" if the term overlap matters in chrome strings. |
...
- [ ] **User explicitly says "description approved" (or equivalent)** — this unlocks Stage 2 (`split-workspace-procomp-plan.md`).
