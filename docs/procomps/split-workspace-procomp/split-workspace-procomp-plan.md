# `split-workspace` — Pro-component Plan (Stage 2)

> **Stage:** 2 of 3 · **Status:** Signed off + shipped (v0.1.0 → v0.1.1 → v0.1.2 → next: v0.2.0).
> **Slug:** `split-workspace` · **Category:** `layout`
> **Inputs:** description signed off ([split-workspace-procomp-description.md](split-workspace-procomp-description.md)). All twelve description-stage decisions are inherited as fixed inputs.
...
export type SplitWorkspaceProps = {
  ...
  "aria-label"?: string;       // landmark name on the root region; default "Split Workspace"
  className?: string;
};
...
// Throws if called outside a Split Workspace area's render() subtree.
...
```
src/registry/components/layout/split-workspace/
...
**Split Workspace's pattern: registry + render fn.** Consumers register `SplitWorkspaceComponent`s; each declares a `render()` returning `ReactNode`. This is closer to "headless host + consumer-supplied content" than to render-props because the workspace is the host, not the consumer.

Sub-components (`Area`, `AreaHeader`, etc.) are **all private** — not exposed. The `<SplitWorkspace>` is the only public component. We don't ship a compound component (`<SplitWorkspace.Area>` etc.). This keeps the surface minimal; advanced composition can come later if real consumers need it.
...
- `demo.tsx` — server component that just renders `<SplitWorkspace>` with the dummy registry. The workspace's "use client" boundary handles hydration.
...
None. `split-workspace` doesn't compose other registry components.
...
- **Landmark structure:** the workspace root has `role="application"` (acceptable for workspace-style UIs) with its `aria-label` sourced from the `aria-label` prop on `<SplitWorkspace>` (default: `"Split Workspace"`). Document this prop in usage.
...
2. Open a STATUS.md note recording the test-debt for `split-workspace`
3. `pnpm new:component layout/workspace`
4. Implement against this plan, file by file in the order listed in §4
5. Author `split-workspace-procomp-guide.md` (Stage 3) alongside the implementation
