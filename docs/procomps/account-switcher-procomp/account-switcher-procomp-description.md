# `account-switcher` — Pro-component Description (Stage 1)

> **Stage:** 1 of 3 · **Status:** 🟡 Drafted, awaiting sign-off
> **Slug:** `account-switcher` · **Category:** `navigation`
...
>
> **Sibling-of:** `app-sidebar` v0.2.0. Designed to mount inside app-sidebar's new `topSlot` (above the brand). No hard dependency — works standalone, slotted anywhere the consumer needs a "current context + switchable other contexts + create-new affordance" UI.
...
- **`isCollapsed?: boolean`** — when true, the trigger collapses to icon-only mode (no label, no chevron). The popover content still shows full labels (the trigger collapses; the popover doesn't). Designed for slotting into `app-sidebar`'s collapse-to-icon mode.
...
The same SaaS teams who consume `app-sidebar`. Specifically:
...
```tsx
import { AccountSwitcher } from "@/registry/components/navigation/account-switcher";
...
### 5.1 Workspace switcher in a app-sidebar topSlot
The canonical pairing. Consumer wires memberships → items array, derives activeKey from pathname, slots into `<AppSidebar topSlot={...} />`. Demonstrates the full multi-context UX end-to-end.
...
8. **Composes with app-sidebar.** Drop into `<AppSidebar topSlot={<AccountSwitcher ... />} />` and the styling cooperates (width, color tokens, collapsed-mode aware).
...
| **L10** | **Collapsed-mode trigger renders icon-only**; popover content shows full labels regardless. Designed to slot into `app-sidebar`'s icon-collapsed mode. Detail behaviors (popover side, focus return after icon-trigger close, label-hide aria-label fallback) locked at GATE 2 plan. |
...
| R5 | Collapsed-mode trigger and popover positioning interact badly when slotted in a fixed-width 80px sidebar. | Medium | Tested in the v0.1 demo against app-sidebar's collapsed mode. Plan stage spells out exact CSS. |
...
GATE 2 plan authoring (`account-switcher-procomp-plan.md`) begins now: file structure, commit chain, internal implementation strategy, demo plans.

---

## Appendix A — relationship to app-sidebar v0.2.0

`account-switcher` is the canonical occupant of app-sidebar v0.2.0's new `topSlot`. **No hard dependency** — account-switcher ships standalone in the registry under `@ilinxa/account-switcher`. Consumers who use only one of the two install only the one they want.

When both are used together:

- account-switcher reads its `isCollapsed` from the consumer (passed through from app-sidebar's collapsed state via a render-prop or via a small consumer-side context).
- app-sidebar doesn't import anything from account-switcher. It just accepts `topSlot: ReactNode`.

This matches the `task-card` ↔ `task-tree` sibling pattern shipped 2026-05-20/21: two independently-versionable procomps that compose without hard coupling.
...
| **I-7** | **Collapsed-mode (icon-only) trigger** | Source has no collapse mode — desktop sidebar is fixed-width. | Library exposes `isCollapsed?` so the trigger can render as a 40px icon-only button when slotted into app-sidebar's collapse-to-icon mode (an ilinxa extension beyond the source). | L10 |
