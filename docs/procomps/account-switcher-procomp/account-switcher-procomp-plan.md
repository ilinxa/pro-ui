# `account-switcher` — Pro-component Plan (Stage 2)

> **Stage:** 2 of 3 · **Status:** 🟡 Drafted, awaiting sign-off
> **Slug:** `account-switcher` · **Category:** `navigation`
> **Companion to:** [`account-switcher-procomp-description.md`](account-switcher-procomp-description.md) (GATE 1, closed 2026-05-23)
> **Sibling-of:** `app-sidebar` v0.2.0 (GATE 2 plan: [`../app-sidebar-procomp/app-sidebar-procomp-plan-v0.2.0.md`](../app-sidebar-procomp/app-sidebar-procomp-plan-v0.2.0.md))
...
```
src/registry/components/navigation/account-switcher/
├── account-switcher.tsx          ← main component (~180 LOC, single file)
├── parts/
...
pnpm dlx shadcn@4.6.0 add @ilinxa/account-switcher @ilinxa/account-switcher-fixtures
pnpm tsc --noEmit
```
...
account-switcher has **zero** dependency on app-sidebar (per description §10). Sibling-of, not depends-on.
...
`pnpm validate:meta-deps` audit grows from 48 → 49 slugs at close (account-switcher added). Lint should be clean immediately.
...
|---|---|---|---|
| **C1** | `feat(account-switcher): scaffold + types` | `pnpm new:component navigation/account-switcher` + edit `types.ts` + initial `index.ts` barrel + `meta.ts` stub (empty deps to start, grow per §13 as imports land) | tsc clean; lint clean; new-component scaffolder ran |
| **C2** | `feat(account-switcher): pure lib helpers` | `lib/resolve-active-item.ts`, `lib/enforce-unique-keys.ts` | tsc clean; pure functions exercised at import time |
| **C3** | `feat(account-switcher): useControllableState hook` | `hooks/use-controllable-state.ts` | tsc clean; hook signature stable; no callers yet |
| **C4** | `feat(account-switcher): main component + parts` | `account-switcher.tsx`, `parts/switcher-trigger.tsx`, `parts/switcher-item-row.tsx`, `parts/empty-placeholder.tsx` | tsc clean; demo not yet wired; component compiles standalone |
| **C5** | `feat(account-switcher): dummy data + manifest + meta deps` | `dummy-data.ts`, edit `src/registry/manifest.ts` (3 lines printed by scaffolder), populate `meta.ts` deps to `["popover","button","separator"]` + lucide-react peer | tsc + lint + `validate:meta-deps` 49/49 clean |
| **C6** | `feat(account-switcher): demo + usage + dev page renders` | `demo.tsx`, `usage.tsx` | Dev server: `/components/account-switcher` → 200; all 3 demos (workspace-in-sidebar, topbar standalone, collapsed-mode) render |
| **C7** | `feat(account-switcher): registry.json entries + GATE 3 spotcheck + v0.1.0 ship` | `registry.json` (base + fixtures items), procomp guide.md, GATE 3 spotcheck review file, STATUS.md, `.claude/decisions/2026-05-23-account-switcher-v0.1.0-first-ship.md` | `pnpm registry:build` artifact clean; tsc + lint + meta-deps + build all clean; GATE 3 verdict ≥ Pass with follow-ups |
...
| **L7** | Aria pattern: combobox + button-list | §3.1, §11 | `account-switcher.tsx` |
...
|---|---|---|
| `account-switcher.tsx` | ~180 | main composition; absorbs Popover wiring + state + handlers |
