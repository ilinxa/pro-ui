# category-cloud — procomp description

> Stage 1: what & why.
>
> **Migration origin:** [`docs/migrations/category-cloud/`](../../migrations/category-cloud/) (kasder `NewsMagazineGrid.tsx` sidebar).
...
```tsx
<CategoryCloud items={["All", "Tech", "Design"]} value={cat} onChange={setCat} />
```

**3. Tag cloud with counts only on some items:**
```tsx
<CategoryCloud items={tags.map(t => ({ value: t.slug, label: t.name, count: t.count }))} />
```
