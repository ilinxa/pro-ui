# share-bar — procomp description

> Stage 1: what & why.
>
> **Migration origin:** [`docs/migrations/share-bar/`](../../migrations/share-bar/) (kasder `kas-social-front-v0` news detail page article column footer — `src/app/(platform)/news/[id]/page.tsx` lines 163–184)
>
> Sibling migration: [`article-meta`](../../migrations/article-meta/) — top-of-article meta strip from the same page.
...
```tsx
<ShareBar targets={[{ kind: "twitter" }, { kind: "copy" }]} />
```
...
10. **Verification** — `pnpm tsc --noEmit` + `pnpm lint` + `pnpm build` clean (1 pre-existing card-tree warning OK; no new); SSR `HTTP 200` for `/components/share-bar` with all demo tabs rendered.
