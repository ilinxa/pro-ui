# share-bar — procomp plan

> Stage 2: how. Implementation blueprint for [`share-bar-procomp-description.md`](./share-bar-procomp-description.md).

## File map

```
src/registry/components/marketing/share-bar/
├── share-bar.tsx            # root, "use client", memoized, owns copy state
├── parts/
...
export const SHARE_BAR_DEFAULT_LABELS: Required<ShareBarLabels>;
```
...
```ts
// parts/clipboard.ts (or inline in share-bar.tsx — small enough)
...
function ShareBarInner(props: ShareBarProps) {
  const {
    targets,
    url,
    title,
...
export const ShareBar = memo(ShareBarInner);
...
export const SHARE_BAR_DUMMY_URL =
  "https://ilinxa-proui.vercel.app/components/share-bar";
...
3. `pnpm build` clean — `/components/share-bar` prerendered
...
6. Registry artifacts at `public/r/share-bar.json` + `public/r/share-bar-fixtures.json`
