# team-progress-bar — consumer guide

> Stage 3: how to use it. Companion to the [description](./team-progress-bar-procomp-description.md) (GATE 1) and [plan](./team-progress-bar-procomp-plan.md) (GATE 2). Shipped v0.1.0, 2026-07-01. First component of the [`gamification-system`](../../systems/gamification-system/gamification-system-description.md).
...
- A **standalone "% done" drop-in** — `<TeamProgressBar team value />` with no other infrastructure.
...
- **A milestone editor or timeline** → the host owns milestone data + mutation; chapter/beat framing is the (future) `team-quest-log`.
- **A completion celebration overlay** → that's the (future) `team-feedback-loop` (E6). This bar may animate its fill, but the burst lives elsewhere.
...
It ships as a **light shadcn-style compound**. The batteries-included `TeamProgressBar` is just `TeamProgressBarRoot` + `TeamProgressBarLabel` + `TeamProgressBarTrack`. Compose the parts directly for a lighter / custom build — drop the `Label` and it tree-shakes away:
...
# fixtures (sample milestones): add @ilinxa/team-progress-bar-fixtures
```
