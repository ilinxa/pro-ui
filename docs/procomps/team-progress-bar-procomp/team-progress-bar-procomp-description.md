# `team-progress-bar` — Pro-component Description (Stage 1 / GATE 1)

> **Stage:** 1 of 3 · **Status:** DRAFT — pending GATE 1 sign-off
> **Slug:** `team-progress-bar` · **Category:** `gamification` · **Tier:** pro-component (light shadcn-style compound — see §0)
...
| **A — assembly** | `TeamProgressBar` | `Root` + `Track` + `Label`, gated by `showLabel` / `showTicks`. Contains no logic the parts don't. Demo + screenshot use this. |
...
- **Animated celebration on completion** — that's E6 (`team-feedback-loop`), a separate procomp. This bar may animate its fill transition, but the celebration overlay is not here.
...
- **Not a milestone editor or timeline.** Chapter/beat framing is E5 (`team-quest-log`); the editor is the host's.
...
```ts
// Re-declared locally in team-progress-bar/types.ts (no shared import in v1).
interface Milestone {
  id: string;
...
Illustrative; canonical shape lands in `src/registry/components/gamification/team-progress-bar/types.ts` at plan stage. **This is a small surface** — roughly **~10 feature concepts**, well under the ~25 ceiling. It deliberately stays tiny: the whole point is a drop-in bar.
...
```tsx
import { TeamProgressBar } from "@/registry/components/gamification/team-progress-bar";

<TeamProgressBar team={{ id: "T-001", name: "Team Aurora" }} value={62} />
```
...
} from "@/registry/components/gamification/team-progress-bar";
...
- [ ] **User sign-off** → Stage 2 (`team-progress-bar-procomp-plan.md`, GATE 2).
