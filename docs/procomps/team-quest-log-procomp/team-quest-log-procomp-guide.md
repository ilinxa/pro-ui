...
Install: `pnpm dlx shadcn@latest add @ilinxa/team-quest-log` (add `@ilinxa/team-quest-log-fixtures` for sample data).
...
- **Scheduling** — no time axis, no drag-to-reschedule, no date grid → `gantt-timeline` / `event-calendar`.
...
import { TeamQuestLogRoot, TeamQuestNameEditor } from "@/components/team-quest-log";
...
A ref on `TeamQuestLog` (or `TeamQuestLogRoot`) exposes:
