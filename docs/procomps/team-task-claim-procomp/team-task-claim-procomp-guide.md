# team-task-claim — consumer guide
...
Install: `pnpm dlx shadcn@latest add @ilinxa/team-task-claim` (add `@ilinxa/team-task-claim-fixtures` for sample data).
...
- Full board layout → `kanban-board`. Single-card detail → `task-card`.
...
```tsx
// Display-only:
<TeamTaskClaim teamId={team.id} members={team.members} value={task.choice} readOnly />
```
...
```tsx
import { OpenForAnyoneToggle, ClaimButton, AssigneeChip } from "@/components/team-task-claim";
...
`resolveTaskClaimState(value)` and `resolveMember(id, members)` / `initialsFor(...)` are exported so a hand-assembly resolves identically.

## Density

`density="compact"` (kanban) collapses labels to icons and truncates the assignee name (the avatar is the floor — the name never truncates to nothing; full name in a native `title`). `density="comfortable"` (default, card-tree row) is labelled.
...
`onEvent` (optional) emits `{ type: "task-claim.interaction", teamId, taskId }` on **meaningful committed interactions only** — toggle / claim / reassign / release — never on render, hover, or opening the picker. No SDK, no `next/*`.
