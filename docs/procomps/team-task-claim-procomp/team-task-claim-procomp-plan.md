# `team-task-claim` — Pro-component Plan (Stage 2 / GATE 2)

> **Stage 2 of 3 · Status: DRAFT — pending GATE 2 sign-off**
> **Slug:** `team-task-claim` · **Category:** `gamification` · **Tier:** pro-component · **Structure:** single-unit control with flat à-la-carte sub-parts (NOT a `Root`/context compound)
> **Element:** E4 — Task Choice / Volunteering · **SDT need:** Autonomy (catalogue C6)
> **Predecessor:** [`team-task-claim-procomp-description.md`](./team-task-claim-procomp-description.md) (GATE 1)
...
A small, droppable **autonomy affordance** for one team task: an "open for anyone" toggle, an "I'll take this" claim/volunteer action, and an assignee chip with a neutral release + reassign path. It renders **three states** — `open` / `claimed` / `unassigned` — plus the legal `assigned-and-open` edge, deterministically from a controlled `TaskClaimState` slice + a team `members` list. Releasing or reassigning is a **neutral, no-penalty transition** (the defining constraint). Ships as a **single-unit control** exposing flat à-la-carte sub-parts (`OpenForAnyoneToggle` / `ClaimButton` / `AssigneeChip`) under a logic-free `TeamTaskClaim` assembly — **no headless `Root`, no React context** (D-06 forbids requiring a provider; there is nothing cross-cutting to hold). Imports no other registry component (D-03).

---

## 2. Client vs server

**`"use client"` on every module that holds state, refs, effects, or event handlers** — i.e. `team-task-claim.tsx`, all `parts/*`. Pure `lib/*` (the state-resolution + member-resolution functions) and `types.ts` are **framework-free, no directive** (importable from a server component's type position).
...
**None (D-03).** The component imports **no** other registry component and re-declares the `TaskClaimState` / `TeamMember` slice locally in its own `types.ts` (§5 of the description). Therefore:
...
| `resolveTaskClaimState` | **pure helper** | `lib/state.ts` | — | `(value) => "open" | "claimed" | "unassigned"` — the deterministic resolver (description §4 table). Exported so a hand-assembly resolves identically. |
...
> **Subset paths the flat exports unlock (the soft-trigger answer):** a host wanting *only* the open-flag imports `OpenForAnyoneToggle`; a host wanting *only* the assignee display imports `AssigneeChip`; a host wanting the full three-state behavior imports `TeamTaskClaim`. Dropping the assembly drops nothing the parts need; dropping a part drops its `Switch`/`Command` import from that consumer's graph. **Tree-shaking is real** because each part is a separate module re-exported from `index.ts` and nothing is wrapped in a namespace object.

### 4.2 — Assembly is logic-free

`TeamTaskClaim` does exactly one non-trivial thing — call `resolveTaskClaimState(value)` and branch the JSX — and that resolver is **exported**, so the branch is reproducible by hand. It owns **no state** except the picker-open flag, which lives in `AssigneeChip` (the only part that needs it), not the assembly. A reviewer rejects any behavior that lives in the assembly but not in a part + the resolver.
...
```ts
// lib/state.ts
export type TaskClaimRenderState = "open" | "claimed" | "unassigned";

export function resolveTaskClaimState(v: TaskClaimState): TaskClaimRenderState {
...
export const isOpenFlagVisible = (v: TaskClaimState) => true; // always renderable; "claimed" doesn't hide it
```
...
Sealed folder under `src/registry/components/gamification/team-task-claim/` (the `gamification` category must be plumbed first — see §8 prerequisite). Sub-parts are flat à-la-carte modules under `parts/`; pure logic under `lib/`. **No `hooks/` folder** (the only internal state is the picker-open flag, local to `AssigneeChip` — no shared hook needed). **No `Root`/context module.**

| File | Contents |
|---|---|
| `team-task-claim.tsx` | **Assembly** `TeamTaskClaim` — `"use client"`. Calls `resolveTaskClaimState(value)`; renders the relevant sub-parts per the §5 table; fans props out. Emits `onEvent` (§8). Capability-gates each affordance on its callback (§9). Holds **no** state. |
| `index.ts` | Barrel — flat exports: `TeamTaskClaim`, `OpenForAnyoneToggle`, `ClaimButton`, `AssigneeChip`, `resolveTaskClaimState`, `resolveMember`, `initialsFor`, + all public types. **No namespace object.** |
| `types.ts` | Framework-free. Locally-declared `TaskClaimState`, `TeamMember` (D-03 — re-declared, NOT imported; byte-faithful to system §4). Plus `TeamTaskClaimProps`, `OpenForAnyoneToggleProps`, `ClaimButtonProps`, `AssigneeChipProps`, `TaskClaimInteraction`, `TaskClaimRenderState`, `TaskClaimEvent`. |
...
| `lib/state.ts` | Pure `resolveTaskClaimState` + `isOpenFlagVisible` (§5). |
| `lib/members.ts` | Pure `resolveMember` + `initialsFor` (§5). |
| `dummy-data.ts` | Fixtures: a `members: TeamMember[]` team + several `TaskClaimState` examples exercising **all three states + the legal edge + a stale-assigneeId** + a member with no `avatarUrl` (initials path). **Fixtures registry item.** |
...
export type TaskClaimInteraction =
...
export type TaskClaimEvent = { type: "task-claim.interaction"; teamId: string; taskId: string };
```

### 8.2 — The control props (locked)

```ts
export type TeamTaskClaimProps = {
  ...
  /** Telemetry (D-07). Emits { type:"task-claim.interaction"; teamId; taskId } on meaningful interactions only. */
  onEvent?: (e: TaskClaimEvent) => void;

  /** Display-only: shows current state, hides ALL actions. Default false — choice is the default (§5.2). */
  readOnly?: boolean;
  /** Density to fit a kanban card vs a card-tree row. Default "comfortable". */
...
- **Generic-over-host-task drift.** Mitigated by D-03 local slice (no `TaskItem`, no `<T>` generic) + the §12 grep for any `@/registry/...`/`../<other>` import. Accepted cost: the slice is duplicated from system §4 — plan keeps it byte-faithful.
...
1. `pnpm tsc --noEmit` clean · `pnpm lint` clean · `pnpm validate:meta-deps` clean (trivial — no internal/npm deps; **meta-deps gotcha:** if any `parts/*` file has a side-effect import, put the dep-declaring `from`-import first; never name a dep only in a comment — blackboard + content-composer lessons).
...
4. Docs render at `/components/team-task-claim`; all demo sections interactive (three states + edge + density + read-only + à-la-carte).
5. `pnpm registry:build`; spot-check `public/r/team-task-claim.json` + `team-task-claim-fixtures.json` (targets follow the locked `components/team-task-claim/<sub>` convention; **no** demo/usage/meta shipped; **no** `registryDependencies`).
...
8. **(Recommended) cross-backend consumer-tsc smoke:** `pnpm dlx shadcn add @ilinxa/team-task-claim` in a tmp consumer on **both** Radix and Base UI backends; `pnpm tsc --noEmit` clean post-install (the `popover`/`command`/`switch` controlled-prop divergence surface; F-cross-13).
9. **Pure-lib unit tests (Vitest — informed-defer per house convention, but `lib/*` is written test-ready):** `resolveTaskClaimState` for all four input shapes incl. the legal edge; `resolveMember` stale-id; `initialsFor` fallback.
...
|---|---|---|---|
| **D7** | Control layout | 🔒 **Inline cluster** — `OpenForAnyoneToggle` on the inline-start, primary action / assignee chip on the inline-end; `density` switches compact (icon-led, kanban) vs comfortable (labelled, card-tree row). | No — confirm at design polish only. |
...
- [ ] **User sign-off** → scaffold `gamification/team-task-claim` + implement (GATE 2 cleared).
