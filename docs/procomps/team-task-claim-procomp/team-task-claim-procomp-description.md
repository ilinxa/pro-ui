# `team-task-claim` — Pro-component Description (Stage 1 / GATE 1)

> **Stage:** 1 of 3 · **Status:** DRAFT — pending GATE 1 sign-off
> **Slug:** `team-task-claim` · **Category:** `gamification` · **Tier:** pro-component
...
> **Sibling, not a fork.** This is a **standalone, droppable affordance** — it does **not** extend, embed, or modify [`kanban-board`](../../../src/registry/components/data/kanban-board/) or [`task-card`](../../../src/registry/components/data/task-card/). Per system **Q3** (signed off) it ships as its own procomp so a host can drop it into *any* task card. Per **D-03** it imports no other registry component.
...
- Telemetry via optional `onEvent` (**D-07**): emits `{ type: "task-claim.interaction"; teamId; taskId }` on a meaningful interaction (open toggled, claimed, reassigned, released).
...
- **NOT a fork or modification of `kanban-board` / `task-card` / `task-tree`.** It is a *droppable control* a host wires into those cards (or any other card). It imports none of them (**D-03**).
- **No task list / board / drag-and-drop** — it controls one task's choice state, nothing more. The card around it is the host's.
- **No persistence / backend** — controlled; the host owns `TaskClaimState` and writes via the callbacks (**D-06**).
...
| **Tier-host gamification page** *(integration)* | [`src/app/systems/gamification-system/page.tsx`](../../systems/gamification-system/gamification-system-description.md) wiring all six elements | Same `TaskClaimState` shape as the host's example state; `onEvent` for the demo telemetry log |

Non-targets: full board layout (→ `kanban-board`), single-card detail (→ `task-card`), assignment *analytics* (out of scope — would drift toward per-member ranking, forbidden by §5.3).
...
```ts
// Declared locally in team-task-claim/types.ts — NOT imported.
...
Illustrative; the canonical shape lands in `src/registry/components/gamification/team-task-claim/types.ts` at plan stage. No imports from other registry components (**D-03**); the §4 types are declared locally.
...
export type TeamTaskClaimProps = {
  ...
  /** Telemetry (D-07). Emits { type: "task-claim.interaction"; teamId; taskId }. */
  onEvent?: (e: { type: "task-claim.interaction"; teamId: string; taskId: string }) => void;

  /** Display-only (still shows state; hides all actions). Default false — choice is the default. */
  readOnly?: boolean;
  /** Layout density to fit a kanban card vs a card-tree row. Default "comfortable". */
...
```tsx
import { TeamTaskClaim } from "@/registry/components/gamification/team-task-claim";
...
The same control sits identically inside a `kanban-board` card body or a `task-card` footer — the host owns the card, this owns the choice.
...
| # | Question | Decision |
|---|---|---|
| **D1** | **Standalone vs. fork** | 🔒 **Standalone procomp** (system Q3, signed off). Droppable into any task card; imports no other registry component (**D-03**). NOT a modification of `kanban-board` / `task-card`. |
...
| **D5** | **Telemetry** | 🔒 Optional `onEvent` emitting `{ type: "task-claim.interaction"; teamId; taskId }` (**D-07**). Library code stays env-free; host wires transport. |
...
| **D11** | **Slug / category** | 🔒 `gamification/team-task-claim` (system §2, Q1 signed off). New `gamification` category plumbed once before any element scaffolds (**D-01**, system-level). |
...
- **Generic-over-host-task drift.** The control must stay decoupled from any specific card shape — it consumes only the `TaskClaimState` slice + `members`, never a full `TaskItem`. Re-declaring the slice locally (**D-03**) avoids the cross-procomp import trap (`content-composer` F-01 lesson) — but means the slice is duplicated; accepted as the price of independence (system §7.3). Plan must keep the local slice byte-faithful to the system §4 model.
...
7. **Telemetry** — `onEvent` emits `{ type: "task-claim.interaction"; teamId; taskId }` on meaningful interactions only (**D-07**).
...
- [ ] Compact (kanban card — avatar-only chip, icon actions) · comfortable (card-tree row — labelled) · narrow-column truncation behavior.
...
- [ ] **User sign-off** → Stage 2 (`team-task-claim-procomp-plan.md`, GATE 2).
