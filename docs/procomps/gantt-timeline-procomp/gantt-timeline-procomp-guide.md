# `gantt-timeline` — Consumer Guide (Stage 3)

> **Stage:** 3 of 3 · **Version:** v0.4.0 · **Status:** alpha
> Install: `pnpm dlx shadcn@latest add @ilinxa/gantt-timeline` (pulls `@ilinxa/task-card` + `@tanstack/react-virtual` + `@dnd-kit/core` + `lucide-react`).

A read-only, fully-navigable project timeline (Gantt) over the canonical `TaskItem[]`. The time-axis sibling of `task-card` (cards) and `kanban-board` (columns).

---

## When to use

- A **"Timeline" tab** beside List (rich cards) and Board (kanban) — the same `TaskItem[]`, on a time axis.
...
- You need a **date-grid calendar** (month/week/day cells) → that's the planned `event-calendar`, not this.
- You need a **rich single-card editor** as the primary surface → that's `task-card`. (This gantt *embeds* it for detail edits when `editable`, but it's a timeline first; editing is opt-in.)
...
The component consumes `TaskItem[]` verbatim — the **same canonical type** `task-card` renders. Each bar is drawn from:
...
`children` become indented WBS sub-rows; a parent renders a **collapsible summary bar** spanning `min(child start) → max(child end)`. Color: status tone (`done`=gray, `blocked`=red, `active`=time-urgency ramp green→red, **imported from task-card**); `borderColor` overrides per item.
...
```tsx
import { GanttFullCardTooltip } from "@/components/gantt-timeline";
...
/>
```
`task-card`'s **value** is `React.lazy`-loaded only here — the default lightweight tooltip keeps it out of your bundle.
...
Opt in with `editable` (default **off** → byte-identical v1 read-only). Data stays **controlled** — every edit fires a typed event **and** `onChange(next)` with the full mutated `TaskItem[]`; echo it back into `data`.
...
| Double-click a bar | Open the detail editor (embedded `<TaskCard editable>`) |
