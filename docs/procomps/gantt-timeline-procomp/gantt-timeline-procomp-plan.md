# `gantt-timeline` — Pro-component Plan (Stage 2 / GATE 2)

> **Stage:** 2 of 3 · **Status:** 🟢 Reviewed (internal + external consistency pass, 2026-06-20) · approved to proceed → scaffold + implement
> **Slug:** `gantt-timeline` · **Category:** `data` · **Tier:** pro-component · **Structure:** shadcn-style compound
> **Predecessor:** [`gantt-timeline-procomp-description.md`](./gantt-timeline-procomp-description.md) (GATE 1, approved 2026-06-20)

This is the **how**. It is the contract the implementation must follow. Once signed off (**GATE 2**), `pnpm new:component data/gantt-timeline` scaffolds the folder and code begins. Nothing here re-opens GATE-1 decisions (D1–D12); it operationalises them.
...
A **read-only, fully-navigable** project timeline that lays the canonical `TaskItem[]` on a continuous time axis. Bars from effective start→end, `children` rolled into collapsible WBS summary rows, milestone diamonds, a two-tier auto-adapting header, a "now" line, filled status/urgency bars (ramp imported from `task-card`), and a **pannable/zoomable canvas** (drag-pan + momentum + boundary resistance, continuous focal-point pinch/wheel zoom). Ships as a **compound**: `GanttTimelineRoot` (headless) + flat parts + Tier-C primitives + `GanttTimeline` assembly. State model is **architected so v2 drag-to-reschedule drops in** without a rewrite.

---

## 2. Client vs server

**`"use client"` on every module that holds state, refs, effects, or gestures** — i.e. `gantt-timeline.tsx`, all `parts/*`, all `hooks/*`. Pure `lib/*` (math, scale, flatten, color) and `types.ts` are **framework-free, no directive** (importable from a server component's type position).

Justification: pointer/wheel/pinch gesture handlers, `useReducer` state, the `now`-tick interval, `@tanstack/react-virtual`, and imperative `useImperativeHandle` all require the client. The component still server-renders (client components SSR + hydrate in Next 16); **SSR determinism** is preserved by seeding the "now" line + urgency from the `now` prop on first paint when provided, and — when `now` is omitted — deferring now-dependent visuals to **post-mount** (a `mounted` flag) so the server and first client render match (the `card-tree` hydration-mismatch lesson — never `new Date()`/`Date.now()` during render). The tick interval only starts in a client effect.
...
- `@tanstack/react-virtual` — **already a dependency** (task-tree / file-tree / file-manager). Row virtualization. No new package.
...
### 3.3 — internal registry dependency: `task-card` *(load-bearing — locks the §8/description risk)*

- **Imports (types, value):** `TaskItem`, `TaskStatusOption`, `TaskPriorityOption`, `TaskLabelOption`, `TaskColorRamp` (types) + `RAMPS` (value) — from the **barrel** `../task-card` (same-category **relative** import; rewriter-safe — it only strips category from `@/registry/...` static imports, never relative ones).
- **`registryDependency` MUST be declared in BOTH `meta.ts` and `registry.json`** (`@ilinxa/task-card`). Even though the type imports are erased at build, the **relative `../task-card` path must resolve in a consumer install** → `task-card` must be installed. *Type-only ≠ skip-the-dep* (the `content-composer` F-01 lesson, now locked here).
- **Lazy boundary:** the **full-card** tooltip path is `React.lazy(() => import("../task-card").then(m => ({ default: m.TaskCard })))`, mounted only when a consumer passes `renderTooltip` that opts into it (via an exported `<GanttFullCardTooltip>` helper). The default lightweight `BarTooltip` never imports the card **value** → `task-card`'s runtime weight stays out of the default bundle. (The *types* are compile-time only; they don't add runtime weight.)
- **`validate:meta-deps`** must pass: `task-card` appears in `meta.ts` deps AND is actually imported. ✅ by construction.

---

## 4. Composition pattern — the compound (export surface)

Per [`.claude/rules/compound-component-structure.md`](../../../.claude/rules/compound-component-structure.md). **Flat exports, never a `GanttTimeline.Root` namespace.** Each part module co-locates a **dumb Tier-C core** + a **thin Tier-B context wrapper** (the media-library one-file-two-exports pattern).
...
| `GanttFullCardTooltip` | **C (lazy host)** | `parts/bar-tooltip-full.tsx` | no | opt-in helper that lazy-loads `<TaskCard>`; for consumers who want the full card in `renderTooltip`. |
...
| `GanttTimeline` | **A (assembly)** | `gantt-timeline.tsx` | no (composes B parts) | `Root` + `Toolbar?` + `Gutter` + `Axis` + `Body`; only `Toolbar` is `show*`-gated (the core three always mount in the assembly; drop them only via hand-assembly). **Contains no logic the parts don't.** Demo + screenshot use this. |
...
- `task-card`'s **value** (`TaskCard`) enters the graph **only** via `GanttFullCardTooltip`'s `React.lazy`. Default usage (lightweight `BarTooltip`) ⇒ the card never loads. **Verified at GATE 3** by inspecting the built bundle / the lazy chunk boundary.
...
`GanttTimelineRoot` is the single source of state + the cross-cutting gesture context. `GanttTimeline` is a fixed child tree with `show*` toggles and **zero** state of its own — so a hand-assembled layout (description §6.3) gets identical behavior. A reviewer rejects any logic that lives in the assembly but not the parts.
...
- **Boundary resistance** — past the data extents (fit range ± padding), applied delta ×0.25; momentum reaching a boundary eases to rest (no hard clamp jerk). Mirrors `story-viewer`.
...
Sealed folder under `src/registry/components/data/gantt-timeline/` (compound layout; `parts/` co-locates Tier-B wrappers + Tier-C cores per the house pattern):

| File | Contents |
|---|---|
| `gantt-timeline.tsx` | **Tier A** `GanttTimeline` assembly — `"use client"`; `Root` + `show*`-gated parts; the only public default-layout entry. |
| `index.ts` | Barrel — flat exports (every Tier A/B/C name + `useGanttTimeline` + all public types). Re-exports `TaskItem` + option types from `../task-card` for consumer ergonomics (tail re-export; rewriter-safe). |
| `types.ts` | Framework-free. `GanttZoom`, `GanttTimelineProps`, `GanttTimelineRootProps`, `GanttTimelineHandle`, `Viewport`, `GanttRow`, `GanttNode`, context value type, primitive prop types. Imports `TaskItem` etc. as **types** from `../task-card`. |
...
| `parts/bar-tooltip-full.tsx` | **Tier C (lazy)** `GanttFullCardTooltip` — `React.lazy` `<TaskCard>`; the only module importing the card **value**. |
...
| `dummy-data.ts` | `TaskItem[]` fixture: a deep epics→tasks→subtasks tree exercising every state (overdue, blocked, done, milestone, inactive, locked, borderColor override, all-milestone parent). **Fixtures registry item.** |
...
| `meta.ts` | Full `ComponentMeta`; `registryDependencies: ["@ilinxa/task-card"]`; dep list incl. `@tanstack/react-virtual` + shadcn primitives. |
...
- `colorRamp?: TaskColorRamp` default `"default"` → resolved via `lib/color.ts` over imported `RAMPS`.
- `now?: Date | (() => Date)`. If provided, it **seeds first paint deterministically** (SSR-safe). If omitted, now-dependent visuals (today line + urgency tint) compute **after mount** via a `mounted` flag so server and first client render match (the `card-tree` SSR-determinism lesson — no `new Date()` during render); the `colorRefreshIntervalMs` tick then keeps them fresh.
...
- **Generics:** none. `TaskItem` is a fixed schema (not generic) — matches `task-card`. No `<T>`.
...
- **Structure:** the chart is a labelled region (`role="group"` + `aria-label`); rows form a **tree** in the gutter (`role="tree"`/`treeitem`/`aria-expanded`/`aria-level`) mirroring `task-tree`'s proven a11y.
...
1. `pnpm tsc --noEmit` clean · `pnpm lint` clean · `pnpm validate:meta-deps` clean (`task-card` + `@tanstack/react-virtual` declared & imported). **meta-deps gotchas:** in any file with side-effect imports, place the dep-declaring `from`-import FIRST (the audit regex stops at the first `from`); never reference a dep name only inside a comment (false-positive). (blackboard + content-composer lessons.)
2. `pnpm build` succeeds.
3. Docs render at `/components/gantt-timeline`; all demo tabs interactive.
4. `pnpm registry:build`; spot-check `public/r/gantt-timeline.json` + `gantt-timeline-fixtures.json` (targets follow the locked `components/gantt-timeline/<sub>` convention; no demo/usage/meta shipped; `task-card` in `registryDependencies`).
5. **Compound proof:** the demo's "Composed / lighter" tab renders a hand-assembled subset; bundle inspection confirms `TaskCard` value is behind the lazy boundary.
...
- [x] Dependencies + the `task-card` `registryDependency` wiring locked (§3.3).
...
- [x] **User approved to proceed** ("review and confirm… then move on") → scaffolding `data/gantt-timeline` + implementation **now in progress**.
