# `gantt-timeline` v0.2.0 — Pro-component Plan (Stage 2 / GATE 2)
...
> **Slug:** `gantt-timeline` · **Category:** `data` · **Bump:** v0.1.0 → **v0.2.0** (minor, public-API-touching)
> **Predecessors:** v0.2.0 [description](./gantt-timeline-procomp-description-v0.2.0.md) (GATE 1, approved 2026-06-20; D13–D22) + the v0.1.0 [plan](./gantt-timeline-procomp-plan.md) (the §4 compound surface + §5 geometry engine + §6 drag seam this builds on).
...
Make the v1 read-only Gantt **editable**, additively. Editing is gated behind `editable` (default `false` → byte-identical v1). Five capabilities: **direct bar manipulation** (drag-move / edge-resize / milestone-drag), **task CRUD** (create via draw-on-canvas + gutter ＋ / delete / inline rename), **gutter reparent-reorder** (@dnd-kit, task-tree model), **detail editing** (embedded `<TaskCard editable>`), and the **shared permission matrix**. Data stays controlled — every edit fires a typed event + `onChange(next: TaskItem[])`; the consumer echoes. No new `TaskItem` fields. No internal data state (the Root computes the next forest from `props.data` and hands it over).
...
|---|---|---|
| `input` | gutter inline rename | the rename text field (mirrors task-tree's use) |
...
| Package | Used by | Why |
|---|---|---|
| `@dnd-kit/core` `^6.3.1` | gutter reparent/reorder | the exact dep + sensor set task-tree uses; already a consumer peer via task-tree/kanban |

No date lib (still epoch-ms + `Intl`). `@dnd-kit/sortable`/`utilities` **not** needed — task-tree proved `@dnd-kit/core` alone suffices for tree reparent.

### 2.3 — internal (unchanged)

`task-card` stays the single internal `registryDependency`. v2 imports MORE from it (now also the **event types** + **`TaskPermissions`/`TaskPermissionRule`**) via the same rewriter-safe relative `../task-card`. The **value** import (`<TaskCard>` in editable mode) stays behind `React.lazy` in the edit-popover module — the card's runtime weight loads only when a consumer opens an editor.
...
Pure, framework-free forest operations over `TaskItem[]` — the array analogue of `task-card`'s reducer mutations, returning a **new** forest (structural sharing; never mutate in place):

```ts
buildIndex(data): Map<id, { item; parentId: string|null; index: number; level: number }>
setWindow(data, id, patch: { startAt?; expireAt?; duration? }): TaskItem[]   // reschedule / resize / milestone-move
renameItem(data, id, name): TaskItem[]
addItem(data, parentId: string|null, item, index?): TaskItem[]               // null parent = root
removeItem(data, id): { next: TaskItem[]; removed: TaskItem; parentId: string|null }
moveItem(data, id, newParentId: string|null, newIndex): TaskItem[]           // reparent + reorder
...
Mirror of `task-tree/lib/permissions.ts`. Maps gantt actions onto `task-card`'s `TaskPermissionRule` keys and resolves against `TaskPermissions` (`default`/`byLevel`/`byItem`/`inherit`) + `item.locked`:
...
### 4.3 — gutter reparent/reorder (`@dnd-kit/core`, task-tree model)

A `<DndContext>` wraps the gutter track (Mouse + Touch + Keyboard sensors). Edge-zone drop math (top 25% sibling-before / middle 50% reparent-as-last-child + auto-expand / bottom 25% sibling-after, 8px cap) lifted from task-tree. Drop targets gated by `canDropAsSibling`/`canDropIntoChildren` (→ `evalGanttPermission(... "create"/"move")`); circular drops banned via `isAncestor`. Virtualization **suspends during drag** (task-tree precedent; `use-gantt-virtual.ts` gains a `suspended` flag). Commit → `moveItem` → `onItemMoved` + `onChange`.
...
| `parts/gantt-edit-popover.tsx` | **new** — `popover` hosting the lazy `<TaskCard editable value onChange permissions>`; splices the edited subtree back + fires events. (Supersedes/extends `bar-tooltip-full.tsx`'s lazy import — keep one lazy boundary.) |
...
| `gantt-timeline.tsx` | pass `editable` + edit handlers through; empty-state CTA can create the first task when editable. |
...
- `onChange?: (data: TaskItem[]) => void` — full mutated forest after any edit.
...
- Permissions: `permissions?: TaskPermissions` + `canMoveItem`/`canResizeItem`/`canDeleteItem`/`canCreateChild`/`canEditItem` + `onPermissionDenied`.
...
4. `registry:build`; artifact spot-check (no demo/usage/meta; `popover`/`input` in `registryDependencies`; `@dnd-kit/core` in `dependencies`; relative `../task-card` preserved).
...
- [x] Gesture arbitration order locked (§4.1); keyboard scope split (§4.2); gutter DnD = `@dnd-kit/core` task-tree model (§4.3).
