# `gantt-timeline` — Pro-component Description (Stage 1 / GATE 1)

> **Stage:** 1 of 3 · **Status:** 🟢 Reviewed (internal + external consistency pass, 2026-06-20) · approved to proceed → GATE 2
> **Slug:** `gantt-timeline` · **Category:** `data` · **Tier:** pro-component (ships as a **shadcn-style compound** — see §0)
...
> **Display sibling of [`task-card`](../../../src/registry/components/data/task-card/) and [`kanban-board`](../../../src/registry/components/data/kanban-board/).** It does **not** extend or embed either — it is a fresh, standalone procomp that consumes the **same canonical `TaskItem[]`** the card renders, and lays the items out on a horizontal time axis instead of in cards or columns. *Same data genes, different surface.* The fourth member of the project-management set; its twin, **`event-calendar`, is the next procomp (not yet built)**.
...
| **A — assembly** | `GanttTimeline` | `Root` + the parts above, gated by `show*` toggles. Contains no logic the parts don't. Demo + screenshot use this. |

**Tree-shaking story:** each part is its own module re-exported from the barrel. Row virtualization is isolated; the **full-card tooltip path** (`renderTooltip` → `task-card`) is `React.lazy` so a consumer who keeps the default lightweight tooltip never pulls `task-card` into their graph. A read-only, gutter-only, or body-only subset must fall out for free.
...
**We already carry the data for this.** The canonical `TaskItem` — the JSON that drives `task-card`, nests in `task-tree`, and can render inside a `kanban-board` column via the card's `taskCardKanbanRenderer` adapter — ships `setAt` / `startAt` / `expireAt` / `duration`, a consumer-defined `status`, `priority`, `labels`, people, and infinite `children`. What's missing is the **primitive that lays those items on a time axis** — a Gantt/timeline that:
...
3. paints **urgency + status** onto each bar using the **same color engine** as `task-card` (imported, not re-implemented),
4. and consumes **`TaskItem[]` directly** — so a product's "Timeline" tab is literally the same task data its "List" (rich cards) and "Board" (kanban) tabs show, on a third surface.

Today every product hand-rolls this (1–3 weeks each, accessibility usually skipped). This procomp is that primitive.

> **Correction vs the original brief:** `kanban-board` does **not** consume `TaskItem[]` natively — it has its own `KanbanData` (columns + items) model and bridges to the card through `taskCardKanbanRenderer`. The honest shared spine is the **canonical `TaskItem` type**, which `task-card` and `task-tree` render directly and which this gantt consumes directly. "Same array as the board" is replaced by "same canonical item type."

### Release strategy — phased

| Version | Scope |
|---|---|
| **v1 (this doc)** | Read-only timeline. Bars from effective start→end, WBS hierarchy from `children` with collapsible summary bars, milestone diamonds, a **pan/swipe/zoom-navigable** two-tier time axis (continuous zoom; hour→quarter header buckets), today line, overdue treatment, status/urgency **filled** bars, hover tooltip, selection, keyboard nav. Consumes `TaskItem[]`. Standalone-usable. **State model architected for drag (§7-D3) — write callbacks reserved, not shipped.** |
...
| **v3** | Dependencies: a `dependsOn?: string[]` field on `TaskItem` + finish-to-start arrows (deferred, design-reserved §7-D1). |
...
**Rows & hierarchy**
- One **row per top-level `TaskItem`**. A row's `children` render as **indented sub-rows** (WBS / outline), to arbitrary depth.
...
- Collapse/expand is **UI-only state** (not part of the `TaskItem` schema) — exactly like `task-card`'s `collapsedIds`.
...
**Color** — reuse `task-card`'s engine (consistency is a hard requirement)
- **Filled bars** (locked §7-D5). Default fill comes from the item's **status tone** via consumer-supplied `statusOptions`: `active` tones use the **time-urgency ramp** (fresh green → red as the window elapses), `done` → gray, `blocked` → red. The ramp is imported from `task-card` (`TaskColorRamp` + its ramp lib), not re-derived.
...
- **Pan / swipe** the chart body by dragging: single-pointer drag with **dominant-direction lock** (horizontal = time, vertical = rows), **flick momentum**, and **boundary resistance** (×0.25) at the data extents — mirrors `story-viewer`'s swipe + `media-editor`'s drag-to-pan.
...
- Read-only: pan + zoom move the viewport only; they never mutate `TaskItem` data. (Drag-to-*reschedule* is the separate v2 feature — §7-D3.)
...
- **Hover** a bar → **lightweight tooltip** (name + dates + assignee + status) by default; a `renderTooltip` slot overrides it (and may lazy-embed the full `<TaskCard>`) — locked §7-D7.
...
**Portability**
- Zero `next/*` imports; no `process.env`; no app context. Registry-import rules: only `react`, `@/components/ui/*`, `@/lib/utils`, the declared third-party deps, and same-category relative imports of `task-card`. SSR-safe (the "now" line + urgency color compute from a `now` prop on first paint, then a client interval refreshes — same contract as the card's `colorRefreshIntervalMs`).
...
- **Dependencies / arrows** → v3 (the `TaskItem` schema has no dependency field; we do **not** add one in v1, locked §7-D1).
- **Progress %, baselines, critical path, resource levelling, swimlanes** → v4.
- **Recurrence** (no RRULE — consumer produces fresh `TaskItem`s).
...
- **Not a calendar.** Date-grid layout (month/week/day cells, agenda) is the planned sibling **`event-calendar`** (next procomp). This component is a continuous-axis bar chart with task rows.
- **Not a task editor.** It renders `TaskItem`s; editing is `task-card`'s / the list shell's job.
...
|---|---|---|
| **Project / task module** *(primary)* | A "Timeline" tab next to List + Board | Same `TaskItem[]` as the card/tree, on a time axis; WBS from `children`; urgency color matching the card |
...
Non-targets: calendar grids (→ `event-calendar`), single-card detail (→ `task-card`), status-flow boards (→ `kanban-board`).

---

## 4. Data structure — what every bar is drawn from

**The component consumes `TaskItem[]` verbatim.** No new data shape is introduced in v1. This is the canonical type (source of truth: [`src/registry/components/data/task-card/types.ts`](../../../src/registry/components/data/task-card/types.ts), re-exported from its `index.ts`). Treat this as the contract — every visual element maps to a field below.
...
type TaskItem = {
  ...
  targetPerson?: TaskPerson; // → assignee avatar in the gutter + tooltip
  creatorPerson?: TaskPerson;// → tooltip
  images?: TaskImage[];      // → tooltip only (not on the bar)
  links?: TaskLink[];        // → tooltip only
  ...
  children?: TaskItem[];     // → indented WBS sub-rows; parent becomes a summary bar
};

type TaskPerson = { id: string; name: string; avatar?: string };
type TaskImage  = { src: string; alt?: string; caption?: string };
type TaskLink   = { url: string; label?: string; icon?: string };
```

### Field → visual mapping (the design table)

| `TaskItem` field | Where it shows | Notes for design |
|---|---|---|
...
Illustrative. The canonical shape lands in `src/registry/components/data/gantt-timeline/types.ts` at plan stage; defer to it on naming. **Option + ramp types are imported from `task-card`** — same color/label language, zero duplication. (Internally this is a *same-category relative* import: `../task-card`; consumers installing both via the registry get the dependency declared in `meta.ts`.)

```ts
import type {
  TaskItem, TaskStatusOption, TaskPriorityOption, TaskLabelOption, TaskColorRamp,
} from "@/registry/components/data/task-card";

export type GanttZoom = "hour" | "day" | "week" | "month" | "quarter";

export type GanttTimelineProps = {
  /** The tasks to lay out. The canonical TaskItem[] the card/tree render. */
  data: TaskItem[];

  // Color + label language (shared with task-card)
  ...
  renderTooltip?: (item: TaskItem) => React.ReactNode;
...
```tsx
import { GanttTimeline } from "@/registry/components/data/gantt-timeline";
import { STATUS_OPTIONS } from "@/domains/tasks/status"; // consumer's own status palette

function TasksTimeline({ tasks }: { tasks: TaskItem[] }) {
  return (
    <GanttTimeline
      data={tasks}                       // same TaskItem[] as the List + Board tabs
...
The same `TaskItem[]` that renders as rich cards (List) and can render in kanban columns (Board) now renders as bars — `children` become indented WBS rows, a parent becomes a collapsible summary bar over its subtasks, and a blocked item past its `expireAt` shows the overdue red treatment.
...
} from "@/registry/components/data/gantt-timeline";
...
| # | Question | Decision |
|---|---|---|
| **D1** | **Dependencies** | 🔒 **No dependencies in v1.** The `TaskItem` schema has no dependency field and we do not add one now. Ordering is by date only. Arrows + a `dependsOn?: string[]` field → **v3**. |
...
| **D5** | **Bar color: fill vs border** | 🔒 **Filled bars** (user's GATE-1 call). Status-tone fill + time-urgency tint; overdue = red end-cap. Verified against light + dark `--card`/`--background`. The ramp engine is **imported from `task-card`**, not re-implemented. |
...
| **D7** | **Hover tooltip content** | 🔒 **Lightweight summary by default** (name + dates + assignee + status); `renderTooltip` slot overrides and may lazy-embed the full `<TaskCard>` (user's GATE-1 call). Keeps `task-card` out of the default bundle. |
...
| **D9** | **Slug / category** | 🔒 `data/gantt-timeline`. `-01` suffix follows `kanban-board`, `media-library`. |
...
- **Urgency-color consistency with the card.** Bars MUST use the same ramp/tones as `task-card` or the tabs look incoherent. Plan **imports** the card's color engine (`TaskColorRamp` + ramp lib), never re-implements it.
...
- **Frozen-gutter + horizontal-scroll perf** on large trees. Plan must **virtualise rows** — the house choice is `@tanstack/react-virtual` (already used by `task-tree`, `file-tree`, `file-manager`); confirm at plan stage.
...
- **`onWheel` is passive in React** (known `story-composer` trap): `preventDefault` on ⌘/ctrl+wheel zoom needs a **non-passive** native listener via `addEventListener`, not the React `onWheel` prop. Document it.
...
- **Cross-procomp dependency wiring** (`task-card`): the shared types (`TaskItem`, option types, `TaskColorRamp`) come from `task-card` via a **same-category relative import** (`../task-card`), which the registry rewriter leaves untouched (rewriter-safe). But `task-card` must still be a **declared `registryDependency`** (in `meta.ts` *and* `registry.json`) so the relative path resolves in a consumer install. **Type-only imports still require the package installed** for consumer-tsc — the `content-composer` F-01 lesson. Plan locks both.
...
1. **`TaskItem[]` renders** — every field in §4's table produces its mapped element.
...
9. **Color matches `task-card`** — the **time-urgency ramp** (active tones) is the *imported* engine and matches the card exactly; `done`/`blocked` tones are *adapted to filled bars* (done → gray, blocked → red) rather than the card's gray-border-plus-overlay treatment. The visual diff focuses on the ramp.
10. **Compound is real** — flat exports; a hand-assembled subset (§6.3) renders; default tooltip keeps `task-card` out of the bundle; the demo includes a "Composed / lighter" example.
...
- [x] Data structure pinned to the **real** canonical `TaskItem` (§4), verified against `task-card/types.ts`.
...
- [x] **User approved to proceed** ("review and confirm… then move on") → Stage 2 (`gantt-timeline-procomp-plan.md`, GATE 2) **now in progress**.
