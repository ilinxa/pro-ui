# `gantt-timeline` v0.3.0 — Pro-component Description (Stage 1 / GATE 1)
...
> **Predecessors:** v0.2.0 [description](./gantt-timeline-procomp-description-v0.2.0.md) (D13–D21) + [plan](./gantt-timeline-procomp-plan-v0.2.0.md), and the v0.2.1 polish (bracket-drag → pan). This doc records ONLY the **group-move** delta; everything not restated is inherited from v0.2.x unchanged.
...
Group-move mutates **existing fields only** (`startAt`, and `expireAt` for expireAt-driven leaves) — it is single-bar move applied to each scheduled leaf. No new `TaskItem` field, no new event shape, no new permission key. It reuses the `drag` rule and `onFieldEdited`/`onTaskReschedule` events already shared from `task-card`. (Same "adopt, don't invent" discipline as v0.2.0.)
