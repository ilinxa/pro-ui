# `gantt-timeline` v0.2.0 — Pro-component Description (Stage 1 / GATE 1)
...
> **Slug:** `gantt-timeline` · **Category:** `data` · **Tier:** pro-component (shadcn-style compound)
...
> **Predecessors:** v0.1.0 [description](./gantt-timeline-procomp-description.md) (D1–D12) + [plan](./gantt-timeline-procomp-plan.md) (the §6 "drag seam"). This doc records ONLY the deltas; everything not restated here is inherited from v0.1.0 unchanged.
...
The whole point of v1's D3 was that v2 drops in without an API/state rewrite. This holds, and it's now *over-delivered* by the ecosystem: **every edit event + the permission matrix v2 needs already exists as a shared type in [`task-card/types.ts`](../../../src/registry/components/data/task-card/types.ts)**, and [`task-tree`](../../../src/registry/components/data/task-tree/) already proved cross-procomp reuse of both (its `evalPermission` maps tree actions onto `task-card`'s `TaskPermissionRule`). So v2 doesn't invent an editing vocabulary — it **adopts the one the card + tree already speak**, importing it via the same rewriter-safe relative `../task-card`.
...
- **Reparent + reorder** (D16) — drag rows in the **gutter** to change order and hierarchy, reusing the `task-tree` DnD model (drop-as-sibling / drop-into-children, circular-drop ban).

**C. Detail editing (reuse, don't rebuild)**
- **Edit-in-card** (D17) — open the **already-lazy `<TaskCard editable>`** in a popover/sheet for `status` / `targetPerson` / `priority` / `labels` / `description` / color. The card emits its edited subtree; the gantt host splices it back into the forest. The gantt's `permissions` thread straight into the embedded card.
- **Quick status** — context-menu shortcut over `statusOptions` (sugar for the above).

**D. Edit infrastructure**
- **Per-item permissions** (D20) — adopt `task-card`'s `TaskPermissions` (`default`/`byLevel`/`byItem`/`inherit`) + per-action predicates (`canEditItem` / `canMoveItem` / `canResizeItem` / `canDeleteItem` / `canCreateChild`) + `onPermissionDenied`. `item.locked` blocks all edits (already in v1). A tiny gantt-side resolver maps gantt actions → `TaskPermissionRule` keys (`move`/`resize`→`drag`, `delete`→`remove`, `create`→`addChildren`, `editDetails`→`edit`), mirroring `task-tree/lib/permissions.ts`.
...
- **Controlled echo** (D14) — data stays controlled (v1 D3). Every edit fires its typed event **and** an `onChange(next: TaskItem[])` carrying the whole mutated forest; the consumer echoes it back into `data`. No internal uncontrolled-data mode.
...
type GanttEditProps = {
  ...
  // CRUD (event shapes reused from task-card)
  onItemAdded?:   (e: TaskItemAddedEvent) => void;     // create
  onItemRemoved?: (e: TaskItemRemovedEvent) => void;   // delete
  onItemMoved?:   (e: TaskItemMovedEvent) => void;     // reparent / reorder
  onFieldEdited?: (e: TaskFieldEditedEvent) => void;   // rename + reschedule keys
  onStatusChanged?: (e: TaskStatusChangedEvent) => void;

  // Permissions (reused from task-card; mirrors task-tree)
  ...
  onPermissionDenied?: (action: keyof TaskPermissionRule, itemId: string, reason: string) => void;
};
```
...
|---|---|---|
| **D13** | **Edit enablement / backward-compat** | 🟢 Single `editable` master toggle, **default `false`** → byte-for-byte v1 behavior for existing consumers. Affordances are capability-gated (handler present AND permission granted), exactly like `task-card`. |
...
| **D16** | **Reparent + reorder** | 🟢 **In v2**, via gutter DnD reusing the `task-tree` model (drop-as-sibling / into-children + circular-drop ban). *(User-confirmed.)* |
| **D17** | **Detail editing** | 🟢 **Reuse `<TaskCard editable>`** in a popover/sheet (the lazy boundary already exists from the v1 tooltip). No bespoke editor. *(User-confirmed.)* |
...
| **D20** | **Permissions + which bars edit** | 🟢 Adopt `task-card`'s `TaskPermissions` + predicates + `onPermissionDenied`; `locked` blocks all. **Summary bars are NOT directly draggable/resizable** (derived span — you edit children; the summary recomputes). Milestones **are** draggable. |
...
- **Forest mutation correctness.** No `parentId` means create/delete/move operate on nested `children`; the host needs a normalized `parentId`+`index` map (mirror `task-card`'s `TaskNode`). Reparent must ban circular drops (task-tree precedent). Summary spans + virtualization must recompute after every edit without a full remount.
- **Embedded-card round-trip.** The card edits a *single* `TaskItem` subtree; splicing its result back into the forest must preserve sibling order + identity. The card's `duration` is edited in whole minutes (lossy) — document it. Permissions must pass through consistently so the card doesn't offer an edit the gantt would deny.
...
3. Create (both gestures), delete, inline rename, and gutter reparent/reorder all fire the correct `task-card`-shaped events **and** a coherent `onChange(forest)`.
4. Detail editing mounts `<TaskCard editable>`, round-trips a subtree, and threads permissions.
5. The `TaskPermissions` matrix + predicates + `locked` gate **every** edit path (keyboard, pointer, DnD, card); `onPermissionDenied` fires with a reason. Summary bars are non-manipulable; milestones are.
...
- [x] Edit-surface inventory (§1) grounded in the **validated** shared vocabulary (`TaskPermissions` + event types from `task-card`; reuse proven by `task-tree`).
- [x] Data contract = no new fields (§2); controlled echo `onChange(TaskItem[])`.
...
- [x] **User sign-off (GATE 1)** — confirmed 2026-06-20 (D14/D19/D20/D21 recommendations accepted) → `gantt-timeline-procomp-plan-v0.2.0.md` (GATE 2) now in progress.
