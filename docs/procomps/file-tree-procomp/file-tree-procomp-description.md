...
1. **Slug — `file-tree` or `file-tree-01`?** Existing pattern is mixed: composed-surface flagships drop the suffix (`data-table`, `markdown-editor`, `entity-picker`, `properties-form`, `workspace`, `detail-panel`, `stat-card`, `filter-panel`, `pdf-viewer`); pattern-style numbered variants keep it (`media-carousel`, `kanban-board`, `story-viewer`, etc.). **Recommendation:** drop the suffix → `file-tree`. This is a flagship primitive (anticipated to be referenced by name, not numbered), aligned with `data-table` and `pdf-viewer` precedent.
