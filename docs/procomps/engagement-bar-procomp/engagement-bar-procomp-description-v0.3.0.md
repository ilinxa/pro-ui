# `engagement-bar` v0.3.0 — Description Addendum (Stage 1)
...
`engagement-bar` v0.2.x ships a binary heart only (`kind: "like"` with `liked?: boolean` + `count: number` — see [`types.ts:9-24`](../../../src/registry/components/data/engagement-bar/types.ts#L9-L24)). The `social-moduls-python` backend has a fully built + tested **multi-kind reaction model** (one reaction per user per content, choose from `{ love, laugh, wow, sad, angry, ... }`, with per-kind tallies + total + viewer's current reaction). The current UI surface cannot express it. Their only workaround is `kind: "custom"` × N buttons, which loses both the picker UX and the aggregate count.
...
2. **New `EngagementBarProps.reactionsPreview?: ReactNode` slot** — parallel to `likersPreview`, per ILX-2.
...
6. **1 new `EngagementBarHandle` method** — `triggerReaction: (kind: string | null) => void`. Plus 1 new read: `getCurrentReaction: () => string | null` (parity with `getCurrentState`).
...
- **`PostCard` ripple.** Covered in `post-card-procomp-description-v0.3.0.md` (queued separately). That ripple adds `Post.reactionCounts` + `Post.viewerReaction` + `onReact` handler + `reactions` branch in `defaultPostEngagementActions`.
...
- **`post-card`** — picks up `reaction` kind via its own v0.3.0 ripple (separate doc).
- **`comment-thread`** — future v0.3+ candidate (reactions on comments). Not blocked by this ship.
...
### 4.4 New `EngagementBarHandle` methods
...
### 4.5 New `EngagementBarProps` slot
...
`comment-thread` ripple is NOT in this minor. Reactions-on-comments is v0.4+ candidate.
...
| Cross-procomp ripple to post-card v0.3.0 acknowledged | ✅ 2026-05-28 |
| Effort estimate accepted | ✅ 2026-05-28 |

**Next step:** GATE 2 plan authored at `engagement-bar-procomp-plan-v0.3.0.md`. No code touches before plan sign-off.
