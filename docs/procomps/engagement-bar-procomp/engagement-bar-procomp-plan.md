# engagement-bar — procomp plan

> Stage 2: how. The implementation contract.
>
> See [`engagement-bar-procomp-description.md`](./engagement-bar-procomp-description.md) for the what & why.
>
> Migration origin: [`docs/migrations/social-posts-system/`](../../migrations/social-posts-system/) — kasder `PostEngagementPanel.tsx` (468 LOC). We extract only the action-row concern; comments → `comment-thread` (next ship), likers carousel → `likersPreview` slot.
...
| Q-P4 | Heart-burst sub-export path? | **`./parts/engagement-heart-burst.tsx` re-exported from `index.ts`.** Sibling CSS file `engagement-heart-burst.css` shipped as a `registry:file` peer; component imports the CSS sibling. Optional deep-import path: `@ilinxa/engagement-bar/parts/engagement-heart-burst` if Next's barrel-file analyzer ever trips for retrofit consumers. |
...
- **`<EngagementBar>` is `"use client"`** — owns optimistic state + ref handle + subscription effect.
...
- **Subscribe contract** identical shape to the one comment-thread will use (`Subscribe<T>` / `Unsubscribe`) — `Subscribe` type lives in this folder for now; if comment-thread ends up needing it, we promote to a shared utils path then. Until then: local.
...
```
src/registry/components/data/engagement-bar/
├── engagement-bar.tsx                # 1 — root component
├── parts/
...
- `useImperativeHandle` exposes `triggerLike` / `triggerBookmark` / `getCurrentState` / `reset`.
  ...
  - `getCurrentState()` returns the current `state` snapshot via a `currentStateRef` (stable identity pattern matching media-carousel).
...
Five tabs (matches media-carousel demo style):
...
5. **Heart-burst** — composes `<MediaCarousel>` (any variant) + `<EngagementHeartBurst trigger={burstKey} />` + `<EngagementBar ref={barRef}>`. Double-tap the carousel → burst overlays + like flips. The canonical Instagram flow.

### 16. `usage.tsx`

Same shape as expandable-text's `usage.tsx`: short markdown-ish prose + 4–6 code recipes. Cover:
- Minimal usage (post)
...
Already covered file-by-file. Match patterns established by `media-carousel` and `video-player`:
...
Two items: base (`engagement-bar`) + fixtures (`engagement-bar-fixtures`).
...
```jsonc
{
  ...
  "files": [
    { "path": "src/registry/components/data/engagement-bar/engagement-bar.tsx", "type": "registry:component", "target": "components/engagement-bar/engagement-bar.tsx" },
    { "path": "src/registry/components/data/engagement-bar/parts/action-button.tsx", "type": "registry:component", "target": "components/engagement-bar/parts/action-button.tsx" },
    { "path": "src/registry/components/data/engagement-bar/parts/like-action.tsx", "type": "registry:component", "target": "components/engagement-bar/parts/like-action.tsx" },
    { "path": "src/registry/components/data/engagement-bar/parts/comment-action.tsx", "type": "registry:component", "target": "components/engagement-bar/parts/comment-action.tsx" },
    { "path": "src/registry/components/data/engagement-bar/parts/share-action.tsx", "type": "registry:component", "target": "components/engagement-bar/parts/share-action.tsx" },
    { "path": "src/registry/components/data/engagement-bar/parts/bookmark-action.tsx", "type": "registry:component", "target": "components/engagement-bar/parts/bookmark-action.tsx" },
    { "path": "src/registry/components/data/engagement-bar/parts/view-count-action.tsx", "type": "registry:component", "target": "components/engagement-bar/parts/view-count-action.tsx" },
    { "path": "src/registry/components/data/engagement-bar/parts/custom-action.tsx", "type": "registry:component", "target": "components/engagement-bar/parts/custom-action.tsx" },
    { "path": "src/registry/components/data/engagement-bar/parts/engagement-heart-burst.tsx", "type": "registry:component", "target": "components/engagement-bar/parts/engagement-heart-burst.tsx" },
    { "path": "src/registry/components/data/engagement-bar/parts/engagement-heart-burst.css", "type": "registry:file", "target": "components/engagement-bar/parts/engagement-heart-burst.css" },
    { "path": "src/registry/components/data/engagement-bar/hooks/use-engagement-state.ts", "type": "registry:component", "target": "components/engagement-bar/hooks/use-engagement-state.ts" },
    { "path": "src/registry/components/data/engagement-bar/utils/format-count.ts", "type": "registry:component", "target": "components/engagement-bar/utils/format-count.ts" },
    { "path": "src/registry/components/data/engagement-bar/types.ts", "type": "registry:component", "target": "components/engagement-bar/types.ts" },
    { "path": "src/registry/components/data/engagement-bar/index.ts", "type": "registry:component", "target": "components/engagement-bar/index.ts" }
  ]
}
```

`engagement-bar-fixtures` is a tiny sibling: just `dummy-data.ts`, `registryDependencies: ["engagement-bar"]`, locked target convention.
...
1. **Build verification**
   ...
   - `pnpm lint` — clean
   - `pnpm build` — clean prerender (route count `/components/engagement-bar` added)
   - `pnpm registry:build` — generates `public/r/engagement-bar.json` + `public/r/engagement-bar-fixtures.json`; spot-check heart-burst CSS file shows up in the JSON's `files[]` entry with `type: "registry:file"`.
...
6. **Smoke test from a tmp consumer (one-off)** — `pnpm dlx shadcn@latest add http://localhost:3000/r/engagement-bar.json` from a tmp Next 16 app; verify the CSS file lands at the expected target path AND the burst animates.
...
6. `engagement-bar.tsx` root + `useImperativeHandle`.
...
12. Draft `engagement-bar-procomp-guide.md` (Stage 3, alongside this).
