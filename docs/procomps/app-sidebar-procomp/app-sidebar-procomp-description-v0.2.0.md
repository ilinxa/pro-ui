# `app-sidebar` v0.2.0 — Description Addendum (Stage 1)
...
Because none of the v0.1.x semantics change. Items that don't have `ownerOnly` / `minMembers` filter the same way as before (no filtering). Items without `{xxx}` placeholders in `href` render literally (no substitution). Consumers without a `topSlot` see no top zone. The slug stays `app-sidebar`; exports keep their `AppSidebar*` names.
...
1. **`topSlot?: ReactNode`** (new prop) — single named slot above the brand zone. Consumer mounts `<AccountSwitcher>` + any inter-section widgets (governance bars, breadcrumbs, status banners) here, stacked via Fragment if multiple. Renders nothing when omitted (v0.1 behavior).
...
- **A canned `<AccountSwitcher>` recipe** baked into app-sidebar. The switcher is a separate procomp (`account-switcher`); app-sidebar accepts it via `topSlot` and nothing more. (analysis §8.13 #4)
- **Bottom-tab-bar-01 integration.** Carried into a separate procomp's GATE 1.
- **Mobile menu sheet composition.** The composition is a guide.md recipe using v0.1's existing `<AppSidebar>` mobile drawer mode + `<Sheet>` from shadcn. No new prop needed.
...
import {
  ...
  type NavContext,
  type NavSection,
} from "@/registry/components/navigation/app-sidebar";

// Consumer-side: derive NavContext from URL (app-sidebar doesn't ship the hook)
...
Consumer renders their own arbitrary sidebar UI but uses `useFilteredNavSections` to do the filter logic. Demonstrates the hook's independence from `<AppSidebar>`.
...
| **L49** | **`<AppSidebar>`'s collapse-to-icon mode stays.** Source has no equivalent — ilinxa extension beyond the source. (analysis §8.1) |
...
|---|---|---|---|
| R8 | F-cross-13 surface unchanged from v0.1 (Tooltip + Sheet + DropdownMenu). No new Radix primitives in v0.2.0 unless we add a new slot uses Popover (not for app-sidebar's own scope; account-switcher ships that). | Low | No new pre-emption work in app-sidebar v0.2.0. |
...
| R12 | A consumer renders `topSlot` with content that includes its own `<nav>` element — could create nested-nav semantic confusion. | Low | Document: `topSlot` is INSIDE app-sidebar's `<nav>` — content should not introduce a second `<nav>` element. ARIA landmark guidance in guide.md. |
...
- [x] Re-validation pass (2026-05-23) — 11 findings on addendum applied (9 self + 2 cross-consistency); no further dynamicity-primacy gaps surfaced for app-sidebar (the two findings from that pass landed in the sibling account-switcher description).
- [x] User signed off 2026-05-23 ("Apply both — then accept all defaults").

GATE 2 plan authoring for v0.2.0 begins now at [`app-sidebar-procomp-plan-v0.2.0.md`](app-sidebar-procomp-plan-v0.2.0.md) (companion to this addendum). The plan will list the additive files, the migration shape inside `derive-visible-entries.ts`, the new hook implementation, the demo update, the type re-exports, etc.
...
5. This file (`app-sidebar-procomp-description-v0.2.0.md`) AND its companion plan (`app-sidebar-procomp-plan-v0.2.0.md`) are **deleted** as part of the v0.2.0 close commit; their content lives in the merged base description + plan.
...
## Appendix D — relationship to account-switcher

`account-switcher` ships SEPARATELY as a sibling procomp at version 0.1.0 (see [`docs/procomps/account-switcher-procomp/account-switcher-procomp-description.md`](../account-switcher-procomp/account-switcher-procomp-description.md)). The two procomps are **independently installable** via the shadcn registry; app-sidebar v0.2.0 does NOT pull account-switcher as a registry dependency.

The composition happens at the consumer's app code (drop `<AccountSwitcher>` into `<AppSidebar topSlot={...} />`). Documented as a recipe in app-sidebar's guide.md and demo'd on the docs site.

If consumers want to slot something else (custom workspace dropdown, command palette trigger, status indicator), the slot accepts anything. account-switcher is just the canonical occupant.
