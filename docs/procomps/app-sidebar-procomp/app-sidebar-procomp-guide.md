# `app-sidebar` — Consumer guide

> **Stage:** 3 of 3 (Consumer-facing usage + migration notes)
> **Current version:** `0.3.0` · **Status:** alpha
> **Companion docs:** [base description](app-sidebar-procomp-description.md) · [v0.2.0 addendum](app-sidebar-procomp-description-v0.2.0.md) · [v0.3.0 addendum](app-sidebar-procomp-description-v0.3.0.md)
...
```bash
pnpm dlx shadcn@4.6.0 add @ilinxa/app-sidebar
# Optional — adds the demo dummy data:
pnpm dlx shadcn@4.6.0 add @ilinxa/app-sidebar-fixtures
```
...
The `<AppSidebar>` itself is CSS-hidden below the `mobileBreakpoint` (default `lg`). To open the drawer from your app header, mount `<AppSidebarTrigger>`:
...
function AppShell() {
  return (
    <>
      <header>
        <AppSidebarTrigger className="lg:hidden" />
        {/* …other header bits… */}
      </header>
      <AppSidebar items={items} currentPath={pathname} />
    </>
  );
}
```

If `<AppSidebarTrigger>` is in a separate React subtree from `<AppSidebar>` (e.g., sibling of the sidebar, not descendant), pass an explicit `controls` ref:

```tsx
const sidebarRef = useRef<AppSidebarHandle>(null);

<AppSidebarTrigger controls={sidebarRef} />
<AppSidebar ref={sidebarRef} items={items} currentPath={pathname} />
```
...
|---|---|
| `"trigger"` | The companion `<AppSidebarTrigger>` was clicked (or a custom trigger called `toggleMobile("trigger")`). |
...
```tsx
import { type NavUserMenuItemSelectEvent } from "@/components/app-sidebar";
...
The new named alias `NavUserMenuItemSelectEvent` is re-exported from `@/components/app-sidebar` and is the recommended type for custom `onClick` callbacks.
...
- **`<AppSidebarTrigger>` is a sibling-aware companion**, not a wrapper. Place it anywhere in your header; it resolves the handle via React context (when nested inside `<AppSidebar>`'s subtree) or an explicit `controls` ref (when outside).
...
- [`@ilinxa/account-switcher`](../account-switcher-procomp/account-switcher-procomp-description.md) — canonical occupant of `topSlot` for multi-context SaaS shells.
