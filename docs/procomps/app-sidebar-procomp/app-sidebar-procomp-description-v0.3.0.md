# `app-sidebar` v0.3.0 — Description Addendum (Stage 1)
...
- **Two ⚠️ High bugs hit consumers immediately.** `<li><li>` nesting fires the moment any consumer uses the documented `renderItem={({ defaultRender }) => defaultRender}` slot pattern — invalid HTML, fails a11y scans, hydration warnings. `mobileOpenChange.reason` has been stuck on `"imperative"` since v0.1's C2 — the entire 5-case `AppSidebarMobileOpenReason` union is a lie to consumers who wired analytics or routing decisions to the reason discriminator.
- **Three 🔸 Medium quality issues.** A broken CSS.escape fallback (escapes only `"` and `\`), ~50 LOC of imperative-handle builder duplicated between `app-sidebar.tsx` and `use-sidebar-nav-state.ts` (already drifted once between v0.2.x bumps), and an unsafe `eventArg as React.MouseEvent` cast in `NavUser.onSelect`.
...
The handle-method widening from `closeMobile()` → `closeMobile(reason?: AppSidebarMobileOpenReason)` is ADDITIVE (optional parameter, default `"imperative"` preserves v0.2.x signatures); not breaking.
...
2. **F2 ⚠️ Reason discriminator plumbing** (C2). Add `lastMobileOpenReason` field to reducer state; SET / TOGGLE write it; Defense-1 effect reads `state.lastMobileOpenReason ?? "imperative"` for the microtask callback. Extend `AppSidebarHandle.openMobile` / `closeMobile` / `toggleMobile` with optional `reason?` parameter (additive). Wire reason at every carrier site: `item-click` (sidebar-nav-list click handler), `outside-click` + `escape` (Sheet's `onPointerDownOutside` + `onEscapeKeyDown`), `trigger` (companion `<AppSidebarTrigger>` button).
...
interface AppSidebarHandle {
  openMobile(reason?: AppSidebarMobileOpenReason): void;   // was: openMobile(): void
  closeMobile(reason?: AppSidebarMobileOpenReason): void;  // was: closeMobile(): void
  toggleMobile(reason?: AppSidebarMobileOpenReason): void; // was: toggleMobile(): void
...
19. **Imperative handle is built by a single shared `buildHandle()` factory** — `git diff` shows the same factory consumed in both `app-sidebar.tsx` and `use-sidebar-nav-state.ts` (proves the drift risk is closed).
...
| **Q26** | Cross-codebase `data-stage` sweep + fix in C3? | ✅ **YES — fix everywhere in C3.** Grep run 2026-05-23 returned ONE hit total across `src/` (the app-sidebar one). C3 scope expansion is therefore trivial in practice: deleting one attribute IS the cross-cutting sweep. Record in commit message that the grep was run and app-sidebar was the sole offender; no `F-cross-NN` entry needed because there is no cross-cutting class. |
...
When closed, GATE 2 plan authoring begins at [`app-sidebar-procomp-plan-v0.3.0.md`](app-sidebar-procomp-plan-v0.3.0.md) (companion to this addendum). The plan will lock the C1–C7 chain, the file-touch matrix, the verification ladder, and the GATE 3 spotcheck framing.
...
If a subsequent bump after v0.3.0 needs further breaking changes (e.g., removing the `AppSidebar.defaultMatch` prop or renaming `linkComponent`), that's the v0.x → v1.0.0 conversation — not on the near roadmap.
...
5. This file (`app-sidebar-procomp-description-v0.3.0.md`) AND its companion plan (`app-sidebar-procomp-plan-v0.3.0.md`) are **deleted** as part of the v0.3.0 close commit; their content lives in the merged base description + plan.
