# `app-sidebar` v0.3.0 — Plan Addendum (Stage 2)

> **Stage:** 2 of 3 · **Status:** ✅ Signed off 2026-05-23 — C1 in progress
> **Slug:** `app-sidebar` (unchanged) · **Target version:** `0.3.0`
> **Companion to:** [`app-sidebar-procomp-description-v0.3.0.md`](app-sidebar-procomp-description-v0.3.0.md) (GATE 1 v0.3.0, closed 2026-05-23)
...
> **Prior addendum:** [`app-sidebar-procomp-plan-v0.2.0.md`](app-sidebar-procomp-plan-v0.2.0.md) (v0.2.0 plan)
...
**Extend (ADDITIVE) `AppSidebarHandle` mobile methods** (F2 + L54):
...
1. Add `lastMobileOpenReason: AppSidebarMobileOpenReason | null` to `SidebarReducerState` interface.
...
**`types.ts` changes:**
1. Widen `AppSidebarHandle.openMobile` / `closeMobile` / `toggleMobile` with optional `reason?: AppSidebarMobileOpenReason` parameter (default `"imperative"`).
...
1. Widen `onCloseMobile: () => void` prop to `onCloseMobile: (reason: AppSidebarMobileOpenReason) => void`.
...
- `src/registry/components/navigation/app-sidebar/parts/tooltip-wrapper.tsx` (audit + maybe-edit)
...
**Files touched:**
- `src/registry/components/navigation/app-sidebar/lib/build-handle.ts` (NEW)
- `src/registry/components/navigation/app-sidebar/app-sidebar.tsx`
- `src/registry/components/navigation/app-sidebar/hooks/use-sidebar-nav-state.ts`
- `src/registry/components/navigation/app-sidebar/lib/sidebar-reducer.ts` (action discriminant change)
...
import type { NavEntry, NavItem, AppSidebarHandle, AppSidebarStateValue } from "../types";
...
3. The outer hook's final `useMemo` returning `AppSidebarStateValue` stays (it carries the state fields onto the handle).
...
- `src/registry/components/navigation/app-sidebar/app-sidebar.tsx`
- `src/registry/components/navigation/app-sidebar/types.ts`
- `src/registry/components/navigation/app-sidebar/demo.tsx`
...
1. Add JSDoc to `onCollapsedChange`:
   ...
   ```
2. Add JSDoc to `collapseAllSections` in `AppSidebarHandle`:
   ```ts
   /**
...
**Files touched:**
- `docs/procomps/app-sidebar-procomp/app-sidebar-procomp-guide.md`
- `src/registry/components/navigation/app-sidebar/meta.ts`
- `src/registry/component-versions.ts`
- `registry.json`
- `docs/procomps/app-sidebar-procomp/reviews/<YYYY-MM-DD>-v0.3.0-spotcheck.md` (NEW)
- `.claude/STATUS.md`
- `.claude/decisions/<YYYY-MM-DD>-app-sidebar-v0.3.0-a-plus-pass.md` (NEW)
...
2. **Mobile-drawer reason discriminator:** "`onMobileOpenChange.reason` now fires correctly for all 5 cases: `"trigger"` (companion `<AppSidebarTrigger>` click), `"item-click"` (nav row in drawer), `"outside-click"` (backdrop click), `"escape"` (Esc key), `"imperative"` (direct `handle.openMobile()` / `closeMobile()` / `toggleMobile()` call from consumer code). v0.2.x had this stuck on `"imperative"`."
3. **Custom trigger reason guidance (Q25):** "If you wire your own hamburger trigger button instead of using `<AppSidebarTrigger>`, pass `\"trigger\"` to keep analytics-style reason discriminators aligned with the built-in companion: `handle.toggleMobile(\"trigger\")` instead of `handle.toggleMobile()`."
4. **`openMobile` / `closeMobile` / `toggleMobile` additive `reason?` param:** "Each accepts an optional `AppSidebarMobileOpenReason` arg. Default `\"imperative\"` preserves v0.2.x signatures — `handle.closeMobile()` works exactly as before."
...
- Bump app-sidebar entry to `0.3.0`.
...
- Path: `docs/procomps/app-sidebar-procomp/reviews/<YYYY-MM-DD>-v0.3.0-spotcheck.md`
...
**Decision file (NEW):**
- Path: `.claude/decisions/<YYYY-MM-DD>-app-sidebar-v0.3.0-a-plus-pass.md`
...
**Push:** Commit chain `C1..C7` to master. Vercel auto-runs `pnpm vercel-build` → registry regenerated → installable as `pnpm dlx shadcn@4.6.0 add @ilinxa/app-sidebar` at v0.3.0.

---

## 5. Verification ladder

Per [.claude/rules/component-readiness-review.md](../../../.claude/rules/component-readiness-review.md) closure conditions + project precedent (task-card, task-tree, json-form same-day-patch loops):
...
4. `pnpm dev` — visit `/components/app-sidebar`, confirm every existing demo tab still renders.

### After C6 (full pre-ship gate)
5. `pnpm registry:build` — `public/r/app-sidebar.json` regenerated; spot-check (a) `lib/build-handle.ts` appears with `type: "registry:component"` + `target: "components/app-sidebar/lib/build-handle.ts"`, (b) `demo.tsx` / `usage.tsx` / `meta.ts` still absent.
...
### After C7 (push gate)
8. **Path-b consumer-tsc smoke** at `e:/tmp/ilinxa-smoke-consumer/`: install via `pnpm dlx shadcn@4.6.0 add @ilinxa/app-sidebar` against the local registry artifact → `pnpm tsc --noEmit` in the consumer tree → clean. Expected no F-cross-13 hits on the new `<SheetContent>` `onPointerDownOutside` / `onEscapeKeyDown` props (Radix-typed today; defensive fallback through `onOpenChange` if shadcn migrates to Base UI in a future release).
9. **Browser interaction tests** (manual, in `/components/app-sidebar` demo page):
...
10. Confirm:
    - No `data-stage` anywhere in the app-sidebar folder (grep).
    ...
    - No `replace(/(["\\])/g, "\\$1")` fallback in `app-sidebar.tsx`.
    ...
    - `buildHandle` is the SAME factory in both `app-sidebar.tsx` and `use-sidebar-nav-state.ts` (no inline duplication left).
...
4. This file (`app-sidebar-procomp-plan-v0.3.0.md`) AND the description addendum (`app-sidebar-procomp-description-v0.3.0.md`) are **deleted** in the v0.3.0 close commit.
