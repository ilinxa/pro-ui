# schedule-list — procomp plan

> Stage 2: how. The implementation contract.
>
> See [`schedule-list-procomp-description.md`](./schedule-list-procomp-description.md) for the what & why.
...
```ts
// src/registry/components/data/schedule-list/types.ts
...
export interface ScheduleListLabels {
...
```ts
export const DEFAULT_SCHEDULE_LIST_LABELS: Required<ScheduleListLabels> = {
  timeRangeSeparator: " - ",
...
```
src/registry/components/data/schedule-list/
├── schedule-list.tsx           # 1 — root component
├── parts/
...
**Polymorphic root** via `linkComponent: ElementType`. Same pattern as event-card / news-card.
...
- [ ] `pnpm build` clean — all routes prerendered including `/components/schedule-list`
