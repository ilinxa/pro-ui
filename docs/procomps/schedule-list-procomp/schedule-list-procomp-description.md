# schedule-list — procomp description

> Stage 1: what & why.
>
> **Migration origin:** kasder `kas-social-front-v0` `src/app/(platform)/events/[id]/page.tsx` lines 197–213 (the "Program" / Schedule block on the event detail page). Source page already deep-analyzed during event-card migration; intake doc skipped per session agreement.
...
Pro-ui has no existing answer. `data-table` is tabular. `thumbnail-list` is image-led. `article-meta` is horizontal flex strip. `info-list` (next sibling) is icon-prefixed; this is time-prefixed.
...
```tsx
<ScheduleList heading="Program" items={event.schedule} />
```
...
<aside className="rounded-2xl border bg-card p-4">
  <ScheduleList items={today.items} framed={false} headingAs="h3" heading="Today" />
</aside>
```
...
- `pnpm tsc --noEmit` + `pnpm lint` + `pnpm build` clean; `/components/schedule-list` SSR returns 200.
...
3. **Heading semantic level** — `h2` default, configurable via `headingAs`. Same convention as news-card / author-card.
