# video-player — procomp description
...
>
> Second of 8 components in the social-posts-system arc. Narrow primitive — useful even if downstream `media-carousel` / `story-viewer` ships are deferred. Establishes the slot-based-controls pattern, the `isActive` auto-pause contract, and the `useDoubleTap` helper that the carousel will consume.
...
- **Double-tap callback** — `onDoubleTap?: () => void`. Fires on a 300ms-window double-tap detection. Composes into post-card's double-tap-to-like UX. Component itself does NOT render any visual — that's the host's concern (heart-burst lives in `engagement-bar`'s `<EngagementHeartBurst>` sub-export).
...
- **`media-carousel`** (next ship after this) — post media (photos + videos in one strip). Sets `isActive` per slide.
- **`story-viewer`** — full-screen story playback with segmented progress.
- News article inline videos (could retrofit via `news-card`'s slot-based design).
...
**1. Post media carousel slide** (the originating use case — paired with `media-carousel`):
```tsx
{media.map((item, idx) => (
...
**2. Story viewer item** (composed inside `story-viewer`):
```tsx
<VideoPlayer
...
1. **Component category — `media` (NEW) or `data`?** The analysis defaulted to `data`; the project has a `media` category at [src/registry/categories.ts:46-50](../../../src/registry/categories.ts#L46) defined as "Image galleries, video players, carousels, file viewers" — exactly this. **Recommendation: `media`.** First occupant. Same posture for upcoming `media-carousel`, `story-rail`, `story-viewer` (3 more `media`-category components in this arc).
...
- **No framer-motion** in this component. Control fade is CSS opacity transition. Heart burst lives in `engagement-bar`'s sibling sub-export, not here.
- **No Embla** dep. This component is just `<video>` + control overlay; Embla is `media-carousel`'s concern.
