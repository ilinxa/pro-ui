# video-player — consumer guide

> Stage 3: usage notes for hosts. Authored alongside implementation.
>
> See [description](./video-player-procomp-description.md) for what & why; [plan](./video-player-procomp-plan.md) for how it's built.
...
- Post media (Instagram-style cards) — composes inside `media-carousel` (next ship)
- Story viewers — composes inside `story-viewer` (later ship)
...
```tsx
import { useDoubleTap } from "@ilinxa/video-player";
...
- [`media-carousel`](../media-carousel-procomp/) (next ship) — composes this for video items in mixed image+video carousels. Sets `isActive` per slide.
- [`story-viewer`](../story-viewer-procomp/) (later ship) — composes this with `controls={false}` + onTimeUpdate for segmented progress bar fill.
