# video-player — procomp plan

> Stage 2: how. The implementation contract.
>
> See [`video-player-procomp-description.md`](./video-player-procomp-description.md) for the what & why.
>
...
|---|---|---|
| Q-P1 | Component category — `media` (new) or `data`? | **`media`.** First occupant of the existing `media` category at [src/registry/categories.ts](../../../src/registry/categories.ts). Sets precedent for media-carousel / story-rail / story-viewer. |
...
- **No framer-motion** — control fade is CSS opacity transition. Heart burst is `engagement-bar`'s sibling sub-export, not here.
- **No Embla** — that's `media-carousel`'s concern.
...
```ts
// src/registry/components/media/video-player/types.ts
...
```
src/registry/components/media/video-player/
├── video-player.tsx                    # 1 — root
├── parts/
...
function VideoPlayerInner({
  ...
  const labels: Required<VideoPlayerLabels> = useMemo(
...
1. **Default** — `<VideoPlayer src={SAMPLE_VIDEO_URL} poster={SAMPLE_POSTER_URL} />` — proves muted-autoplay-friendly defaults + control overlay + auto-hide
...
5. **isActive auto-pause** — 3 videos in a row with a "active index" toggle button; only one plays at a time (proves the carousel use case before media-carousel ships)
...
export const meta: ComponentMeta = {
  ...
  tags: ["video-player", "video", "media", "player", "carousel"],
  ...
  related: ["expandable-text", "media-carousel", "story-viewer"],
};
```
...
- `next/*`
- `framer-motion` (CSS opacity transitions only here; framer is `engagement-bar` / `story-viewer` territory)
- `embla-carousel-react` (carousel concerns belong to `media-carousel`)
...
- [ ] `pnpm build` clean — `/components/video-player` prerendered (36th route)
...
- `pnpm new:component media/video-player` — scaffolds the sealed folder
...
- Author `video-player.tsx` root (composes the above)
...
- Run `pnpm registry:build` — verify `public/r/video-player.json` + `public/r/video-player-fixtures.json`
- Update `.claude/STATUS.md`
- Author `video-player-procomp-guide.md`

### Estimated total: ~4.5 hours focused work

Larger than `expandable-text` (~3 hours) due to the state machine + slot architecture. Establishes the `media` category, the `useDoubleTap` public hook, and the `renderXxx` slot pattern for the rest of the social-posts arc.
