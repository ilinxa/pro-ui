# `card-tree` — Pro-component Plan (Stage 2)

> **Stage:** 2 of 3 · **Status:** Draft — awaiting sign-off
> **Slug:** `card-tree` · **Category:** `data`
> **Inputs:** description signed off ([card-tree-procomp-description.md](card-tree-procomp-description.md)). All sixteen description-stage decisions (Q1–Q16) are inherited as fixed inputs. **Markdown is OUT of card-tree** per the top-of-description deviation banner.
...
- Added **`ParsedCardTree` (private)** — the normalized internal shape. Not exported.
- Added **`ParsedCardTree.parentKey`** — stores the property name each child was found under, so serialize can re-emit children at their original keys. **Required for Q3 round-trip fidelity** (caught during plan-review consistency pass — without this, child keys like `section1` / `section2` would be lost on serialize).
- Added **`Action` (private)** — strict-typed reducer-action union. Not exported.
- Added **`'aria-label'` prop** for the landmark name on the tree root, default `"Card tree"`.
...
export type CardTreeProps = {
  defaultValue: CardTreeJsonNode;                      // seed; remount via key prop to reset
...
export type CardTreeHandle = {
  ...
  getTree(): CardTreeJsonNode;                         // parsed object form, with auto-attached IDs
};
...
type ParsedCardTree = {
  ...
  children: ParsedCardTree[];                            // sorted by order ascending
};
...
Public API: `CardTreeJsonNode` — the loose, dynamic-key shape consumers write.
Internal: `ParsedCardTree` — normalized, statically-keyed, sorted, type-tagged.
...
**Serialize pass** (`lib/serialize.ts`) walks `ParsedCardTree` and rebuilds the dynamic-key object. Property emission order:
1. `__rcid`
2. `__rcorder`
...
`card-tree.tsx` renders a `<ul role="tree">` containing a single `<Card tree={root} level={1} />`. `parts/card.tsx` renders its own structure recursively:
...
```ts
{
  tree: ParsedCardTree;                  // parsed once; replaced only by `replace-tree` action (used internally on key remount)
...
| `replace-tree` | Wholesale replace the tree. Used internally by `card-tree.tsx` if the consumer remounts via `key`. |
...
| `serialize.ts` | `serializeTree(tree) => string`, `treeToJsonNode(tree) => CardTreeJsonNode` | Tree → canonical JSON / object form |
...
├── card-tree.tsx                  ← root; "use client"; useReducer + useImperativeHandle + ARIA tree
├── parts/
...
├── lib/
│   ├── parse.ts                   ← input → ParsedCardTree (pure)
│   ├── serialize.ts               ← ParsedCardTree → canonical JSON (pure)
...
Parse errors are collected, not thrown. Caller (`card-tree.tsx`) decides what to do with them: in v0.1, they're surfaced as `console.warn` lines with a single `console.error` summary listing all errors, and the tree renders best-effort with bad entries skipped.
...
- Single public component `<CardTree>`. `forwardRef` exposing `CardTreeHandle`.
...
`card-tree.tsx`: **`"use client"`**. Required for: `useReducer`, `useRef`, `useImperativeHandle`, keyboard event handlers, click handlers on collapse toggle / meta trigger.

Server-safe (no directive needed):
- `types.ts`, `lib/*.ts`, `meta.ts`, `index.ts`, `dummy-data.ts` — pure modules, no React runtime.
- `demo.tsx` — server component that renders `<CardTree>` with the dummy data. The "use client" boundary inside `<CardTree>` handles hydration.
...
None. `card-tree` doesn't compose other registry components.
...
| `aria-label` empty string | Falls back to default `"Card tree"`. |
...
- **Root container:** `<div role="region" aria-label={ariaLabel}>` wrapping the tree (default label `"Card tree"`).
...
| Tree walks on every render | `ParsedCardTree` is parsed once and stored in reducer state. No re-parsing per render. |
...
| Memory | Internal `ParsedCardTree` is ~2× the input shape (extra type tags + sorted children); acceptable for ≤ 500 nodes. |
...
**No test runner** is wired in this repo (description risk §8). Plan-stage stance: ship v0.1 with extensive demo-driven verification + an explicit STATUS.md test-debt entry. Round-trip property tests (random `CardTreeJsonNode` → `parse` → `serialize` → `parse` → assert tree-equal) are the highest-priority test once Vitest lands. **Flagged Q-P5.**
...
| Markdown-adapter creep | This plan contains **zero** markdown-shaped concessions. No definition-list field encoding. No HTML-comment meta. v0.5 adapter (if ever) is a clean external module operating on `CardTreeJsonNode`. |
...
- **Compound-component API (`<CardTree.Card>` etc.).** Adds a parallel surface that only matters once we have customization beyond classNames; v0.3 is the right time. Defer.
...
2. Open a STATUS.md note recording the test-debt for `card-tree`
3. `pnpm new:component data/card-tree`
4. Implement against this plan, file by file. Suggested order:
   ...
   10. `card-tree.tsx` — root, ARIA tree wrapper, `forwardRef` + `useImperativeHandle`
   11. `dummy-data.ts` — three demo trees
   12. `demo.tsx`
   13. `usage.tsx`
   14. `meta.ts`
   15. `index.ts`
5. Author `card-tree-procomp-guide.md` (Stage 3) alongside the implementation
