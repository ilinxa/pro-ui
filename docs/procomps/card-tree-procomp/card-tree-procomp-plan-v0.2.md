# `card-tree` v0.2 — Pro-component Plan (Stage 2 revision)

> **Stage:** 2 of 3 · **Phase:** v0.2 (inline editor) · **Status:** Draft — awaiting sign-off
> **Slug:** `card-tree` · **Category:** `data`
> **Inputs:**
> - [Description (Stage 1)](card-tree-procomp-description.md) — all 16 Q-decisions inherited as fixed inputs
> - [v0.1 plan (Stage 2)](card-tree-procomp-plan.md) — v0.1 architecture inherited; v0.2 adds, never replaces
> - [v0.1 guide (Stage 3)](card-tree-procomp-guide.md) — public surface inherited
...
"What's currently being edited" is **transient UI state**, not committed state. It lives in a dedicated `useState<EditMode | null>(null)` in [`card-tree.tsx`](../../../src/registry/components/data/card-tree/card-tree.tsx) and the new [`hooks/use-edit-mode.ts`](../../../src/registry/components/data/card-tree/hooks/use-edit-mode.ts).
...
[`lib/reducer.ts`](../../../src/registry/components/data/card-tree/lib/reducer.ts) gains 12 new actions and 3 state slots:

```ts
// new state slots
type CardTreeStateV2 = CardTreeStateV1 & {
...
type ActionV2 =
  ...
  | { type: "predefined-edit";  cardId: string; key: PredefinedKey; entry: CardTreePredefinedEntry }
...
New file [`lib/validate-edit.ts`](../../../src/registry/components/data/card-tree/lib/validate-edit.ts) — pure synchronous validators per action type. Returns `{ ok: true } | { ok: false, errors: ValidationError[] }`. The reducer calls the validator before mutating; if invalid, state is unchanged and the caller (the editor part) surfaces the error inline.
...
```ts
type CardTreePropsV2 = CardTreePropsV1 & {
...
```ts
type CardTreeHandleV2 = CardTreeHandleV1 & {
...
export type CardRemovedEvent = {
  cardId: string;
  removed: CardTreeJsonNode;     // the full removed subtree (for host-side undo)
...
| File | Changes |
|---|---|
| [`types.ts`](../../../src/registry/components/data/card-tree/types.ts) | Add `CardTreePropsV2` union (or extend in place), 9 event types, expanded `CardTreeHandle` |
| [`lib/reducer.ts`](../../../src/registry/components/data/card-tree/lib/reducer.ts) | Add ~12 actions, `selectedId`/`version`/`cleanVersion` state slots |
| [`parts/card.tsx`](../../../src/registry/components/data/card-tree/parts/card.tsx) | Accept editing props via config; render add buttons in edit mode; click selects card; switch field/predefined rendering to editors when in edit mode for that target |
| [`parts/card-header.tsx`](../../../src/registry/components/data/card-tree/parts/card-header.tsx) | Render `<CardActions>` in edit mode; switch title to `<CardTitleEdit>` when in edit-mode for this card's title |
| [`parts/field-row.tsx`](../../../src/registry/components/data/card-tree/parts/field-row.tsx) | Switch to `<FieldEdit>` when edit-mode targets this row's value or key; show hover-× when in edit mode |
| [`card-tree.tsx`](../../../src/registry/components/data/card-tree/card-tree.tsx) | Wire `editable` prop, edit-mode state, all event handlers; expand imperative handle |
...
[`parts/predefined-edit.tsx`](../../../src/registry/components/data/card-tree/parts/predefined-edit.tsx) is one file with five inner components:
...
2. **Coarse event** — `onChange(tree)` if registered, where `tree` is the post-commit `CardTreeJsonNode` (a fresh `treeToJsonNode(state.tree)`).
...
[`hooks/use-dirty.ts`](../../../src/registry/components/data/card-tree/hooks/use-dirty.ts) reads `state.version` and `state.cleanVersion`:
...
Validators in [`lib/validate-edit.ts`](../../../src/registry/components/data/card-tree/lib/validate-edit.ts):
...
[`parts/inline-error.tsx`](../../../src/registry/components/data/card-tree/parts/inline-error.tsx):
...
1. Update [STATUS.md](../../../.claude/STATUS.md): bump card-tree version target to 0.2.0; add a v0.2 in-flight entry.
2. Implement against this plan, file by file. Suggested order:
   ...
   10. **Wire in card-tree.tsx** — new props, edit-mode state, event firing, expanded handle.
...
4. Manual browser smoke-test at `/components/card-tree`.
