# `card-tree` v0.3 — Pro-component Plan (Stage 2 revision)

> **Stage:** 2 of 3 · **Phase:** v0.3 (movement + permissions + bulk + custom keys + virtualization + meta) · **Status:** Draft — awaiting sign-off
> **Slug:** `card-tree` · **Category:** `data`
> **Inputs:**
> - [Description (Stage 1)](card-tree-procomp-description.md) — all 16 Q-decisions inherited
> - [v0.1 plan](card-tree-procomp-plan.md) — viewer architecture inherited
> - [v0.2 plan](card-tree-procomp-plan-v0.2.md) — editor architecture inherited
> - [v0.2 guide](card-tree-procomp-guide.md) — public surface as of v0.2
...
[`card-tree.tsx`](../../../src/registry/components/data/card-tree/card-tree.tsx) wraps its tree in `<DndContext>` from `@dnd-kit/core` when `editable={true}` and any DnD scope is allowed. The context provides:
...
[`lib/reducer.ts`](../../../src/registry/components/data/card-tree/lib/reducer.ts) gains 9 new actions and 2 new state slots:

```ts
type CardTreeStateV3 = CardTreeStateV2 & {
...
New file [`lib/permissions.ts`](../../../src/registry/components/data/card-tree/lib/permissions.ts) — pure resolver. The host can specify permissions at three scopes:
...
```ts
type CardTreeHandleV3 = CardTreeHandleV2 & {
...
A pure search function in [`lib/search.ts`](../../../src/registry/components/data/card-tree/lib/search.ts) walks `ParsedCardTree` and returns a `SearchResult`. The host owns the search query (controlled): pass `search={{ query: "OKLCH" }}` and the component re-runs the walk on every render where the search props change.

**No DOM dependence.** The walk operates on the parsed `ParsedCardTree`, so:
...
```ts
type CardTreePropsV3 = CardTreePropsV2 & {
  // DnD
...
**Counts:**
- **Types:** v0.2 had 21 public types. v0.3 adds 5 event types + `CardTreePermissions` + `PermissionRule` + `DndScopes` + `CustomPredefinedKey` + `CustomKeyContext` + `SearchOptions` + `SearchMatch` + `SearchMatchType` + `SearchResult` = 14 new → **35 public types total**.
...
```
src/registry/components/data/card-tree/
├── parts/
...
| File | Changes |
|---|---|
| [`types.ts`](../../../src/registry/components/data/card-tree/types.ts) | Add 5 event types, 4 search types, `CardTreePermissions`, `PermissionRule`, `DndScopes`, `CustomPredefinedKey`, expanded `CardTreeProps` + `CardTreeHandle` |
| [`lib/reducer.ts`](../../../src/registry/components/data/card-tree/lib/reducer.ts) | Add 9 movement/bulk/meta actions + 3 search actions, multi-select state, drag-transient state, search state |
| [`lib/validate-edit.ts`](../../../src/registry/components/data/card-tree/lib/validate-edit.ts) | Add `validateMetaEdit`, `validateMetaAdd`, `validateCardMove` |
| [`parts/card.tsx`](../../../src/registry/components/data/card-tree/parts/card.tsx) | Wrap in `useSortable`; render drag handle; multi-select click handlers; permission gates on add/remove buttons; wrap rendered text in `<MatchHighlight>` |
| [`parts/card-header.tsx`](../../../src/registry/components/data/card-tree/parts/card-header.tsx) | Render `<DragHandle>` in edit mode; wrap title in `<MatchHighlight>` |
| [`parts/field-row.tsx`](../../../src/registry/components/data/card-tree/parts/field-row.tsx) | Wrap field key + value renders in `<MatchHighlight>` |
| [`parts/card-actions.tsx`](../../../src/registry/components/data/card-tree/parts/card-actions.tsx) | Add cascade/promote picker; "Duplicate" entry; root-removal flag handling |
| [`parts/meta-popover.tsx`](../../../src/registry/components/data/card-tree/parts/meta-popover.tsx) | Edit-mode integration — switch to `<MetaEdit>` content when in edit mode; wrap meta key/value renders in `<MatchHighlight>` |
| [`parts/meta-inline.tsx`](../../../src/registry/components/data/card-tree/parts/meta-inline.tsx) | Click-to-edit on inline meta values; wrap in `<MatchHighlight>` |
| [`parts/predefined-codearea.tsx` / `predefined-quote.tsx` / `predefined-list.tsx` / `predefined-table.tsx` / `predefined-image.tsx`](../../../src/registry/components/data/card-tree/parts/) | Wrap displayed strings in `<MatchHighlight>` for search-result rendering |
| [`card-tree.tsx`](../../../src/registry/components/data/card-tree/card-tree.tsx) | Mount `<DndContext>` (when editable + any DnD scope on); virtualization integration; search hook integration; handlers for drag-end, bulk ops; multi-select state management; expanded imperative handle |
| [`demo.tsx`](../../../src/registry/components/data/card-tree/demo.tsx) | Add multi-select demo, drag-drop demo, permission demo, search demo (toggles for each) |
...
For consumers who want a built-in input UI, drop in `<CardTreeSearchBar>` (in `parts/search-bar.tsx`) — exported as a sibling component:

```tsx
import { CardTree, CardTreeSearchBar } from "@/registry/components/data/card-tree";

const [query, setQuery] = useState("");

<CardTreeSearchBar value={query} onChange={setQuery} matchCount={result?.matches.length ?? 0} />
<CardTree defaultValue={data} search={{ query }} onSearchResults={setResult} />
```
...
Default keyboard binding (when `<CardTreeSearchBar>` is used): `F3` / `Shift+F3` for next/previous match. Cmd+G / Cmd+Shift+G also bound. Hosts wiring their own input handle their own bindings.
...
3. If callback returns a `CardTreeJsonNode` → that becomes the new root via `replace-tree`
...
```tsx
<CardTree defaultDeletePolicy="promote" />
```
...
- **Search**: the optional `<CardTreeSearchBar>` exposes a native `<input type="search">` with `aria-label="Search cards"`. Match count fires into a `aria-live="polite"` region ("3 matches"). Matched text is wrapped in `<mark>` (semantic, screen-readers announce it). Active-match navigation also moves keyboard focus to the matched card so `aria-activedescendant` updates; screen readers re-announce the card's content on jump. When `query` is non-empty and there are zero matches, the live region announces "No matches found".
...
| Q-P8 | **`onRootRemoved` return shape** — `CardTreeJsonNode | null` (consumer picks new root) or `'replace-with-empty' | CardTreeJsonNode` enum? | **`CardTreeJsonNode \| null`.** Null = empty tree placeholder. | Simpler. Consumers who want explicit empty-state behavior can return a synthetic empty card. |
...
1. Update [STATUS.md](../../../.claude/STATUS.md): bump card-tree target version to 0.3.0; add v0.3 in-flight entry.
...
3. Implement against this plan, file by file. Suggested order:
   1. **Types first** — extend `types.ts` with new event types, search types, `CardTreePermissions`, `PermissionRule`, `DndScopes`, `CustomPredefinedKey`, expanded handle.
   ...
   11. **Wire into card-tree.tsx** — DndContext, virtualizer integration, multi-select, expanded handle, permission resolution, custom-keys merging, root-removal flow, search hook + active-match scrollToView, sibling export of `<CardTreeSearchBar>`.
...
5. Manual browser smoke test at `/components/card-tree` covering: drag-drop, multi-select bulk delete, permission lock via `__rcmeta.locked`, virtualization on a 1000-node tree, meta editing, root-removal with callback, **search across collapsed subtrees + meta + virtualized cards with `findNext` jump-to-match auto-expanding ancestors**.
