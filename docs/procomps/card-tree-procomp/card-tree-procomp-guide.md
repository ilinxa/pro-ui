# `card-tree` — Pro-component Guide (Stage 3)

> **Stage:** 3 of 3 · **Status:** Aligned with shipped v0.4.0 (2026-04-28) · **Maturity: beta**
> **Slug:** `card-tree` · **Category:** `data` · **Version:** 0.4.0 (beta)
> **Source:** [src/registry/components/data/card-tree/](../../../src/registry/components/data/card-tree/) — 56 files
> **Demo route:** `/components/card-tree`
> **Procomp planning trail:** [description](card-tree-procomp-description.md) · [v0.1 plan](card-tree-procomp-plan.md) · [v0.2 plan](card-tree-procomp-plan-v0.2.md) · [v0.3 plan](card-tree-procomp-plan-v0.3.md) · [v0.4 plan](card-tree-procomp-plan-v0.4.md)

This guide is the **complete developer reference** for `CardTree` v0.4. Every feature, prop, gotcha, composition pattern, and behavior the component supports — in one place. If you're integrating, customizing, or porting `CardTree` into another codebase, this is your map.

---

## 1. What it is

`CardTree` is a JSON-driven recursive card-tree component for hierarchical structured content. It is — in one component — a **viewer**, **inline editor**, **structural manager**, and **safety net**:
...
Markdown source/serialization is **intentionally not in `card-tree`**. A v0.5 markdown adapter is deferred indefinitely as a separate companion module operating on the JSON model.
...
1. **Copy the folder** [src/registry/components/data/card-tree/](../../../src/registry/components/data/card-tree/) into your project's component tree.
...
### 5.1 The `CardTreeJsonNode` shape
...
type CardTreeProps = {
  ...
  // Container
  className?: string;
  'aria-label'?: string;                                // default "Card tree"
};
```
...
## 9. Imperative API (`CardTreeHandle`)

```tsx
const ref = useRef<CardTreeHandle>(null);
<CardTree ref={ref} defaultValue={...} />
```

| Method | Returns | Phase | Notes |
|---|---|---|---|
| `getValue()` | `string` | v0.1 | Canonical pretty-printed JSON (with auto-IDs). |
| `getTree()` | `CardTreeJsonNode` | v0.1 | Object form. |
...
```ts
type CardTreeValidators = {
  fieldEdit?: (event, tree) => CardTreeValidationResponse;
  fieldAdd?: ...;
  cardAdd?: ...;
  cardRemove?: ...;
  cardMove?: ...;
...
type CardTreeValidationResponse =
  | { ok: true }
...
```ts
validate?: (action: { type: string; cardId?: string }, tree: CardTreeJsonNode) => CardTreeValidationResponse;
```
...
<CardTreeSearchBar
  ...
  onClear={() => { setQuery(""); ref.current?.clearSearch(); }}
/>
<CardTree ref={ref} defaultValue={data} search={{ query }} onSearchResults={setResult} />
```
...
`<CardTreeSearchBar>` binds `F3` / `Shift+F3` for next/previous. Cmd+G / Cmd+Shift+G also bound. **Cmd+F is NOT intercepted** — browser's native find is preserved (Q-P14 lock).
...
- A `CardTreeJsonNode` → that becomes the new root
...
### 21.5 `<CardTreeUndoToolbar>`

Optional sibling export. Drop in next to `<CardTree>` for default undo / redo affordances:
...
- `<CardTreeSearchBar>` uses native `<input type="search">` with `aria-label`
...
```tsx
<CardTree defaultValue={someJson} />
```
...
<CardTreeUndoToolbar canUndo={canUndo} canRedo={canRedo} onUndo={undo} onRedo={redo} />
<CardTree
  defaultValue={data}
  editable
  validators={validators}
...
| 6 | Root has no `parentKey` | Title comes from `aria-label` (default `"Card tree"`) |
...
| 11 | Browser `Ctrl+F` doesn't find content inside collapsed cards | Use native search (`<CardTreeSearchBar>`) instead — walks the data model and auto-expands the path to matches |
| 12 | Custom keys aren't searched in v0.3 | `searchableText` API deferred. Search the card's `parentKey` or other fields meanwhile. |
| 13 | `Cmd+F` is NOT intercepted by card-tree | Browser-native find still works; native search is `F3` / explicit input |
...
| **v0.5** | planned (no commit date) | Virtualization (the wiring work the v0.3 prop never received), markdown ↔ JSON adapter (separate companion module, NOT inside card-tree) |
...
From [`src/registry/components/data/card-tree/index.ts`](../../../src/registry/components/data/card-tree/index.ts):
...
export type {
  ...
  ListValue, PredefinedKey, QuoteValue, CardTreeHandle, CardTreeJsonNode,
  CardTreeProps, TableValue,
} from "./types";
...
export type {
  ...
  CardTreePermissions, PermissionRule, EffectivePermissions, PermissionDenialReason,
...
export type {
  CardTreeValidators, CardTreeMasterValidator,
  CardTreeValidationResponse, CardTreeValidationError,
...
- **Source:** [src/registry/components/data/card-tree/](../../../src/registry/components/data/card-tree/) — 56 files (1 root + ~22 parts + ~10 hooks + ~10 lib + 7 anatomy + assorted)
- **Pure helpers (testable):** [lib/parse.ts](../../../src/registry/components/data/card-tree/lib/parse.ts), [serialize.ts](../../../src/registry/components/data/card-tree/lib/serialize.ts), [classify-key.ts](../../../src/registry/components/data/card-tree/lib/classify-key.ts), [infer-type.ts](../../../src/registry/components/data/card-tree/lib/infer-type.ts), [reducer.ts](../../../src/registry/components/data/card-tree/lib/reducer.ts), [validate-edit.ts](../../../src/registry/components/data/card-tree/lib/validate-edit.ts), [permissions.ts](../../../src/registry/components/data/card-tree/lib/permissions.ts), [search.ts](../../../src/registry/components/data/card-tree/lib/search.ts), [dnd-helpers.ts](../../../src/registry/components/data/card-tree/lib/dnd-helpers.ts), [bulk-actions.ts](../../../src/registry/components/data/card-tree/lib/bulk-actions.ts), [validators.ts](../../../src/registry/components/data/card-tree/lib/validators.ts)
- **Procomp planning trail:**
  - [Description (Stage 1)](card-tree-procomp-description.md)
  - [v0.1 plan (Stage 2)](card-tree-procomp-plan.md)
  - [v0.2 plan (Stage 2)](card-tree-procomp-plan-v0.2.md)
  - [v0.3 plan (Stage 2)](card-tree-procomp-plan-v0.3.md)
  - [v0.4 plan (Stage 2)](card-tree-procomp-plan-v0.4.md)
