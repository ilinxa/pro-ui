# `kanban-board` — Pro-component Description (Stage 1)
...
1. **`dragHandle: "shell" | "header"` on `KanbanCardRenderer`** — opt-in per renderer. `"shell"` (default) keeps v0.1 behavior (whole-card grab). `"header"` renders a thin grip strip above the item; only that strip activates dnd-kit drag, so the body stays fully interactive. Required for renderers that own internal pointer interactions — click-to-edit fields, embedded inputs, internal DnD. The card-tree adapter (see #2) is the canonical use case.
2. **Rich-card adapter pattern (demonstrated)** — the demo now registers a third renderer that wraps `<CardTree>` (the structural-content tree editor in this registry) with full feature passthrough — `editable`, `onChange`, `defaultCollapsed`, predefined keys, internal DnD, all preserved. Items with `rendererId: "card-tree"` carry a `CardTreeJsonNode` as their data; the renderer mounts the real `<CardTree>` so all of its features work inside a kanban column. This is the renderer-registry's marquee feature delivered: heterogeneous columns with first-class rich content.
3. **Column body uses native `overflow-y-auto`** — replaces the radix `<ScrollArea>` that wasn't reliably constraining inside the `flex-1` chain. Tall items (card-tree with expanded children) now scroll natively within the column. `min-h-0` propagates from `Column` → body so the flex chain shrinks correctly. `scroll-area` is no longer a registryDependency.
...
- **Built-in card detail / activity feed** — the `note` item handles a *contextual* between-cards comment use case, but kanban does NOT ship a full Trello-style "comments on a card" thread. Use `comment-thread` inside a custom card-detail surface for that.
...
Non-targets: dense data tables (use `data-table`), pure timelines (use `progress-timeline` or similar), Gantt charts.
...
type KanbanCardRenderer<TData = unknown> = {
  ...
  //  Use for renderers that own internal pointer interactions (e.g. card-tree).
...
// ─────────────────────────────────────────────────────────────────────
// Built-in renderers (named exports from the kanban-board package)
// ─────────────────────────────────────────────────────────────────────

// import { kanbanCardRenderer, kanbanNoteRenderer } from "@/components/kanban-board";
...
```tsx
import { KanbanBoard, kanbanCardRenderer, kanbanNoteRenderer } from "@/components/kanban-board";
...
11. **Compiles and renders** at `/components/kanban-board` with no console warnings.
...
|---|---|---|---|
| Q1 | **Slug name** — `kanban-board` vs alternatives | **`kanban-board`** | Matches `-01` versioned-component convention. Future variants (`kanban-board-02` swimlane-first?) are reachable. |
| Q2 | **Decomposition** — single sealed folder or multiple registry items? | **Single sealed folder** `kanban-board` with internal parts (`column.tsx`, `card.tsx`, `note.tsx`, `swimlane.tsx`, `palette.tsx`). | Matches `data-table` precedent. Tightly coupled — only meaningful together. One install: `pnpm dlx shadcn add @ilinxa/kanban-board`. |
...
| Q15 | **Comment thread per card?** | **Out of scope.** The note item handles between-cards annotation; per-card threads use `comment-thread` inside a consumer-built card-detail surface. | Two different patterns; conflating them bloats the kanban surface. |
...
- [ ] **User explicitly says "description approved" (or equivalent)** — this unlocks Stage 2 (`kanban-board-procomp-plan.md`).
