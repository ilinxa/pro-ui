# `kanban-board` — Pro-component Plan (Stage 2)

> **Stage:** 2 of 3 · **Status:** Signed off · v0.4.0 (alpha)
> **Slug:** `kanban-board` · **Category:** `data`
> **Inputs:** description signed off ([kanban-board-procomp-description.md](kanban-board-procomp-description.md)). All fifteen description-stage decisions (Q1–Q15) are inherited as fixed inputs.
...
2. **Rich-card adapter pattern** — the demo now wires `<CardTree>` (sibling registry component) as a third renderer with full feature passthrough: `editable`, `defaultCollapsed`, `onChange`, predefined keys, internal DnD. State for card-tree items lives in the demo's hoisted `boardData` and the renderer closes over a setter that walks the columns to update the matching item's data on CardTree's `onChange`. **No core-kanban changes were needed beyond `dragHandle`** — the renderer-registry already supports any consumer renderer.
...
export type KanbanCardRenderer<TData = unknown> = {
  ...
  // for renderers that own internal pointer events (card-tree, embedded WYSIWYG, etc.).
...
One `useReducer` lives in `kanban-board.tsx`. The state is `KanbanData` plus a small bag of UI state: `activeItemId` (currently dragged), `activeColumnId` (currently dragged column), `draftItem` (in-progress inline create).
...
```
src/registry/components/data/kanban-board/
├── kanban-board.tsx           ← root; "use client"; props → reducer → DnDContext + render
├── parts/
...
**Counts:** 7 mandatory anatomy files (`kanban-board.tsx`, `types.ts`, `dummy-data.ts`, `demo.tsx`, `usage.tsx`, `meta.ts`, `index.ts`) + 18 parts + 4 hooks + 5 lib = **34 files total**.
...
`kanban-board.tsx`: **`"use client"`**. Required for: pointer events, keyboard handlers, `useReducer`, `useRef`, `<DndContext>`.
...
- `parts/*.tsx` and `hooks/*.ts` — inherit "use client" from `kanban-board.tsx`'s import boundary.
...
1. **Open a STATUS.md note** recording the test-debt for `kanban-board` (matches workspace's pattern).
2. `pnpm new:component data/kanban-board`
3. **Implement against this plan, file by file in the order listed in §4.** Suggested order:
   ...
   11. `hooks/*` then `kanban-board.tsx`
   12. `dummy-data.ts`, `demo.tsx`, `usage.tsx`, `meta.ts`, `index.ts`
4. **Author `kanban-board-procomp-guide.md` (Stage 3)** alongside the implementation.
...
7. `pnpm registry:build`, spot-check `public/r/kanban-board.json`.
