# `filter-panel` — Pro-component Description

> **Status:** **signed off 2026-04-28.** Stage 2 (`filter-panel-procomp-plan.md`) authoring may begin.
> **Slug:** `filter-panel`
> **Category:** `forms`
> **Created:** 2026-04-28
...
This is Stage 1 of the [procomp gate](../README.md). It answers *should we build this at all, and what shape should it be?* It does NOT specify implementation — that's Stage 2 (`filter-panel-procomp-plan.md`).
...
- **AND-across / OR-within composition** (system decision #9). filter-panel composes per-category predicates with `&&`. Within a category (e.g., a checkbox-list with `mode: "union"`), the host's predicate handles "does any active value pass." This is the filter-composition pattern locked at the system level.
...
- **Empty-detection per category** via a mandatory `isEmpty(value)` method on each category definition. filter-panel uses this to gate per-category clear visibility and global clear-all enable state.
...
- **Generic typing**: `FilterPanel<T>` parameterized over the item type, defaulting to `unknown`. Predicates take `(item: T, value: FilterValue) => boolean`.
...
- **Filter presets / saved filter sets.** Out of scope; would be a separate companion (`<FilterPanel.Presets>`) if ever needed.
- **Search-overrides-filters semantics** (system decision #12). This is a HOST-level concern: filter-panel returns who passes the filter; the host then unions that result with search-matched items independently. filter-panel does not have a "search override" prop.
...
2. **`data-table` column-filter feature** (future enhancement) — when data-table grows column-filter UI, filter-panel is the right substrate. Existing data-table doesn't have this today; would be a v0.2 data-table feature.
3. **Tier 3 graph-system page** — wires the filter-panel instance to force-graph's filter state.
4. **Generic faceted-search surfaces** (any pro-component or page that needs multi-dimensional filtering).

filter-panel has zero graph dependency. This is per [system decision #35](../../systems/graph-system/graph-system-description.md): Tier 1 components are independent.
...
Predicate composition is automatic: filter-panel ANDs every category's `predicate(item, values[id])` to produce the filtered list. The `mode: "union" | "intersection"` for `checkbox-list` is a hint to the host's predicate (filter-panel stores mode but doesn't impose semantics — the host's `predicate` reads it and decides).
...
```tsx
<FilterPanel<NodeWithGraphContext>
...
11. **Demo at `/components/filter-panel`** demonstrates: 3 built-in types side-by-side, custom slot example, mode toggle, per-category clear, global clear, debounced text input.
...
7. **`isEmpty` defaults — locked with refinement:** filter-panel ships per-type default `isEmpty` checkers for `checkbox-list` (empty when value is empty array) and `text` (empty when string is empty). **`toggle` and `custom` REQUIRE explicit `isEmpty`** — for `toggle`, "empty" depends on host intent (`true` could mean "show all" OR "filter to true-only"); for `custom`, only the host knows the value shape. Refined from the original "defaults for all built-in types" recommendation on re-validation.
...
**Signed off 2026-04-28.** Stage 2 (`filter-panel-procomp-plan.md`) authoring may begin. Parallel description authoring for `entity-picker` and `markdown-editor` is now unblocked. Plan must build against the §8 locked decisions and address the §8.5 plan-stage tightenings, defining the file-by-file structure per the [component-guide.md anatomy](../../component-guide.md#5-anatomy-of-a-component-folder).
