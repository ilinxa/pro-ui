# `filter-panel` — Pro-component Guide (Stage 3)

> **Stage:** 3 of 3 · **Shipped:** v0.1.0 (alpha) · **Date:** 2026-05-09
> **Slug:** `filter-panel` · **Category:** `forms`
> **Files:** [src/registry/components/forms/filter-panel/](../../../src/registry/components/forms/filter-panel/)
> **Detail page:** `/components/filter-panel` (after `pnpm dev`)

The consumer-facing reference. The description ([filter-panel-procomp-description.md](filter-panel-procomp-description.md)) explains *why*; the plan ([filter-panel-procomp-plan.md](filter-panel-procomp-plan.md)) explains *how we built it*; this doc explains *how to use it*.
...
3. **Schema-driven config.** You'd rather declare categories as data than wire JSX per filter. The host owns predicates and value shapes; FilterPanel owns the layout, controls, and composition mechanics.

## When NOT to use FilterPanel

- **Single-axis filtering.** If you have one search box, use an `Input`. If you have one dropdown, use `Select`. FilterPanel's machinery is justified at 2+ categories.
...
- **Server-driven asynchronous filters.** v0.1 predicates are synchronous (`(item, value) => boolean`). If you need debounce-then-fetch-from-API, FilterPanel returns who passes locally; the host pairs that with its own request layer or waits for v0.2 async support.
- **Saved filter presets / URL sync as a built-in.** FilterPanel is pure controlled — `values` is a prop, `onChange` is yours. Persistence (localStorage, URL params, named saved filters) is host-side. Wire it once around the panel.
...
That's a working two-axis filter panel. The user types a name, ticks a status; FilterPanel ANDs the active categories and pushes the filtered array via `onFilteredChange`. `values` state is yours — persist it to URL, localStorage, or a server preset whenever you want.

## The mental model

FilterPanel is **a layout + state-orchestration component**, not a filtering library:
...
- FilterPanel renders the controls, debounces text input, manages mode toggles for `checkbox-list`, calls your predicates, and ANDs the results.

The contract:

| FilterPanel owns | You own |
|---|---|
...
The composition rule is **AND across categories, OR (or AND, or whatever you like) within**. FilterPanel ANDs every active category's predicate; *within* a category, your predicate decides what counts as a match. For `checkbox-list` with `modeToggle`, FilterPanel stores the user's chosen mode at `values["${id}__mode"]`; your predicate reads it and switches between OR-of-options and AND-of-options.

A category is "empty" when its `isEmpty(value)` returns true; empty categories are skipped — FilterPanel doesn't even build their predicate. That's how the panel behaves correctly with no values set (no filtering happens) and how the per-category clear and global clear-all decide their own visibility/disabled state.
...
The host's `render` is wrapped in an error boundary; if it throws, FilterPanel shows a fallback message and the rest of the panel keeps working.
...
If `categories` reference changes more than 5 times in succession, FilterPanel emits a dev-only warning (`[filter-panel] \`categories\` prop is changing every render…`).
...
For `checkbox-list` and `text`, FilterPanel supplies sensible defaults (empty array / empty string). For `toggle` and `custom`, only the host knows what counts as empty — TypeScript enforces it on the schema. A common toggle empty check is `(v) => v !== true` (the toggle is "active" only when it's true).

### 3. Predicates and `isEmpty` should not throw

If they do, FilterPanel catches the throw, logs `[filter-panel] predicate threw for category "<id>":` in dev, and treats:
...
FilterPanel does **not** prune `values["${removedId}__mode"]` when you drop a category from the schema. The orphan stays in `values` until you `delete` it via `onChange`. Same goes for the value key itself.

### 8. Category id ending in `__mode` is rejected (dev-only)

The reserved-suffix validation logs `console.error` if any category's `id` ends in `__mode`. Pick a different id; "__mode" is FilterPanel's territory. The list is exported as `RESERVED_SUFFIXES` if you ever need to test against it.
...
<div className="flex flex-col gap-3">
  ...
  <FilterPanel<Item> items={items} categories={CATEGORIES} values={values} onChange={setValues} />
</div>
```

The preset just replaces `values`; FilterPanel picks up the new state via its controlled prop.
...
<FilterPanel<Item>
  items={[]} // FilterPanel doesn't filter; it only renders chrome + dispatches values
...
`defaultIsEmpty` is re-exported from `@/components/filter-panel` for exactly this purpose.
...
- **Filter presets / saved sets** — possibly as a companion `<FilterPanel.Presets>` component, possibly as a host concern.

All v0.2+ items are designed as additive — no breaking changes to the v0.1 API.

## Migration notes

- **From hand-rolled stacked checkboxes + a search box.** Move each filter into a `FilterCategory` with the right `type`. The hardest part is usually moving the global "any active?" + "clear all" logic — FilterPanel does both for free; you can delete that code.
- **From a single `<Combobox>` with multi-select.** If your filter is one-axis, FilterPanel is overkill; stay with the combobox. If you've grown to 2+ axes, lift each into a category.
- **From a render-prop filter library (e.g., react-table's column filters).** FilterPanel is its own panel — the items live with the panel, not on a row. For data-table column filters, FilterPanel will ship as the substrate in a future data-table release; until then, render-prop column filters and FilterPanel-as-sidebar coexist.
...
```ts
// from @/components/filter-panel
export { FilterPanel, defaultIsEmpty, meta };
export type {
  FilterCategory,
  CheckboxListFilter,
  ToggleFilter,
...
|---|---|---|---|
| `items` | `ReadonlyArray<T>` | required | The full source array; FilterPanel derives `filtered` from this. |
...
| `onChange` | `(values) => void` | required | Mandatory; FilterPanel is pure-controlled. |
...
| `ref` | `Ref<FilterPanelHandle>` | undefined | React 19 ref-as-prop; preserves the `<T>` generic. |
...
In non-production builds, FilterPanel runs schema validation on every `categories` reference change and emits warnings/errors for: reserved-suffix collisions, duplicate category ids, `checkbox-list` with empty options, `defaultMode` set without `modeToggle: true`. It also tracks `categories` reference instability and warns after 5+ unstable renders. All checks are stripped from production bundles.

---

*End of guide. Pair with [filter-panel-procomp-description.md](filter-panel-procomp-description.md) for the *why* and [filter-panel-procomp-plan.md](filter-panel-procomp-plan.md) for the *how*.*
