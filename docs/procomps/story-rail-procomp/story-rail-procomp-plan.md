# story-rail — procomp plan

> Stage 2: how. The implementation contract.
>
> See [`story-rail-procomp-description.md`](./story-rail-procomp-description.md) for the what & why.
>
> **Migration origin:** kasder `StoriesSection.tsx` + `StoryThumbnail.tsx` (story-rail concern only).
>
> **Embla used directly — NOT cross-importing media-carousel (sealed-folder rule).**
...
| Q-P4 | Always-uncontrolled — `items` prop is mount-only initial state. `reset(next)` + `dispatch(action)` imperative escape hatches. | comment-thread R-Plan-15 + post-card |
| Q-P5 | Realtime via `Subscribe<StoryRailDelta>` + `onSubscribeDelta` callback. Identity-stable (`useCallback` host-side). Same `controlledRef` + `onSubscribeDeltaRef` mirror pattern. | engagement-bar + comment-thread carry-over |
...
| Q-P13 | `<StoryRail>` is `"use client"` (Embla state + handle); `<AddStoryThumbnail>` is `"use client"` (button click). | inherited |
| Q-P14 | `React.memo` at export + ref-as-prop. | project standard |
| Q-P15 | `commentReducer`-style `storyRailReducer` exported from day-1 (per dynamicity primacy). `useStoryRailState` hook also public. | post-card carry-over |
| Q-P16 | `onItemClick(item, index)` signature — both args. `linkComponent` + `getHref` co-exist (both can fire on click). | engagement-bar / media-carousel callback shape |
| Q-P17 | `useId()` per thumbnail wired into `renderThumbnail` slot helpers — for hosts attaching ARIA via the slot. | comment-thread R-Plan-12 carry-over |
| Q-P18 | No new shadcn primitives — `avatar` already installed. NOT using the shadcn `card` primitive (R-Plan-1: render `<section>` with conditional card-style classes). Embla peer dep already installed (media-carousel). | R-Plan-1 |
...
| Q-P21 | Locked target convention for `registry.json`: `registry:component` + `target: "components/story-rail/<sub>"`. Never ship demo / usage / meta. Fixtures via `-fixtures` sibling. | project lock |
...
export const DEFAULT_STORY_RAIL_LABELS: Required<
  Omit<StoryRailLabels, "thumbnailAriaLabel">
> = {
  railLabel: "Stories",
...
export interface StoryRailProps {
  ...
  labels?: StoryRailLabels;

  className?: string;
  thumbnailClassName?: string;

  ref?: React.Ref<StoryRailHandle>;
}
...
```
src/registry/components/data/story-rail/
├── story-rail.tsx                # 1 — root component (Embla inline, no wrapper)
├── parts/
...
### 1. `story-rail.tsx` — root

- `"use client"`, `React.memo`, `forwardRef<StoryRailHandle, StoryRailProps>`.
...
- Visual structure (kasder-exact):
  ...
  ```
- `initials(username)` helper: same shape as comment-thread's.
...
Same shape as `useCommentState` from comment-thread.
...
- `notes`: "Stories rail at the top of a feed. Pairs with story-viewer (next ship)."
...
3 lines into `src/registry/manifest.ts` (after post-card imports):
...
{ meta: storyRailMeta, Demo: StoryRailDemo, Usage: StoryRailUsage }
```
...
```jsonc
{
  ...
  "files": [
    /* 7 entries: story-rail.tsx + 2 parts + 1 hook + types + index */
  ]
}
```

Plus `story-rail-fixtures` sibling carrying only `dummy-data.ts`.
...
3. `pnpm registry:build` — generates `story-rail.json` + `story-rail-fixtures.json`; spot-check `files[]` count = 7 + fixtures dummy-data.
...
7. **Component docs route** at `/components/story-rail` returns HTTP 200.
...
| `useEmblaCarousel` not memoized properly causes re-init on every render | Low | Medium | Options memoized via `useMemo([], [])` — empty deps since the config is constant for story-rail (no variant prop). |
...
| Touch swipe + click conflict (Embla drag absorbs click) | Low | Medium | Embla handles this natively via its `clickAllowed` logic. Tested in media-carousel. |
| `subscribe` identity churn from non-memoized host callback | Medium | Medium | Document `useCallback` requirement loudly in usage.tsx (same convention as engagement-bar / comment-thread / post-card). |
...
4. **`<StoryRail>` root** — Embla wiring + handle + edge gradients + framed/bare branch.
