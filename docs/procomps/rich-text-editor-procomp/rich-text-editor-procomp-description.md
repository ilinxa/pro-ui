# rich-text-editor — procomp description

> Stage 1: what & why.
>
> **Greenfield component** — no migration origin. The kasder source has a markdown-text-renderer (line-by-line regex parser); rich-text-editor is a separate paradigm (WYSIWYG editor + JSON viewer) and a multi-year architectural commitment.
>
...
- **Two-export sealed folder.** `<RichTextEditor>` (`"use client"`, ~150KB gzip) and `<RichTextViewer>` (RSC-friendly, ~30KB gzip). Same JSON shape; different bundle profiles.
...
const [value, setValue] = useState<RichTextValue>(initial);
...
1. **SSR works in Next 16.** `/components/rich-text-editor` prerenders with `HTTP 200`. Both editor (in a client boundary) and viewer (server-rendered) appear in the demo and behave correctly.
...
9. **TypeScript strict.** All exports typed; `RichTextValue`, `ImageUploader`, `ImageUploadResult` exported.
10. **Lint clean.** No new warnings (1 pre-existing card-tree warning OK).
...
2. ✅ **`platejs/static` SSR works in Next 16** — verified via `pnpm build` prerender of `/components/rich-text-editor`.
