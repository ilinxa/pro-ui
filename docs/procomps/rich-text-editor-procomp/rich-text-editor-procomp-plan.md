# rich-text-editor — procomp plan

> Stage 2: how. Implementation blueprint for [`rich-text-editor-procomp-description.md`](./rich-text-editor-procomp-description.md).
>
...
>
> **Files added in v0.2:** `lib/lowlight-instance.ts`, `lib/serialize-html.ts`, `parts/image-element.tsx`, `parts/toolbar-buttons.tsx`, `parts/floating-toolbar.tsx`. Files modified: `rich-text-editor.tsx` (mount FloatingToolbar), `parts/editor-toolbar.tsx` (refactored to import from toolbar-buttons), `parts/element-renderers.tsx` (re-export ImageElement from new module), `static-elements/static-element-renderers.tsx` (StaticImage reads width + caption), `plugins/editor-kit.ts` (configure CodeBlock with lowlight + register CaptionPlugin), `plugins/viewer-kit.ts` (configure CodeBlock with lowlight), `index.ts` (export `serializeRichTextToHtml` + `SerializeHtmlOptions`), `meta.ts` (v0.1.0 → v0.2.0 + new feature list + new npm deps), `dummy-data.ts` (added `RICH_TEXT_DUMMY_IMAGE` fixture + lang-tagged code-block fixtures), `demo.tsx` (8 sub-tabs covering all v0.2 features), `src/app/globals.css` (highlight.js token CSS classes mapped to chart palette).
>
> **Verification:** tsc + lint + build clean (1 pre-existing card-tree warning, no new); 27 routes prerendered; SSR HTTP 200 with default editor tab content rendering. Lowlight server-side render not directly verified — Radix Tabs lazy-mounts inactive tabs, so the Code tab fixture renders client-side only on tab click. Build success + plugin set type-checks both ways = lowlight wiring correct; runtime visual verification deferred to in-browser smoke.
>
> ## v0.1 plan (below)
...
```
src/registry/components/data/rich-text-editor/
├── rich-text-editor.tsx              # RichTextEditor — "use client", main editor
├── rich-text-viewer.tsx          # RichTextViewer — RSC-friendly viewer
├── plugins/
...
`RICH_TEXT_EMPTY_VALUE` and `RICH_TEXT_DEFAULT_PLACEHOLDER` are exported as sentinels.
...
A fixed top bar with role `"toolbar"` + `aria-label="Rich text editor toolbar"`. Internal layout: `<ToolbarGroup>` clusters separated by `<ToolbarSeparator />`.
...
export function RichTextEditor(props) {
  ...
  const initialValue = value ?? defaultValue ?? RICH_TEXT_EMPTY_VALUE;
...
export function RichTextViewer({ value, className, fallback }) {
...
3. ✅ `pnpm build` clean — 27 routes prerendered including `/components/rich-text-editor`
4. ✅ SSR `HTTP 200` (~94KB) on `/components/rich-text-editor` with 5 demo tabs
