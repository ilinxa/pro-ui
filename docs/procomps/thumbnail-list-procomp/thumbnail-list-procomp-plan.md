# thumbnail-list — procomp plan

> Stage 2: how. Implementation blueprint for [`thumbnail-list-procomp-description.md`](./thumbnail-list-procomp-description.md).

## File map

```
src/registry/components/data/thumbnail-list/
├── thumbnail-list.tsx           # root component, memoized default export
├── parts/
│   ├── thumb-row.tsx           # single <li> with image + title + meta + link wrap
│   └── empty-state.tsx         # default empty fallback
├── types.ts                    # ThumbnailListItem + props + defaults
...
export function ThumbnailListEmpty({ custom, message }: EmptyStateProps) {
...
import type { ThumbnailListItem } from "../types";
...
function ThumbnailListInner(props: ThumbnailListProps) {
  ...
  return (
    ...
    >
      ...
      {isEmpty ? (
        <ThumbnailListEmpty custom={emptyState} message={resolvedLabels.emptyText} />
      ) : (
...
export const THUMBNAIL_LIST_DUMMY: ReadonlyArray<ThumbnailListItem> = [
  {
    id: "1",
...
export const THUMBNAIL_LIST_DUMMY_TR: ReadonlyArray<ThumbnailListItem> = [
...
export const THUMBNAIL_LIST_DUMMY_DATED: ReadonlyArray<ThumbnailListItem & { publishedAt: string }> = [
...
3. `pnpm build` clean — `/components/thumbnail-list` prerendered
...
6. Registry artifacts at `public/r/thumbnail-list.json` + `public/r/thumbnail-list-fixtures.json`

## Risks / known unknowns

1. **Image error fallback** — broken `src` shows browser default. Acknowledged.
2. **Heading-order pitfalls** — if a consumer uses `<thumbnail-list headingAs="h2">` deep inside an article whose top-of-page is also `<h2>`, document order may be wrong. Default `h3` is the safe choice; documented in guide.
