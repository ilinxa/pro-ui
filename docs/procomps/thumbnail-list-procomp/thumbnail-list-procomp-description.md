# thumbnail-list — procomp description

> Stage 1: what & why.
>
> **Migration origin:** [`docs/migrations/thumbnail-list/`](../../migrations/thumbnail-list/) (kasder `kas-social-front-v0` news detail page sidebar — `src/app/(platform)/news/[id]/page.tsx` lines 208–234)
>
> Cousin migration: [`author-card`](../../migrations/author-card/) — sibling extraction from the same sidebar.
...
- **Pagination / infinite-scroll** — small lists only. If the consumer needs more, they reach for `magazine-layout` or future `data/list-XX`.
...
2. **Sibling-rhythm match** — header typography, card chrome, spacing rhythm matches `author-card` / `newsletter-signup` / `category-cloud` / `filter-bar` so the family composes.
3. **Generic shape works** — fixed `ThumbnailListItem` shape (`{ id, title, imageSrc, imageAlt?, meta?, href? }`) covers the realistic 80% case; `renderMeta` slot covers the long tail.
...
12. **Verification** — `pnpm tsc --noEmit` + `pnpm lint` + `pnpm build` clean; SSR `HTTP 200` for `/components/thumbnail-list` with all demo tabs rendered.
...
- **Generic `<ThumbnailList<T>>` with accessor functions?** — The fixed-shape `ThumbnailListItem` covers the realistic 80% with less prop ceremony. Consumers with custom shapes do `items.map(toThumbnailListItem)` upstream. Consistent with `author-card` flat-prop API.
- **shadcn `Card` wrapper around the framed variant?** — kasder visual is `bg-card rounded-2xl p-6 border border-border/50`; shadcn `Card` is `rounded-xl border` with different padding semantics. Match the kasder visual exactly via raw classes; same approach as `author-card` / `newsletter-signup`.
- **Compound component pattern (`<ThumbnailList.Header>` etc.)?** — Overkill. Flat-prop API mirrors the family.
- **Combine with `data-table`?** — Different pattern. `data-table` is column-driven and selection-aware; `thumbnail-list` is row-as-link, image-forward. Different use cases.
