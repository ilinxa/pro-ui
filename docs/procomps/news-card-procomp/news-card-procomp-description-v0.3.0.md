# `news-card` v0.3.0 — Description Addendum (Stage 1)
...
- An optional `NewsCardPermissions` matrix for per-action granular overrides
...
- Editor-mutation handlers separated from engagement handlers (same pattern as post-card's `PostMutationHandlers`)
...
| `quotedArticle?: NewsCardItem`              | analysis pieces that quote a source article — recursive nested mini-card   | **add (optional, recursive type — same pattern as post-card `repostOf`)** |

v0.3.0 grows the type to **40 fields total** (9 existing + 31 new), all new ones optional. v0.2.x consumers' `NewsCardItem` shape still satisfies. Existing `author: string` field stays alongside new structured `authorEntity` — host can supply either (soft-compat per Q-D8).
...
v0.2.0 has none of these as slots/sub-exports. v0.3.0 ships each as **(default render) + (renderXxx slot for full takeover) + (disableXxxRender opt-out) + (sub-export for standalone use)** — the same 4-axis pattern post-card v0.2.0 locked.
...
CMS dashboards need a programmatic escape hatch. v0.3.0 ships the same `ref` pattern as post-card:

```ts
interface NewsCardHandle {
  openKebab: () => void;
  triggerEdit: () => void;
  triggerDelete: () => void;
  ...
  reset: (next: NewsCardItem) => void;
  getCurrentItem: () => NewsCardItem;
}
```

11 handle methods, same mental model as post-card.
...
| Block                                           | Status            | Notes                                                                                                                      |
|-------------------------------------------------|-------------------|----------------------------------------------------------------------------------------------------------------------------|
| **Role-aware mode** (`viewerMode` + permissions + canPerformAction predicate) | ✅ in scope | Mirror post-card |
...
| **Kebab system** (defaults + role-aware items + moderator section + full takeover slot) | ✅ in scope | Reuses `CommentMenuItem` from comment-thread for parallelism                                                          |
...
| **Sensitive content gate** (default render + slot + opt-out + sub-export) | ✅ in scope | Mirrors post-card's SensitiveGate                                                                                 |
...
| **Quoted article mini-card** (recursive nested) | ✅ in scope       | Mirrors post-card `repostOf`                                                                                            |
| **Per-entity click handlers** (×10 listed §1.4) | ✅ in scope       |                                                                                                                            |
| **Imperative handle** (11 methods)              | ✅ in scope       | `ref` prop, mirrors post-card handle                                                                                    |
...
Identical structural shape to `PostPermissions` in post-card, editorial vocab.

```ts
export type NewsCardPermissionAction =
  | "edit"
  | "delete"
  | "publish"
  | "unpublish"
...
export interface NewsCardPermissions {
...
**Resolution order** (most → least specific) — identical to post-card:
...
Same handler-driven gating as post-card v0.1 legacy mode. When no handlers wired, kebab doesn't render.
...
Same orthogonal pattern as post-card v0.3.0. When `permissions.canModerate === true` OR `canPerformAction("moderate", item) === true`, AND `moderatorActions(item)` returns ≥1 item, the moderator group renders **between common items and reader-destructive items** with a divider above. Host supplies the actual menu items (e.g. "Take down", "Lock comments", "Flag for review", "Approve flag"). Suppressed when `kebabActions` full-takeover slot is used.

### 5.5 Full takeover

`kebabActions={(item) => CommentMenuItem[]}` — same shape as post-card. When supplied, all default items + moderator section + permission gating bypassed; host's items render verbatim. Use case: editorial CMS dashboards with their own action catalog.

---

## 6. Slots / sub-exports / opt-outs

Same 4-axis pattern as post-card v0.2.0 — for each sub-feature: default render + `renderX` slot for full takeover + `disableXRender` opt-out + sub-export of the default part.
...
All `renderX` slots receive `(item, helpers)` where `helpers` carries pre-wired callbacks the host needs (`onReveal`, `onClick`, `handlers` bag, etc.). Same shape as post-card.

### 6.2 Engagement integration with `engagement-bar` (the news-detail-page pattern)
...
For the **news article detail page** (or any surface where the news card IS the engagement surface — e.g. a Reddit-style feed where readers vote/comment inline), the same card composes the full `engagement-bar` via the `renderEngagementCounts` slot:

```tsx
import { EngagementBar } from "@ilinxa/engagement-bar";
import { NewsCard } from "@ilinxa/news-card";
...
- Keeps `engagement-bar` OFF the news-card's peer-dependency list when consumers only want light counts (the 95% case). No bundle bloat for index-page consumers.
...
- Engagement data shape on `NewsCardItem` (likeCount / isLiked / commentCount / bookmarkCount / isBookmarked / shareCount) deliberately *matches* engagement-bar's counts shape, so passing data through is trivial.
...
interface NewsCardProps {
  ...
  onClick?: (args: { item: NewsCardItem; event: MouseEvent }) => void;
  ...
  onQuotedClick?: (quotedArticle: NewsCardItem) => void;
  ...
  onComment?: (articleId: string) => void;  // alias for onCommentCountClick — kept for symmetry with post-card
...
```ts
export interface NewsCardHandle {
  openKebab: () => void;
  triggerEdit: () => void;
  triggerDelete: () => void;
  triggerPublish: () => void;
...
1. Every v0.2.x `NewsCardItem` shape still type-satisfies — new fields all optional.
...
- **Q-D1** Role-aware mode opt-in semantics — `viewerMode === undefined` means v0.2 legacy mode (no kebab, no role-aware items). Host explicitly picks `"editor"` or `"viewer"`. No auto-derivation from `currentUser`. **`(rec)` confirm — mirrors post-card.**
...
- **Q-D2** Naming — `viewerMode: "editor" | "viewer"` vs `"owner" | "viewer"`?
  ...
  - (b) `"owner" | "viewer"` — strict parallelism with post-card

- **Q-D3** Moderator section — orthogonal `canModerate` + `moderatorActions(item)` slot, divider-separated kebab group. **`(rec)` confirm — mirrors post-card v0.3.0.**
...
- **Q-D5** Quoted article recursion — `quotedArticle?: NewsCardItem`. Renders as compact mini-card. Nested actions/kebab suppressed. No further recursion (a quoted article's `quotedArticle` is ignored). **`(rec)` confirm — mirrors post-card repostOf.**
...
- **Q-D6** Engagement model — light count-chip with handlers as DEFAULT, with a slot for full-bar composition.
  ...
  - (b) Compose engagement-bar like post-card (full inline panels)
  - (c) Skip engagement entirely — counts not in v0.3, defer

- **Q-D6b** Detail-page composition — `renderEngagementCounts` slot lets consumers compose `<EngagementBar>` on the news article detail page. Engagement data shape on `NewsCardItem` deliberately matches `engagement-bar`'s counts shape so the slot is drop-in (§6.2). `engagement-bar` is NOT a peer dep of `news-card` (consumer imports it themselves when they opt in). **`(rec)` confirm — keeps light-counts the default while making full-bar composition trivial.**
...
- **Q-D8** `author` vs `authorEntity` soft-compat — both fields supported on `NewsCardItem`; `authorEntity` wins when both set. Renderer falls back to `author` string. **`(rec)` confirm.**
...
- **Q-PF** Sensitive gate scope — gates the **media only** (excerpt + title still visible above the gated image). Mirrors post-card v0.2 sensitive-gate scope. **`(rec)` confirm.**
...
- **Q-PH** Locale of default kebab labels — English defaults via `DEFAULT_NEWS_CARD_LABELS`, host overrides via `labels` prop. Same pattern as post-card. **`(rec)` confirm.**

- **Q-PI** Touch targets — every interactive (chip, kebab trigger, count chip, paywall CTA) ≥44×44px per WCAG 2.5.5. Same standard as post-card v0.2.0. **`(rec)` confirm.**
...
- **Q-V3** Shadcn primitive additions — needs `dropdown-menu` (kebab). v0.2 only depends on `badge`. Adding `dropdown-menu` is a `pnpm dlx shadcn add dropdown-menu` away (already present in the project per post-card). Adds to meta `dependencies.shadcn`. **`(rec)` confirm.**
...
|---------------------------------------------------------------------|----------------|--------------------------------------------------------------------------------------------------------------------------------------------------------|
| Related articles ribbon inside the card                             | sibling component (**queued: `related-articles-ribbon-01`**) | Breaks magazine-card visual model. Will ship as its own procomp that consumers stack *under* a news-card in their detail-page layout. Added to active component queue per user sign-off 2026-06-02. |
...
| Full engagement-bar composition as a built-in mode (the card always composes the bar) | never as built-in | Made *connectable* via the `renderEngagementCounts` slot in v0.3 (see §6.2). Consumers opt-in on the detail page by passing `<EngagementBar>` into the slot. Card stays light by default; engagement-bar stays out of the peer-dep list. |
...
| Inline poll widget (like post-card)                              | never          | News articles don't carry polls in the card preview.                                                                                                   |
| Inline link-preview card (like post-card)                        | never          | News card IS the link-preview equivalent; nested previews aren't editorial UX.                                                                         |
...
- [ ] **2.** `NewsCardPermissions` matrix with 19 capabilities
...
- [ ] **8.** Schema expansion — 31 new optional `NewsCardItem` fields
...
- Stage 2 plan can begin authoring at `news-card-procomp-plan-v0.3.0.md`
