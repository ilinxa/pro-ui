# news-card — procomp description

> Stage 1: what & why. The "should we build this at all?" doc.
>
> **Migration origin:** [`docs/migrations/news-card/`](../../migrations/news-card/) (kasder `kas-social-front-v0` `NewsCard.tsx`)
>
> Family: 4-component news-domain set (`news-card`, `page-hero`, `magazine-layout`, `detail-page-news-01`-deferred). All four totally independent, fully compatible, dynamic.
...
Pro-ui has no existing answer here. `card-tree` is a tree editor; `data-table` is tabular; force-graph is graph viz. None render an article preview with image + title + excerpt + meta in 5 calibrated densities driven by a single prop.
...
- **Layout orchestration** — composing cards into magazine grids, infinite scroll, search/filter UIs. That's `magazine-layout`.
- **Rich text rendering** — full article view with markdown / prose / sidebar / share. That's `detail-page-news-01` (deferred).
...
**1. Magazine landing page** (composes with `magazine-layout`):
...
- SSR `/components/news-card` returns 200 with all 7 demo tabs (5 variants + Composed + Actions slot).

## Open questions

1. **Component export name.** `NewsCard` (verbose, version-explicit, future-proof for `02`/`03`) vs. `NewsCard` (clean JSX, requires alias on import if multiple variants installed). **Resolved (analysis sign-off):** `NewsCard` — verbose but unambiguous.
...
5. **Loading skeleton variants.** Should the card ship a `<NewsCard.Skeleton variant="medium" />` companion for loading states? **Open** — v0.1 ships without; consumers can render a `bg-muted` placeholder. v0.2 candidate.
