# `news-card` v0.3.0 — Plan Addendum (Stage 2)

> **Stage:** 2 of 3 · **Status:** 🟡 Drafted, awaiting sign-off
> **Slug:** `news-card` (unchanged) · **Target version:** `0.3.0`
> **Depends on:** [news-card-procomp-description-v0.3.0.md](./news-card-procomp-description-v0.3.0.md) (GATE 1 ✅ signed off 2026-06-02 — all defaults + 2 user amendments: engagement-bar slot + related-articles-ribbon-01 sibling queued)
>
> This addendum is the **implementation contract** for v0.3.0 — *how* the description's 14 A+ traits land in code. The v0.1/v0.2 base plan ([news-card-procomp-plan.md](./news-card-procomp-plan.md)) carries unchanged; this file extends with the v0.3.0-specific commit chain, resolver semantics, file deltas, and risk register.
...
| **Q-P26** | Permissions resolver lives at `lib/permissions.ts`. Scoped to news-card. Cross-card lift to a shared `@/lib/permissions` is v0.4+; explicit pattern parallelism with post-card's `lib/permissions.ts` for now (two files, identical shape, different action enum). | parallelism w/ post-card |
| **Q-P27** | Resolution order: `canPerformAction(action, item)` returning `true \| false` → `permissions[canX]` → `viewerMode`-derived defaults → legacy mode (no kebab). `canPerformAction` returning `undefined` falls through. Same shape as post-card. | GATE 1 §4 |
| **Q-P28** | `NewsCardMutationHandlers` interface flattened onto `NewsCardProps` (not a nested prop bag). Same pattern as post-card's `PostMutationHandlers`. Editor-side (12) + reader-side (4) handlers in one interface. | parallelism w/ post-card |
| **Q-P29** | `defaultNewsCardKebabActions` helper is **new** (v0.2 ships no kebab). Single helper, dual-mode: when all 3 role-aware args undefined AND ≥1 engagement handler wired → minimal default kebab (Bookmark / Share / Copy link / Translate / Report). When ANY role-aware arg set → assembles role-aware items per the matrix. Exported sub-export for extend-not-replace. **Refinement (locked C3, 2026-06-02):** signature uses a single `DefaultNewsCardKebabArgs` object arg instead of post-card's positional `(item, handlers, labels, viewerMode?, …)` — 8+ positional args are hard to call + adding new optional args breaks call sites. Object args follow the same shape as React component props and let future optional args land non-breakingly. post-card may adopt the same pattern in a future bump for parallelism. | GATE 1 §5.1 |
| **Q-P30** | `NewsVisibility` is **extensible string union** with 5 base values: `"public" \| "members" \| "subscribers" \| "staff" \| "unlisted"` + `(string & {})` branded extension. Library renders default labels for the 5 base values via `DEFAULT_NEWS_CARD_LABELS`. Custom values get "Custom" fallback. Same pattern as `PostVisibility`. | GATE 1 §3.1 |
...
| **Q-P32** | `quotedArticle?: NewsCardItem` is recursive — recursion-strip applies: a quoted article's own `quotedArticle` field is ignored when rendering the inner card. **Refined 2026-06-02 post-ship:** the inner card renders as `<NewsCard variant="small" disableQuotedRender disableEngagementCounts disablePaywallGate disableSensitiveGate disableBadgesRender actions={undefined} viewerMode={undefined} className="border-0 bg-transparent p-3 rounded-md shadow-none">` — `variant="small"` gives a compact thumb-left, body-right preview (initial impl used `variant="medium"` which rendered a full nested medium card with its own paywall + sensitive + badge stack, breaking the compact-citation pattern). All gates suppressed so a paywalled quoted article doesn't show "Premium content" inside the quote; badges suppressed so the inner small variant's top-right badge doesn't overlap the inner title in the compact preview. Mirrors post-card `repostOf` Q-P30. | GATE 1 §1.3 + Q-D5 |
| **Q-P33** | `paywall` gate is **per-card**, gates **media block only** (refined 2026-06-02 post-ship — original draft said "excerpt + media" but the impl wrapping both produced a `paywall.preview`-overlapping-the-badge-stack bug; corrected scope below). Title + author byline + footer + engagement counts stay visible above and below the gate. `paywall.preview` SUBSTITUTES for `item.excerpt` in the body (variants compute `displayExcerpt = paywall.preview ?? item.excerpt` and render it in the excerpt slot). Tap on CTA fires `onRevealPaywall(articleId)`. Library renders default CTA using `paywall.ctaLabel` + `paywall.ctaHref` per Q-D7=(a). **CTA element resolution:** when `paywall.ctaHref` set → renders as `<a href={ctaHref}>` (consumer can pass `linkComponent={NextLink}` to swap), `onClick` fires `onRevealPaywall` BEFORE navigation; when `ctaHref` not set → renders as `<button>` that fires `onRevealPaywall` only. **Per-variant placement:** featured uses `className="absolute inset-0"` (gate covers full hero; content overlay div bumped to `z-20` so title stays visible above the gate's `z-10`); large + medium wrap just the media block in normal flow above the title; list (no image) wraps the excerpt block. NewsPaywallGate is purely a "blur children + overlay with CTA + lock + heading" component — it does NOT render the preview text itself (that's the variant's job in the body excerpt slot). | GATE 1 Q-D7 + Q-PG |
| **Q-P34** | `sensitivity` gate is **per-card** (gates media only — excerpt + title remain visible). Single reveal button fires `onRevealSensitive(articleId)` (analytics hook) + sets local `sensitiveRevealed=true`. Keyboard-operable. `prefers-reduced-motion` snap-reveal. Mirrors post-card SensitiveGate Q-P33. **Refined 2026-06-02 post-ship:** `ContentSensitiveGate` accepts a `compact: boolean` prop. When `true`: heading text + warnings list dropped, icon shrinks `size-6 → size-5`, 44px reveal button → 24px "Show" pill (aria-label carries full heading + reveal copy for screen readers). Total content height ~60px — fits cleanly in the `small` variant's 96×96 thumb. `small.tsx` passes `compact` to the gate. All other variants use the full overlay. | GATE 1 Q-PF |
| **Q-P34b (locked 2026-06-02 post-ship)** | `small` variant article uses `flex items-start gap-4` (not the flex-default `align-items: stretch`) so when grid layout stretches the article vertically — e.g. when paired with a taller medium variant in a `grid-cols-2` row — the 96×96 thumb's gate wrapper stays at 96×96 instead of stretching to fill the article height. Defensive against ambient grid stretching. | post-ship visual sweep |
| **Q-P35** | **Engagement model = light count chips by default + slot-driven full-bar composition.** `renderEngagementCounts` slot receives `(item, { handlers })` where `handlers = { onLike, onComment, onShare, onBookmark }` pre-wired. Consumers compose `<EngagementBar>` via the slot on detail pages (§6.2 of description). `engagement-bar` is NOT a peer-dep of news-card. Data-shape on `NewsCardItem` matches engagement-bar's counts shape for drop-in composition. | GATE 1 Q-D6 + Q-D6b |
...
| **Q-P42** | Badge stack priority (Q-PC) implemented in `parts/news-badges.tsx` via a fixed ordered render — first match wins for `small` variant; full stack renders for other variants. Order frozen: `isBreaking` → `isLive` → `isExclusive` → `isFeatured` → `isPinned` → `isSponsored` → `status` (editor mode only). | Q-PC |
| **Q-P42b (locked 2026-06-02 post-ship)** | Badge **placement split** per Q-PC2 — `NewsBadges` accepts `group: "all" \| "state" \| "curation"`. State group = Breaking / Live / Pinned / Sponsored / Status (urgency, admin, commercial); curation group = Exclusive / Featured (editorial product label). v0.3.0 `medium` variant renders TWO `<NewsBadges>` instances: state at top-right overlay, curation as a kicker row above the title in the body. Priority order preserved within each group. Other variants (`featured` / `large` / `list` / `small`) keep `group="all"` for now — kicker pattern can be adopted in v0.3.1 if the same visual conflict surfaces. Default `"all"` preserves backward-compat for any consumer rendering the standalone `<NewsBadges>` sub-export. **Uniform shape (locked alongside the split):** all badges share a `BADGE_BASE` const (`h-5 px-1.5 text-[10px] font-semibold uppercase tracking-wide leading-none rounded shrink-0`). Hierarchy via saturation: vivid (Breaking/Live red solids), accent (Exclusive amber / Featured primary solids), subtle (Pinned card-tone). Drop-shadow on the vivid tier reads against bright hero images. Filled-icon (★ Featured, 📍 Pinned) reinforces editorial vs urgency semantics. | Q-PC2 |
| **Q-P42c (locked 2026-06-02 post-ship)** | Medium variant badge wrapper at top-right uses `max-w-[75%] flex-wrap justify-end gap-1` so 3 tightened state badges fit on a single line at the medium card width (~280px); beyond that they wrap-end INWARD toward the title (not into the category chip at top-left). Top-left of the medium image area belongs to the category chip exclusively. | post-ship visual sweep |
...
Each commit lands on its own; `pnpm tsc --noEmit && pnpm lint && pnpm validate:meta-deps` must pass at every step. Smoke harness runs at **C12** (final). No prerequisite C0 (engagement-bar stays untouched — composition is slot-driven, not built-in).
...
|---|---|---|---|
| **C1** | **Type additions** — All new types and interfaces. `ContentStatus` (closed enum) / `NewsVisibility` (extensible) / `NewsArticleAuthor` / `NewsPublisher` / `ContentSensitivity` / `ContentPaywall` / `NewsCardPermissionAction` (19 action discriminators: edit / delete / publish / unpublish / schedule / feature / pin / changeVisibility / changeCategory / markSensitive / seeAnalytics / pushToTop / share / bookmark / report / blockAuthor / muteAuthor / unfollowTopic / moderate) / `NewsCardPermissions` (19 capabilities: 12 editor-side + 2 host-policy + 4 reader-side + 1 moderator) / `NewsCardMutationHandlers` (16: 12 editor + 4 reader) / `NewsCardHandle` (11 methods). Extend `NewsCardItem` with **31 new optional fields** (per description §3.1: slug, authorEntity, publisher, publishedAt, updatedAt, scheduledFor, status, visibility, topics, tags, language, availableTranslations, isPinned, isFeatured, isBreaking, isLive, isExclusive, isSponsored, sponsorLabel, liveUpdateCount, lastLiveUpdateAt, sensitivity, paywall, commentsEnabled, commentCount, likeCount, isLiked, bookmarkCount, isBookmarked, shareCount, quotedArticle). Extend `NewsCardProps` with **~50 new optional props** (13 per-entity click handlers + 16 mutation handlers + 9 render slots incl. kebabActions/moderatorActions + 7 opt-outs + 3 role-aware args + ref + onComment alias). Extend `NewsCardLabels` with **~49 new keys** per description §9 (22 kebab + 6 visibility + 3 status + 6 editorial badges + 4 paywall + 3 sensitive + 4 engagement + 1 live). Extend `DEFAULT_LABELS` with new defaults. Update `ResolvedPartProps` to carry all new resolved fields downstream. | `types.ts` only | tsc + lint clean |
| **C2** | **Permissions resolver** — `lib/permissions.ts` (`resolveNewsCardPermissions` + `canPerformActionInternal` + `PERMISSION_DEFAULTS_BY_MODE` const). Pure, no React, no DOM. Hand-walked truth table (no Vitest — project lock). | `lib/permissions.ts` (new) | tsc + lint + hand-walked truth table |
| **C3** | **Default kebab helper** — `lib/defaults.tsx` new file. Implement `defaultNewsCardKebabActions(item, handlers, viewerMode?, permissions?, canPerformAction?, mutationHandlers?, moderatorActions?, labels?)`. Dual-mode entry-branch per Q-P29. Returns `CommentMenuItem[]` (reuses shape from comment-thread — same as post-card's kebab). Sub-export from `index.ts`. | `lib/defaults.tsx` (new) | tsc + lint |
| **C4** | **Local-mirror + handle** — extend `news-card.tsx` with `statefulItem` (mirrors `item`) + `paywallRevealed` + `sensitiveRevealed` local state. Add `useImperativeHandle` for 11 handle methods. Add `ref` to `NewsCardProps`. | `news-card.tsx` | tsc + lint |
...
`resolveNewsCardPermissions(viewerMode, permissions)`:

| `viewerMode` | `permissions` | Resulting `Required<NewsCardPermissions>` |
|---|---|---|
...
```
src/registry/components/data/news-card/
├── news-card.tsx              [edit — local-mirror, handle, kebab integration]
...
│   ├── permissions.ts                    [NEW — resolveNewsCardPermissions]
│   └── defaults.tsx                      [NEW — defaultNewsCardKebabActions + recursion-strip helper]
└── parts/
...
Currently the news-card item ships **9 files**. After v0.3.0:

- **Base item `news-card`:** add 13 new file entries (5 parts + 1 author + 1 publisher + 1 paywall + 1 sensitive + 1 quoted + 1 engagement + 2 lib). Total file count: **9 → 22**.
- **Fixtures item `news-card-fixtures`:** unchanged (just `dummy-data.ts` — gets a content refresh but not new files).
- **registryDependencies:** **unchanged** (engagement-bar NOT added; composition is consumer-side via the slot per Q-P35).
...
| **R-2** | `CommentMenuItem` cross-procomp import (kebab item shape comes from comment-thread) | Low | Use F-S1 lock — RELATIVE path import to `../comment-thread/types` (not the barrel) to dodge shadcn 4.6.0's path-rewriter bug. Precedent: post-card v0.3.1. |
...
| **R-7** | shadcn DropdownMenu primitive Radix→Base UI divergence (F-cross-13) | Med | Apply the established defensive-callback pattern (don't pass divergent props; use direct trigger, not asChild). Precedent: engagement-bar v0.3.2 + post-card v0.3.x. |
...
| **R-9** | Quoted-article card calling itself recursively could leak `actions` / `onClick` from the outer card | Low | Q-P32 explicit strip — recursion-strip helper at `lib/defaults.tsx` returns a pruned `NewsCardItem` + the inner card sets `actions={undefined}` + `viewerMode={undefined}` + `disableQuotedRender` to prevent further nesting. |
| **R-10** | `~22 new optional fields` on `NewsCardItem` could surprise users into making them required | Low | JSDoc on every new field explicitly states "optional" + soft-fail behavior. Demo tabs deliberately mix populated/absent fields to demonstrate. |
...
- `pnpm registry:build` regenerates `public/r/news-card.json` artifact
- **Manual registry-roster audit** — diff `find src/registry/components/data/news-card` vs `registry.json files[]` to catch any missing-file blocker (per `project_registry_roster_manual_audit_pattern` memory — established 2026-05-30 from story-viewer v0.4.3)
- Smoke harness `pnpm dlx shadcn add @ilinxa/news-card` in `e:/tmp/ilinxa-smoke-consumer/` + consumer-side `pnpm tsc --noEmit` (F-cross-11 path-b)
...
- Author `docs/procomps/news-card-procomp/reviews/<YYYY-MM-DD>-v0.3.0-spotcheck.md`
...
| C8 (sensitive gate) | 30 min (mostly mirrors post-card's part) |
