# news-card — procomp guide

> Stage 3: how to use it. Authored alongside the implementation.
>
> See [`news-card-procomp-description.md`](./news-card-procomp-description.md) for *why* and [`news-card-procomp-plan.md`](./news-card-procomp-plan.md) for *how*.
...
- **Tree-structured content** with editing — use `card-tree`.
- **Rich text / full prose page** — that's `detail-page-news-01` (deferred sibling).
...
<div className="space-y-8">
  <NewsCard item={featured} variant="featured" href={`/news/${featured.id}`} />
  <div className="grid gap-6 lg:grid-cols-12">
    <div className="space-y-6 lg:col-span-8">
      <NewsCard item={lead} variant="large" href={`/news/${lead.id}`} />
      <div className="grid gap-6 md:grid-cols-2">
        {middleArticles.map(item => (
          <NewsCard key={item.id} item={item} variant="medium" href={`/news/${item.id}`} />
        ))}
      </div>
    </div>
    <aside className="lg:col-span-4">
      <div className="rounded-2xl border bg-card p-4">
        {topPopular.map(item => (
          <NewsCard key={item.id} item={item} variant="list" href={`/news/${item.id}`} />
        ))}
      </div>
    </aside>
  </div>
...
```tsx
<NewsCard item={item} variant="medium" loading="eager" />
```
...
The original lives in [`docs/migrations/news-card/original/`](../../migrations/news-card/original/) for historical reference; never imported.

## Open follow-ups

### v0.2 candidates

- **Skeleton companion** (`<NewsCard.Skeleton variant="medium" />`) for loading states matching each variant's shape.
...
These were the original v0.3 candidates carried forward from v0.1 — v0.3.0 actually went much wider (full backend-shaped feature parity with post-card). The original candidates below are still queued for v0.4+:
...
v0.3.0 mirrors the post-card v0.3.2 trait set translated into editorial vocabulary. **Strictly additive on v0.2** — every v0.2.x consumer keeps working unchanged.

See [`news-card-procomp-description-v0.3.0.md`](./news-card-procomp-description-v0.3.0.md) for the full A+ trait set and [`news-card-procomp-plan-v0.3.0.md`](./news-card-procomp-plan-v0.3.0.md) for the implementation contract.
...
```tsx
const paywalledArticle: NewsCardItem = {
  // ...
...
```tsx
const sensitiveArticle: NewsCardItem = {
  // ...
  sensitivity: {
    isSensitive: true,
...
```tsx
const ref = useRef<NewsCardHandle>(null);
// ... later:
...
```tsx
import { ContentSensitiveGate } from "@ilinxa/news-card";
...
```tsx
import { NewsBadges } from "@ilinxa/news-card";
...
```tsx
const analysisItem: NewsCardItem = {
  // ...
...
### Engagement counts → engagement-bar composition
...
**Detail page composition** — compose `<EngagementBar>` for the full bar:
...
`engagement-bar` is **NOT** a peer dep of news-card — consumers who only want light counts don't pull it in. Engagement data shape on `NewsCardItem` matches engagement-bar's counts shape so the slot is drop-in. See description §6.2 for the full pattern.
...
<NewsCard ref={ref} item={article} viewerMode="editor" onEdit={...} onDelete={...} />
...
import {
  ...
  NewsPublisherRow,
  // Helpers
  defaultNewsCardKebabActions,
  resolveNewsCardPermissions,
  stripQuotedRecursion,
  DEFAULT_LABELS,
} from "@ilinxa/news-card";
```
...
- **No virtualization** at the card level — that's the layout's job (`magazine-layout` will support `virtualize: 'auto'` when it ships).
