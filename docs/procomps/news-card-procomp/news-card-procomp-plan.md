# news-card — procomp plan

> Stage 2: how. The implementation contract.
>
> See [`news-card-procomp-description.md`](./news-card-procomp-description.md) for the what & why.
>
> **Migration origin:** [`docs/migrations/news-card/`](../../migrations/news-card/) — see [`analysis.md`](../../migrations/news-card/analysis.md) for the design-DNA inventory + 16 numbered rewrite items + dependency audit.
...
```ts
// src/registry/components/data/news-card/types.ts
...
export interface NewsCardProps {
  ...
  onClick?: (item: NewsCardItem, event: React.MouseEvent) => void;
...
```ts
// src/registry/components/data/news-card/index.ts
...
The card uses `NewsCardItem` directly, not `<T extends ItemBase>`. Generic shape was considered and rejected — strict shape is more ergonomic for the 99% case (consumers `.map(raw => mapToNewsCardItem(raw))` once before render). Power users still customize via `titleClassName` / `imageClassName` / `categoryStyles` / `actions`.
...
### 1. `news-card.tsx` — root component
...
```ts
export const dummyNewsCardItems: NewsCardItem[] = [
...
export const newsCardMeta: ComponentMeta = {
  slug: 'news-card',
  name: 'News Card',
  category: 'data',
  status: 'alpha',
...
**Client component** — `news-card.tsx` declares `"use client"`. Required because it uses React reconciler hooks (`useId`, `useMemo`, `useCallback`) and `React.memo`, all of which need the client runtime in Next.js App Router.

The component still SSRs fine (the demo at `/components/news-card` returns 200 with content rendered); the client boundary just means hydration applies hover/focus interactivity on the consumer side. Function-rich props (`onClick`, `formatRelativeTime`, `formatDate`, `linkComponent`) are practical to use only from a client parent — passing functions across the RSC boundary isn't supported, so a client boundary somewhere in the tree is required anyway. Putting it on the card itself keeps consumer choice simple: import the card, pass props, done.
...
- [ ] `pnpm build` clean — all routes prerendered including `/components/news-card`.
- [ ] SSR `curl -s http://localhost:3000/components/news-card` returns 200 with all 7 demo tab triggers rendered (Featured / Large / Medium / Small / List / Composed / Actions slot) + Featured tab default content visible.
...
3. **Generic over item shape** (`<NewsCard<T>>`). Rejected — strict shape is more ergonomic; users can map to it cheaply. Generic adds complexity without enabling new use cases.
...
- v0.2: skeleton companion (`<NewsCard.Skeleton variant="medium" />`).
