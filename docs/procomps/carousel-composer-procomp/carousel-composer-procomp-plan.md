# carousel-composer — procomp plan

> Stage 2: how (the implementation contract). **GATE 2 — awaiting sign-off.** No code until signed off.
>
> Builds on the signed-off [description](./carousel-composer-procomp-description.md). Category `media`. Composes the shipped [`media-editor`](../media-editor-procomp/) v0.1.2 as a **single, serially-loaded edit panel** — media-editor source is NOT modified.

## 0. The one fact that drives the whole design

media-editor supports **edit-only mode**: `enabledModes={[]}` + `initialSource={…}` → it skips the entire capture surface (no camera, **no photo/video tabs**) and lands directly on the edit canvas for the supplied media ([types.ts:402](../../../src/registry/components/media/media-editor/types.ts#L402), [`MediaEditorProps`](../../../src/registry/components/media/media-editor/types.ts#L400)). The carousel mounts exactly one such instance, keyed by the item being edited. This is why "don't separate photo/video" falls out for free: intake is file-based (MIME decides `kind`), and the editor never shows capture chrome.
...
//   actually exposes (VERIFIED: MediaEditorProps/Handle, MediaEditorState,
...
//   media-editor untouched); the carousel defines its own `MediaCarouselError`.
...
// Own error type — media-editor's `ValidationError` is NOT re-exported from its
// .tsx entry, and we keep media-editor untouched. Structurally compatible.
...
export interface CarouselComposerProps {
  ...
  // forwarded subset of media-editor dials (edit panel). NOTE: crop aspect is
...
export interface CarouselComposerHandle {
   ...
   *  content-composer's active-step gating + media-editor's pull-only export). */
...
`CarouselComposer` (component) + all of: `MediaCarouselItem`, `MediaKind`, `MediaCarouselSource`, `CarouselComposerProps`, `CarouselComposerHandle`, `CarouselComposerLabels`. Plus the public parts (`PreviewRail`, `RailThumb`, `MainPreview`, `MediaDropzone`, `EditPanel`) for sealed-folder consumers, mirroring media-editor's barrel style.

## 2. File-by-file plan (sealed `data-table` shape)

```
src/registry/components/media/carousel-composer/
├── carousel-composer.tsx   root: state, DndContext, layout, handle, "use client"
├── types.ts                        §1.1
├── parts/
...
│   └── edit-panel.tsx              wraps <MediaEditor enabledModes={[]}> + Done/Cancel frame
├── hooks/
│   ├── use-carousel-state.ts       items + selectedId + editingId reducer; URL lifecycle
│   └── use-controllable-state.ts   sealed local copy (navigation/account-switcher + code-block precedent)
├── lib/
...
### Root component responsibilities (`carousel-composer.tsx`)
...
- Layout: a **main region** (top) + a **rail** (bottom). Main region shows, by UI mode:
  ...
  - `editingId != null` → `<EditPanel key={editingId} item={editing} …>` (the single media-editor instance).
- Rail always visible once `items.length > 0`: `<PreviewRail>` + trailing `<MediaDropzone variant="add-more">` when `< maxItems`.
- Forwards `ref` → `CarouselComposerHandle` (via `useImperativeHandle`).

### Edit handshake (`edit-panel.tsx`) — the core contract
1. Mount `<MediaEditor ref={editorRef} key={item.id} enabledModes={[]} presentation="inline" initialSource={initialSourceFor(item)} aspect={resolvedAspect} cropAspects={[resolvedAspect]} enabledTools={editorProps?.enabledTools} … />`.
...
4. Carousel-owned header: **Done** + **Cancel** (media-editor inline renders no publish/close chrome — the consumer owns it, exactly as content-composer's media-substrate does).
...
| Kind | Items | Notes |
|---|---|---|
| npm | `@dnd-kit/core`, `@dnd-kit/sortable`, `@dnd-kit/utilities` | **Already in `package.json`** (kanban-board). No new package. |
| internal registry | `@ilinxa/media-editor` | The ONLY cross-procomp dep. Import `MediaEditor` + types from its `.tsx` entry. Declare as `registryDependency`. |
...
**Cross-procomp import surface = exactly one module:** `@/registry/components/media/media-editor/media-editor` (the component value + the types `AspectRatio` / `ExportMetadata` / `InitialSource` / `MediaEditorState` / `MediaEditorProps` / `MediaEditorHandle` — all confirmed present in that file's tail re-export band). `validateGalleryFile` is **reimplemented locally** (`lib/validate-media-file.ts`) and the carousel defines its **own `MediaCarouselError`** (the `.tsx` does not re-export `ValidationError`). Net: media-editor source stays untouched and the cross-procomp surface is exactly one module. F-01 mitigation, made concrete.
...
- **Imperative handle** (`forwardRef` + `useImperativeHandle`) — pull-only export, mirrors media-editor.
- **Single delegated editor instance**, keyed remount per edit session (no N instances; respects media-editor's multi-instance guard).
- **dnd**: one `DndContext` at root; `SortableContext` (items = ids, `horizontalListSortingStrategy`) in the rail; `useSortable` per thumb. **Drag `listeners`/`attributes` attach to a dedicated drag-handle inside each thumb (NOT the whole thumb)** so the thumb body stays a click-to-select button and the Remove button works; Remove `onClick` calls `stopPropagation`. Sensors: `PointerSensor` (`activationConstraint: { distance: 4 }`) + `KeyboardSensor` (`sortableKeyboardCoordinates`). `onDragEnd` → `arrayMove` → `reorder`. Mirror kanban-board's setup ([use-drag-handlers.ts](../../../src/registry/components/data/kanban-board/hooks/use-drag-handlers.ts)).

## 5. Client vs server

Entire component is `"use client"` — object URLs, `FileReader`/`Image`/`<video>` dimension reads, `@dnd-kit`, refs, and the composed media-editor (Konva, `dynamic({ssr:false})`) are all browser-only. No server entry. Registry rule: no `next/*` imports (portable).
...
| **Video item Edit (v0.1)** | Edit **disabled** for `kind:"video"` (tooltip `videoNotEditable`). Preview + reorder + remove only. (Q4: limited edit; full video editing tracks media-editor maturity.) |
| Re-edit seed (remote URL, no blob) | `initialSource = {kind:"url"}`. CORS/fetch failures surface via media-editor's `onInitialSourceError`; carousel shows the item uneditable + a toast-free inline note. |
...
- **Edit panel**: on open, focus moves into the editor surface; **Done/Cancel** are labelled buttons; on close, focus returns to the edited item's thumb. media-editor owns its internal canvas a11y.
...
| # | Risk | Mitigation |
|---|---|---|
| R1 | **F-01 cross-procomp import** (rewriter mangles deep subpaths; type-only deps still need install) | Import component + types from the `.tsx` entry only; reimplement validation locally; declare `@ilinxa/media-editor` registryDependency; **local-registry re-smoke** before push (serve `public/r` → repoint tmp consumer → `shadcn add` + `tsc`). |
| R2 | media-editor inline + `enabledModes:[]` might render unexpected chrome | **Impl-start spike**: mount edit-only inline, confirm it lands on the edit canvas with no capture UI and no publish bar (content-composer already proves inline export works). If stray chrome appears, suppress within the EditPanel frame. |
| R3 | Video editing immaturity | v0.1 disables video Edit (R6/Q4). Honest tooltip; revisit when media-editor ships video tools. |
...
| — | **Alt: extend media-editor** | Rejected at GATE 1 (breaking a shared shipped procomp). |
...
- `pnpm tsc --noEmit` clean; `pnpm lint` no new findings; `pnpm validate:meta-deps` clean (declare the media-editor dep + shadcn primitives in `meta.ts`).
- `pnpm build` green; docs render at `/components/carousel-composer`.
- `pnpm registry:build`; spot-check `public/r/carousel-composer.json` (base + `-fixtures`).
...
`"library"` source (clamped), camera-as-add-source, full video editing, hard aspect-crop-on-add, video poster extraction, cross-item bulk ops, per-item caption/alt-text, and the **content-composer post-slot integration** (separate follow-on gate; content-composer v0.2 breaking — `MediaSlotValue` single-URL → array).
