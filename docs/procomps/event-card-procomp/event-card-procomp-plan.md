# event-card — procomp plan

> Stage 2: how. The implementation contract.
>
> See [`event-card-procomp-description.md`](./event-card-procomp-description.md) for the what & why.
>
> **Migration origin:** [`docs/migrations/event-card/`](../../migrations/event-card/) — see [`analysis.md`](../../migrations/event-card/analysis.md) for the design-DNA inventory + rewrite items + dependency audit. Locked-in intake decisions: [`source-notes.md` § Locked-in decisions](../../migrations/event-card/source-notes.md#locked-in-decisions-signed-off-2026-05-02).
...
```ts
// src/registry/components/data/event-card/types.ts

import type { EventStatus } from './lib/event-status';
export type { EventStatus };

export type EventCardVariant = 'grid' | 'feed' | 'list' | 'compact';
...
```ts
// src/registry/components/data/event-card/lib/event-status.ts
...
```ts
// src/registry/components/data/event-card/lib/format-default.ts
...
```ts
// src/registry/components/data/event-card/index.ts
...
The card uses `EventCardItem` directly, not `<T extends EventCardItem>`. Consumers `.map(raw => mapToEventCardItem(raw))` once before render. Power users still customize via `typeStyles` / `titleClassName` / `imageClassName` / `actions`. Same call as news-card.
...
```
src/registry/components/data/event-card/
├── event-card.tsx              # 1
├── parts/
│   ├── grid.tsx                   # 2
│   ├── feed.tsx                   # 3
...
Matches news-card (14 files = 5 parts + 2 lib/hooks + 7 standard; we have 4 parts + 3 lib + 7 standard = 14).

### 1. `event-card.tsx` — root component
...
- Link overlay: `<linkComponent href={href} aria-labelledby={titleId} aria-label={ariaLabel} className="absolute inset-0 z-0 rounded-2xl focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 outline-none" onClick={handleClick} />` — applied to image area? **No** — link overlay covers the WHOLE article, not just the image. Same overlay-link pattern as news-card.
...
**Event-specific delta over news-card's `list` variant:**
...
**`dummyTrLabels: EventCardLabels`** — Turkish-locale labels for the "Localized" demo tab.
...
export const eventCardMeta: ComponentMeta = {
  slug: 'event-card',
  name: 'Event Card (01)',
  category: 'data',
  status: 'alpha',
...
**Headless wrapping + presentational parts** — same as news-card.
...
**Polymorphic root** via `linkComponent: React.ElementType`. Same pattern as news-card.

---

## Client vs server

**Client component** — `event-card.tsx` declares `"use client"`. Required for `useId` + `useMemo` + `React.memo` + the function-rich props (`onClick`, `formatDate`, `linkComponent`, `getHref`).
...
- The link overlay is the sole keyboard-focusable element on the card (matches news-card).
...
- [ ] `pnpm lint` clean (no new warnings; pre-existing card-tree warning OK).
- [ ] `pnpm build` clean — all routes prerendered including `/components/event-card`.
- [ ] SSR `curl -s http://localhost:3000/components/event-card` returns 200 with all 6 demo tab triggers rendered (Grid / Feed / Featured / Localized / Custom types / Actions slot) + Grid tab default content visible.
...
- [ ] Helper-only imports work — write a quick `pnpm tsc --noEmit` test consumer that does `import { getEventStatus } from "@/registry/components/data/event-card"` and uses it without rendering the card; passes type-check.
...
1. **Compound API** (`<EventCard.Image>`, `<EventCard.Status>`, `<EventCard.Capacity>`). Rejected — variants vary too much in structure (grid stacks; feed overlays); compound API fights the variant system. Same call as news-card.
2. **Generic over event shape** (`<EventCard<T extends EventCardItem>>`). Rejected — strict shape is more ergonomic; users map to it cheaply. Generic adds complexity without enabling new use cases.
...
- v0.2: skeleton companion (`<EventCard.Skeleton variant="grid" />`).
- v0.2: status-tinted progress bar (via a `progressClassName` prop or a `progressTone` prop derived from status).
- v0.2: register-in-place dialog as an opt-in slot (`<EventCard.RegisterDialog>` compound, or just a worked usage example with a consumer-supplied dialog).
- v0.2: `events-grid-layout-01` sibling — search + filter + infinite scroll + featured-item slot, mirroring `magazine-layout`. Composes with this card.
