# event-card — procomp description

> Stage 1: what & why. The "should we build this at all?" doc.
>
> **Migration origin:** [`docs/migrations/event-card/`](../../migrations/event-card/) (kasder `kas-social-front-v0` `EventCard.tsx` + `SocialEventCard.tsx`)
>
> **Locked-in intake decisions:** [`source-notes.md` § Locked-in decisions](../../migrations/event-card/source-notes.md#locked-in-decisions-signed-off-2026-05-02). Extraction analysis: [`analysis.md`](../../migrations/event-card/analysis.md).
>
> Family: events-domain set begins here. Future siblings — `events-grid-layout-01` (search + filter + infinite scroll over a card array, mirrors `magazine-layout`), `event-detail-page-01` (deferred). All independent, fully compatible, dynamic — same convention as the news-domain family.
...
Pro-ui has no existing answer here. `news-card` is editorial (passive read-link, no status, no capacity, no CTA semantics). `data-table` is tabular. Nothing in the registry renders an event preview with status-aware visuals + capacity bar + CTA in two calibrated layouts driven by a single prop.
...
- **Overlay-link pattern from day 1** — clickable whole-card surface that doesn't lock out nested interactives (register-in-place, add-to-calendar, share, save). Same pattern as `news-card`.
...
These are exported alongside `EventCard` from the package — consumers can use them WITHOUT rendering the card:
...
```tsx
import { getEventStatus, EVENT_STATUS_CONFIG } from '@ilinxa/event-card';
...
- SSR `/components/event-card` returns 200 with all 6 demo tabs rendered.
...
1. **Component export name.** `EventCard` (verbose, version-explicit, future-proof for `event-card-02` if a richer schema emerges). **Resolved:** `EventCard` — matches the `<Slug><NN>` convention already established by `news-card`, `page-hero`, etc.
...
6. **Title font — sans or serif?** **Resolved:** sans (default `--font-sans`, Onest). Events are utilities (when/where/how-to-register), not editorial. Differs intentionally from `news-card` (which uses `--font-serif`).
...
10. **Loading skeleton.** Should the card ship a `<EventCard.Skeleton variant="grid" />` for loading states? **Open** — v0.1 ships without; consumers render a `bg-muted` placeholder. v0.2 candidate if real demand surfaces.

---

**Stage 1 contract:** if you sign off on the above, the plan doc (`event-card-procomp-plan.md`) will lock down: file shape, type definitions, prop/state/effect/render flow per variant, helper signatures, design-system token wiring, demo plan, success-gate verification steps, and known plan deviations to flag during implementation.
