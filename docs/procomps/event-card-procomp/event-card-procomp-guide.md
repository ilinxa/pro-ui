# event-card — procomp guide

> Stage 3: how to use it. Authored alongside the implementation.
>
> See [`event-card-procomp-description.md`](./event-card-procomp-description.md) for *why* and [`event-card-procomp-plan.md`](./event-card-procomp-plan.md) for *how*.
...
- **Static event listings with no status semantics** — a plain card without the state machine is lighter; use `news-card` or a custom card.
...
This component started as a port from the kasder `kas-social-front-v0` project — the original `EventCard.tsx` (grid variant) + `SocialEventCard.tsx` (feed variant). See [`docs/migrations/event-card/`](../../migrations/event-card/) for the full extraction analysis: design DNA preserved, structural debt rewritten, dependency audit, and the `--warning` token addition that landed alongside this card.
