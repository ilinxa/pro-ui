# `sidebar-nav-01` — Pro-component Plan (Stage 2)

> **Stage:** 2 of 3 · **Status:** ✅ Signed off (GATE 2 closed 2026-05-22 — PQ1–PQ8 all defaults confirmed)
> **Slug:** `sidebar-nav-01` · **Category:** `navigation` · **Target version:** `v0.1.0`
> **Inherited from GATE 1:** [`sidebar-nav-01-procomp-description.md`](./sidebar-nav-01-procomp-description.md) — 40 locks (L1–L40), 14 confirmed Qs (Q1–Q14), 13 risks (R1–R13). All defaults accepted at GATE 1 sign-off.
>
...
├── sidebar-nav-01.tsx                 [SHIPPED] — entry; "use client"; default export <SidebarNav>
├── types.ts                           [SHIPPED]
...
**Cross-procomp import:** `bottom-tab-bar-01` imports the following from `sidebar-nav-01` via **relative paths** (F-S1 lock):

```ts
// In bottom-tab-bar-01/bottom-tab-bar-01.tsx:
import { NavBadge } from "../sidebar-nav-01/parts/nav-badge";

// In bottom-tab-bar-01/types.ts:
import type { NavItem, NavLinkProps, NavLinkComponent } from "../sidebar-nav-01/types";
```

Never via `@ilinxa/sidebar-nav-01` alias in shipped source.
...
export function SidebarNavTrigger({
  ...
  if (!resolved) {
    if (process.env.NODE_ENV !== "production") {
      console.warn("[sidebar-nav-01] <SidebarNavTrigger> rendered with no <SidebarNav> in context and no `controls` ref — renders disabled.");
    }
...
dependencies: {
  ...
  internal: [],  // No internal procomp deps — sidebar-nav-01 owns the shared NavBadge
}
```

When `bottom-tab-bar-01` ships next, it declares `internal: ["sidebar-nav-01"]` for the relative-path import.

---

## 20. Implementation order (commit chain)

Single-feature ships break into ~12–14 commits per recent procomps. For sidebar-nav-01 with this surface, plan **14 commits** (C1–C14) — each landing a coherent slice that compiles + the docs-site detail page renders without regression.

| # | Commit | What lands | Validates |
|---|---|---|---|
| **C1** | scaffold + types | `pnpm new:component navigation/sidebar-nav-01`; populate `types.ts` with all interfaces + type aliases; empty `index.ts` barrel; empty `meta.ts` shell; placeholder `<SidebarNav>` returning null. | tsc clean; `/components/sidebar-nav-01` 200s. |
...
| **C14** | demo.tsx + usage.tsx + meta.ts + manifest + registry.json + STATUS + spotcheck | All docs-site + registry distribution + STATUS.md update + GATE 3 spotcheck review at `reviews/2026-MM-DD-v0.1.0-spotcheck.md`. F-cross-11 path-b smoke. | Component closes. `/components/sidebar-nav-01` fully demonstrates all features. Ready to push. |
...
2. `pnpm dlx shadcn@4.6.0 add @ilinxa/sidebar-nav-01`.
...
| PQ4 | **Should bottomTabBar's `<NavBadge>` import be tested via path-b smoke during sidebar-nav-01's commit chain?** Or wait until bottom-tab-bar-01's commit chain? | **Wait.** sidebar-nav-01's C14 smoke covers its own surface; cross-procomp resolution tested when bottom-tab-bar-01 lands. |
