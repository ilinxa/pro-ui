# `task-card` — Pro-component Description (Stage 1)

> **Stage:** 1 of 3 · **Status:** ✅ Signed off (GATE 1 closed 2026-05-20)
> **Slug:** `task-card` · **Category:** `data`
> **Conceptual lineage:** task cards (Linear, Things 3, Height, Notion task boards), time-aware UI (calendar urgency gradients, SLA traffic-lights). **Fixed-schema sibling to [`card-tree`](../card-tree-procomp/)** — it borrows card-tree's architecture (uncontrolled + imperative handle, granular events, permission predicates, JSON I/O) but ships a closed `TaskItem` schema and a time-driven auto-color engine that card-tree deliberately doesn't carry.
...
> ⚠ **Architectural deviation flagged here for visibility:** the original brief mentioned "we can reuse the rich card as well but im not sure we must verify that." Verification result — **`task-card` does NOT extend, embed, or import `card-tree`.** Rationale: card-tree is an arbitrary-JSON outline editor; task-card is a fixed-schema task renderer with a time-driven color engine. Extending card-tree would inherit its full surface (~30 props, custom-key registry, reserved-key catalog, master validators) for no schema gain. **Instead** we mirror card-tree's architectural patterns — uncontrolled state with imperative ref handle, granular event surface, permission predicates, JSON-native input/output — so consumers who learn one component already know the other's shape. Same family, separate genes.
...
4. and is composable into both **its own list shell** (sibling procomp `todo-list`) AND **existing host surfaces** — `card-tree-node`-style canvases and `kanban-board`-style boards.
...
| **Companion adapters (parallel)** | `todo-card-tree-node` (flow-canvas node renderer, mirrors `card-tree-node` shape). Kanban integration ships as a `KanbanCardRenderer<TaskItem>` either inside this folder or via `todo-list`'s kanban variant — decision in GATE 2. |
...
**Data**
- Fixed `TaskItem` schema (see §4). Closed set of fields; consumer-defined `status` enum (string).
...
- `editable=false`: edit button on each item opens a **popup** (canonical convention locked by card-tree-node v0.1.0).
...
**JSON I/O surface** (mirrors card-tree v0.3 patterns)
- `defaultValue: TaskItem` — uncontrolled seed; remount via `key` prop to reset.
- Imperative ref handle: `getValue(): string` (canonical JSON), `getTree(): TaskItem`, `isDirty()`, `markClean()`, `focusItem(id)`.
...
- DnD payload: card is a DnD source for its own JSON payload AND a drop target for matching payloads (same `application/x-ilinxa-task+json` MIME type). Cross-card paste within a list is the list shell's concern; intra-card paste of children is in scope here.

**Granular event surface** (mirrors card-tree)
...
**Permission predicates** (mirrors card-tree's per-action predicate pattern)
- `canEditItem?(id)`, `canRemoveItem?(id)`, `canAddChildren?(id)`, `canDragItem?(id)`, `canToggleActive?(id)`, `canOverrideColor?(id)`.
- Declarative shortcut: `permissions?: { default?, byLevel?, byItem?, inherit? }` — same shape as card-tree's `CardTreePermissions` (the `inherit` flag controls whether child rules cascade from their parent's effective rule).
- `onPermissionDenied?(action, itemId, reason)` fires for analytics when an action would have been blocked (mirrors card-tree's [`onPermissionDenied`](../../src/registry/components/data/card-tree/types.ts) pattern).
...
- **Standalone**: drop a `<TaskCard defaultValue={item} />` anywhere.
- **`card-tree-node` style** (flow-canvas node): adapter ships as the sibling procomp `todo-card-tree-node` (mirrors card-tree-node's shape exactly). v0.1 of this procomp ships before the flow adapter; adapter follows in its own PR.
- **`kanban-board` card slot**: ships a `taskCardKanbanRenderer: KanbanCardRenderer<TaskItem>` export with `dragHandle: "header"` (the kanban-board pattern explicitly designed for renderers with internal pointer interactions — same lock card-tree uses). Lives either in this folder's `parts/kanban-adapter.tsx` or in todo-list's kanban variant; decision in GATE 2.
...
- Built-in undo/redo (mirrors card-tree v0.4 — not needed in v0.1; consumer can wire via events if required).

### Deliberate non-goals (any version of `task-card`)
...
- **Not a recurrence engine.** No RRULE, no "repeats every Monday." Recurrence is consumer-owned and produces fresh `TaskItem`s.
...
| **Embedded in `flow-canvas`** *(primary)* | Pipeline editor where nodes ARE tasks | Card-as-node via the companion `todo-card-tree-node` adapter; same shape as `card-tree-node` |
...
This is illustrative. **The canonical, shipped shape lives in [`src/registry/components/data/task-card/types.ts`](../../../src/registry/components/data/task-card/types.ts)** — defer to it on any naming difference (sketch updated to current names: `addChildren`, `showEditButton`, `key: TaskEditableField`, non-null event `parentId`).
...
export type TaskColorRampPreset = 'default' | 'muted' | 'vivid' | 'monochrome';
export type TaskColorRamp = TaskColorRampPreset | ((elapsed: number) => string);

// ─── Permissions (mirrors card-tree's pattern) ───
...
```tsx
const ref = useRef<TaskCardHandle>(null);
...
1. **TaskItem parses + renders** — every field in the schema produces the expected visual element.
...
14. **Compiles + renders** at `/components/task-card` with no console warnings, validate-meta-deps clean, build clean, F-cross-11 path-b consumer-tsc smoke clean.
15. **Companion-adapter readiness** — `taskCardKanbanRenderer` ships as a named export (so kanban-board demos can register it); `todo-card-tree-node` adapter procomp scaffolded but its v0.1 ships separately.
...
| # | Question | Decision |
|---|---|---|
| Q1 | **Slug + category** | `data/task-card`. No `-01` suffix (foundational primitive; convention follows `card-tree`, `stat-card`, `data-table`, `file-tree`). Sibling list shell is `data/todo-list`, sibling flow adapter is `data/todo-card-tree-node`. |
| Q2 | **Architecture** | **Two procomps, shipped in order: `task-card` first, then `todo-list`.** Card ships standalone, registerable in `card-tree-node`-style canvases and `kanban-board`. List shell follows in a separate PR. Reuse of `card-tree` itself was considered and **rejected** (card-tree is arbitrary-JSON; task-card is fixed-schema). We **mirror card-tree's patterns** — uncontrolled + imperative handle, granular events, permission predicates, JSON I/O — without sharing code. |
...
| Q7 | **Edit mode semantics** | **`editable` enables a toggle**, popup remains default click affordance. `editable=false` → click edit button = popup (canonical convention from card-tree-node v0.1.0). `editable=true` → edit button toggles per-item between view and inline-edit; popup still available as a secondary action. Per-item state — host doesn't dictate. `locked` blocks both. |
...
| Q11 | **JSON I/O surface** | **Mirror card-tree v0.3** — `defaultValue` for seed, imperative handle with `getValue(): string` (canonical JSON) + `getTree(): TaskItem`. Clipboard via `copy() / paste()` methods + Cmd/Ctrl+C / Cmd/Ctrl+V when focused. DnD as source + target with `application/x-ilinxa-task+json` MIME. Cross-card moves are list-shell concern; intra-card child paste is in-scope here. |
| Q12 | **Permission model** | **Mirror card-tree's pattern** — declarative `permissions?: { default?, byLevel?, byItem?, inherit? }` + per-action predicates (`canEditItem`, `canRemoveItem`, `canAddChildren`, `canDragItem`, `canToggleActive`, `canOverrideColor`). |
| Q13 | **Standalone vs embedded** | **Both.** Standalone usable as `<TaskCard />`. Registerable in `kanban-board` via a `KanbanCardRenderer<TaskItem>` named export with `dragHandle: "header"` (the kanban-board pattern for renderers with internal pointer interactions, e.g., inline editors). Registerable in `flow-canvas` via a sibling adapter procomp `todo-card-tree-node` (mirrors `card-tree-node` shape exactly — separate procomp, separate PR, separate v0.1). |
...
| Q16 | **Identifier strategy** | `id: string` is **required** on input (no auto-generation, unlike card-tree's `__rcid`). Consumers carrying a real-world entity already have one; auto-generating UUIDs would obscure data flow. Plan stage will lock collision-detection behavior. |
...
- **F-S1 lock applies** — once `todo-card-tree-node` ships, cross-procomp imports between it and `task-card` MUST use relative paths and MUST NOT re-export through a barrel. Existing lock pattern (`json-form v0.1.4`, `card-tree-node v0.1.0`) is the template.
...
- **Test coverage.** Same as card-tree — no test runner wired. Color engine is the kind of thing that benefits from property tests (random `now/startAt/expireAt` → assert clamp + monotonicity). Either land Vitest as a STATUS decision before v0.1, or ship with extensive demo-driven verification + test-debt note.
...
Before moving to Stage 2 (`task-card-procomp-plan.md`):

- [x] Sections 1–8 reviewed.
- [x] Q1–Q16 each carry an agreed answer (see §7).
- [x] Architectural decision flagged at top: **do not extend `card-tree`** — build fresh, mirror patterns.
- [x] Companion adapter strategy stated: `todo-card-tree-node` ships as a sibling procomp following `card-tree-node` precedent; kanban integration ships as a named `KanbanCardRenderer<TaskItem>` export.
...
- [x] **User explicitly says "description approved" (or equivalent)** — this unlocks Stage 2 (`task-card-procomp-plan.md`). Signed off 2026-05-20.
...
> ...
> ----
> some dynamik system like our card-tree pro component
...
> ---
> also these todo-rich-cards must be usable in richcard-flow as one of the component types supported by card-tree-flow
> ---
>
