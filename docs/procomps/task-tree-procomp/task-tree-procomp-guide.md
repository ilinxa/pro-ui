# `task-tree` — Pro-component Guide (Stage 3)
...
Reach for `task-tree` when you need a **lightweight hierarchical task outline** — a thin two-line row per item with multi-select, sort/filter/search, and drag-and-drop, sitting in a side panel or split view rather than commanding the full canvas.
...
- **Split-view companions** to `task-card` — click a row in the tree → open the card in a dialog or right pane (use `<TaskTreeWithEditor>` for the batteries-included wiring)

## When NOT to use

- **You want the full time-aware card chrome (border ramp, time-info, person chips, color override)** → use `task-card` directly. task-tree intentionally renders a thin row.
- **You need a calendar grid / gantt timeline** → use a calendar/gantt primitive.
- **Your items aren't hierarchical** → use a flat table (`data-table`) or a kanban (`kanban-board`). The whole point of this component is the tree.
- **You need recurring tasks** → no RRULE engine. Generate fresh `TaskItem`s on the consumer side.
...
```bash
pnpm dlx shadcn@latest add @ilinxa/task-tree
```

This pulls the sealed-folder source files PLUS the `task-card` peer (task-tree shares the `TaskItem` schema with it; the `<TaskTreeWithEditor>` convenience wrapper also imports `<TaskCard>` for the edit dialog).
...
```bash
pnpm dlx shadcn@latest add @ilinxa/task-tree-fixtures
```
...
- **Chevron** (left, 16px slot) — toggles collapse for that row's subtree. Always rendered; renders as an empty 16px gap when the row has no children, so all rows align vertically. UI-only state (`collapsedIds`), not stored in `TaskItem`. Default expanded.
...
- **Checkbox** — toggles `active`. Indeterminate on partial selection in multi-select mode is rendered, but a click always commits to `true` (F-cross-13 defensive coerce in [`parts/task-tree-checkbox.tsx`](../../src/registry/components/data/task-tree/parts/task-tree-checkbox.tsx)).
...
The tree is intentionally **read-only on click** — clicking a row fires `onItemClick(args)`, and the consumer decides what edit surface to open. This keeps task-tree's surface lightweight; the edit dialog can be a `<TaskCard>` (the canonical pairing), a right-pane detail view, or a custom modal.

For the canonical wiring use the included wrapper:

```tsx
import { TaskTreeWithEditor } from "@/components/task-tree";

<TaskTreeWithEditor defaultValue={items} />
```

`<TaskTreeWithEditor>` mounts a centered `<Dialog>` containing `<TaskCard editable>` on every row click; saves flow back into the tree via the imperative handle (state-transparent — controlled mode stays authoritative, uncontrolled mode keeps the tree's internal state). Consumers who want a stricter integration (confirm before edit, custom editor surface) should compose `<TaskTree>` + their own dialog using `onItemClick`.
...
`<TaskTreeWithEditor>` passes `editable={true}` automatically. For a bare `<TaskTree>` with your own editor, pass `editable` yourself.
...
**Deferred-commit** (`onCreateRequest` provided): click → factory runs → `onCreateRequest(item)` fires → toolbar does NOT mutate the tree. The consumer is now responsible for committing via `handle.addItem(...)` when ready. This is how `<TaskTreeWithEditor>` opens the edit dialog on a pending item and only commits on Submit.
...
### Deferred-commit flow (the `<TaskTreeWithEditor>` pattern)
...
The wrapper does exactly this — see [`task-tree-with-editor.tsx`](../../src/registry/components/data/task-tree/task-tree-with-editor.tsx).
...
| Grip handle → another `task-tree` / `task-card` instance on the same page | Native HTML5 `dataTransfer` with `application/x-ilinxa-task+json` MIME | Cross-procomp drops |

The MIME payload is shared with `task-card`, so consumers can drag rows from a tree onto a card's children area (and back via the card's HTML5 drop target). Touch DnD is **internal-only** by design — the native HTML5 transport doesn't fire reliably on touch.
...
Mirrors `task-card`'s declarative matrix. **The only permission API is the `permissions` prop + `onPermissionDenied`** — there are no per-action predicate props (`canEditItem`, `canDragItem`, … do not exist).
...
The matrix gates six actions: `edit`, `toggleActive`, `drag`, `dropAsSibling`, `dropIntoChildren`, `remove`. The two drop actions map onto `task-card`'s rule keys: `dropAsSibling` → `drag`, `dropIntoChildren` → `addChildren`. `byLevel` keys are 0-indexed (top-level = `0`).
...
`<TaskTree>` forwards a `ref` of type `TaskTreeHandle`. The full surface:
...
// Focus / lookup (2)
h.focusItem(itemId);
h.getItemById(itemId);                         // TaskItem | undefined
...
For headless consumers, prefer the `useTaskTreeState` hook — same surface plus live state values and a `dispatch` escape hatch.

---

## Headless hook — `useTaskTreeState`

```tsx
import { useTaskTreeState } from "@/components/task-tree";

function Custom() {
  const state = useTaskTreeState({ defaultValue: items });
...
The hook returns a superset of `TaskTreeHandle` plus live state values (`items` / `visibleItems` / `selectedIds` / `focusedItemId` / `collapsedIds` / `sort` / `filter` / `query`) and a `dispatch` escape hatch (raw reducer access; use sparingly — the action types are public but the reducer's invariants are easier to break than the wrapped methods).
...
| Slot | Args | Notes |
|---|---|---|
| `renderRow` | `TaskTreeRowRenderArgs` | Replace the whole row content (grip + chevron + checkbox + name + description + person). Most consumers should reach for the field-level slots below instead. |
| `renderName` | `TaskTreeFieldRenderArgs` | Replace just the bold name slot. |
| `renderDescription` | `TaskTreeFieldRenderArgs` | Replace just the second-line description. |
| `renderPerson` | `TaskTreeFieldRenderArgs` | Replace the right-side person label (good fit for avatars). |
| `renderStatusIndicator` | `TaskTreeStatusRenderArgs` | Replace the status dot (e.g. icon, colored ring). |
| `renderToolbar` | `TaskTreeToolbarRenderArgs` | Replace the top toolbar (search + sort + filter + bulk-action bar). Get `{ defaultToolbar, state }` (compose the built-in toolbar or read live state). |
| `renderEmptyState` | `TaskTreeEmptyRenderArgs` | Replace the empty-state when the visible-items pipeline returns 0 rows. |
| `renderDragOverlay` | `TaskTreeDragOverlayArgs` | Replace the floating overlay shown during `@dnd-kit` drag. Defaults to a thin row preview. |
...
```tsx
<TaskTree defaultValue={items} onChange={(e) => console.log(e.items)} />
```
...
**Three-defenses controlled-mode pattern** is fully wired in [`hooks/use-controlled-mode.ts`](../../src/registry/components/data/task-tree/hooks/use-controlled-mode.ts):
...
This is the same pattern locked in `flow-canvas` v0.2.2 + v0.2.3 + v0.2.4. See [`project_controlled_mode_two_defenses.md`](../../.claude/projects/.../memory/project_controlled_mode_two_defenses.md) (memory entry) for the rationale.
...
|---|---|---|
| `onChange` | `{ items, reason }` | Any tree mutation (`reason: TaskTreeChangeReason`). |
| `onItemClick` | `TaskTreeItemEvent` (`{ item, level, event }`) | Row click. `event` is `MouseEvent \| KeyboardEvent` (Enter fires this too). |
| `onItemContextMenu` | `TaskTreeItemEvent` | Right-click / context-menu on a row. |
| `onActiveToggled` | `{ item, nextActive }` | Active checkbox toggled. |
| `onCollapseToggled` | `{ item, collapsed }` | Per-row chevron toggle. |
| `onItemMoved` | `TaskTreeMoveEvent` | `@dnd-kit` internal move (reorder / reparent). |
| `onItemDropped` | `TaskTreeDropEvent` | HTML5 drop from another procomp. |
| `onItemAdded` | `TaskTreeAddEvent` | Item added (handle, toolbar, or drop). |
| `onItemRemoved` | `TaskTreeRemoveEvent` | Item removed. |
...
## Cross-procomp shape — sharing `TaskItem` with `task-card`

Both procomps import `TaskItem` from a single source: `task-card/types`. task-tree's shipped source uses **relative imports** to that file (`../task-card/types`) per the F-S1 cross-procomp lock — same-category sibling procomp imports must be relative because shadcn 4.6.0's path rewriter substitutes the current slug for the target slug otherwise.

For consumers, this means once you've installed `@ilinxa/task-card` (auto-pulled when you install `@ilinxa/task-tree`), you can import `TaskItem` from either:

```tsx
import type { TaskItem } from "@/components/task-card";
// or, equivalently, since task-tree re-exports for convenience:
import type { TaskItem } from "@/components/task-tree";
```
...
- **TaskTreeStateValue identity changes per render.** The `useTaskTreeState` hook returns a fresh object every render — don't destructure into a dep array. The inner imperative methods are stable; if you need a stable selector, useMemo it yourself.
- **Inline editing is not built in.** The tree is read-only on click; consumer-owned dialogs (or `<TaskTreeWithEditor>`) provide the edit surface. Inline rename / status pickers remain deferred to a future minor.
- **`<TaskTreeWithEditor>` is state-transparent.** It owns only the dialog open/close — your `value` / `defaultValue` / `onChange` flow through unchanged. Saves inside the dialog route through the tree's imperative handle so controlled-mode consumers see them through their normal `onChange`.

---

## Migration notes

This is a new component — no migration path. If you're coming from a custom tree built on `<details>`/`<summary>` or `react-arborist`, map your data to `TaskItem` at the boundary and use `defaultValue` to seed.

---

## Open follow-ups

- ~~**v0.1.2 (patch)** — Thread `permissions` matrix end-to-end through `useTaskTreeState` + `useTreeKeyboard` so Space/Delete shortcuts honor predicates (F-perm).~~ ✅ Closed in v0.1.2.
...
- **v0.3** — Server-rendered initial state with deferred hydration (currently full client-side); per-tree theming via `--task-tree-*` CSS variables.
