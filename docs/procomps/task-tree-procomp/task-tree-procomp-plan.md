# `task-tree` — Pro-component Plan (Stage 2)

> **Stage:** 2 of 3 · **Status:** ✅ Signed off (GATE 2 closed 2026-05-20)
> **Inherits from:** [description doc](task-tree-procomp-description.md) (GATE 1 closed 2026-05-20)
> **Sibling-of:** [`task-card@v0.1.1`](../task-card-procomp/) — shared `TaskItem` schema; cross-procomp DnD via `application/x-ilinxa-task+json`.
...
```ts
// Top-level files (task-tree.tsx, index.ts, types.ts):
import type {
  TaskItem,
  TaskStatusOption,
  TaskPermissions,
...
NEVER `@ilinxa/task-card` in shipped source (F-S1 lock).

### 2.2 Public types

```ts
import type { ReactNode, MouseEvent } from "react";
import type { TaskItem, TaskStatusOption, TaskPermissions } from "../task-card/types";
...
export type TaskTreeAction =
  ...
  | { type: "ADD_ITEM"; item: TaskItem; parentId: string | null; index?: number; via: "imperative" | "drop-from-external" }
...
// State value (returned by useTaskTreeState; passable via `state` prop)
export interface TaskTreeStateValue extends TaskTreeHandle {
  items: TaskItem[];
  visibleItems: ReadonlyArray<{ item: TaskItem; level: number; parentId: string | null; index: number }>;  // flattened + filtered + sorted snapshot, level included for renderer
...
export interface TaskTreeProps {
  ...
  state?: TaskTreeStateValue;  // lifted; supersedes value/defaultValue
  ...
  dndContext?: "internal" | "external";           // default "internal" — "external" skips internal <DndContext> mount (use when parent already provides one, e.g., nested inside kanban-board)
...
**F-S1 lock:** NO cross-procomp re-exports from this barrel. Shipped source uses relative paths to task-card; downstream consumers import TaskItem etc. directly from `@ilinxa/task-card`.
...
```
              ...
              │   • mounts <TaskTreeToolbar> (if default) │
              │   • mounts <TaskTreeList> (virtualized)   │
              └─────────┬──────────────────────┬──────────┘
                        ▼                      ▼
          ┌─────────────────────────┐  ┌─────────────────────┐
          │  useTaskTreeState       │  │  <TaskTreeList>     │
...
All state mutations flow through `lib/reducer.ts`. Reducer is **pure** — no side effects, no consumer-callback firing. Side effects (firing `onChange` / `onItemMoved` / `onActiveToggled` etc.) happen in **`hooks/use-tree-events.ts`**, called by the host (`<TaskTree>`) after each dispatch. The `dispatch` escape hatch exposed on `TaskTreeStateValue` lets advanced consumers fire actions directly.

### 3.2 Three-defenses controlled mode

When `value` is provided (controlled), `<TaskTree>` runs the canonical pattern (lifted from `flow-canvas` v0.2.2/v0.2.3/v0.2.4 saga, locked in [`project_controlled_mode_two_defenses.md`](../../../C:/Users/AsiaData/.claude/projects/e--2026-ilinxaDOC-ilinxa-ui-pro/memory/project_controlled_mode_two_defenses.md)):
...
Every `<TaskTreeRow>` carries **both** drag bindings simultaneously. Activation rules:
...
| Mouse-down on row body (outside grip) + native drag threshold | native `draggable={true}` + `onDragStart` on row body | HTML5 | Native drop targets — both internal rows AND external `task-card` drop targets |
...
items[] (raw tree)
    │
    ▼
lib/filter-items.ts  (apply TaskTreeFilter + search query)
    │
    ▼
lib/sort-items.ts    (apply TaskTreeSort)
    │
    ▼
...
`<TaskTreeToolbar>` is a self-contained component that reads state from the `useTaskTreeState` context (provided by `<TaskTree>` via `<TaskTreeStateContext.Provider>`). Three sub-parts:

- `<TaskTreeSearchInput>` — debounced 200ms via `useDebouncedCallback` (rebuild local `useEffect` since we already have lib-internal debounce). Fires `setQuery` + `onSearchChanged`.
- `<TaskTreeSortDropdown>` — Select primitive; F-cross-13 pre-emption (L11): `onValueChange` widened to `(v: string \| null) => void` + null-coerce.
- `<TaskTreeFilterDropdown>` — Popover + Checkbox group; same F-cross-13 pre-emption posture.
- `<TaskTreeBulkActionBar>` — appears when `selectedIds.size > 0`; surfaces buttons for `onBulkToggleActive`, `onBulkRemove`, `onBulkEdit`.

---

## 4. File structure

Sealed folder at `src/registry/components/data/task-tree/`. Expected 53 files (50 shipped via registry; demo/usage/meta docs-site only).

```
src/registry/components/data/task-tree/
├─ task-tree.tsx                  (main component, ~250 LOC)
├─ task-tree-with-editor.tsx      (convenience wrapper, ~80 LOC)
...
│  ├─ task-tree-list.tsx          (virtualized list root; reads from context)
│  ├─ task-tree-row.tsx           (the 2-line row; dual DnD bindings)
│  ├─ task-tree-row-content.tsx   (defaultRender for slot)
│  ├─ task-tree-chevron.tsx       (collapse toggle button)
│  ├─ task-tree-status-indicator.tsx (dot / strip / none paint)
│  ├─ task-tree-checkbox.tsx      (native checkbox with permission gate)
│  ├─ task-tree-name.tsx          (name span; bold)
│  ├─ task-tree-description.tsx   (truncated description preview)
│  ├─ task-tree-person-label.tsx  (person label, right edge)
│  ├─ task-tree-grip.tsx          (hover-revealed drag handle)
│  ├─ task-tree-drop-indicator.tsx (top/bottom line OR middle glow)
│  ├─ task-tree-drag-overlay.tsx  (cursor-follow visual)
│  ├─ task-tree-empty-state.tsx   (default placeholder)
│  ├─ task-tree-toolbar.tsx       (default toolbar root)
│  ├─ task-tree-search-input.tsx  (debounced input)
│  ├─ task-tree-sort-dropdown.tsx (Select primitive)
│  ├─ task-tree-filter-dropdown.tsx (Popover + checkbox group)
│  ├─ task-tree-filter-active-toggle.tsx (segmented control: all/active/inactive)
│  ├─ task-tree-bulk-action-bar.tsx (appears on selection)
│  └─ task-tree-keyboard-handler.tsx (invisible div with keydown handlers)
│
├─ hooks/                         (state + interaction logic)
│  ├─ use-task-tree-state.ts      (the headless hook; main reducer host)
│  ├─ use-task-tree-context.ts    (context bridge for parts/)
...
│
└─ lib/                           (pure functions)
   ├─ reducer.ts                  (TaskTreeAction → state)
   ├─ visible-items.ts            (filter + sort + flatten pipeline)
   ├─ filter-items.ts             (apply TaskTreeFilter)
   ├─ search-items.ts             (case-insensitive .includes on name + description)
   ├─ sort-items.ts               (apply TaskTreeSort; 4 named kinds + custom = 5 total)
   ...
   ├─ dnd-payload.ts              (TASK_TREE_MIME + serialize/parse)
   ├─ permissions.ts              (predicate evaluator, mirrors task-card)
...
**File-count summary:**
- 8 top-level (5 shipped: task-tree.tsx, task-tree-with-editor.tsx, types.ts, index.ts, dummy-data.ts; 3 docs-site: demo, usage, meta)
...
`lib/reducer.ts` exports a pure `function reducer(state: State, action: TaskTreeAction): State`.
...
>
  {/* tree content */}
  <DragOverlay>{activeItem && <TaskTreeDragOverlay item={activeItem.item} level={activeItem.level} />}</DragOverlay>
</DndContext>
```
...
`PointerSensor` (which unifies mouse + touch) is NOT used here because we want **divergent activation timings** between mouse (immediate) and touch (long-press). Splitting into Mouse + Touch sensors gives the correct UX per modality. This is a deliberate divergence from kanban-board (which uses unified PointerSensor) — locked here because the description doc L17 specifies 300ms long-press semantics that kanban-board doesn't actually deliver via its current sensor config.
...
Drop indicators (`<TaskTreeDropIndicator>`) render based on the current `over` + `edgeZone`. Top/bottom: horizontal line between rows. Middle: inner-glow ring on the target row.

### 6.3 Circular drop ban

```ts
// lib/circular-drop.ts
export function isAncestor(items: TaskItem[], sourceId: string, targetId: string): boolean;
```
...
<div
  ...
  onDragStart={(e) => {
    e.dataTransfer.setData(TASK_TREE_MIME, JSON.stringify(item));
    e.dataTransfer.setData("text/plain", JSON.stringify(item));
    e.dataTransfer.effectAllowed = "copy";
  }}
  onDragOver={(e) => {
    if (e.dataTransfer.types.includes(TASK_TREE_MIME)) {
...
Where `TASK_TREE_MIME = "application/x-ilinxa-task+json"` (same MIME task-card uses; defined in `lib/dnd-payload.ts`).

### 6.5 Activator mutual exclusion

Wire `<TaskTreeGrip>` as the `useDraggable` listener target. When grip-drag activates, set `isInternalDragRef.current = true`. The row-level `onDragStart` (native) checks this flag — if true, call `e.preventDefault()` to suppress the HTML5 drag. Otherwise (drag started outside grip), let HTML5 proceed and skip the @dnd-kit drag.
...
User drags row → grip activator? → YES → @dnd-kit DnD
                              ...
                              → NO  → HTML5 drag
                                        ...
                                        → drops on external (card-tree) → external handles drop natively; tree emits onItemDropped({ from: "internal", ... }) when its drag ends if drop happened in an HTML5 drop target
```
...
### 7.1 `<TaskTreeToolbar>` composition

```tsx
<div role="toolbar" aria-label="Tree controls" className="flex items-center gap-2 px-2 py-1.5">
  <TaskTreeSearchInput value={query} onChange={setQuery} />
  <TaskTreeFilterActiveToggle value={filter.active} onChange={(active) => setFilter({ ...filter, active })} />
  <TaskTreeSortDropdown value={sort} onChange={setSort} />
  <TaskTreeFilterDropdown value={filter} onChange={setFilter} statusOptions={statusOptions} />
...
All `Select.onValueChange` callbacks in `<TaskTreeSortDropdown>` and `<TaskTreeFilterDropdown>` are widened:
...
```ts
const SORT_OPTIONS: Array<{ value: string; label: string; spec: TaskTreeSort }> = [
...
```ts
function handleRowClick(item: TaskItem, e: React.MouseEvent) {
...
`lib/permissions.ts` mirrors task-card's pattern:
...
`<TaskTreeList>` reads `isDraggingRef` from context. When drag is in flight, virtualizer is forced disabled (renders all visible items, no windowing) so dragged item can't scroll out of windowed range.
...
Implementation in `hooks/use-controlled-mode.ts`. Mirrors flow-canvas v0.2.4 verbatim where possible.

```ts
export function useControlledMode(args: {
  value: TaskItem[] | undefined;
  defaultValue: TaskItem[] | undefined;
  ...
  const fireOnChange = useCallback((next: TaskItem[], reason: TaskTreeChangeReason) => {
...
`parts/task-tree-row-content.tsx`:

```tsx
export function TaskTreeRowContent({ item, level, isSelected, isCollapsed }: TaskTreeRowRenderArgs & { /* internal */ }) {
  const { renderName, renderDescription, renderPerson, renderStatusIndicator, statusIndicator } = useTaskTreeContext();
  return (
    <div
      data-level={level}
    ...
    >
      <div className="flex items-center gap-1.5">
        <TaskTreeChevron item={item} />
        {renderStatusIndicator
          ? renderStatusIndicator({ item, level, statusOption })
          : <TaskTreeStatusIndicator variant={statusIndicator} item={item} />}
        <TaskTreeCheckbox item={item} />
        <span className="flex-1 truncate font-semibold">
          {renderName ? renderName({ item, level }) : item.name}
        </span>
        {item.targetPerson && (
          renderPerson ? renderPerson({ item, level }) : <TaskTreePersonLabel person={item.targetPerson} />
        )}
      </div>
      {item.description && (
        <div
...
```tsx
// In <TaskTreeRow>
const rowContent = <TaskTreeRowContent {...args} />;
return renderRow ? renderRow({ ...args, defaultRender: rowContent }) : rowContent;
```

For toolbar:
```tsx
const defaultToolbar = toolbar === "default" ? <TaskTreeToolbar /> : null;
...
const rendered = renderStatusIndicator
  ? renderStatusIndicator({ item, level, statusOption })
  : <TaskTreeStatusIndicator variant={statusIndicator} statusOption={statusOption} />;
```

---

## 13. `<TaskTreeWithEditor>` composition

`task-tree-with-editor.tsx`:
...
import { TaskCard } from "../task-card";       // relative cross-procomp import (F-S1)
import type { TaskItem } from "../task-card/types";
import type { TaskTreeProps } from "./types";

export function TaskTreeWithEditor(props: TaskTreeProps) {
...
**Important:** the wrapper does NOT propagate edit changes back into the tree by default — the edited item exists only inside the dialog's TaskCard. Closing the dialog discards changes unless consumer also wires `onChange` on the TaskCard.
...
|---|---|---|---|
| `@dnd-kit/core` | `^6.3.1` | Internal drag (Dual DnD) | ✅ already used by kanban-board |
| `@dnd-kit/utilities` | `^3.2.2` | @dnd-kit transform util | ✅ already used by kanban-board |
...
`dependencies.internal: ["task-card"]` — `<TaskTreeWithEditor>` mounts TaskCard. Bare `<TaskTree>` has only TYPE deps on task-card (`TaskItem`, `TaskStatusOption`, `TaskPermissions`) — those are type-only imports erased at compile time. Still declared in `dependencies.internal` because the convenience wrapper carries the runtime dep.
...
| `popover` | filter dropdown | medium (used in task-card v0.1.1, no issues yet) |
...
| `dialog` | only via `<TaskTreeWithEditor>` | none |
...
Total: ~12 shadcn primitives. All already installed (per task-card v0.1.1).
...
| # | Case | Behavior |
|---|---|---|
| E1 | Empty tree (zero items) | `<TaskTreeEmptyState>` renders. If filter is active, message reads "No matches"; else "No tasks." Slot via `renderEmptyState`. |
...
| E9 | Drop from external (task-card) onto top-level (no row hit) | `<TaskTreeList>` itself is a drop zone for the bottom whitespace; drop becomes new top-level item at end. |
...
| E17 | Touch-drag attempts cross-procomp drop into card-tree | Touch drag uses @dnd-kit (no native dataTransfer) — card-tree cannot receive. Documented as a known limit (L17 lock, R11 risk). |
...
| E19 | TaskItem with `children: []` (empty array) vs `children: undefined` | Both treated as "no children" — no chevron, no expand affordance. |
...
- Each `<TaskTreeRow>` wrapped in `React.memo` with custom equality: shallow-compare item ref + level + isSelected + isCollapsed.
- `useTaskTreeContext` returns a STABLE context object via `useMemo` with narrow deps.
...
| # | Title | Scope |
|---|---|---|
| **C1** | scaffold | `pnpm new:component data/task-tree`. Top-level files stubbed. types.ts complete. meta.ts populated. manifest.ts entry added. Builds + ts-checks pass. |
...
| **C3** | hooks layer | hooks/use-task-tree-state.ts (the hook), hooks/use-task-tree-context.ts, hooks/use-controlled-mode.ts, hooks/use-tree-events.ts, hooks/use-selection.ts, hooks/use-debounced-callback.ts, hooks/use-status-option-by-value.ts. Hook standalone-usable post-this commit. |
| **C4** | row primitives | parts/task-tree-chevron, status-indicator, checkbox, name, description, person-label, row-content. Pure presentational. Storybook-friendly. |
| **C5** | list + virtualization | parts/task-tree-list, hooks/use-tree-virtual. Renders rows without DnD. |
| **C6** | Dual DnD wiring | parts/task-tree-row (the full row with both bindings), parts/task-tree-grip, parts/task-tree-drop-indicator, parts/task-tree-drag-overlay. hooks/use-tree-dnd-internal.ts (the @dnd-kit hooks). hooks/use-tree-dnd-html5.ts (HTML5 handlers). Internal + cross-procomp drag work. |
| **C7** | toolbar | parts/task-tree-toolbar, task-tree-search-input, task-tree-sort-dropdown, task-tree-filter-dropdown, task-tree-filter-active-toggle, task-tree-bulk-action-bar. F-cross-13 pre-emption applied. |
| **C8** | keyboard + a11y | hooks/use-tree-keyboard.ts, parts/task-tree-keyboard-handler, parts/task-tree-empty-state. Full WAI-ARIA tree pattern. |
| **C9** | wrapper + demo + usage + meta sync | task-tree-with-editor.tsx. demo.tsx with 6+ sub-demos. usage.tsx with 7+ sections. meta.ts final pass. dummy-data.ts comprehensive. |
...
| **C11** | GATE 3 spotcheck + status + tracking | docs/procomps/task-tree-procomp/reviews/2026-MM-DD-v0.1.0-spotcheck.md. STATUS.md row + version-pointer + recent-activity. Decision file in `.claude/decisions/`. |

Tip-of-master after C11: task-tree shipped via push to master, Vercel deploys, consumer can `pnpm dlx shadcn@latest add @ilinxa/task-tree`.
...
- [x] F-S1 cross-procomp imports use relative paths in all shipped source (verified via `grep -rE "@ilinxa/" src/registry/components/data/task-tree/` — must return 0 matches in shipped files).
- [x] No barrel re-exports from cross-procomp (task-card types are type-only, imported per file).
- [x] `dependencies.internal: ["task-card"]` declared in meta.ts.

### 19.2 Smoke run

```bash
cd e:/tmp/ilinxa-smoke-consumer
pnpm dlx shadcn@4.6.0 add @ilinxa/task-tree @ilinxa/task-tree-fixtures --yes --overwrite
pnpm tsc --noEmit
```

Expected: 0 task-tree errors. The 97 pre-existing errors in code-block / flow-canvas / json-form / pdf-viewer remain unchanged.

### 19.3 If smoke fails

- F-cross-13 hits → widen the callback / drop the divergent prop → bump v0.1.1 → re-run smoke. Same pattern as task-card v0.1.0→v0.1.1.
...
| RP2 | Visible-items memoization breaks when consumer passes new `permissions` object each render (identity churn) | `useTaskTreeContext` stabilizes permissions via shallow-compare in a `useMemo`. Documented in usage.tsx perf section. |
| RP3 | @dnd-kit's DragOverlay re-renders on every drag tick — heavy if drag overlay uses TaskCard-sized children | Default `<TaskTreeDragOverlay>` is the simplified one-line view; `renderDragOverlay` slot exists for full control. |
| RP4 | TaskTreeWithEditor wraps consumer's `onItemClick` — possible double-fire if consumer also passes their own | Wrapper composes both: setEditTarget(item) + delegate to consumer. Documented in usage.tsx. |
...
|---|---|
| Use @dnd-kit for cross-procomp too (drop HTML5 entirely) | task-card uses native HTML5; cross-procomp would not work. |
| Use HTML5 only (drop @dnd-kit) | No touch support. Q8 = a rejected this. |
| Headless-only (no `<TaskTree>` component, just hook + caller renders) | Too low-level for the "lightweight" promise; consumers would re-implement the 80% common UI. |
| Re-use task-card's `<TaskCard>` rendering in tree mode | Defeats the entire "lightweight" purpose. Already rejected in description doc. |
...
|---|---|---|
| Q-P1 | **`<TaskTreeWithEditor>` edit-save behavior** — does v0.1 ship auto-persistence (wire TaskCard's onChange → setValue), or stays unwired (consumer wires onChange themselves if they want persistence)? | **Suggest**: ship auto-persistence. The wrapper IS the convenience path; making consumers wire onChange defeats the convenience. Internal: wrapper tracks its own items state, lifts changes via setValue on dialog close (or live during edit). |
...
| Q-P6 | **`@dnd-kit` `DndContext` scope — mount it inside `<TaskTree>` OR require consumer to mount it externally?** | **Suggest**: mount internally. Consumers who already have a `DndContext` higher in their tree (e.g., kanban-board wrapping TaskTree) need a way to opt out — provide `dndContext="external"` prop (boolean) that disables internal mounting. |
| Q-P7 | **Reducer dispatch from outside — should `dispatch` on `TaskTreeStateValue` accept the full `TaskTreeAction` union, or filter to public-safe actions only?** | **Suggest**: full union exposed. Marks as "escape hatch" in docs. Power users can dispatch SELECT_ALL directly; sanity checks live in reducer (e.g., MOVE_ITEM bans circular). |
| Q-P8 | **Touch-drag cross-procomp graceful degradation** — when a user tries to touch-drag a tree row toward a card-tree and nothing happens (because @dnd-kit doesn't write to dataTransfer), do we show a hint? | **Suggest**: no hint in v0.1. The behavior is internal-drag works on touch; cross-procomp doesn't. Document the limit in usage.tsx + dev-mode console warning if a touch drag exits the tree's drop zones. |
...
|---|---|---|---|---|
| LD1 | §2.2 `visibleItems` | `TaskItem[]` (per description's API sketch) | `ReadonlyArray<{ item, level, parentId, index }>` | Renderer + slot consumers need level/parentId/index alongside the item for indent + DnD reparent. Description's English text ("flattened + filtered + sorted snapshot") supports this richer shape; the type annotation in §4 sketch was imprecise. Description's sketch is "NOT final" (line 189). |
| LD2 | §6.1 sensor config | Description L17 mentions "long-press 300ms" with claim "same UX as kanban-board" | Plan uses `MouseSensor` (distance: 5) + `TouchSensor` (delay: 300, tolerance: 5) separately — NOT kanban-board's unified PointerSensor | kanban-board's PointerSensor with no constraint activates immediately on touch, which would conflict with scroll. Splitting Mouse + Touch sensors gives the correct divergent UX (immediate mouse, long-press touch) that L17 actually intends. |
...
| L5 (click → consumer popup) | §13 TaskTreeWithEditor, §9 canEditItem gate |
...
| L16 (useTaskTreeState hook) | §2.2 TaskTreeStateValue, §5 state model, §18 C3 |
...
| L20 (26-method handle) | §2.2 TaskTreeHandle, §3.1 reducer-as-engine |
...
Larger than task-card (~4000 LOC). Comparable to code-block (~4500 LOC) and json-form (~5500 LOC). Within the project's authoring norm.
