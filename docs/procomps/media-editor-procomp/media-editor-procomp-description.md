# media-editor — procomp description
...
2. **Wrap story-composer inappropriately** — open a "Create Story" UI with story-shaped chrome to edit a news hero image. Mode mismatch; users don't understand why they're "publishing a story" to edit a thumbnail.
...
|---|---|---|---|---|---|
| `story-composer` v0.2.0 (refactor) | photo / video / text | all 6 | 9:16 locked | camera + gallery | mobile-fullscreen / desktop-modal |
| `content-composer` news hero | photo | crop / filters / adjust | 16:9 locked | upload only (no camera in CMS) | inline (inside step 2 of composer) |
| `content-composer` post hero | photo | crop / filters / adjust / text / stickers | 1:1 locked | upload + library | inline |
| `content-composer` event cover | photo | crop / filters / adjust | 16:9 or 4:5 (consumer choice) | upload | inline |
...
## Substrate decisions (inherited from story-composer v0.1.5 — locked, not re-litigated)

| Concern | Choice | Source |
|---|---|---|
| Canvas engine | `react-konva` v19.x + `konva` v10 (peer deps) | story-composer §"Substrate decisions" (project root resolves konva to v10; C1 verified) |
...
**Consumer (story-composer / content-composer / chat-panel) owns:**
...
This split means **media-editor has NO `onPublished` callback, NO `uploadUrl` prop, NO `uploader` prop.** It exports blob+metadata via the ref handle on demand. The consumer takes that blob and does whatever its content type needs.
...
**Required-prop rule for dialog mode**: when `presentation` is `"dialog"` (or `"auto"` resolves to dialog), `isOpen` AND `onClose` are required props. Dev-only TypeScript-level enforcement is impractical (props depend on a runtime-resolved enum); runtime validation in dev (`process.env.NODE_ENV !== "production"`) fires `console.error("media-editor: presentation='dialog' requires isOpen + onClose props")` once per instance. Inline mode ignores both props.
...
export interface MediaEditorProps {
  ...
  // === Localization ===
  labels?: Partial<MediaEditorLabels>;
...
Live in `media-editor/hooks/` and are exported via the procomp's `index.ts` barrel:
...
`useImageUploader()` is **NOT exported by media-editor** — it stays in story-composer (upload is consumer concern). story-composer v0.2.0 retains its `useImageUploader` export from v0.1.5 unchanged.

**Backward-compat for v0.1.5 consumers of story-composer hooks:**
- `useMediaCapture` — was exported by story-composer v0.1.5. After v0.2.0, the canonical home is media-editor. story-composer v0.2.0 re-exports it (`export { useMediaCapture } from "../media-editor"`) for one minor version with a JSDoc `@deprecated` pointing at the new import path; removed in v0.3.0.
- `useStoryComposerState` — was exported by story-composer v0.1.5 and has a story-shaped return type (includes `publishStatus`, `uploadProgress`, story-specific draft fields that media-editor doesn't own). Stays in story-composer v0.2.0 with **identical return shape**. Internals **compose `useMediaEditorState` + augment** with the story-specific fields — NOT pure delegation. Plan locks the exact composition shape so the return-type invariant is provable.

The plan also locks the exact re-export shims for any other names that v0.1.5 exposed (sticker types, filter preset types, etc. — if they're now in media-editor, story-composer v0.2.0 re-exports them from its types.ts barrel).
...
A+ procomps don't ship without explicit a11y. Carries forward from story-composer §13 + adapts for the editor-only scope (no publish button):
...
Inherited from story-composer v0.1.5 — media-editor holds the line on `.claude/CLAUDE.md` Design system mandate:
...
Carries forward from story-composer — manual matrix per the `feedback_browser_test_documented_headless_patterns` memory (no automated browser tests). Each shipped feature must be exercised on each row:
...
Inherited from story-composer §14. `"use client"` at the procomp boundary; Konva-touching parts (canvas-shell + capture-shell) wrapped in `dynamic(import(...), { ssr: false })` so they don't crash during the RSC pass when the host page is server-rendered.
...
// GradientPreset, DrawingStroke, CropRect, ValidationError, SourceError, EditAction, EditorCtx, MediaEditorLabels)
// move from story-composer v0.1.5 types.ts as-is — types.ts re-locates them, NOT renamed (locked in plan §"Type system":
// ComposerMode stays ComposerMode, not MediaEditorMode, because v0.1.5 published it and renaming breaks backcompat;
// story-composer v0.2.0 re-exports them from media-editor via the type re-export band).
// New types added by this procomp: MediaEditorProps, MediaEditorHandle, MediaEditorLabels, EditorCtx, InitialSource,
...
### Files that MOVE from `story-composer/` → `media-editor/`
...
### Files that STAY in `story-composer/`
...
- **`lib/defaults.ts`** — editor defaults (color presets, font list, filter list, sticker positions) move to `media-editor/lib/defaults.ts`. Story defaults (labels strings for "Your story" / "Close friends", upload defaults) stay.
- **`types.ts`** — editor-shaped types (`ComposerMode`, `EditTool`, `TextOverlay`, `PlacedSticker`, `ImageAdjustments`, `StickerOption`, `StickerSet`, `FilterPreset`, `FontOption`, `AspectRatio`, `ValidationError`, `GradientPreset`, `DrawingStroke`, `CropRect`, etc.) move to `media-editor/types.ts`. Story-shaped types (`PublishedStory`, `PublishMetadata`, `PublishResult`, `StoryComposerProps`, `StoryComposerHandle`, `StoryComposerLabels`) stay in story-composer's types.ts. **Critical non-breaking rule**: story-composer v0.2.0/types.ts MUST `export type { ComposerMode, EditTool, TextOverlay, PlacedSticker, ImageAdjustments, StickerOption, StickerSet, FilterPreset, FontOption, AspectRatio, ValidationError } from "../media-editor"` so any v0.1.5 consumer doing `import type { ComposerMode, ValidationError } from "@ilinxa/story-composer"` still type-checks. Removing the re-exports is a breaking change reserved for v0.3.0+. Plan §"Type re-exports audit" locks the full list against v0.1.5's actual exported surface (read via `tsc --emitDeclarationOnly` against the pre-extraction snapshot to catch any miss).

### story-composer v0.2.0 — the consumer side

After extraction, story-composer becomes a thin wrapper:

```tsx
// src/registry/components/media/story-composer/story-composer.tsx
"use client";
import { MediaEditor, type MediaEditorHandle } from "../media-editor";

export function StoryComposer({ uploadUrl, uploader, onPublished, ... }: StoryComposerProps) {
...
**Public API of story-composer v0.2.0 is identical to v0.1.5.** No breaking change. The only diff is one new internal peer dep (`@ilinxa/media-editor`) declared in `meta.ts`.
...
- **Backwards-compat shim for story-composer v0.1.x** — refactor is internal; public API preserved; no shim needed.
...
No new peer deps. story-composer v0.2.0 adds `@ilinxa/media-editor` as a registry-side dependency (declared in registry.json `registryDependencies` + meta.ts), not a peer dep.
...
- **~7–8 days** end-to-end (Phase A 5 days + Phase B 2 days + Phase C 1–2 days):
  - Phase A — Day 1–2: file moves (git mv hooks/lib/parts + path-renames + types.ts split + lib/defaults.ts split) with story-composer staying green
  - Phase A — Day 3: media-editor root + state + presentation + capability gating + initialSource + ExportOpts + multi-instance guard
  ...
  - Phase B — Day 6: story-composer v0.2.0 wrapper refactor (handlePublish + ref handle wiring + presentation="dialog") + useStoryComposerState compose + demo regression
  - Phase B — Day 7: meta.ts bump + registryDependencies + manifest sync + every v0.1.5 demo tab visually verified identical
  - Phase C — Day 7–8: GATE 3 spotcheck for media-editor + wrapper-equivalence spotcheck for story-composer v0.2.0 + cross-procomp install smoke + STATUS + decision file + push to master
...
- **Browser-test matrix** carries over from story-composer — Chrome desktop + Safari iOS + Chrome Android.
