...
| **Q-P34** | `sensitivity` gate is **per-card** (gates media only — excerpt + title remain visible). Single reveal button fires `onRevealSensitive(articleId)` (analytics hook) + sets local `sensitiveRevealed=true`. Keyboard-operable. `prefers-reduced-motion` snap-reveal. Mirrors post-card-01 SensitiveGate Q-P33. **Refined 2026-06-02 post-ship:** `ContentSensitiveGate` accepts a `compact: boolean` prop. When `true`: heading text + warnings list dropped, icon shrinks `size-6 → size-5`, 44px reveal button → 24px "Show" pill (aria-label carries full heading + reveal copy for screen readers). Total content height ~60px — fits cleanly in the `small` variant's 96×96 thumb. `small.tsx` passes `compact` to the gate. All other variants use the full overlay. | GATE 1 Q-PF |
