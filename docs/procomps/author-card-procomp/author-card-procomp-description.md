# author-card — procomp description

> Stage 1: what & why.
>
> **Migration origin:** [`docs/migrations/author-card/`](../../migrations/author-card/) (kasder `kas-social-front-v0` news detail page sidebar — `src/app/(platform)/news/[id]/page.tsx` lines 191–205)
>
> Cousin migration: [`thumbnail-list`](../../migrations/thumbnail-list/) — sibling extraction from the same sidebar. Shared family member already shipped: [`newsletter-signup`](../../../src/registry/components/marketing/newsletter-signup/).
...
- **Tone variants** — `primary` / `accent` / `muted` matching the `newsletter-signup` resolver pattern.
...
2. **Sibling-rhythm match** — heading typography (`text-lg font-serif font-bold`), card chrome (`rounded-2xl p-6 border border-border/50`), and overall vertical feel matches `newsletter-signup` / `category-cloud` / `filter-bar` so the three blocks visually compose without retuning.
3. **Tone resolver consistency** — same `primary` / `accent` / `muted` mapping as `newsletter-signup`.
...
11. **Verification** — `pnpm tsc --noEmit` + `pnpm lint` + `pnpm build` clean (1 pre-existing card-tree warning OK; no new); SSR `HTTP 200` for `/components/author-card` with all demo tabs rendered.
...
- **Compound component pattern (`<AuthorCard.Avatar>` etc.)?** — Overkill for ≤ 4 visible parts. Flat-prop API matches the cousin `newsletter-signup` shape; consistent learning curve across the family.
