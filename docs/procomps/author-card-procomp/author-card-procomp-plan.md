# author-card — procomp plan

> Stage 2: how. Implementation blueprint for [`author-card-procomp-description.md`](./author-card-procomp-description.md).

## File map

```
src/registry/components/marketing/author-card/
├── author-card.tsx          # root component, memoized default export
├── parts/
...
Pure, no React. Mirrors `newsletter-signup/parts/tone-resolver.ts` shape (different output class set).
...
Note: registry-wide ESLint override already disables `@next/next/no-img-element` for `src/registry/components/**` (set during `news-card` ship). The per-file disable comment is belt-and-suspenders; can also be omitted. **Decision:** omit the per-file directive — rely on the rule override.
...
- `Required<AuthorCardLabels>` enforces that `AUTHOR_CARD_DEFAULT_LABELS` covers every `labels` field.
...
Built on shadcn primitives `Tabs` + `Badge` (badge optional) for demo navigation. Same structure as `newsletter-signup/demo.tsx`.
...
3. `pnpm build` clean — `/components/author-card` prerendered
...
6. Registry artifacts at `public/r/author-card.json` + `public/r/author-card-fixtures.json`
