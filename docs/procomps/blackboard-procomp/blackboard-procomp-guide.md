# blackboard — consumer guide

> Stage 3: how to use it. Companion to the [description](./blackboard-procomp-description.md) (GATE 1) and [plan](./blackboard-procomp-plan.md) (GATE 2). Shipped v0.1.0, 2026-06-18.
...
- **Threaded discussion** with replies/nesting → `comment-thread`.
...
It ships as a shadcn-style **compound**. The batteries-included `Blackboard` is just `BlackboardRoot` + the flat parts gated by `show*` toggles. Compose the parts directly for a lighter / custom build:
