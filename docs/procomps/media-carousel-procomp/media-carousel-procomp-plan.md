# media-carousel — procomp plan

> Stage 2: how. The implementation contract.
>
> See [`media-carousel-procomp-description.md`](./media-carousel-procomp-description.md) for the what & why.
>
...
|---|---|---|
| Q-P1 | Cross-folder import of `video-player`? | **Allow (option A).** Built-in video handler imports `<VideoPlayer>` directly. shadcn `registryDependencies: ["video-player"]` handles install dependency. Establishes refined sealed-folder rule for the project: **cross-folder imports allowed within a designed system, declared via `registryDependencies`; not allowed across unrelated domains.** Documented in meta.ts + memory. |
...
| Q-P7 | Double-tap unified across image + video items? | **Yes.** Single `onDoubleTap(item, index)` callback on the carousel root. Image items: carousel attaches `useDoubleTap(...)` to the slide wrapper. Video items: carousel forwards to `<VideoPlayer onDoubleTap>`. Both trigger the same callback. |
...
- **`<MediaCarousel>` is `"use client"`** — owns Embla state, observers, refs.
- **`React.memo` at export.**
- **Cross-folder import** — `import { VideoPlayer } from "@/registry/components/media/video-player"` for the built-in video handler. **First explicit cross-import in pro-ui's registry.** Documented as the locked refinement of the sealed-folder rule.
...
```ts
// src/registry/components/media/media-carousel/types.ts

import type { ReactNode } from "react";

export type MediaCarouselVariant = "gallery" | "linear";
...
export const DEFAULT_MEDIA_CAROUSEL_LABELS: Required<MediaCarouselLabels> = {
...
```
src/registry/components/media/media-carousel/
├── media-carousel.tsx                # 1 — root (single-item vs carousel dispatch)
├── parts/
...
- **Critical:** the dispatch is just a component switch — `media-carousel.tsx` itself calls **no hooks beyond `useMemo` for labels** (which fires unconditionally). Each child component owns its own hooks.
...
interface SingleItemProps {
  ...
  onDoubleTap?: (item: MediaItem, index: number) => void;
  labels: Required<MediaCarouselLabels>;
  variant: MediaCarouselVariant;
  aspect: LinearAspect;
  className?: string;
  slideClassName?: string;
...
- Each slide wraps `<SlideRenderer>` + carousel-level `useDoubleTap` if `onDoubleTap` provided AND item type is image (video items get the double-tap forwarded via VideoPlayer).
...
3. `item.type === "video"` → built-in video handler (imports `VideoPlayer`).

```tsx
"use client";

import { cn } from "@/lib/utils";
import { VideoPlayer } from "@/registry/components/media/video-player";
import type {
  MediaItem,
  RenderItemContext,
  MediaCarouselVariant,
...
export function SlideRenderer({
  ...
  // Built-in video handler — imports VideoPlayer (cross-folder, locked Q-P1).
  return (
    <div className={cn(aspectClass, "overflow-hidden", slideClassName)}>
      <VideoPlayer
        src={item.url}
...
export const meta: ComponentMeta = {
  ...
  description:
    "Embla-driven image+video carousel with two variants (gallery peek-scale / linear snap). Composes video-player for video items via the isActive contract — videos in inactive slides pause cleanly.",
  context:
...
- **`@/registry/components/media/video-player`** — `VideoPlayer` + `useDoubleTap` (cross-folder import per Q-P1)
...
- Built-in video: inherits VideoPlayer's accessibility (aria-label, mute aria-pressed, etc.).
...
- [ ] `pnpm lint` clean (1 pre-existing card-tree warning OK)
- [ ] `pnpm build` clean — `/components/media-carousel` prerendered (37th route)
- [ ] SSR returns 200 with all 5 demo tab triggers
- [ ] `/components` index lists the new entry under the `media` category section (joining video-player)
- [ ] Visual sanity:
...
- [ ] Cross-folder import works at build time (`VideoPlayer` resolves)
...
This is the architectural inflection point. Risk: if the cross-import goes wrong (e.g., `VideoPlayer` evolves a breaking API, our peer-dep declaration is wrong), media-carousel ships broken.

**Mitigation:** explicit `registryDependencies: ["video-player"]` in `registry.json` ensures install-time dependency. internal: ["video-player"] in meta.dependencies for visibility. Document in usage.tsx + meta.ts. Future audit: when video-player ships v0.2, verify media-carousel still works at the boundary.
...
If pro-ui ever extracts to NPM packages, media-carousel would need video-player as a peer dep (or runtime dep). Different distribution mechanism but same logical dependency.
...
2. **Built-in minimal `<video>` instead of cross-importing VideoPlayer** — rejected. Degraded UX vs the just-shipped video-player; ships duplicate code.
...
- `pnpm new:component media/media-carousel` — scaffolds the sealed folder.
...
- Author `parts/slide-renderer.tsx` (built-in handlers + cross-import of VideoPlayer).
- Author `parts/nav-buttons.tsx`.
- Author `parts/indicator-dots.tsx`.
- Author `parts/carousel-track.tsx` (Embla orchestrator).
- Author `media-carousel.tsx` root (single-item dispatch + variant dispatch + ref forwarding).
...
- Add to `registry.json` (base + fixtures items, with `registryDependencies: ["video-player"]` on the base).
- Run `pnpm registry:build`.
- Update `.claude/STATUS.md` (note the cross-folder import precedent).
- Author `media-carousel-procomp-guide.md`.

### Estimated total: ~4 hours focused work

Slightly less than video-player (~4.5h) because the state machine is simpler (Embla owns most of it). Establishes the cross-folder import precedent for the rest of the social-posts arc.
