# media-carousel — procomp description
...
>
> Third of 8 components in the social-posts-system arc. Composes [`video-player`](../video-player-procomp/) for video items via the `isActive` contract — the whole reason video-player shipped first. **Adds Embla** as a peer dep (~25KB ESM, no peer deps of its own; approved during analysis sign-off).
...
- **Built-in video item handler** — for `type: "video"` items, renders `<VideoPlayer src={url} poster={poster} isActive={isCurrentSlide} onDoubleTap={...} />`. **This is a deliberate cross-folder import** from `video-player` (sealed-folder rule deviation; see Q-P1 below). Composing the kasder UX without it would force consumers into massive boilerplate.
...
- **Double-tap callback** — `onDoubleTap?: (item, index) => void`. For image items, fires on double-tap of the slide. For video items, the carousel's onDoubleTap is forwarded to the underlying `<VideoPlayer onDoubleTap>` so the same gesture works in both contexts.
...
- **`onSlideChange` callback** — `(index: number) => void` fires when active slide changes. Hosts use this for analytics / preloading / heart-burst overlay positioning (see post-card).
...
- **`feature-strip` variant** — the third variant from the analysis was deferred since no concrete consumer exists in the 8-component scope (engagement-bar's likers-preview is a slot pattern; story-rail uses raw Embla). Add when a third consumer surfaces.
...
- **`post-card`** (later ship) — primary consumer. Sets `variant="gallery"`, items from `post.media`, forwards `onDoubleTap` to engagement-bar's heart-burst overlay.
...
```tsx
<MediaCarousel items={post.media} variant="gallery" />
```
...
<article className="bg-card rounded-2xl p-4">
  ...
  <ExpandableText content={post.body} maxLines={3} />
  <MediaCarousel
    items={post.media}
    variant="gallery"
    onDoubleTap={() => triggerLike(post.id)}
  />
  <EngagementBar actions={[/*...*/]} />
</article>
```
...
1. **Cross-folder import of `video-player` for the built-in video handler — allow or refuse?** This is the **biggest architectural decision** for this ship. Options:
   - **(A) Allow the cross-import.** media-carousel's built-in video handler renders `<VideoPlayer isActive={...} />`. shadcn `registryDependencies: ["video-player"]` handles install-time dependency. **Pros:** kasder UX works out of the box; the `isActive` contract is delivered for free. **Cons:** establishes precedent that cross-folder imports are OK within a "system" (social-posts arc); future audits need to draw the line carefully.
   - **(B) Slot-based only.** Carousel handles image items; video items REQUIRE `renderItem` slot. Consumer wires video-player. **Pros:** zero cross-folder imports; sealed-folder discipline preserved. **Cons:** consumer boilerplate for the 99% case (post cards, story rails — every social surface needs videos).
   - **(C) Built-in minimal `<video>`.** Carousel ships its own `<video src muted loop playsInline>` for video items (no controls, no double-tap, no proper `isActive`). Consumers wanting full features override via `renderItem`. **Pros:** zero cross-folder imports; works out of the box. **Cons:** degraded video UX vs the just-shipped video-player; double maintenance.
   
   **Recommendation: (A) — allow the cross-import for this arc.** The 8 social-posts components are designed as a coherent system; treating them as 8 isolated primitives forces consumer boilerplate that defeats the purpose of shipping them together. Document explicitly in this description + meta.ts as a deliberate exception. Set the rule going forward: **cross-folder imports allowed within a designed system (declared via shadcn `registryDependencies`); not allowed across unrelated domains.** If the user prefers (B), the carousel ships with image-only built-in handler + clear "wire video-player here" docs.
...
7. **Double-tap on image items vs video items — different handling?** Video items: forward to `VideoPlayer.onDoubleTap`. Image items: carousel attaches `useDoubleTap(onDoubleTap)` to the slide wrapper. Both fire the same `onDoubleTap(item, index)` callback. **Lock: yes, both fire the same callback.**
...
**Awaiting your sign-off before I draft the plan doc.** The **big call to make** is open question #1 — cross-folder import of `video-player` (recommendation A: allow). If you redirect to (B) or (C), the entire built-in video handler design changes. Other questions are smaller and can be resolved in plan-stage Q-P locks.
