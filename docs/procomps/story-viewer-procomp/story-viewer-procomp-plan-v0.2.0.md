# story-viewer v0.2.0 — procomp plan

> **Stage 2: how.** Addendum to the v0.1.0 [plan](story-viewer-procomp-plan.md). v0.1 plan remains authoritative for the cursor / progress timer / tap-zone / keyboard nav architecture; v0.2 layers engagement on top without retouching the viewer core.
>
> **Prerequisite:** [GATE 1 description v0.2.0](story-viewer-procomp-description-v0.2.0.md) signed off (7 Q-Ps locked + 10 pre-locked from precedent).
>
> **A+ template:** mirror [`post-card-procomp-plan.md`](../post-card-procomp/post-card-procomp-plan.md) where post-card v0.2.0 established the role-aware-permissions + slot/disable + engagement-layer pattern.
...
- [`src/registry/components/media/story-viewer/parts/item-view.tsx:1`](../../src/registry/components/media/story-viewer/parts/item-view.tsx#L1) — `import { VideoPlayer } from "../../video-player"` → `"../../video-player/video-player"` (F-S1 specific-file). Same surgical fix as media-carousel v0.1.3 did to `slide-renderer.tsx`.
- [`src/registry/components/media/story-viewer/parts/viewer-header.tsx`](../../src/registry/components/media/story-viewer/parts/viewer-header.tsx) — pause / mute / close buttons `h-8 w-8` (32×32) → `h-11 w-11` (44×44). Optional `md:h-9 md:w-9` (36×36) for desktop-tighter (acceptable since desktop has mouse precision).
- [`src/registry/components/data/story-rail/usage.tsx`](../../src/registry/components/data/story-rail/usage.tsx) lines 32, 53, 91 — positional `onItemClick={(item, index) => …}` → object-shape `onItemClick={({ item, index }) => …}`. Plus the prose mention at line 10. This is the bundled story-rail v0.2.1 ship; bump `src/registry/components/data/story-rail/meta.ts` v0.2.0 → v0.2.1 + features list entry.
- `meta.ts` (story-rail) — version bump + features entry.
...
**Commit message:** `fix(story-rail): v0.2.1 + chore(story-viewer): F-S1 + touch-target (rolls into v0.2.0)` — story-rail is bumped to v0.2.1 in this commit (its own ship); story-viewer hygiene patches stage for v0.2.0 (no separate version bump here)

### C1 — types expansion (story-viewer)

**Files touched:** 1 (types.ts massive expansion)

Add to [`src/registry/components/media/story-viewer/types.ts`](../../src/registry/components/media/story-viewer/types.ts):
...
// Inherits EngagementLikerProfile shape from engagement-bar via inline-copy
...
export interface StoryViewerHandle {
  // v0.1 (unchanged):
...
export interface StoryViewerLabels {
  ...
  /** Nested forward to engagement-bar labels (per Q-V14 — inherit). */
  engagementLabels?: EngagementBarLabels;
  /** Nested forward to comment-thread labels (composer keys only matter). */
...
Export `resolveStoryPermissions` + `canPerformStoryActionInternal` with identical signatures to post-card's helpers.

**Verification:** tsc + lint clean.

**Commit message:** `feat(story-viewer): v0.2.0 C2 — lib/permissions.ts (resolver mirroring post-card)`

### C3 — engagement overlay (compose engagement-bar v0.3.x)

**Files touched:** 2 NEW (`parts/engagement-overlay.tsx` + `hooks/use-story-engagement-state.ts`); 1 modified (`story-viewer.tsx`)

**Cross-category type imports — Bug 3 inline-copies required:**

`engagement-bar` is in `data/` category; `story-viewer` is in `media/`. So **cross-category** for all engagement-bar / comment-thread composition. Per F-S1 Bug 3 lock:

- **COMPONENT imports** use absolute-with-suffix `@/registry/components/data/engagement-bar/engagement-bar` (works through the rewriter)
- **TYPE imports** must be **inline-copied** locally — `@/registry/components/data/engagement-bar/types` does NOT survive the rewriter (mangles to wrong slug or invalid `<cat>/` prefix)

Inline-copies needed in `types.ts` (add to C1):

```ts
// Inlined from @ilinxa/engagement-bar's EngagementAction discriminator.
// MUST be kept structurally identical. If engagement-bar adds a new arm,
...
export interface StoryEngagementBarLabels {
  like?: string;
  unlike?: string;
  /* ...full 15-key shape verbatim from engagement-bar/types.ts EngagementBarLabels... */
}
```
...
Document all inline-copies in JSDoc with a sync-warning: *"if engagement-bar changes the upstream type, this copy MUST be updated manually."*
...
import { EngagementBar } from "@/registry/components/data/engagement-bar/engagement-bar";
import type { StoryEngagementAction, Story, StoryItem, StoryViewerLabels } from "../types";
...
export interface EngagementOverlayProps {
  ...
  labels: Required<StoryViewerLabels>;
  className?: string;
}
...
Mirror post-card's `defaultPostEngagementActions`. Build the action list from:
...
Per-item engagement state (likes / reactions / viewerLiked / viewerReaction). Mirror engagement-bar's `useEngagementState` shape but scoped per-(storyId, itemId). Subscribe wiring fires deltas; reset on cursor change.

**Wire into story-viewer.tsx** — render `<EngagementOverlay>` inside the ViewerShell when `viewerMode === "viewer"` AND `!disableEngagement`.
...
**Commit message:** `feat(story-viewer): v0.2.0 C3 — engagement overlay (compose engagement-bar stacked)`

### C4 — reply composer (compose comment-thread CommentComposer)

**Files touched:** 1 NEW (`parts/reply-composer.tsx`); 1 modified (`story-viewer.tsx`)
...
import { CommentComposer } from "@/registry/components/data/comment-thread/parts/comment-composer";
import type { CommentComposerHandle } from "@/registry/components/data/comment-thread/parts/comment-composer";
// Inline-copy CommentThreadCurrentUser (cross-cat type — Bug 3 fix)
import type { Story, StoryItem, StoryViewerLabels } from "../types";
...
**Wire into story-viewer.tsx** — render `<ReplyComposer>` when `viewerMode === "viewer"` AND `!disableReplyComposer` AND `canPerformAction("reply", story, item) !== false`.
...
**Commit message:** `feat(story-viewer): v0.2.0 C4 — reply composer (compose CommentComposer + auto-pause)`

### C5 — owner overlay (view-count + lazy viewers list)

**Files touched:** 1 NEW (`parts/owner-overlay.tsx`); 1 modified (`story-viewer.tsx`)
...
import { LikersStrip } from "@/registry/components/data/engagement-bar/parts/likers-strip";
import type { Story, StoryItem, StoryViewerLabels, ViewerListItem } from "../types";
...
**Wire into story-viewer.tsx** — render `<OwnerOverlay>` when `viewerMode === "owner"` AND `!disableOwnerOverlay`.
...
**Commit message:** `feat(story-viewer): v0.2.0 C5 — owner overlay (view-count + lazy viewers list)`
...
**F-cross-13 alert:** Both render sites use `<DropdownMenuTrigger>` — apply the `buttonVariants(…)` pattern (no `asChild + Button` wrapping). Same fix as post-card v0.3.1 + comment-thread v0.2.1. Required pattern:
...
**Commit message:** `feat(story-viewer): v0.2.0 C6 — kebab assembly (engagement-overlay + header fallback)`
...
Pattern (same as post-card's variant parts):
...
**Commit message:** `feat(story-viewer): v0.2.0 C7 — render slots (7 connective seams)`

### C8 — disable opt-outs (8 flags)

**Files touched:** 1 (story-viewer.tsx + dispatch table)
...
**Commit message:** `feat(story-viewer): v0.2.0 C8 — 8 disable opt-outs`

### C9 — long-press pause + StoryItem.link CTA + linkComponent

**Files touched:** 1 NEW (`hooks/use-long-press-pause.ts`); 1 NEW (`parts/link-cta.tsx`); 1 modified (`story-viewer.tsx`)
...
**Commit message:** `feat(story-viewer): v0.2.0 C9 — long-press pause + StoryItem.link CTA + linkComponent`
...
- Add a sample `reactionKinds` array for the engagement overlay (reuse engagement-bar's reaction kinds shape)
...
features: [
  ...existing,
  "v0.2.0 — Engagement overlay composing engagement-bar v0.3.x (variant=stacked) — like + reaction (host-supplied kinds) + comment + share + bookmark + kebab as 6th item",
  "v0.2.0 — Reply composer composing comment-thread v0.2.1 CommentComposer with auto-pause-on-type + currentUser-gated visibility",
...
dependencies: {
  ...
  internal: ["video-player", "engagement-bar", "comment-thread"], // expanded
},
```
...
**Spotcheck file:** `docs/procomps/story-viewer-procomp/reviews/2026-MM-DD-v0.2.0-spotcheck.md`. Per readiness-review rule: public-API-touching minor bump → spotcheck required; rotating dim = Public API (major surface expansion); 4 fixed dims + Public API. Self-review acceptable per pro-component tier.

**Decision file:** `.claude/decisions/2026-MM-DD-story-viewer-v0.2.0-engagement-layer-ship.md`.

**STATUS.md + component-versions.md** bumps for story-viewer 0.1.2 → 0.2.0 + story-rail 0.2.0 → 0.2.1 (bundled).

**Verification gates:** tsc + validate:meta-deps + lint + registry:build + manual demo run-through.

**Commit message:** `feat(story-viewer): v0.2.0 C10 — handle + demo + meta + guide + spotcheck + STATUS`
...
```
story-viewer v0.2.0 (media/)
├── compose engagement-bar v0.3.x (data/) — variant=stacked
│   ├── EngagementBar (component) — absolute-with-suffix import
...
├── compose comment-thread v0.2.1 (data/) — CommentComposer
...
├── compose video-player v0.1.x (media/) — VideoPlayer + useDoubleTap
│   └── Same-cat relative + specific-file (existing C0 hygiene fix)
└── self
    ├── lib/permissions.ts NEW (mirror post-card)
    ├── lib/kebab.ts NEW
...
- New `<Popover>` usage? — engagement-bar's ReactionPicker uses Popover internally; we don't directly mount Popover here, so we inherit engagement-bar's defensive wiring (post-v0.3.2 Popover 3-facet fix). No additional surface from story-viewer.
...
**Smoke harness:** post-push, run `node scripts/smoke-all.mjs --slug story-viewer` from `e:/tmp/ilinxa-smoke-consumer`. Will transitively install story-viewer + comment-thread + engagement-bar + expandable-text + media-carousel + video-player. Consumer-tsc must pass for all 6.
...
| Same-cat cross-procomp imports | video-player (same media/ category) uses relative + specific-file (`../../video-player/video-player`); engagement-bar / comment-thread (cross data/) use absolute-with-suffix | ✅ Plan §C0 fixes existing relative-barrel; §C3/C4 use absolute-with-suffix for cross-cat components |
...
| Permissions resolver auto-derivation | viewerMode does NOT auto-derive from currentUser.id === story.userId; host opts in explicitly (Q-V11 lock) | ✅ Plan §C2 mirrors post-card pattern verbatim — explicit-only |
...
| meta.ts dependencies.internal | Updated from `["video-player"]` to `["video-player", "engagement-bar", "comment-thread"]` | ✅ Plan §C10 |
...
| story-rail v0.2.1 docs bundled | 3 stale onItemClick snippets + prose mention fixed | ✅ Plan §C0 bundles |

---

## Acceptance criteria (for v0.2.0 "ready to ship" verdict)

Per the [GATE 1 description acceptance §](story-viewer-procomp-description-v0.2.0.md#acceptance-criteria). All 12 items met. Quick re-list:
...
12. Bundled story-rail v0.2.1 docs patch ✅ (C0)
...
| EngagementBar `variant="stacked"` layout doesn't fit the modal's right edge cleanly | Medium | Manual visual test in C3; may need `className` overrides on the stacked bar; engagement-bar's stacked variant is designed for this |
...
**Estimated wall-clock:** ~1-2 working days. Each commit is ~30-90 min including verification. Same pace as post-card v0.2.0 (13 commits, single working day).
