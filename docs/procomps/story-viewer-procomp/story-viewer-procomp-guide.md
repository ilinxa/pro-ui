...
> [`description.md`](./story-viewer-procomp-description.md) +
> [`plan.md`](./story-viewer-procomp-plan.md) (v0.1),
> [`description-v0.2.0.md`](./story-viewer-procomp-description-v0.2.0.md) +
> [`plan-v0.2.0.md`](./story-viewer-procomp-plan-v0.2.0.md) (v0.2). This
> guide is the only live consumer-facing doc.

## 30-second mental model

`<StoryViewer>` is a **full-screen sequential story viewer** — Radix Dialog modal with mobile-fullscreen / desktop centered portrait (`md:w-100 md:h-175`), segmented progress bars, tap zones (left/middle/right thirds), desktop nav arrows for story-level navigation, keyboard nav (← → space escape), pause-preserving accumulator timer, ID-anchored cursor, video-player composed for video items. v0.2 added a role-aware engagement layer (composing `engagement-bar` + `comment-thread`); v0.3 added Instagram-canonical comments + share bottom-sheet panels; v0.4 added a 3D cube transition + finger-driven swipe between stories (pure CSS — no framer-motion).
...
Auto-pulls `video-player` (cross-folder dep), shadcn `dialog` / `avatar` / `button`.
...
- **Item duration resolution:** `currentItem.duration` (explicit) → video metadata (from video-player's `onLoadedMetadata`) → `defaultItemDuration` (default 5s) — image fallback. Non-video custom items via `renderItem` MUST set `item.duration` explicitly.
...
```tsx
const TR_LABELS: StoryViewerLabels = {
...
<StoryViewer labels={TR_LABELS} {...rest} />
```
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <ul className="flex flex-col gap-4">
        {posts.map((post) => (
          <li key={post.id}>
            <PostCard variant="feed" post={post} currentUser={viewer} />
          </li>
        ))}
      </ul>
...
This is the canonical Tier-3 wiring for the social-posts-system arc — story-rail + story-viewer in lockstep, post-card below, all 8 components composed once. The `/sandbox/social-feed-page-01` Tier-3 sandbox demonstrates this end-to-end.
...
vary (mirrors post-card v0.3.0 resolver).
...
`@ilinxa/engagement-bar` `variant="stacked"` on the viewer's right edge,
...
`@ilinxa/comment-thread`'s `CommentComposer`. Auto-pauses on first character
...
the panel. Always-mounted so consumer state (e.g., CommentThread's
...
v0.3.9 added four new keys to `StoryViewerLabels` (all overridable;
defaults shipped):
...
engagement-bar, `commentLabels` for comment-thread).
...
When this component composes another registry component (cross-folder import), it imports only from the OTHER component's `<slug>.tsx` file — never from `lib/`, `hooks/`, or `parts/` sub-folders. Conversely, when other registry components compose `story-viewer`, they import only from `story-viewer.tsx`.
