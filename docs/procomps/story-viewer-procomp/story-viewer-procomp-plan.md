# story-viewer — procomp plan

> Stage 2: how (the implementation contract).
>
> See [`story-viewer-procomp-description.md`](./story-viewer-procomp-description.md) for what & why. Description signed off — all R-D-1..8 refinements absorbed.
>
...
```
src/registry/components/media/story-viewer/
├── story-viewer.tsx                  # root: Radix Dialog wrapper + state orchestration + handle
├── parts/
...
│   └── item-view.tsx                    # image OR video item renderer (composes video-player)
├── hooks/
...
| `VideoPlayer`, `DEFAULT_VIDEO_PLAYER_LABELS` | `@/registry/components/media/video-player` | Cross-folder dep — declared via `registryDependencies` in registry.json |
...
export const DEFAULT_STORY_VIEWER_LABELS: Required<StoryViewerLabels> = {
...
export interface StoryViewerProps {
  ...
  isOpen: boolean;
  onClose: () => void;
  ref?: React.Ref<StoryViewerHandle>;
  // Realtime
...
`story-viewer` imports `video-player` as a cross-folder registry component. Declared via `registryDependencies: ["https://ilinxa-proui.vercel.app/r/video-player.json"]` in registry.json (locked precedent — same pattern as comment-thread importing engagement-bar and post-card importing all 5 priors).

## Behavioral contracts per file

### `story-viewer.tsx` (root)
...
Memoized at export: `export default memo(StoryViewer)` from inside the file? No — root is exported by name (matches story-rail / post-card / comment-thread pattern; consumers wrap externally if needed).
...
Subscribe wiring (locked Subscribe-pattern from story-rail):
...
Documents installation, mental model, mount-only items, click → viewer wiring (with rail integration), realtime pattern, imperative handle. Same structure as story-rail usage.

### `meta.ts`

Standard pro-ui meta: `slug: "story-viewer"`, `category: "media"`, `version: "0.1.0"`, status `stable`, dependencies (lists `video-player` cross-folder), shadcn primitives (`dialog`, `avatar`, `button`), peer deps (none beyond what video-player declares).
...
|---|---|---|
| `bg-gradient-to-X` | `bg-linear-to-X` | None in this scope (gradient ring is in story-rail) |
...
{ meta: storyViewerMeta, Demo: StoryViewerDemo, Usage: StoryViewerUsage },
```
...
```json
{
  ...
  "registryDependencies": ["https://ilinxa-proui.vercel.app/r/video-player.json"],
  "dependencies": [],
  "files": [
    { "path": "src/registry/components/media/story-viewer/story-viewer.tsx", "type": "registry:component", "target": "components/story-viewer/story-viewer.tsx" },
    { "path": "src/registry/components/media/story-viewer/parts/viewer-shell.tsx", "type": "registry:component", "target": "components/story-viewer/parts/viewer-shell.tsx" },
    { "path": "src/registry/components/media/story-viewer/parts/progress-bars.tsx", "type": "registry:component", "target": "components/story-viewer/parts/progress-bars.tsx" },
    { "path": "src/registry/components/media/story-viewer/parts/viewer-header.tsx", "type": "registry:component", "target": "components/story-viewer/parts/viewer-header.tsx" },
    { "path": "src/registry/components/media/story-viewer/parts/tap-zones.tsx", "type": "registry:component", "target": "components/story-viewer/parts/tap-zones.tsx" },
    { "path": "src/registry/components/media/story-viewer/parts/nav-arrows.tsx", "type": "registry:component", "target": "components/story-viewer/parts/nav-arrows.tsx" },
    { "path": "src/registry/components/media/story-viewer/parts/item-view.tsx", "type": "registry:component", "target": "components/story-viewer/parts/item-view.tsx" },
    { "path": "src/registry/components/media/story-viewer/hooks/use-story-viewer-state.ts", "type": "registry:component", "target": "components/story-viewer/hooks/use-story-viewer-state.ts" },
    { "path": "src/registry/components/media/story-viewer/hooks/use-story-progress.ts", "type": "registry:component", "target": "components/story-viewer/hooks/use-story-progress.ts" },
    { "path": "src/registry/components/media/story-viewer/hooks/use-story-keyboard-nav.ts", "type": "registry:component", "target": "components/story-viewer/hooks/use-story-keyboard-nav.ts" },
    { "path": "src/registry/components/media/story-viewer/lib/format-time.ts", "type": "registry:component", "target": "components/story-viewer/lib/format-time.ts" },
    { "path": "src/registry/components/media/story-viewer/types.ts", "type": "registry:component", "target": "components/story-viewer/types.ts" },
    { "path": "src/registry/components/media/story-viewer/index.ts", "type": "registry:component", "target": "components/story-viewer/index.ts" }
  ]
},
{
  "name": "story-viewer-fixtures",
  "type": "registry:component",
  "registryDependencies": ["https://ilinxa-proui.vercel.app/r/story-viewer.json"],
  "files": [
    { "path": "src/registry/components/media/story-viewer/dummy-data.ts", "type": "registry:component", "target": "components/story-viewer/dummy-data.ts" }
  ]
}
```
...
2. `pnpm lint` clean (only pre-existing card-tree warning).
3. `pnpm dev` → `/components/story-viewer` HTTP 200, all 6 demo tabs render.
4. `pnpm registry:build` → `public/r/story-viewer.json` + `public/r/story-viewer-fixtures.json` regenerate.
