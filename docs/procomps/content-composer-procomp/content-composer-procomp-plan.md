...
```
src/registry/components/media/content-composer/
├── content-composer.tsx          # root forwardRef orchestrator — "use client"          (EXPORTED via barrel)
├── parts/
...
│   ├── body-substrate-plate.tsx     # React.lazy default-export wrapping <RichTextEditor> (INTERNAL, default-export)
...
│   └── post-composer.config.ts      # post config (modeled; blocked on media-editor v0.2)     NEW  (EXPORTED)
├── adapters/
...
├── dummy-data.ts                    # sample draft + sample NewsCardItem (re-edit demo)       NEW  (EXPORTED via fixtures)
...
**Final registry roster**: **28 base files** (`type: "registry:component"`, `target: "components/content-composer/<sub-path>"`) = on-disk 32 − `demo.tsx`/`usage.tsx`/`meta.ts` (docs-site only) − `dummy-data.ts` (→ fixtures) + 1 fixtures item (`content-composer-fixtures`, depends on base, adds `dummy-data.ts`) = **29 distributed artifacts**. `demo.tsx` / `usage.tsx` / `meta.ts` **EXCLUDED** per the locked convention (docs-site only). Counts are plan-time estimates — reconcile at C16 (registry.json) per the manual roster audit, exactly as `media-editor` did.
...
import { RichTextEditor, RICH_TEXT_EMPTY_VALUE, type RichTextValue }
...
import type { MediaEditorProps, MediaEditorHandle, MediaEditorState, ExportMetadata,
  InitialSource, ComposerMode, EditTool, MediaSource, AspectRatio }
  from "@/registry/components/media/media-editor/types";
...
import type { NewsCardItem, NewsArticleAuthor, NewsPublisher, ContentSensitivity,
  ContentPaywall, ContentStatus, NewsVisibility }
  from "@/registry/components/data/news-card/types";
```

`registry.json` MUST add **`registryDependencies`** (NOT `dependencies` — those are NPM peers) listing the live URLs of `json-form`, `rich-text-editor`, and `media-editor`:

```json
"registryDependencies": [
  "https://ilinxa-proui.vercel.app/r/json-form.json",
  "https://ilinxa-proui.vercel.app/r/rich-text-editor.json",
  "https://ilinxa-proui.vercel.app/r/media-editor.json"
]
```

`news-card` is a **type-level dependency only** (the adapter's forward target) — list it in `meta.ts` `dependsOn` but it need NOT be a runtime `registryDependency` (types are erased; the consumer only needs its `types.ts` to typecheck, which the page that mounts the news config already installs).
...
import type { NewsCardItem, ContentStatus } from "@/registry/components/data/news-card/types";
import type { FormSchema, Condition } from "@/registry/components/forms/json-form/types";

export type { MediaEditorState, ExportMetadata, InitialSource, RichTextValue, NewsCardItem };
...
export interface BodySlotConfig {
  ...
  fieldName: string;
  placeholder?: string;
  emptyValue?: RichTextValue;  // plate-only sentinel; plaintext ignores it and uses "" — defaults to RICH_TEXT_EMPTY_VALUE
}

/**
 * mediaSlot → 1:1 passthrough of media-editor's dials. Every key is a verified
 * MediaEditorProps prop (media-editor/types.ts:400-475). The shell spreads these
 * straight onto <MediaEditor>, EXCEPT `mediaSources` which the substrate CLAMPS to the
...
/**
 * MediaEditorState minus the non-serializable live Blob (videoBlob:Blob, media-editor/types.ts:218).
...
export interface ComposerDraft {
  ...
  status: ContentStatus;                       // "draft"|"scheduled"|"published"|"archived" → NewsCardItem.status (same CLOSED enum)
...
export interface ContentComposerProps {
  ...
  // Re-edit
  initialItem?: NewsCardItem;                 // drives the INVERSE adapter (item.image → media initialSource)
  initialBody?: BodySlotValue;                   // persisted body — NOT on NewsCardItem (§5)
...
- **`ComposerDraft.status`** (`ContentStatus`) — the persisted publish axis; written only by the draft reducer's `set-status` on the terminal-ish arms; maps 1:1 to `NewsCardItem.status`.
...
export function useComposerState({
  ...
  // latest-onChange ref — stable dispatch identity across parent re-renders (media-editor.tsx:165-173).
...
Shell wiring (mirrors `kanban-board.tsx:47-48`):
```ts
...
| `bodySlot` | current `BodySlotValue` | **shell-derived** baseline JSON-compare (§autosave) — Plate has NO dirty signal | config `minLength` over flattened text vs `RICH_TEXT_EMPTY_VALUE` | `setBodyValue(v)` + **reset baseline** |
...
export const richTextSubstrate: SlotSubstrate<"bodySlot"> = { kind: "bodySlot", render: (args) => /* lazy <RichTextEditor> or <Textarea> per slotConfig.substrate */ null };
export const mediaEditorSubstrate: SlotSubstrate<"mediaSlot"> = { kind: "mediaSlot", render: (args) => /* mounts <MediaEditor> + clampMediaSources */ null };

export const DEFAULT_SUBSTRATES: SlotSubstrateMap = {
  metadataFields: jsonFormSubstrate,
  bodySlot: richTextSubstrate,   // dispatches plate-vs-plaintext on slotConfig.substrate
  mediaSlot: mediaEditorSubstrate,
};

export const findSubstrate: FindSubstrate = (substrates, key) => substrates[key]; // analogous to kanban findRenderer (kanban-board.tsx:216), but a keyed-MAP object-index — NOT an array .find()
```
...
`<RichTextEditor>` exposes **NO** dirty surface (props are `value/defaultValue/onChange/onSave/readOnly/placeholder/onImageUpload/...hideToolbar/autoFocus` only). The shell derives bodySlot dirty by baseline JSON-compare, reusing rich-text-editor's OWN content key (`rich-text-editor.tsx:148-154` — `JSON.stringify` is adequate; Plate JSON has no functions/cycles):
...
//   richtext-empty → flattened-text length 0 vs RICH_TEXT_EMPTY_VALUE
...
bodyValidate = async () => {
  ...
  if (bodyContentKey(current) === bodyContentKey({ kind: "richtext", value: RICH_TEXT_EMPTY_VALUE })) return false;
...
```ts
// parts/missing-substrate.tsx — VERBATIM port of kanban-board/parts/missing-renderer.tsx
...
export function warnMissingSubstrate(slotKind: string) {
  ...
  warned.add(slotKind);
  if (typeof console !== "undefined") console.warn(`[content-composer] No substrate registered for slot="${slotKind}". Provide one via the substrates prop (defaults ship for metadataFields/bodySlot/mediaSlot).`);
}
...
## News adapter pair (against `NewsCardItem`) (description §5)

Co-located in `configs/news-composer.config.ts` (QP-7), exported as a pure forward+inverse pair, selected by `config.adapterId === "news-content-item"` from `adapters/adapter-registry.ts`. Forward target = the **real shipped `NewsCardItem`** (`news-card/types.ts:144`).

```ts
const NEWS_STEP = { headline: "headline", hero: "hero", body: "body", meta: "meta", gates: "gates" } as const;
```

### Forward — `toContentItem(draft, ctx) → NewsCardItem`
...
```ts
function toContentItem(draft: ComposerDraft, ctx: { now: Date; currentUser?: { id: string; name: string } }): NewsCardItem {
...
```ts
function fromContentItem(item: NewsCardItem): { draft: Partial<ComposerDraft>; mediaInitialSource?: InitialSource } {
...
Body is **NOT** on `NewsCardItem` (the card carries only `excerpt`). **Out:** the shell exposes the raw `BodySlotValue`; the wrapping pro-page persists it to its own body column (`toContentItem` does NOT emit body — display-card projection only; HTML egress via `serializeRichTextToHtml(value)` — listed in description §5 as an adapter responsibility — is **consciously descoped to page-side / v0.2** here: the serializer is async/server-leaning and is NOT reachable from the `.tsx` tail import the plan locks, so the news-v0.1 client never calls it). **In:** `fromContentItem` CANNOT source body — the page reads the persisted `RichTextValue` from its body column and feeds the shell separately via `initialBody?: BodySlotValue`. `NewsCardItem`-shaped fields round-trip through the adapter; the body round-trips through this separate seam.

### Exported adapter

```ts
export const newsContentItemAdapter: ContentTypeAdapter<NewsCardItem> = { contentType: "news", toContentItem, fromContentItem };
```
...
## media-editor clamp-unknown-source rule (description §15 + §"two examples" #5)

The `post` config DECLARES `mediaSources: ["upload","library"]`, but media-editor's real `MediaSource` is `"camera" | "upload"` only (`media-editor/types.ts:129`; `"library"` deferred to its v0.2). The mediaSlot substrate must **CLAMP** the unknown `"library"` to upload-only WITHOUT crashing.

**Precedent:** media-editor does NO validation of `mediaSources` — it reads membership only (`mediaSources.includes("camera")`, `.includes("upload")` — `media-editor.tsx:211/:304-305/:1161`). An unknown `"library"` simply fails every `.includes()` (and leaks into the dev-debug `sources:` display string at :1166-1167 if passed unfiltered). So the clamp is a **pre-filter on the way in**, mirroring the dial's own no-crash degradation — NOT a new rejection path.

```ts
// lib/clamp-media-sources.ts
import type { MediaSource } from "@/registry/components/media/media-editor/types";
const KNOWN: readonly MediaSource[] = ["camera", "upload"];

/** Drop any mediaSource media-editor v0.1.x doesn't understand (e.g. "library"). Never throws.
...
import { RichTextEditor, RICH_TEXT_EMPTY_VALUE, type RichTextValue }
  from "@/registry/components/data/rich-text-editor/rich-text-editor";   // the .tsx file path
```

Only `RichTextEditor`, `RICH_TEXT_EMPTY_VALUE`, and type `RichTextValue` are reachable from this `.tsx` tail re-export. `RICH_TEXT_DEFAULT_PLACEHOLDER` and `serializeRichTextToHtml` are NOT in the tail — pass the placeholder as a **literal string**; the (async, server-only) serializer is out of news-v0.1 client scope.
...
export default function BodySubstratePlate({ value, onChange, slotConfig, uploader, ariaProps }: BodySubstrateProps) {
  const safe = Array.isArray(value) ? (value as RichTextValue) : RICH_TEXT_EMPTY_VALUE;  // guard like field-richtext
  return (
...
// richTextSubstrate.render wraps <BodySubstratePlate> in <Suspense> (RichTextEditor also has an internal boundary).
```

The plaintext fallback `BodySlotValue` is a plain STRING; richtext is an ARRAY — `BodySlotValue` is the discriminated union; `getValue`/`getIsDirty`/`loadValue`/`validate` branch on `kind`. **`RichTextEditor` has NO dirty signal** — shell-derived via JSON baseline-compare (§autosave); baseline MUST reset after every save/autosave (#1 trap).

---

## Composition pattern / client-vs-server (description §14)

- `"use client"` at the procomp boundary (`content-composer.tsx`).
- Konva-touching parts arrive via the lazy `media-editor` slot (already `dynamic(..., { ssr: false })` internally) — the composer does NOT re-wrap it.
...
Single greenfield phase. News ships at C12; the post config is authored-but-deferred at C13 (blocked on media-editor v0.2 `"library"`).

| # | Commit | Lands |
|---|---|---|
| **C1** | `chore(content-composer): scaffold + F-cross-13 substrate verify` | `pnpm new:component media/content-composer`. Skeleton root that throws. Manifest entry. F-cross-13 grep of transitive primitives (`dialog`/`textarea`/json-form `select`/`command`/`popover`/Plate/media `slider`/`popover`); record divergence findings in the commit message. tsc clean. |
| **C2** | `feat(content-composer): types.ts + index.ts barrel scaffold` | Full `types.ts` per §Type-system: `SlotKind`, `ComposerConfig`, `ComposerStep`, the 3 slot configs (1:1 media dials), `BodySlotValue`, `ComposerDraft` + `ComposerStepValue`, `SerializableMediaEditorState` + `MediaSlotValue`, `SlotSubstrate {kind,render}` + `SlotSubstrateMap`, `SlotHandle`, `ContentTypeAdapter<TItem>`, `PublishMode`/`PublishIntent`/`ComposerPhase`/`GateResult`, `ComposerCtx`/`ComposerStepCtx`, props + handle. Types-only barrel. tsc clean. |
| **C3** | `feat(content-composer): useComposerState (fork useKanbanState) + reducer + phase-reducer` | `hooks/use-composer-state.ts` (controlled/uncontrolled triplet — reduce over `state` not `internal`) + `lib/reducer.ts` (`composerReducer` + `replace` arm + `makeEmptyDraft`) + `lib/phase-reducer.ts` (FSM transitions per §7). tsc + lint clean. |
| **C4** | `feat(content-composer): substrate registry + missing-substrate fallback + findSubstrate` | `lib/substrates.ts` (typed `Partial<Record<SlotKind,SlotSubstrate>>`, `DEFAULT_SUBSTRATES`, `findSubstrate`) + `parts/slot-mount.tsx` + `parts/missing-substrate.tsx` (module-level `Set` dedupe `console.warn`, NEVER throw). |
| **C5** | `feat(content-composer): useComposerContext + step indicator + shell frame + dialog` | `hooks/use-composer-context.ts` (createContext + throwing `useComposerStep`/`useComposerContext`, workspace model, useMemo'd value) + `parts/composer-shell.tsx` + `parts/composer-dialog.tsx` (shadcn Dialog + focus trap) + `parts/step-indicator.tsx` (labeled nav + aria-current). Backward nav free; forward nav stubbed-gated. |
| **C6** | `feat(content-composer): jsonForm substrate + hydration layer + tags/author-picker renderers` | `lib/hydration.ts` (`hydrateSchema`/`stripHydration`) + jsonFormSubstrate (mounts `<JsonForm>` `submitButton={false}`, no-op `onSubmit`, `onReady`-captured handle, `onChangeDebounce={0}`, controlled `values`) + `parts/field-tags.tsx` + `parts/field-author-picker.tsx` via `fieldRegistry`. SlotHandle: `getValue→getValues()`, `getIsDirty→isDirty()`, `validate→trigger()+isValid()`, `loadValue→reset()`. |
| **C7** | `feat(content-composer): bodySlot substrate (lazy Plate + eager Textarea) + baseline dirty` | `parts/body-substrate-plate.tsx` (default-export, `@/registry` alias `.tsx` import, `url→src` uploader adapt) + `parts/body-substrate-plaintext.tsx` + `hooks/use-body-dirty.ts` (JSON baseline-compare; reset-after-save). Lazy registration in `lib/substrates.ts`. |
| **C8** | `feat(content-composer): mediaSlot substrate + clampMediaSources + pull-only export` | `lib/clamp-media-sources.ts` + mediaEditorSubstrate (1:1 dial passthrough, `presentation="inline"` forced, `ref<MediaEditorHandle>`). SlotHandle composes `{ editorState: getState()-with-videoBlob-nulled, exportedUrl, pendingBlobRef }`; `loadValue→loadState()`; dirty→`getIsDirty()`. |
| **C9** | `feat(content-composer): blocking validation gates + canAdvance + focus-jump` | Config-sourced BLOCKING gates run imperatively at advance/publish (metadata `trigger()`+`isValid()`; body minLength vs `RICH_TEXT_EMPTY_VALUE`; media `!!exportedUrl\|\|getIsDirty()`). Block STAYS + jumps focus to first invalid step/field + `role="status"` announce. DISTINCT from the non-blocking fallback. |
| **C10** | `feat(content-composer): autosave + aggregate dirty/validity split` | `hooks/use-autosave.ts` (debounced ~800ms effect, skip-initial-mount, re-baseline-on-success) + `hooks/use-slot-handles.ts` (dirty = OR across 3 SlotHandles). `onDraftChange` (per-mutation, in dispatch) SPLIT from `onAutosave` (debounced). |
| **C11** | `feat(content-composer): FSM publish/schedule + uploader + lazy upload-on-publish` | `lib/upload.ts` (`uploader`/`uploadUrl` resolution + `Map<string,Blob>` consumed once at publish) + `lib/publish-cta.ts` (`resolvePublishCtaArms`) + `parts/publish-bar.tsx`. Publish/schedule re-runs ALL gates; pulls media via `handle.export()`; uploads blob→url→`exportedUrl`→`NewsCardItem.image`. Schedule = publish + future `publishAt`. |
| **C12** | `feat(content-composer): news config + co-located news adapter pair` | `configs/news-composer.config.ts` (the §news JSON shape + co-located `toContentItem`/`fromContentItem` against `NewsCardItem` — OMIT counts; nested `sensitivity`/`paywall` from dotted fields; `mediaInitialSource:{kind:"url",url:item.image,mode:"photo"}`) + `adapters/adapter-registry.ts`. Body via separate `initialBody` leg. |
| **C13** | `feat(content-composer): post config (modeled, deferred) + clamp proof` | `configs/post-composer.config.ts` (`["upload","library"]` → clamp proof; `"plaintext"` body; omitted `gates` step; `publishModes:["draft","publish"]`). Proves config-only divergence; ships fully behind media-editor v0.2. |
| **C14** | `feat(content-composer): demo.tsx (SwipeTabsList) + dummy-data + presentation resolver` | demo tabs (News / Re-edit round-trip / Dialog / Dark) using the docs-site `<SwipeTabsList>` wrapper (NOT raw `<TabsList>`). `dummy-data.ts` (sample draft + sample `NewsCardItem`). Presentation resolver (auto→inline-in-CMS / dialog-standalone). |
| **C15** | `feat(content-composer): usage.tsx + meta.ts v0.1.0 + manifest sync` | `usage.tsx` + `meta.ts` v0.1.0 status `alpha`, deps audited (`pnpm validate:meta-deps` clean — incl json-form/rich-text-editor/media-editor transitive). `/components/content-composer` renders all demo tabs. |
| **C16** | `chore(content-composer): registry.json — base + fixtures + registryDependencies` | Base item (23 `registry:component`, `target: components/content-composer/<sub-path>`, NO demo/usage/meta) + fixtures item (`dummy-data.ts`). `registryDependencies` = json-form + rich-text-editor + media-editor URLs. `pnpm registry:build` clean + manual roster audit (reconcile counts). |
| **C17** | `chore(content-composer): GATE 3 readiness spotcheck v0.1.0` | Author `docs/procomps/content-composer-procomp/reviews/<date>-v0.1.0-spotcheck.md`. 4 fixed dims + rotating = **composition integrity** (LOCKED per description §GATE-3). AI-assisted acceptable (procomp v0.1.0). Findings → patches. Verdict ≥ Pass-with-follow-ups. |
| **C18** | `chore(content-composer): consumer-tsc smoke + STATUS + decision file + guide + push` | `pnpm dlx shadcn@latest add @ilinxa/content-composer` into tmp consumer → consumer `pnpm tsc --noEmit` clean (path-b; expect/fix F-cross-13 sub-traps). Author `content-composer-procomp-guide.md`. Update `.claude/STATUS.md` + decision file. Push to master. |
...
| **`image` missing at publish** — QP-10 lazy upload means `exportedUrl` is empty until publish; `NewsCardItem.image` is non-optional. | `toContentItem` is a publish-time-only projection; the shell uploads (`export()`→`uploader()`→write `exportedUrl`) BEFORE calling it; the adapter throws with a clear message if `image` is absent. |
| **`url` vs `src` uploader mismatch** — `RichTextEditor.onImageUpload` wants `{ src }`; the composer uploader returns `{ url }`. Both are valid object shapes, so TS won't catch it. | The bodySlot substrate renames `url→src` before passing `onImageUpload` (§lazy-load). Flagged in the C7 commit + GATE 3 composition-integrity dim. |
...
| **alpha substrate dependency** — `rich-text-editor` is alpha v0.2.2. | Project-accepted (json-form already depends on it); flagged in the Dependencies table so GATE 3 isn't surprised. |
...
- Fine-grained media-edit autosave once media-editor v0.2 emits content-mutation `onEditAction` events.
...
| `post` config declares `mediaSources: ["upload","library"]` | `clampMediaSources` drops `"library"` → `["upload"]`; no crash, no cosmetic leak; SAME shell stays valid until media-editor v0.2. |
...
| Re-edit, body not in `NewsCardItem` | `fromContentItem` emits no body step; page re-seeds via `initialBody`. |
...
- Body editor: `<RichTextEditor>` exposes no `id` (Plate owns a contenteditable) — wrapped in `role="group"` + `aria-labelledby` (the `field-richtext.tsx` wrap). Each metadata/media step's label binds the same way.
...
3. `pnpm validate:meta-deps` — clean once `meta.ts` lands (C15); re-run after C16 (removing/adding any shipped file cascades to orphaned deps — the media-editor v0.1.1 lesson).
4. `pnpm build` — production build succeeds (C15+).
5. `pnpm registry:build` — regenerates `public/r/*.json`; spot-check `content-composer.json` (C16).
6. Docs render — `/components` lists it; `/components/content-composer` renders all demo tabs (C15).
7. Per-commit browser test — the visual walkthrough is the de-facto gate (the media-editor lesson: every chrome issue was invisible to tsc/lint/build). Smoke each new surface in the demo as it lands.

---

## GATE 3 readiness review

Author `docs/procomps/content-composer-procomp/reviews/<YYYY-MM-DD>-v0.1.0-spotcheck.md` using [`docs/reviews/templates/review-spotcheck.md`](../../reviews/templates/review-spotcheck.md). 4 fixed dims (Planning docs / Registry distribution / Meta+manifest sync / Verification) + **1 rotating dim LOCKED = composition integrity** (per description §GATE-3): this shell composes three substrates — verify prop-flow correctness across them, no leaked substrate internals, clean state lifting (no prop-drilling hacks), and correct dirty/validity aggregation across the heterogeneous `SlotHandle`s. Self-review acceptable for a v0.1.0 procomp; AI-assisted pass recommended given the composition surface. Verdict must be `Pass` or `Pass with follow-ups` to close; each follow-up tagged with owner + bump target.

Per-tier smoke (pro-component, runtime): F-cross-11 path-b consumer-tsc — `pnpm dlx shadcn add @ilinxa/content-composer` succeeds AND consumer-side `pnpm tsc --noEmit` clean post-install (C18).
