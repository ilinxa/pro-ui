# content-composer — procomp description

> Stage 1: what & why.
>
> **Greenfield — no source app, no migration intake.** There is no app to port from (confirmed). This is a new shell procomp authored against three already-shipped substrates: `json-form`, `rich-text-editor` (Plate), and `media-editor` (the recently-extracted Konva capture+edit surface, v0.1.2). It generalizes the "controllable surface area via orthogonal capability dials" pattern that `media-editor` proved at the editor level, lifting it to a **per-content-type JSON config**.
>
...
- rich body text → `rich-text-editor` (Plate WYSIWYG, JSON value)
- hero/cover media → `media-editor` (capture + on-canvas edit + export)
...
That is `content-composer`: a **single procomp shell** that mounts three substrate slots per step, driven entirely by a JSON config, with cross-cutting lifecycle owned centrally.
...
Each row is **one JSON composer config** that the shell loads. The divergence between rows is **data, not code** — different dial values, different tool arrays, different aspect, a different metadata-step set. The shell has zero per-content-type branches. `news` ships first (it needs **zero** `media-editor` changes); `post` is modeled now but **blocked on `media-editor` v0.2's `"library"` source** and ships as a follow-on JSON file.
...
| Rich-body slot (`bodySlot`) | `rich-text-editor` `<RichTextEditor>` (Plate); plaintext fallback = shadcn `<Textarea>` | `src/registry/components/data/rich-text-editor/` v0.2.2 (alpha, **already shipped**) |
| Media slot (`mediaSlot`) | `media-editor` `<MediaEditor>` — four dials + `presentation` + `initialSource` | `src/registry/components/media/media-editor/` v0.1.2 (**already shipped + extracted**) |
| Slot-mounting model | **Renderer registry** — runtime array of `{ id, render }` records keyed by slot-kind string; config references substrates by KEY only | `workspace` (`WorkspaceComponent[]` + `useAreaContext`) + `kanban-board` (`KanbanCardRenderer[]` + `useRendererMap`) |
| Controlled/uncontrolled draft state | `value? + defaultValue? + onChange?` triplet; internal reducer when uncontrolled | `kanban-board` `useKanbanState` (copy verbatim) |
| Adapter contract | Per-content-type **bidirectional pure functions** mapping collected draft ↔ backend item (and inverse, for CMS re-edit) | new (this procomp); output target = `NewsCardItem` |

The shell does NOT re-open any substrate's internals. The point is to *compose* the substrates, not reconsider them.

**bodySlot needs NO new procomp.** `rich-text-editor` is already shipped (v0.2.2) and is already proven to mount cross-procomp — `json-form` mounts `<RichTextEditor>` as its `richtext` field via the `.tsx` import path (see `json-form/parts/field-richtext.tsx`). The shell copies that recipe: import from `@/registry/components/data/rich-text-editor/rich-text-editor` (the `.tsx`, NOT the barrel — the F-S1 path-rewriter bug mangles cross-procomp `/index`/`/types`), lazy-load the ~165 KB Plate bundle. Embedding Plate directly, or building a new `body-editor-01` first, is **unnecessary**.
...
**content-composer (the shell) OWNS:**
...
- **The upload** — `uploader` / `uploadUrl` / signed URLs / S3 / Cloudinary / Mux. The shell takes `media-editor`'s exported blob and uploads it.
...
- `bodySlot` → `rich-text-editor` — rich-text editing + **in-body** image upload (`RichTextEditor.onImageUpload`, distinct from the hero upload). Plaintext fallback → shadcn `<Textarea>`.
- `mediaSlot` → `media-editor` — capture / on-canvas edit / **export** of the hero/cover media. Export is **pull-only** (no `onExport` prop) — the shell holds the `MediaEditorHandle` and calls `.export()` at publish time.
...
- Persisting the **full Plate body value** into its own backend body column (`NewsCardItem` carries only `excerpt`, not body — see §5).

The shell has **NO direct DB/API access.** It emits an assembled `ContentItem` + a publish-intent, exactly as `media-editor` emits a blob.

> **DELTA vs `media-editor` (read this — the two siblings' boundaries look contradictory otherwise):** `media-editor` has **NO** `uploadUrl` / `uploader` / `onPublished` (its NOTABLY-ABSENT block). The content-composer **shell DOES own upload** — it is precisely the layer that takes `media-editor`'s exported `{ blob, metadata }` and writes the resulting URL into `NewsCardItem.image`. media-editor exports a blob; the composer uploads it. Not a contradiction — a layering.
...
- **End-to-end news author → publish.** A blank `news-composer.json` mount collects all steps and emits a valid `NewsCardItem` through `onPublish` — required `id` / `title` / `image` present; nested `sensitivity` / `paywall` assembled from dotted fields; runtime counts (`likeCount`/`views`/…) **omitted, not zeroed**.
- **Re-edit round-trips.** `initialItem` + the inverse adapter rehydrate the authored metadata and re-open the hero via `media-editor` `initialSource`; the body re-seeds from the separately-supplied `initialBody` (§5); a re-publish PATCHes without wiping engagement counts.
...
- **Config-only divergence holds.** `post-composer.json` (modulo the `media-editor` v0.2 `"library"` block) runs through the **same shell** with **zero** per-content-type code branches.
...
`presentation: "inline" | "dialog" | "auto"` (default `"auto"`). Inline when embedded in a CMS composer step; dialog (shadcn `dialog` + focus trap) standalone. **Note:** `media-editor`'s own dialog mode is unused inside the composer — the hero is always mounted `inline` at a step; the *composer* is the dialog when `presentation: "dialog"`. The shell owns the cross-step / close-composer **discard guard** (`media-editor`'s `confirmOnDiscard` is dialog-only, so inline-mounted media has no discard guard — the shell reads `editorRef.getIsDirty()` before allowing navigation away).
...
| `bodySlot` | `<RichTextEditor>` / `<Textarea>` | **shell-derived** (ref/struct diff vs loaded baseline — Plate has no dirty signal) | config min-length over flattened text | key-bump remount with `defaultValue` |
| `mediaSlot` | `<MediaEditor>` | `handle.getIsDirty()` + `onDirtyChange` | required? `!!value.exportedUrl \|\| handle.getIsDirty()` | `handle.loadState()` + `initialSource:{kind:"url"}` |
...
- `mediaSlot` → a **1:1 passthrough** of `media-editor`'s dials: `{ enabledModes, enabledTools, mediaSources, aspect, presentation?, cropAspects?, maxFileSizeMb?, fieldName }` — the shell spreads these straight onto `<MediaEditor>` props (every key verified on `MediaEditorProps`).
...
The **news adapter's output target is the real `NewsCardItem`** (`news-card/types.ts:144`, exported via `index.ts`). It authors:
...
**Inverse adapter (CMS re-edit):** `fromContentItem(item)` re-seeds authored fields into the metadata step AND sets the media leg `mediaInitialSource: { kind:"url", url:item.image, mode:"photo" }` so `media-editor` re-opens the hero without re-capture.

**Body is NOT on `NewsCardItem`** (the card carries only `excerpt`). `toContentItem` is the **display-card projection**; the full Plate `RichTextValue` egresses separately — the shell exposes the raw `BodySlotValue` and the wrapping pro-page persists it to its own body column. For an HTML export boundary (RSS/email/OG) the adapter calls `serializeRichTextToHtml(value)`; Plate JSON, never HTML, is the storage format.

**The body leg is two-directional (re-edit can't source body from `initialItem`).** Because body isn't on `NewsCardItem`, `fromContentItem` can't rehydrate it. The wrapping pro-page reads the persisted `RichTextValue` from its own body column and feeds it into the shell **separately** — via an `initialBody?: BodySlotValue` prop (or a `bodySlot` entry on an augmented `initialItem`) — exactly mirroring the outbound `BodySlotValue` it persisted. **Out:** shell → page (body column). **In:** page → shell (re-seed). The `NewsCardItem`-shaped fields round-trip through the adapter; the body round-trips through this separate seam.
...
interface ComposerDraft {
  ...
  status: "draft" | "scheduled" | "published" | "archived"; // → NewsCardItem.status
...
- `bodySlot`: config-declared `minLength` / non-empty check over the Plate value (vs `RICH_TEXT_EMPTY_VALUE`).
...
export interface ContentComposerProps {
  ...
  /** Persisted Plate/plaintext body for re-edit — body is NOT on NewsCardItem, so it re-seeds via this separate leg (§5). */
  ...
  // === The SHELL owns upload (DELTA vs media-editor) ===
...
- **Body editor a11y wrap:** `<RichTextEditor>` exposes no `id` (Plate manages a contenteditable) — the shell wraps it in `role="group"` + `aria-labelledby` (the `field-richtext.tsx` a11y wrap), as must each metadata/media step's label binding.
...
Manual matrix per the documented-headless-patterns convention (no automated browser tests). Most of the heavy media surface lives inside the `media-editor` slot (its own matrix applies); the shell adds:
...
| `media-editor` slot (upload → export → upload) | ✅ | ⚠️ codec/WebP quirks (media-editor matrix) | ✅ |
| Plate body editor mount + a11y wrap | ✅ | ✅ | ✅ |
| `prefers-reduced-motion` step reveal | ✅ | ✅ | ✅ |

> **Desktop Safari** shares the Safari-iOS WebKit caveats for the shell's *own* chrome — the `dialog` focus-trap, the `datetime` metadata field, and the Plate contenteditable behave per WebKit, distinct from Chromium. Validate the shell chrome on desktop Safari too (the canvas-heavy media surface is covered by `media-editor`'s own matrix).

Smoke is documented patterns + consumer-tsc clean post-install (path-b) per shipped-procomp convention. As the **second** inter-procomp registry-dep install path in the library (after `media-editor ← story-composer`), expect F-cross-13 sub-traps against the newly-pulled-in primitives — defensively pre-wire.

### 14. SSR

`"use client"` at the procomp boundary. Konva-touching parts arrive via the lazy-loaded `media-editor` slot (already `dynamic(..., { ssr: false })` internally); the Plate bundle is `React.lazy`-loaded so configs without a `bodySlot` don't pay the ~165 KB cost.
...
// mediaSlot config = 1:1 with media-editor dials (names verbatim from MediaEditorProps)
export interface MediaSlotConfig {
  fieldName: string;
  enabledModes: ("photo" | "video" | "text")[];
  enabledTools: ("text" | "draw" | "stickers" | "filters" | "adjust" | "crop")[];
  mediaSources: ("camera" | "upload")[];   // "library" NOT yet valid (media-editor v0.2) — clamp unknowns
...
### `news-composer.json` (ships FIRST — zero `media-editor` changes)
...
```json
{
  ...
  "steps": [
    ...
    { "id": "body", "title": "Rich text", "slot": "bodySlot",
      "slotConfig": { "substrate": "plate", "fieldName": "body", "placeholder": "Write the article…",
        "emptyValue": [ { "type": "p", "children": [ { "text": "" } ] } ] },
      "validation": { "mode": "custom", "rules": [ { "field": "body", "minLength": 1, "message": "Rich text cannot be empty" } ] } },
...
5. **The `"library"` source declared ahead of capability** — `media-editor`'s real `MediaSource` is `"camera" | "upload"` only. The post config DECLARES `["upload","library"]`; the media substrate must **clamp** the unknown `"library"` (degrade to upload-only) **without crashing** — exactly how media-editor already clamps its dials. This is the concrete cross-procomp version dependency: **post can't fully ship until `media-editor` v0.2.**

## Out of scope for v0.1

- **POST / EVENT / PROJECT configs** — modeled now, shipped as follow-on JSON files (NO new component). POST is hard-**blocked on `media-editor` v0.2** (`"library"` `MediaSource`).
- **`media-editor` `"library"` media source** — upstream gap (its v0.2). News needs upload-only, so it ships unblocked.
...
- **Fine-grained media-edit autosave granularity** — `media-editor`'s `onEditAction` emits only nav/lifecycle events (mode-change/tool-open/tool-close/reset) in v0.1.x; content-mutation events arrive in its v0.2. Media autosave is checkpoint-on-dirty-toggle, not per-edit.
...
| **QP-2** v0.1 scope | news-only vs news+post modeled | **(b) MODEL news + post, SHIP news first.** POST is blocked on `media-editor` v0.2 (`"library"`); modeling both now forces schema honesty. |
| **QP-3** `presentation` default | inline / dialog / auto | **(c) `"auto"`** — inline when embedded in a CMS step, dialog standalone. Mirrors `media-editor`. |
...
| **QP-5** `bodySlot` source | embed Plate directly / NEW body-editor procomp / MOUNT `rich-text-editor` | **(c) MOUNT `rich-text-editor` v0.2.2** — already shipped, already proven cross-procomp inside json-form. **NO new procomp is a prerequisite.** Plaintext fallback mounts shadcn `<Textarea>`. |
...
| **QP-7** Adapter location | co-located in config module / separate registry item | **(a) co-located** in the per-content-type config module, exported as pure forward+inverse functions (collected draft ↔ `NewsCardItem`), NOT components. |
...
| QP-2 | (b) Model news + post; ship news first; post follows on `media-editor` v0.2. |
| QP-3 | (c) `presentation` default `"auto"`. |
| QP-4 | (b) Autosave ON, 800 ms, `onDraftChange`/`onAutosave` split. |
| QP-5 | (c) Mount `rich-text-editor` — no new procomp; `<Textarea>` plaintext fallback. |
...
| `rich-text-editor` | internal procomp | `bodySlot` substrate (Plate). **alpha v0.2.2** — acceptable per project norms (json-form already depends on it); flagged so GATE 3 isn't surprised. Cross-procomp import via the `.tsx` path (F-S1). |
| `media-editor` | internal procomp | `mediaSlot` substrate. **v0.1.2 today; POST blocked on its v0.2** (`"library"` source). `mediaSlot` config = 1:1 dial passthrough; export is pull-only via the ref handle. |
| `news-card` | internal procomp | The news adapter's OUTPUT target type (`NewsCardItem`). Type-level dependency only. |
| `workspace`, `kanban-board` | pattern precedent | Renderer-registry + controlled/uncontrolled-state model. No code dependency. |
