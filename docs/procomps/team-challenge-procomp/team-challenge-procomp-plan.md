# `team-challenge` — Pro-component Plan (Stage 2 / GATE 2)

> **Stage 2 of 3 · Status: DRAFT — pending GATE 2 sign-off**
> **Slug:** `team-challenge` · **Category:** `gamification` · **Tier:** pro-component · **Structure:** light shadcn-style compound
> **Predecessor:** [`team-challenge-procomp-description.md`](./team-challenge-procomp-description.md) (GATE 1, signed off)
...
This is the **how** — the implementation contract. Once signed off (**GATE 2**), `pnpm new:component gamification/team-challenge` scaffolds the folder and code begins. Nothing here re-opens GATE-1 decisions (D-C1…D-C6 resolved below to LOCKED defaults); it operationalises them. Where this plan and the [system description](../../systems/gamification-system/gamification-system-description.md) disagree, the system doc wins — flag it back there.

> **Reviewer focus:** the **never-forced** literal-UI contract (§6 — the load-bearing rule of the whole component), the compound export surface (§4), the controlled-echo opt-in chokepoint + double-emit telemetry guard (§5), and the E6 done-state seam (§6.4). These are where a team-challenge surface drifts.
...
| `skeleton` | `TeamChallengeSkeleton` | empty/loading silhouette (card + meter shimmer) | ✅ — loading is a **Tier-C component**, not a `loading` prop (gantt precedent, §9). |
...
**None (D-03).** This component imports **no** other registry component. It declares its own `Challenge` / `Team` / `GamificationEvent` slice in `types.ts` (§4). `registryDependencies` in both `meta.ts` and `registry.json` is **empty**. (Contrast gantt-timeline, which depends on `task-card`; this component deliberately does not — the type duplication across the pack is the accepted price of registry independence, system D-03/D-04. The deferred `gamification-kit` may dedupe later; do **not** import early.)
...
Per [`.claude/rules/compound-component-structure.md`](../../../.claude/rules/compound-component-structure.md) + system **D-05**. **Flat exports, never a `TeamChallenge.Root` namespace.** Each part module co-locates a **dumb Tier-C core** + a **thin Tier-B context wrapper** (the media-library one-file-two-exports pattern). It is a **light** compound — the "a reasonable consumer wants a subset" trigger fires (card-body-only, or opt-in-control-only), but no heavy-dep / composed-procomp trigger fires → **no `React.lazy`**.

### 4.1 — Tier inventory (LOCK — the GATE-2 enumeration the rule requires)

| Export | Tier | Module | Reads context? | Role |
|---|---|---|---|---|
| `TeamChallengeRoot` | **B (provider)** | `parts/team-challenge-root.tsx` | provides | Owns the **controlled `optedIn` echo** (reads `challenge.optedIn`, never local data state — D-06), derives `progressFraction` + the state machine (`isJoinable`/`isActive`/`isComplete`/`hasReward`), emits **both** telemetry events (`challenge.opened` mount-effect double-emit-guarded + `challenge.opt-in` on toggle), exposes `TeamChallengeContext`. Renders `children`. **No layout opinion.** |
| `TeamChallengeHeader` | **B** | `parts/team-challenge-header.tsx` | yes | challenge `label` (truncate + title attr) + optional `team.name` + `TeamMemberStack`; gated by `showMemberStack`. |
| `TeamChallengeProgress` | **B** | `parts/team-challenge-progress.tsx` | yes | reads derived fraction + count → renders `ChallengeProgressMeter`. Collective only. |
| `TeamChallengeReward` | **B** | `parts/team-challenge-reward.tsx` | yes | reads `reward` + `isComplete` → renders `ChallengeRewardChip` (earned vs available framing); auto-hides if no reward; gated by `showReward`. |
| `TeamChallengeOptIn` | **B** | `parts/team-challenge-optin.tsx` | yes | the opt-in/opt-out control (catalogue C5); reads `optedIn` + handlers → renders `OptInToggle` + `noPenaltyHint`. Hides if `onOptInChange` is absent (capability-gating → read-only). Gated by `showOptIn`. |
| `ChallengeProgressMeter` | **C** | `parts/team-challenge-progress.tsx` | no | dumb meter: `progress` bar fill from `current`/`target` + mono count label. Prop-driven. |
| `ChallengeRewardChip` | **C** | `parts/team-challenge-reward.tsx` | no | dumb reward chip; `earned?: boolean` switches framing ("The team earns:" → "The team earned:"). |
| `OptInToggle` | **C** | `parts/team-challenge-optin.tsx` | no | dumb, **context-free** opt-in button-toggle (`OptInToggleProps`, description §5); usable on any surface. |
...
| `TeamChallengeSkeleton` | **C** | `parts/team-challenge-skeleton.tsx` | no | loading state — card silhouette + meter shimmer; zero state, composes anywhere (used instead of a `loading` prop, §9). |
| `TeamChallenge` | **A (assembly)** | `team-challenge.tsx` | no (composes B parts) | `Root` + `Header` + `Progress` + `Reward?` + `OptIn?`, gated by `show*`. **Contains no logic the parts don't.** Demo + screenshot use this. |
| `useTeamChallenge` | hook | `hooks/use-team-challenge.ts` | — | context consumer for hand-assembled layouts (throws outside Root). |

### 4.2 — Tree-shaking story (must be real)

- Each part is its own module re-exported from `index.ts`. A consumer who wants **only** the penalty-free toggle imports `TeamChallengeOptIn` (with a Root) or the bare context-free `OptInToggle` — and never pulls the card chrome (`Header`/`Progress`/`Reward` modules don't enter the graph).
- A consumer who wants **only the avatar pile** imports `TeamMemberStack` alone.
- The **read-only progress card** falls out for free: omit `onOptInChange` → `TeamChallengeOptIn` renders nothing (capability-gating), no control code paths execute.
...
`TeamChallengeRoot` is the single source of derived state + the telemetry chokepoint + the controlled-echo. `TeamChallenge` is a fixed child tree with `show*` toggles and **zero** state of its own — so a hand-assembled layout (description §6.3) gets identical behavior (same events fire, same never-forced framing). A reviewer rejects any logic that lives in the assembly but not the parts.
...
- `challenge.optedIn` **is** the value. The `OptInToggle` reflects it directly; the Root holds **no** local `optedIn` state (the card-tree / calendar controlled-echo lesson — a toggle that holds local state and a controlled prop will desync).
...
- **The E6 seam (document it explicitly):** the heavy celebration overlay belongs to **E6 `team-feedback-loop`** (catalogue C9, system §7.4). This component's `done` is the *durable inline* surface; a host that wants the *transient flourish* mounts `team-feedback-loop` alongside and routes the milestone/challenge-done event there. **Neither component triggers the other** (system D-16 spirit — though D-16 names trophy-shelf+feedback-loop, the same "host wires exactly one celebration path per event" discipline applies). This component **never** imports or invokes E6. The seam is: *host owns the wiring; we provide the inline ack only.* Keeping this lightweight is what prevents done-state scope-creep into E6 (risk §11).

---

## 7. File-by-file plan

Sealed folder under `src/registry/components/gamification/team-challenge/` (compound layout; `parts/` co-locates Tier-B wrappers + Tier-C cores per the house pattern):

| File | Contents |
|---|---|
| `team-challenge.tsx` | **Tier A** `TeamChallenge` assembly — `"use client"`; `Root` + `Header` + `Progress` + `Reward?` + `OptIn?`, `show*`-gated; the only public default-layout entry. **No logic the parts lack.** |
| `index.ts` | Barrel — flat exports (every Tier A/B/C name + `useTeamChallenge` + all public types: `Challenge`, `Team`, `TeamMember`, `GamificationEvent`, `TeamChallengeProps`, `TeamChallengeRootProps`, `OptInToggleProps`, the Tier-C prop types). **No re-export from another registry component** (D-03). |
| `types.ts` | **Framework-free.** Local slice: `Challenge`, `Team`, `TeamMember` (= `{ id; displayName; avatarUrl? }`), `GamificationEvent` (the local slice of the system §6 union — only the two events this component emits, see §8 note), `TeamChallengeProps`, `TeamChallengeRootProps`, `OptInToggleProps` + Tier-C prop types, context value type. Imports **nothing** from another registry component. |
| `parts/team-challenge-root.tsx` | **Tier B** provider — controlled-echo, `lib/derive` memo, telemetry chokepoint (mount-effect + double-emit guard via `firedRef`), `TeamChallengeContext.Provider`. |
| `parts/team-challenge-header.tsx` | **Tier B** `TeamChallengeHeader` (composes Tier-C `TeamMemberStack`). |
| `parts/team-challenge-progress.tsx` | **Tier B** `TeamChallengeProgress` + **Tier C** `ChallengeProgressMeter`. |
| `parts/team-challenge-reward.tsx` | **Tier B** `TeamChallengeReward` + **Tier C** `ChallengeRewardChip`. |
| `parts/team-challenge-optin.tsx` | **Tier B** `TeamChallengeOptIn` + **Tier C** `OptInToggle` (the context-free bare toggle). |
| `parts/team-member-stack.tsx` | **Tier C** `TeamMemberStack` — avatar pile + initials fallback + `+N` overflow. |
| `parts/team-challenge-skeleton.tsx` | **Tier C** `TeamChallengeSkeleton` — card + meter shimmer for the loading state (§9). |
| `hooks/use-team-challenge.ts` | `TeamChallengeContext` + `useTeamChallenge()` (throws if used outside Root). |
...
export type TeamChallengeRootProps =
  Omit<TeamChallengeProps, "showOptIn" | "showReward" | "showMemberStack" | "className" | "aria-label">
...
**Locks:**
- `TeamChallengeRootProps` = `TeamChallengeProps` **minus** the three `show*` (assembly-only) + `className`/`aria-label` (the assembly's chrome props) **plus** `children`. The assembly's `show*` map to mounting/not-mounting parts.
...
| Case | Handling |
|---|---|
| `reward` undefined / empty string | `hasReward` false → `TeamChallengeReward` renders **nothing** (auto-hide, §5.3); `showReward` is moot. |
...
| Loading / empty | **Tier-C `TeamChallengeSkeleton`** (no `loading` prop — gantt precedent). Consumer mounts the skeleton while fetching. |
| `onOptInChange` omitted | Read-only card; `TeamChallengeOptIn` renders nothing (capability-gating). |
...
- **Loading prop vs `<TeamChallengeSkeleton>`** — **component chosen** (gantt precedent; keeps surface lean, composes).
...
**Base item — `team-challenge`:**
- Every file `type: "registry:component"`, `target: "components/team-challenge/<sub-path>"`.
- Files: `team-challenge.tsx`, `index.ts`, `types.ts`, `parts/*` (root, header, progress, reward, optin, team-member-stack, skeleton), `hooks/use-team-challenge.ts`, `lib/derive.ts`, `lib/events.ts`.
...
**Fixtures item — `team-challenge-fixtures`:**
- `registryDependencies: ["team-challenge"]` (depends on the base).
- One file: `dummy-data.ts`, `target: "components/team-challenge/dummy-data.ts"`.

`pnpm registry:build` regenerates `public/r/team-challenge.json` + `team-challenge-fixtures.json`; spot-check both (targets follow `components/team-challenge/<sub>`; no demo/usage/meta; `registryDependencies: []`).

---

## 13. Verification plan (pre-GATE-3)

1. `pnpm tsc --noEmit` clean · `pnpm lint` clean · `pnpm validate:meta-deps` clean (`shadcn: [progress, avatar, button, badge, skeleton]` = actually imported; `internal: []`). **meta-deps gotchas:** in any file with side-effect imports, place a dep-declaring `from`-import FIRST (the audit regex stops at the first `from`); never reference a dep name only inside a comment (false-positive). (blackboard + content-composer lessons.)
2. `pnpm build` succeeds.
3. Docs render at `/components/team-challenge`; all demo tabs interactive.
4. `pnpm registry:build`; spot-check `public/r/team-challenge.json` + `team-challenge-fixtures.json` (locked target convention; no demo/usage/meta; `registryDependencies: []`).
...
9. **First-ship consumer smoke (F-cross-11 path-b):** `pnpm dlx shadcn add @ilinxa/team-challenge` succeeds + consumer `pnpm tsc --noEmit` clean. Because **no new shadcn primitive** is introduced and **no internal dep** exists, the F-cross-13 "new primitive divergence" + the content-composer "type-only-but-still-needs-registryDependency" traps don't fire — but run the smoke anyway (4-ship pattern discipline).
...
- **Q-P2 (category plumbing):** Confirm this component (as an early gamification ship) **owns the D-01 `gamification` category plumbing** (§0) if it lands before a sibling — or note that `team-progress-bar` (the system's suggested first build, system §10) lands it first. (Recommend: whichever gamification component scaffolds first lands it; reference, don't duplicate.)
...
- **Q-P4 (skeleton vs loading prop):** Confirm `<TeamChallengeSkeleton>` Tier-C export over a `loading` prop (§9, gantt precedent). (Recommend: accept.)
...
- [ ] **User sign-off** → scaffold `gamification/team-challenge` (after §0 plumbing) + implementation.
