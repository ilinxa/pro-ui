# pricing-table — procomp plan

> Stage 2: how. The implementation contract.
>
> See [description](./pricing-table-procomp-description.md) for what & why.
>
> **Greenfield** — no migration origin. Mirrors `newsletter-signup`'s async-aware status / controlled-or-uncontrolled state shape and `share-bar`'s analytics hook.
>
...
```
src/registry/components/marketing/pricing-table/
├── pricing-table.tsx        # 1  root, dispatches to layouts
├── parts/
...
### 1. `pricing-table.tsx` — root

- `"use client"` (uses hooks).
- Wrapped in `React.memo`.
- Resolves controlled-vs-uncontrolled `billing` (matches `newsletter-signup`'s pattern):
...
```ts
{
  ...
  tags: ["pricing-table", "marketing", "pricing", "conversion", "cms-block"],
  version: "0.1.0",
  status: "alpha",
...
`HeadingTag = headingAs ?? "h2"` resolved at the root. (Bumped from h3 vs newsletter-signup because pricing is usually a top-of-section block.)

**Cards vs table dispatch at the root** — like newsletter-signup's variant dispatch. No compound API.
...
- [ ] `pnpm lint` clean.
- [ ] `pnpm build` clean — all routes prerendered including `/components/pricing-table`.
...
- [ ] `registry.json`: base + fixtures entries added; `pnpm registry:build` regenerates `public/r/pricing-table.json`.
