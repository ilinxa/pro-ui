...
- **Three tones** — `primary` / `accent` / `muted`, mirroring `newsletter-signup`'s palette for visual consistency across marketing-category components.
- **Controlled-or-uncontrolled toggle state** — `billing` + `onBillingChange` controlled, OR `defaultBilling` uncontrolled. Mirrors React input convention and `newsletter-signup`'s state shape.
...
- **Analytics hook** — `onTierCtaClick(tierName)` mirrors `share-bar`'s `onShare(targetKind)` pattern. Auto-fires for `CtaSpec`; consumer wires their own when `cta` is a `ReactNode`.
...
- SSR `/components/pricing-table` returns 200 with all demo tabs.
