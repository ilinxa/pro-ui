...
>
> Builds on the signed-off [description](./media-library-procomp-description.md). Category `media`, slug `media-library`. Composes **five** shipped procomps unchanged (`file-tree`, `pdf-viewer`, `code-block`, `markdown-editor`, `video-player`) and imports the `FsNode` model + `mergeLoadedChildren`/`iconForExtension` helpers from `file-manager`/`file-tree`. **No shipped component is modified.**
...
5. **Composability is a hard requirement (shadcn compound model).** The component must ship as a **headless `Root` provider + à-la-carte parts**, with one batteries-included `<MediaLibrary>` assembly on top. A consumer who doesn't need the sidebar / quota bar / type chips / preview / upload **drops those parts** and assembles a lighter version — and the bundler tree-shakes the unused parts (and their lazy viewer imports) away because each part is a separate module export. This drives the export surface (§1.2) and the composition pattern (§4) — it is not optional polish.
...
// boundary (carousel-composer's "define your own type locally" precedent).
...
- `MediaLibrary` — single-prop-driven full assembly (all `MediaLibraryProps`). Internally = `<MediaLibraryRoot>` + the canonical children, gated by the `show*`/`preview`/`enable*` toggles. This is what the demo and the screenshot use.

**Tier B — headless root + context-connected parts (assemble your own lighter version):**
- `MediaLibraryRoot` — the provider. Takes the data + handler props (`MediaLibraryRootProps` — the same surface as `MediaLibraryProps` **minus** the chrome toggles, since composition replaces them) and renders `children`. Holds all state; provides `MediaLibraryContext`.
...
**Types:** all `MediaLibrary*` / `MediaLibraryRootProps` / `MediaNode` / `MediaPreviewKind` / `MediaFilterCategory` / part-prop types.
...
```
src/registry/components/media/media-library/
├── media-library.tsx          Tier A: canonical assembly = <MediaLibraryRoot> + gated children + handle
├── types.ts                       §1.1
├── parts/
...
├── hooks/
│   ├── use-controllable-state.ts  sealed (account-switcher / code-block precedent)
...
- Renders `children` (no fixed layout). `forwardRef` → `MediaLibraryHandle`.

### Canonical assembly (`media-library.tsx` — Tier A)
...
const LazyVideo = React.lazy(() =>
  import("../../video-player/video-player").then(m => ({ default: m.default })));
...
| internal registry | `@ilinxa/video-player` | video preview (lazy). |
...
- **Same-category** (media→media: `pdf-viewer`, `video-player`) → **relative** path (`../../pdf-viewer/pdf-viewer`). The `media/` segment is implied by `../../`, so it survives the consumer's flat layout; relative also dodges the F-S1 same-category slug bug. Lazy is fine here (`lazy(() => import("../../pdf-viewer/pdf-viewer"))`).
...
**`MediaLibraryRoot` is a headless provider; the parts are context-connected presentational modules; `MediaLibrary` is just the canonical assembly.** Three ways to consume, escalating in control:

```tsx
// 1 — Full (the screenshot, zero config). One prop bag, all chrome on.
<MediaLibrary nodes={tree} storage={q} onUpload={up} onLoadChildren={load} onMove={mv} />

// 1b — Full, but toggle chrome off (cheapest customization).
<MediaLibrary nodes={tree} showSidebar={false} showQuota={false} preview="lightbox" />
...
- **`MediaLibrary` contains no logic the parts don't** — it's `<MediaLibraryRoot {...props}>` + a fixed child tree gated by `show*`/`preview`/`enable*`. Anything the full component does, a hand-assembly can do.
- **One `DndContext` + the context provider live in `Root`** (not `MediaLibrary`), so dnd works in hand-assembled layouts too. Marquee/upload-overlay are parts that opt into it.
- **Controlled/uncontrolled** `selectedIds` + `currentFolderId` (standard `value`/`default`/`onChange`; sealed `use-controllable-state`). **Imperative handle** via `forwardRef` on both `MediaLibrary` and `MediaLibraryRoot`.
...
- `pnpm build` green; docs render at `/components` + `/components/media-library`.
- `pnpm registry:build`; spot-check `public/r/media-library.json` (base + `-fixtures`); confirm `demo.tsx`/`usage.tsx`/`meta.ts` are NOT shipped.
- **Local-registry consumer-tsc re-smoke** (R1) — serve `public/r`, repoint a tmp consumer's `components.json`, `pnpm dlx shadcn add @ilinxa/media-library`, then consumer-side `pnpm tsc --noEmit` clean. The F-01 gate; expect + patch F-cross-13 sub-traps (ship→smoke→patch→re-smoke).
...
Real upload transport/chunking/resumable; sharing/permissions UI; rich office-doc preview; tagging/favourites/trash/versions; cross-folder global search; inline image editing (that's `media-editor`); `media-carousel` multi-image lightbox composition; `@ilinxa/file-clipboard` cross-component clipboard sync.
```
