# media-library — procomp guide

> Stage 3: consumer-facing usage. Ships with v0.1.0 (2026-06-09). For the *what & why* see the [description](./media-library-procomp-description.md); for the *how it's built* see the [plan](./media-library-procomp-plan.md).
...
- You need image *editing* → that's `media-editor` / `carousel-composer`.
...
- **Lazy heavy deps**: the four viewers (`pdf-viewer`/`code-block`/`markdown-editor`/`video-player`) are `React.lazy`-loaded. If you never mount `<MediaLibraryLightbox/>` or `<MediaLibraryDetailsPane/>`, none of pdf.js / CodeMirror / shiki / marked enters your bundle.
...
## Composition origin
Built 2026-06-09. Composes (unchanged): `file-tree`, `pdf-viewer`, `code-block`, `markdown-editor`, `video-player`. Reuses the shared `FsNode` model. The authoring/management counterpart to those single-purpose viewers — it routes *to* them rather than duplicating them.
