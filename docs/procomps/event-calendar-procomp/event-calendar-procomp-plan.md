# `event-calendar` — Pro-component Plan (Stage 2 / GATE 2)
...
> **Description (GATE 1, signed off):** [`event-calendar-procomp-description.md`](./event-calendar-procomp-description.md)
> **Reference impl mirrored throughout:** [`gantt-timeline`](../../../src/registry/components/data/gantt-timeline/) — the family's most recent date-driven compound. event-calendar reuses its data model, controlled-echo vocabulary, cross-procomp import pattern, and folder shape.
...
All public types live in `types.ts` and are re-exported from the barrel. Prop **names + shapes mirror `gantt-timeline`** wherever the concern is shared (data, selection, tooltip, the entire v2 editing block) so a consumer who knows the gantt knows the calendar.
...
/** How an event is laid out. Derived (never stored on TaskItem) — see §4.3. */
...
export type CalendarOccurrence = {
  item: TaskItem;          // the source item (calendar never mutates it)
  id: string;              // = item.id
  kind: EventKind;
...
export type CalendarTooltipRenderer = (item: TaskItem, occ: CalendarOccurrence) => ReactNode;
```
...
export type CalendarProps = {
  ...
  data: TaskItem[];                          // controlled; NO internal data state
  statusOptions?: TaskStatusOption[];
  priorityOptions?: TaskPriorityOption[];
  labelOptions?: TaskLabelOption[];
  colorRamp?: TaskColorRamp;                 // urgency ramp; RAMPS imported from task-card
  ...
  classifyEvent?: (item: TaskItem) => EventKind | undefined;  // §4.3 layer 1
  ...
  onPermissionDenied?: (action: keyof TaskPermissionRule, itemId: string, reason: TaskPermissionReason) => void;
};
```
...
export { CalendarFullCardTooltip } from "./parts/event-tooltip-full";  // lazy task-card
...
export type {
  ...
  TaskItem, TaskPerson, TaskStatusOption, TaskPriorityOption, TaskLabelOption,
  TaskColorRamp, TaskPermissions, TaskPermissionRule, TaskPermissionReason,
  TaskItemAddedEvent, TaskItemRemovedEvent, TaskItemMovedEvent,
  TaskFieldEditedEvent, TaskStatusChangedEvent,
} from "./types";
```

> **Re-export note:** event-calendar additionally re-exports `TaskPermissionReason` (its `onPermissionDenied` signature references it). gantt's barrel happens to omit that one type — this is a deliberate, slightly-more-complete choice, not a mirror error. `TaskPermissionReason` is exported by `task-card`'s barrel, so the re-export is valid. The set + ordering otherwise follow gantt.

**Tier map**

| Tier | Exports | Notes |
|---|---|---|
| **A** — assembly | `EventCalendar` | `EventCalendarRoot` + `CalendarToolbar` (if `showToolbar`) + optional `CalendarMiniNav` (if `showMiniNav`) + the active view. **No logic the parts lack.** |
| **B** — headless Root | `EventCalendarRoot` | Owns ALL state (cursor, selection, occurrences memo, now-tick) + context + handle. Renders `children`. |
...
- `CalendarFullCardTooltip` (→ `task-card`) is loaded via `React.lazy` from inside the Root *only when* a consumer passes a `renderTooltip` that needs it; the default `EventTooltip` is lightweight and pulls nothing. A consumer on the default tooltip never bundles `task-card`.
...
```
src/registry/components/data/event-calendar/
├── event-calendar.tsx            Tier A. <EventCalendar> = Root + Toolbar + (MiniNav) + active view via show* toggles. forwardRef → CalendarHandle.
├── types.ts                   All public + internal types (§1). Imports TaskItem & friends from "../task-card"; re-exports them (D4).
├── index.ts                   Barrel (§2).
...
├── dummy-data.ts              TaskItem[] fixtures: timed, multi-day spanning, milestone, overdue, inactive, a dense day, an all-day (date-only) item, deep-nested children.
├── parts/
...
│   ├── event-tooltip-full.tsx     Tier C CalendarFullCardTooltip — lazy-embeds <TaskCard>. Loaded via React.lazy.
...
└── lib/
    ...
    └── color.ts                   Reuse. import { RAMPS } from "../../task-card"; resolve fill/fg/border from status tone + urgency ramp + borderColor override. Mirrors gantt lib/color.ts.
```

**Scaffolding caveat:** `pnpm new:component` emits only the **7 top-level files** (`event-calendar.tsx` / `types.ts` / `index.ts` / `meta.ts` / `demo.tsx` / `usage.tsx` / `dummy-data.ts`). **`parts/`, `hooks/`, and `lib/` are created by hand** (§11 step 1) — the template has no subfolders. (`_template/_template` must stay valid TS — don't break it.)
...
### 4.6 `color.ts`
- `import { RAMPS } from "../../task-card"` — the runtime `Record<preset, (t: number) => string>` ramp functions (defined in task-card's `lib/ramp.ts`, re-exported by its barrel) + the `TaskColorRamp` type (via the local `../types` barrel). Resolve `{fill, foreground, border}` from status tone (`statusOptions`) → done=muted / blocked=red / active=urgency-ramp(elapsed) — **identical output to gantt + card** (Success-criterion 4). `item.borderColor` overrides (skips the engine).
...
| R4 | **Tree-shaking leak** (month-only path pulling time-grid / task-card) | Independent view modules + lazy tooltip; GATE-3 verifies the "Lighter" subset; smoke confirms bundle. |
...
**Alternatives considered + rejected:** (a) *adopt FullCalendar / react-big-calendar* — rejected: heavy, foreign data model, design-system mismatch, defeats the "native `TaskItem` surface" purpose. (b) *render the month grid on react-day-picker* — rejected: it's a date *picker*, not an event grid (D7); used only for the mini-nav. (c) *one mega view component with a `view` switch internally* — rejected: defeats tree-shaking + the compound mandate; views are independent modules. (d) *add `allDay` to `TaskItem`* — rejected: forks the shared type for a calendar-only concern (D5 uses a predicate + date-only strings instead).

---

## 11. Build order (implementation milestones)

1. `pnpm new:component data/event-calendar` → generates the **7 flat files**. **Create `parts/`, `hooks/`, `lib/` by hand** (scaffolder emits no subfolders). Wire `types.ts` (import + re-export task-card) + `meta.ts`.
...
5. **Toolbar + MiniNav**, keyboard model, then **Assembly** `EventCalendar` + `show*` gating.
6. `dummy-data` + `demo` (incl. the "Lighter" subset tab) + `usage`; manifest 3-line edit; verify `/components/event-calendar`.
7. `registry.json` (base + `event-calendar-fixtures`) → `pnpm registry:build` → spot-check artifact.
...
> **On sign-off (GATE 2):** implementation begins at step 1 above. The first code action is `pnpm new:component data/event-calendar` — not before.
