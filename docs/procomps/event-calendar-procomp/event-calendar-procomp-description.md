# `event-calendar` — Pro-component Description (Stage 1 / GATE 1)

> **Stage:** 1 of 3 · **Status:** 🟡 Draft — awaiting sign-off (→ GATE 2)
> **Slug:** `event-calendar` · **Category:** `data` · **Tier:** pro-component (ships as a **shadcn-style compound** — see §0)
...
> **The fifth and final member of the project-management set** — sibling of [`task-card`](../../../src/registry/components/data/task-card/) (list), [`task-tree`](../../../src/registry/components/data/task-tree/) (outline), [`kanban-board`](../../../src/registry/components/data/kanban-board/) (board), and [`gantt-timeline`](../../../src/registry/components/data/gantt-timeline/) (continuous-axis timeline). It does **not** extend or embed any of them — it is a fresh, standalone procomp that consumes the **same canonical `TaskItem[]`** and lays the items out on a **calendar date grid** (month cells / week & day time-grids / agenda list) instead of cards, columns, an outline, or a continuous time axis. *Same data genes, different surface.* This is the **date-grid sibling** the gantt description ([§"Deliberate non-goals"](../gantt-timeline-procomp/gantt-timeline-procomp-description.md)) explicitly reserved: "Date-grid layout (month/week/day cells, agenda) is the planned sibling `event-calendar` (next procomp)."
...
> 🎯 **Read-me-first.** §4 is the **exact data structure** every event is drawn from (the canonical `TaskItem`, the same one verified against [`task-card/types.ts`](../../../src/registry/components/data/task-card/types.ts) that gantt consumes). §10 is the **coverage checklist** — every view, state, and edge case to produce. The component is **read-only in v1** (no drag/resize/create) — a *display* surface, not an editor — but its state model is architected so v2 drag-to-reschedule and create-on-cell drop in without a rewrite (§7-D2/D3), reusing the editing vocabulary `gantt-timeline` already proved.

---

## 0. Compound-structure declaration (mandated)

`event-calendar` trips **multiple** triggers in [`.claude/rules/compound-component-structure.md`](../../../.claude/rules/compound-component-structure.md): it has ≫ 3 distinct mountable regions (toolbar / month grid / week grid / day grid / agenda list / mini-nav), it composes another procomp (`task-card`, optionally lazy-embedded in the hover/detail slot), and a reasonable consumer would want only a subset — "just the month view", "month + agenda, no time-grids", "the agenda list on mobile." The week/day **time-grid** logic (hour positioning + overlap lane-packing) plus the lazy `task-card` tooltip are real weight a month-only consumer shouldn't be forced to pull. **Therefore it ships as a shadcn-style compound** — headless `Root` provider + flat à-la-carte parts + standalone primitives + one logic-free assembly. Flat exports, never a `Name.Root` namespace.

**Rough part inventory** (precise tier split + final names locked at GATE 2):

| Tier | Members (rough) | Role |
|---|---|---|
| **B — headless Root** | `EventCalendarRoot` | Owns data normalization (`TaskItem[]` → calendar **occurrences** with effective start/end + all-day/timed/milestone classification), the **cursor** (current `view` + `focusDate` → derived visible range), selection state, the `now`-tick + color engine, the imperative handle, and the context. Renders `children`. |
...
| **A — assembly** | `EventCalendar` | `Root` + toolbar + (optional) mini-nav + the active view, gated by `show*` toggles. Contains no logic the parts don't. Demo + screenshot use this. |

**Tree-shaking story:** each view is its own module re-exported from the barrel. The **week/day time-grid** logic (hour positioning + overlap lane-packing) lives only in the `CalendarWeekView` / `CalendarDayView` modules — a month-only consumer never pulls it. The **full-card tooltip path** (`renderTooltip` → `task-card`) is `React.lazy` so a consumer who keeps the default lightweight `EventTooltip` never pulls `task-card` into their graph. A month-only, agenda-only, or grid-only subset must fall out for free. (Tier A pulls all four views — that is expected; the subset story is for hand-assembled layouts.)
...
**We already carry the data for this.** The canonical `TaskItem` — the JSON that drives `task-card`, nests in `task-tree`, bridges into `kanban-board`, and lays out as bars in `gantt-timeline` — ships `setAt` / `startAt` / `expireAt` / `duration`, a consumer-defined `status`, `priority`, `labels`, people, and `children`. What's missing is the **primitive that buckets those items into calendar cells** — a calendar that:
...
3. paints **urgency + status** using the **same color engine** as `task-card` and `gantt-timeline` (imported, not re-implemented),
4. and consumes **`TaskItem[]` directly** — so a product's "Calendar" tab is literally the same task data its List / Board / Timeline tabs show, on a fifth surface.

Today every product hand-rolls month/week/day grids (the classic "we'll just use FullCalendar" → 2-4 weeks of theming + data-adapter glue + accessibility usually skipped). This procomp is that primitive, native to the `TaskItem` model and the design system.

### Release strategy — phased

| Version | Scope |
|---|---|
| **v1 (this doc)** | **Read-only**, four views: **Month** (date cells with chips + multi-day spanning bars + "+N more" overflow), **Week** + **Day** (hour time-grids: all-day band + positioned, lane-packed timed blocks + now-line), **Agenda** (chronological list grouped by day). Period navigation (prev/next/today), view switching, jump-to-date mini-nav, hover tooltip, selection, keyboard nav. Status/urgency color from the shared engine. Consumes `TaskItem[]`. Standalone-usable. **State model architected for editing (§7-D2/D3) — write callbacks reserved, not shipped.** |
| **v2** | Interaction: **drag-to-reschedule** (drag a chip/bar to another day; drag a time block to another hour/day), **edge-resize** the span, **create-on-cell-click**, **detail popover** (lazy `<TaskCard editable>`), context menu, delete. Reuses `gantt-timeline`'s `edit-mutations` mental model + an `evalCalendarPermission` mirroring `evalGanttPermission`. Controlled-echo events (no internal data state). |
...
- Event fill/accent comes from the item's **status tone** via consumer-supplied `statusOptions`: `active` tones use the **time-urgency ramp** (fresh → red as the window elapses), `done` → muted/gray, `blocked` → red. The ramp is **imported from `task-card`** (its `RAMPS` runtime presets + the `TaskColorRamp` type) exactly as `gantt-timeline`'s `lib/color.ts` does (`import { RAMPS } from "../../task-card"`) — not re-derived.
...
- Read-only: navigation moves the cursor/viewport only; it never mutates `TaskItem` data. (Drag-to-*reschedule* is the v2 feature — §7-D2.)

**Read-only interactions**

- **Hover** an event → **lightweight tooltip** (name + dates/time + assignee + status) by default; a `renderTooltip` slot overrides it (and may lazy-embed the full `<TaskCard>`) — §7-D6.
...
- **Recurrence (RRULE)** → consumer expands to fresh `TaskItem`s in v1; native pre-expanded recurrence display is v3.
...
- **Not a continuous-axis timeline.** The pan/zoom Gantt bar chart with WBS summary rows is `gantt-timeline`. This component is a **discrete date grid**. They are deliberate twins, not competitors.
- **Not a task editor.** It renders `TaskItem`s; field editing is `task-card`'s job (v2 borrows its editable card in a popover).
...
- **Product teams** adding a "Calendar" tab beside their existing List / Board / Timeline tabs — already holding `TaskItem[]`, want a fifth surface with zero data reshaping.
...
- **Internal-ops dashboards** (agent run calendars, shift views) — read-only display of dated records that already match the `TaskItem` shape.

The shared thread: **anyone already on the `TaskItem` model** who needs the calendar surface. The component must look native to the design system out of the box and require no data adapter.

---

## 4. The data it renders — the canonical `TaskItem`

`event-calendar` consumes the **same `TaskItem[]`** as the rest of the family. It does **not** define its own event type; it imports `TaskItem` (+ option/permission/event types) from `task-card` via the proven same-category relative import + barrel re-export pattern (§7-D4) — exactly as `gantt-timeline` does. The authoritative definition is [`task-card/types.ts`](../../../src/registry/components/data/task-card/types.ts) (≈ lines 43–75); the fields this component reads:

| Field | Type | How event-calendar uses it |
|---|---|---|
...
| `targetPerson?` | `TaskPerson` | optional assignee avatar on event + agenda row |
...
| `children?` | `TaskItem[]` | **flattened** to occurrences in v1 (every item with a date renders, regardless of nesting depth). Calendar has **no WBS summary rows** — that's gantt's surface. See §7-D10. |

**Classification is derived, not stored — resolved at §7-D5.** The `TaskItem` model has **no explicit `allDay` flag**. The calendar classifies each event all-day / timed / milestone via a **three-layer rule** (consumer `classifyEvent?` predicate → date-only-string detection à la Google → span-based heuristic fallback), **without forking the shared type**. See §7-D5 for the full rule, including the floating-local-date parsing requirement that avoids a timezone off-by-one.
...
> Illustrative only — full prop types, the compound export surface, and the imperative handle are locked at GATE 2. Names mirror `gantt-timeline` for cross-component muscle memory.

```tsx
// Tier A — batteries-included assembly
<EventCalendar
  data={items}                       // TaskItem[]  (controlled; no internal data state)
  ...
  renderTooltip={(item) => <TaskCard data={item} />}            // lazy; default is lightweight
/>

// Tier B/C — hand-assembled "lighter" subset (month + agenda only; no time-grids pulled)
<EventCalendarRoot data={items} statusOptions={statusOptions} defaultView="month">
  <CalendarToolbar views={["month", "agenda"]} />
  <CalendarMonthView />
</EventCalendarRoot>
```

**Reserved-but-not-shipped in v1** (architected, fire in v2): `editable`, `permissions: TaskPermissions`, `onChange(next: TaskItem[])`, `onTaskReschedule`, `onItemAdded`/`onItemRemoved`/`onItemMoved`/`onFieldEdited`/`onStatusChanged` — the **identical controlled-echo vocabulary** `gantt-timeline` ships, so v2 is additive, not a re-API.
...
```tsx
<Tabs>
  <TabPanel value="list">     <TaskCardList data={items} /></TabPanel>
  <TabPanel value="board">    <KanbanBoard ... /></TabPanel>
  <TabPanel value="timeline"> <GanttTimeline data={items} statusOptions={s} /></TabPanel>
  <TabPanel value="calendar"> <EventCalendar data={items} statusOptions={s} /></TabPanel>
</Tabs>
```
...
- **D4 — Cross-procomp reuse.** ✅ Import `TaskItem` + option/permission/event types from `../task-card` (same-category relative barrel import) and **re-export** them from `event-calendar/types.ts`, exactly as gantt does — single-import consumer DX, rewriter-safe. Color ramp imported from task-card; no re-implementation.
- **D5 — All-day vs timed classification.** ✅ **Resolved** (was the headline open question; validated against the family's date handling — `gantt-timeline/lib/geometry.ts` parses effective start/end with native `Date.parse`). `TaskItem` has no `allDay` flag, so the calendar classifies every event with a **three-layer rule, first match wins**:
  1. **Consumer predicate** — an optional `classifyEvent?(item): "all-day" | "timed" | "milestone" | undefined` prop wins when it returns a kind (`undefined` defers to the next layer). This is the explicit escape hatch; it is a **pure function prop** (the family's render-prop idiom), **not** a flag bolted onto the shared `TaskItem` — the shared type stays untouched, and a consumer can encode any backend convention ("items labelled `holiday` are all-day").
...
- **D13 — Recurrence.** ✅ Out of scope v1 — consumer pre-expands to `TaskItem` occurrences. Native pre-expanded display is v3.
...
1. **Data-faithful** — any valid `TaskItem[]` renders correctly across all four views with no adapter; milestone / multi-day / timed / overdue / inactive all visually distinct and correct.
...
4. **Color is shared** — identical tone/urgency output to `task-card` and `gantt-timeline` for the same item (imported engine, verified).
5. **Compound + tree-shakeable** — flat exports; a hand-assembled month-only subset renders and does **not** pull the week/day time-grid code or `task-card`; demo includes a "lighter / composed" example.
