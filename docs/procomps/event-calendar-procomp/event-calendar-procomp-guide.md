...
- You hold `TaskItem[]` (the data behind `task-card` / `task-tree` / `kanban-board` / `gantt-timeline`) and want a **calendar tab** — month / week / day / agenda — with **no data adapter**.
...
- **You need a continuous, zoomable timeline with WBS roll-up** → that's `gantt-timeline` (the calendar deliberately has no summary rows; children are flattened).
- **You need a date *picker*** (single/range input) → use the shadcn `Calendar` primitive directly; event-calendar uses it only for its optional jump-to-date mini-nav.
...
<EventCalendar
  ...
  permissions={permissions}              // shared TaskPermissions matrix (optional)
...
<EventCalendar data={tasks} view={view} onViewChange={setView} date={date} onDateChange={setDate} />
```
...
```tsx
<EventCalendarRoot editable data={tasks} onChange={setTasks} statusOptions={statusOptions} views={["month", "agenda"]}>
  <CalendarToolbar />
  <CalendarMonthView />
...
All editing is **opt-in via `editable`** (default off → read-only), **gated** by the shared `TaskPermissions` matrix (+ optional `canMoveItem`/`canResizeItem`/`canDeleteItem`/`canCreateChild`/`canEditItem` predicates), and **controlled-echo**: there is no internal data copy — each edit fires a typed event (`onTaskReschedule` / `onItemAdded` / `onItemRemoved` / `onFieldEdited` / `onStatusChanged`) **and** `onChange(nextForest)`. One `onChange` per gesture = one undo step for you.

**Granular `onFieldEdited` coverage.** As of **v0.2.2**, `onFieldEdited` fires for `name` / `description` / `status` / `active` / `setAt` / `startAt` / `expireAt` / `duration` from **every** edit path — drag-reschedule, inline rename, context-menu status, **and the inspector / modal detail editor** (the modal previously echoed via `onChange` only). **One exception: `priority`.** `priority` is not a `TaskEditableField`, so a priority change (context-menu *or* modal) does **not** emit `onFieldEdited` — it persists via `onChange(nextForest)` only. If you persist purely through granular events, read `priority` off the forest in your `onChange`.
...
- **All-day vs timed is *derived*, not stored.** `TaskItem` has no `allDay` flag. Order (first match wins): your `classifyEvent(item)` → a date-only string (`"2026-06-22"`, no `T`) ⇒ all-day, a full timestamp ⇒ timed → span heuristic (no end ⇒ milestone/all-day; ≥ 1 day ⇒ all-day; else timed).
...
- **Hoist the clipboard** into `task-card` + wire gantt / kanban / tree (the "between all task tools" epic).
