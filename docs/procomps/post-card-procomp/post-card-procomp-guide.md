# post-card — consumer guide

> Stage 3: how to use it. Updated for v0.1.1 (inline panels default-on, share menu, swipable likers, kasder slide layout).
>
> See [`post-card-procomp-description.md`](./post-card-procomp-description.md) for what & why,
> and [`post-card-procomp-plan.md`](./post-card-procomp-plan.md) for the implementation contract.

## 30-second mental model

`<PostCard>` is a **Tier-2 composite** — 95% of its work is wiring the five Tier-1 social primitives ([`expandable-text`](../expandable-text-procomp/), [`media-carousel`](../media-carousel-procomp/), [`engagement-bar`](../engagement-bar-procomp/), [`comment-thread`](../comment-thread-procomp/), plus [`video-player`](../video-player-procomp/) transitively) into one cohesive social-post surface across four variants, with kasder-style **inline engagement panels** (likes / comments / share) on by default.
...
Auto-pulls all 5 Tier-1 social siblings + 6 shadcn primitives (`avatar`, `button`, `card`, `dropdown-menu`, `input`, `popover`). Third cross-folder dependency precedent in pro-ui after `media-carousel → video-player` and `comment-thread → expandable-text + engagement-bar`.
...
| `detail` | Full-page post detail | header → content (no clamp) → media carousel → engagement bar → embedded `<CommentThread>` | `[like, comment, share, bookmark, view-count]` | ✅ auto-wired | ❌ ignores engagementMode (thread is permanent) | ✅ embedded |
...
- **Tap like count** → toggle the inline likers strip (separate click target from the heart — wired via `engagement-bar`'s `onCountClick`).
- **Tap comment icon** → toggle inline `<CommentThread>` panel below the engagement bar (with composer; scrollable; `inlineCommentsMaxHeight` default `24rem`).
...
- `engagementSubscribe` is owned by post-card; deltas patch the local mirror + fire `onSubscribeEngagementDelta`.
- `commentSubscribe` forwards directly to `<CommentThread>` (detail variant's permanent thread, OR feed/compact's inline panel when opened).
...
- **`renderContent(post)`** — full takeover for the body. Default is `<ExpandableText>`. Override for rich Plate content / mentions / hashtags / link previews.
...
- **`commentActions(comment, helpers)`** — forwarded to `<CommentThread>` (inline panel and detail-variant thread).
- **`renderCommentSection(post, handlers)`** — full takeover for the entire comments block (detail variant only). Default builds `<CommentThread>` with all forwarded props.

## i18n

```tsx
const TR_LABELS: PostCardLabels = {
  // header / kebab
...
<PostCard labels={TR_LABELS} {...rest} />
```
...
- Heart-burst is `aria-hidden="true"` (decorative — like state announces via `aria-pressed` on the bar's like action, inherited from engagement-bar).
...
- **Reaction emojis** (Facebook-style 6-emoji popover) — engagement-bar v0.2 candidate.
...
- **Heart vs count split** — wired via `engagement-bar`'s new `like.onCountClick` action property. The split is a feature of the engagement bar (works wherever the bar renders); post-card just supplies the count callback under inline mode.
...
- **New props on `PostCardProps`:** `engagementMode` / `likers` / `onLoadMoreLikers` / `shareSuggestions` / `onShareSearch` / `onShareTo` / `inlineCommentsMaxHeight` / `defaultInlinePanel` / `openLikersOnLike` / `disableHeartBurst` / `onCopyLink` / `onReport`.
...
When this component composes another registry component (cross-folder import), it imports only from the OTHER component's `<slug>.tsx` file — never from `lib/`, `hooks/`, or `parts/` sub-folders. Conversely, when other registry components compose `post-card`, they import only from `post-card.tsx`.
...
- **Mentions:** `post.mentions` carries `{ id, name, username?, range }`. Library default `renderContent` is `<ExpandableText content={post.content}>` and does NOT auto-highlight mentions (ExpandableText's `content: string` API can't accept JSX). Opt-in via the slot:
  ```tsx
  <PostCard
    post={p}
    renderContent={(p) => (
...
import {
  ...
  LikersStrip, ShareMenu, // v0.2.0 — re-exported from engagement-bar for soft-compat
} from "@ilinxa/post-card";
```
