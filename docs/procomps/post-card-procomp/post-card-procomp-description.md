...
> - **Three inline panels:** likers (horizontal swipable avatar strip + `+N` pill), comments (embedded `<CommentThread>` with composer), share (searchable user list).
> - **Split heart-vs-count tap targets** — wired via `engagement-bar`'s new `like.onCountClick` action prop. Heart toggles like; count toggles likers panel. Lives at the bar level (works anywhere the bar renders); post-card supplies the count callback under inline mode.
...
>
> Sections below are the original Stage-1 spec; treat them as the historical record. Live API surface is in [`types.ts`](../../../src/registry/components/data/post-card/types.ts) + the [guide](./post-card-procomp-guide.md).
>
...
>
> **Sixth of 8** in the social-posts-system arc. **Pure composition** — 95% of the work is wiring the five already-shipped Tier-1 primitives ([`expandable-text`](../expandable-text-procomp/), [`video-player`](../video-player-procomp/), [`media-carousel`](../media-carousel-procomp/), [`engagement-bar`](../engagement-bar-procomp/), [`comment-thread`](../comment-thread-procomp/)) into one cohesive card surface. Net-new code is the variant dispatcher + post header + `Post` data shape + a few connective slots.
>
> Cross-folder dependencies declared via `registryDependencies`: all five siblings auto-pull on consumer install. Third cross-folder precedent in pro-ui after media-carousel → video-player and comment-thread → expandable-text + engagement-bar.
...
export interface Post {
  ...
  createdAt: Date | string | number;        // same coercion as event-card
...
**Note** (R-D-1): the pre-loaded comment tree and pagination size live as **separate card props** (`commentThread?: Comment[]` + `commentPageSize?: number`), NOT on the `Post` shape. Posts and comment trees are typically separate API endpoints; mirroring `comment-thread`'s `comments: Comment[]` interface keeps the contract clean.

`MediaItem` re-exported from `media-carousel`. `Comment` re-exported from `comment-thread`. No re-derivation, no shape drift.
...
| `detail` | Full-page post detail | header → content (no clamp) → media carousel (full-width gallery) → engagement bar (default) → **`<CommentThread>` inline** | `[like, comment, share, bookmark, view-count]` | EMBEDDED — `commentThread` from props |
...
post.media[] present + onLike wired:
  → post-card owns burstKey + barRef
  → media-carousel.onDoubleTap = () => { barRef.triggerLike(); setBurstKey(k+1) }
...
Returning `[]` hides the kebab. Same `CommentMenuItem`-style data shape from comment-thread.
...
```ts
engagementSubscribe?: Subscribe<EngagementDelta>;     // forwarded to engagement-bar
commentSubscribe?: Subscribe<CommentDelta>;           // forwarded to comment-thread (detail only)
```
...
- The engagement bar (no — engagement-bar doesn't take `currentUser`; only host wires `onToggle`)
- The comment thread (yes — drives composer avatar + "isOwn" check on the kebab; same shape `comment-thread` already accepts)
...
- `renderContent?: (post) => ReactNode` — full takeover for the body block. Default is `<ExpandableText content={post.content} maxLines={...}>`. Override use case: rich Plate-rendered content in `detail` variant.
...
- `commentActions?: (comment, helpers) => CommentMenuItem[]` — forwarded to `<CommentThread>` in detail variant.
- `renderEngagementBar?: (post, defaults) => ReactNode` — full takeover for the entire engagement row (rare; for hosts with totally custom engagement UI). Default builds `<EngagementBar>` from `engagementActions(post, handlers)`.
...
export interface PostCardHandle {
   ...
   */
  triggerLike: () => void;
  /** Read the underlying engagement-bar handle (bar-only operations). */
  getEngagementHandle: () => EngagementBarHandle | null;
  /** Read the underlying comment-thread handle (detail variant only; null otherwise). */
...
- `formatRelativeTime?: (date, now) => string` — same shape as comment-thread's `formatRelativeTime`; default delegates to `defaultRelativeTime` (re-exported from comment-thread)
- `engagementLabels?: EngagementBarLabels` — forwarded to engagement-bar
- `commentLabels?: CommentThreadLabels` — forwarded to comment-thread
...
- **Reaction emojis** (Facebook-style 6-emoji popover on long-press of like) — explicit out-of-scope; engagement-bar v0.2 candidate.
- **Inline comment thread in `feed` variant** — only `detail` embeds the thread. Feed/compact/list rely on `comment.onClick` to hand off to host (typically navigates to detail). Hosts wanting feed-with-inline-thread compose `<CommentThread>` themselves under the card.
...
export type { Comment, CommentDelta, CommentMenuItem } from "@/registry/components/data/comment-thread";
export type { EngagementAction, EngagementDelta, EngagementBarHandle } from "@/registry/components/data/engagement-bar";
export type { Subscribe, Unsubscribe } from "@/registry/components/data/comment-thread";
...
- **Q1 / variant dispatcher:** **single `<PostCard variant="…">`** with internal switch into `parts/<variant>-variant.tsx`. Matches `event-card`'s 4-variant precedent. One import, one type, one ref handle. Tree-shake cost (~3KB per unused variant) is acceptable.
- **Q2 / comment thread in detail variant:** **slot with auto-default** — card auto-renders `<CommentThread>` for the 90% case AND exposes `renderCommentSection?(post, handlers): ReactNode` slot for full takeover. Same shape as `engagementActions` and `kebabActions`.
...
- **R-D-1 / `commentThread` placement:** lives as **separate card props** (`commentThread?: Comment[]` + `commentPageSize?: number`), NOT on `Post`. Mirrors `comment-thread`'s contract.
- **R-D-2 / `PostHandlers` shape:** typed as engagement-action handlers only (`onLike` / `onComment` / `onShare` / `onBookmark`). Comment handlers stay separate (forwarded directly to `<CommentThread>` in detail variant).
...
- **`<PostCard>` is `"use client"`** — owns kebab open state + burstKey + imperative handle.
- **`<VerifiedBadge>` is RSC-compatible** (no `"use client"`).
- **`React.memo` at export** + ref-as-prop (React 19 pattern; matches engagement-bar / comment-thread).
- **Cross-folder imports allowed** — declared via `registryDependencies: ["expandable-text", "video-player", "media-carousel", "engagement-bar", "comment-thread"]`. Third precedent in pro-ui after media-carousel → video-player and comment-thread → expandable-text + engagement-bar.
...
- **No framer-motion.** CSS keyframes via inherited heart-burst sibling (already handled by engagement-bar).
...
- **`labels` includes `engagementLabels` + `commentLabels` slots** for forwarding to children (locked per pattern with [news-card](../news-card-procomp/) which has `categoryStyles` + similar nested config).
...
- **Locked target convention** for `registry.json`: every file `type: "registry:component"`, `target: "components/post-card/<sub>"`. Never ship `demo.tsx`/`usage.tsx`/`meta.ts`. `dummy-data.ts` ships only via `post-card-fixtures` sibling.
