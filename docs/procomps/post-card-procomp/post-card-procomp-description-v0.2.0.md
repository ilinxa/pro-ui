# `post-card` v0.2.0 — Description Addendum (Stage 1)
...
Sibling cards (project-card, event-card, news-card) inherit the same `viewerMode` toggle + `Permissions` matrix shape — the user explicitly extended the ask to "projects, news, event…". Cross-card charter remains v0.3+ work per Q-D2 lock.
...
Library does NOT model granular cases directly (Facebook's "Friends except…" or "Specific friends only" require carrying user-id lists). Hosts encode their granular cases as their own string keys; e.g. `post.visibility = "list:123"` is valid and renders with the "Custom" fallback. Cross-card consistency: same shape used by `project-card` / `event-card` / `news-card-01` when their permission models adopt visibility.
...
`engagement-bar` is a full sibling procomp, BUT `LikersStrip` + `ShareMenu` sit inside `post-card/parts/` and are not exported. Any other engagement surface (comment-thread, profile, story-viewer) that wants the same UX has to re-implement.

v0.2.0 promotes both to **sub-exports of `engagement-bar`** (alongside the existing `EngagementHeartBurst` sub-export). post-card then composes them via cross-folder import. This is mechanical — no API change visible to post-card consumers, but consumers of `engagement-bar` gain two new sub-exports.
...
- Shares the same `Post` type with `post-card` (single source of truth)
...
export interface PostMutationHandlers {
  ...
  // viewer-side
  // (onReport already exists on PostCardProps from v0.1; it carries unchanged)
...
`PostMutationHandlers` is **flattened onto `PostCardProps`** (same pattern as `PostHandlers extends PostCardProps` today) — no nested `mutations={...}` prop bag. Consumers pass handlers at the top level.

Additional `PostCardProps` fields for the schema-expansion render surfaces (closes F-4 / F-5 / F-6 / F-7):
...
defaultPostKebabActions(
  ...
  labels: Required<Pick<PostCardLabels, ...>>,
  viewerMode?: PostViewerMode,
  permissions?: PostPermissions,
  canPerformAction?: (action: PostPermissionAction, post: Post) => boolean | undefined,
): CommentMenuItem[];
...
Resolution order, **most → least specific** (mirrors card-tree's pattern; closes F-15):
...
Reduced-motion respected (`@media (prefers-reduced-motion: reduce)`) for the heart-burst — already inherited via `engagement-bar`'s CSS, no new work.
...
| `repostOf` | nested `<PostCard variant="compact" post={repostOf}>` mini-card below content | feed / detail |
...
**Workflow note (F-3 closure + Q-P37 lock).** `engagement-bar` is a sibling procomp with its own GATE 1 / 2 / 3 cycle. Per the GATE 2 plan v0.2.0 Q-P37=(a) user lock, the engagement-bar additive ship **MUST land before post-card v0.2.0 C1 begins**. No fallback path: this is committed as a blocking prerequisite (C0 in the plan).
...
- **engagement-bar v0.2.0 owns:** authoring its own description + plan addenda + GATE 3 spotcheck. Separate procomp folder: `docs/procomps/engagement-bar-procomp/`. C0 of the post-card v0.2.0 plan enumerates the steps.
- **Sequencing:** engagement-bar description ✅ → engagement-bar plan ✅ → file moves + sub-exports + meta bump + dual-slug smoke + spotcheck ✅ → THEN post-card v0.2.0 C1 begins.

Proposed shape of the engagement-bar additive ship:

```ts
// engagement-bar/index.ts (additive)
...
Existing post-card consumers see no change either way — `LikersStrip` was never exported from post-card in v0.1.x. Existing engagement-bar consumers gain two new sub-exports if/when that ship lands.

#### Bucket E — Display badges & content gates (new render parts)

Net-new parts inside `post-card/parts/`:
...
- `parts/repost-of-card.tsx` — `"use client"`; nests `<PostCard variant="compact" post={repostOf} viewerMode="viewer" engagementMode="navigate" />` — explicit defaults to prevent recursive engagement panel state.

#### Bucket F — Sibling `post-editor-01` procomp (separate slug)

Lives at `src/registry/components/forms/post-editor-01/`. Sealed-folder convention, GATE 1 + GATE 2 + GATE 3 for the editor are **separate procomp work** and tracked in `docs/procomps/post-editor-01-procomp/` (NOT this addendum). v0.2.0 of `post-card` declares `post-editor-01` as a `related` slug; the editor description ships its own description doc.

**The minimum editor contract referenced by post-card v0.2.0** (the rest is editor's concern):
...
- **Re-export the new permission helpers** + the new types from `post-card/index.ts`.
...
- **Reaction emojis** (Facebook-style 6-emoji popover) — explicit out-of-scope, deferred to `engagement-bar` v0.3.
- **Multi-author posts** — `post.author` stays a single object. Co-authors not modeled.
- **Comment-level permissions** — `commentActions` already takes `isOwn` from comment-thread; v0.2.0 doesn't extend that surface here.
...
The v0.1 demos are **rewritten, not deleted**. Existing tab labels (Feed / Compact / List / Detail / Text-only / Video / Realtime / Inline TR / Custom) stay — same docs-site URL `/components/post-card` continues to surface "Feed" / "Detail" / etc. as the first thing readers click. This is doc churn, not API churn. **Library API stays backwards-compat per §1.6** — `demo.tsx` rewrite is purely the showcase layer.
...
11. **Zero breaking changes verified at the LIBRARY API surface.** Every existing prop signature compiles + renders identically with no source modification. tsc + lint + smoke all green when a v0.1.x consumer imports the new `post-card` without touching their call site. (Note: the docs-site `demo.tsx` is rewritten per Q-D7 lock — this is showcase churn, not API churn. The library stays backwards-compat; only the demonstration narrative is refreshed.)
...
14. **Engagement extraction complete via blocking prerequisite (Q-P37=(a)).** `LikersStrip` + `ShareMenu` are sub-exports of `engagement-bar` v0.2.0 (which shipped first per the C0 prerequisite in the GATE 2 plan). `post-card` v0.2.0 imports cross-folder. Both packages' smoke harness runs clean. No v0.2.1-deferred path — extraction is part of this version.
15. **`post-editor-01` GATE 1 description in flight** in a separate procomp folder. Its GATE 3 closure is NOT a prerequisite for post-card v0.2.0 GATE 3 (the card ships standalone; the editor lands when it lands).
...
| 4 | `LikersStrip` / `ShareMenu` sealed | Both promoted to sub-exports of `engagement-bar` (the bump version + GATE 1/2/3 owned by engagement-bar; post-card v0.2.0 ships either with the cross-folder imports OR with the legacy in-`parts/` files depending on which procomp lands first — see Bucket D) |
| 5 | No editor procomp | Sibling `post-editor-01` GATE 1 description authored in parallel (separate procomp; not gated on post-card) |
...
- **Cross-card permission charter** (sharing the same resolver across project-card / event-card / news-card) — F-cross-NN candidate, NOT a v0.2.0 deliverable; the resolver lives inside `post-card/lib/` for now and gets lifted into a shared module IF the user explicitly wants the cross-cutting charter as a separate work item
...
| Q-D2 | Scoped under `post-card/lib/permissions.ts`; cross-card charter is v0.3+ | Assistant recommendation, confirmed |
...
- **Locked target convention** for `registry.json`: every file `type: "registry:component"`, `target: "components/post-card/<sub>"`. Never ship `demo.tsx` / `usage.tsx` / `meta.ts`. `dummy-data.ts` ships only via `-fixtures` sibling.
