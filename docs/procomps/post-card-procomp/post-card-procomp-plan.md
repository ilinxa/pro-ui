# post-card — procomp plan

> Stage 2: how. The implementation contract.
>
> ### v0.1.1 patch (2026-05-03)
>
> The plan below is the historical Stage-2 record. v0.1.1 layered the following on top — see the [guide](./post-card-procomp-guide.md) for the live consumer-facing surface:
>
...
> - **3 inline panels** (single state, one open at a time): `likes` (`<LikersStrip>`) / `comments` (`<CommentThread>` in scroll wrapper) / `share` (`<ShareMenu>`).
> - **Heart-vs-count split** lives in `engagement-bar` (new `like.onCountClick` field on the action; bar renders two click targets when present). Post-card-01 supplies the count callback under inline mode via `defaultPostEngagementActions(post, handlers, variant, onLikeCountClick?)` — fourth arg.
...
> - **`registry.json`** post-card base item gains 2 new files (`parts/likers-strip.tsx` + `parts/share-menu.tsx`); shadcn deps gain `input` + `popover`.
> - **Demo** grew from 7 → 9 tabs (added Video tab + renamed Inline → Inline TR).
> - **Version bump:** v0.1.0 → v0.1.1.
>
> See [`post-card-procomp-description.md`](./post-card-procomp-description.md) for the what & why.
>
> **Migration origin:** kasder `AdvancedPostCard.tsx` (167 LOC).
>
> **Cross-folder dependencies (registry-declared — third precedent in pro-ui after media-carousel → video-player and comment-thread → expandable-text + engagement-bar):**
> - [`expandable-text`](../expandable-text-procomp/) — body content
> - [`media-carousel`](../media-carousel-procomp/) — media display
> - [`engagement-bar`](../engagement-bar-procomp/) + `<EngagementHeartBurst>` — actions + double-tap burst
> - [`comment-thread`](../comment-thread-procomp/) — embedded thread (detail variant only)

## Q-P locks (most carry from precedents — only novel ones re-stated)

| # | Lock | Source |
|---|---|---|
| Q-P1 | Single component `<PostCard variant="…">` dispatcher; per-variant parts in `parts/<variant>-variant.tsx`. | Stage-1 Q1 + event-card precedent |
| Q-P2 | `renderCommentSection?(post, handlers)` slot with auto-default (`<CommentThread>` in detail variant). | Stage-1 Q2 |
...
| Q-P8 | Always-uncontrolled — `post` prop is initial state on mount. `reset(next: Post)` + `dispatch` imperative escape hatches mirror engagement-bar + comment-thread patterns. Surgical updates flow through `getEngagementHandle()` / `getThreadHandle()` for child-state mutations. | comment-thread R-Plan-15 |
...
| Q-P10 | Verified badge sealed `<VerifiedBadge>` part — RSC-compatible (no `"use client"`). | matches engagement-bar's `<EngagementHeartBurst>` precedent |
...
| Q-P14 | `<PostCard>` is `"use client"` (owns kebab open state + burstKey + handle). | inherited from engagement-bar |
| Q-P15 | Variant parts files are `"use client"` (each owns its own JSX wiring; consistent across the four). | Q-P14 corollary |
| Q-P16 | `React.memo` at export + ref-as-prop. | engagement-bar + comment-thread precedent |
...
| Q-P20 | Locked target convention: `registry:component`, `target: "components/post-card/<sub>"`. Never ship demo/usage/meta. Fixtures via `-fixtures` sibling. | project lock |
| Q-P21 | **Local engagement mirror** in post-card (initialized from `post` on mount via `useState`). Bar runs always-controlled; mirror flows through `defaultPostEngagementActions`. Resolves the always-uncontrolled ↔ engagement-bar-controlled-mode conflict. | R-Plan-1 |
...
| Q-P23 | **`reset(next: Post)` + `getCurrentPost()`** on `PostCardHandle`. Required by always-uncontrolled lock. Re-derives local engagement mirror on reset. | R-Plan-3 |
| Q-P24 | **Post-card-01 owns `engagementSubscribe`** — bar doesn't receive it directly. Card subscribes, dispatches into local mirror, fires `onSubscribeEngagementDelta`. `commentSubscribe` still forwards directly to embedded `<CommentThread>` (its internal reducer handles delta routing). | R-Plan-4 |
...
import type { MediaItem } from "@/registry/components/media/media-carousel";
import type {
  Comment,
  CommentDelta,
  CommentMenuItem,
  ...
  Subscribe,
} from "@/registry/components/data/comment-thread";
import type {
  EngagementAction,
  EngagementBarHandle,
  EngagementBarLabels,
  EngagementDelta,
} from "@/registry/components/data/engagement-bar";
import type { CommentThreadHandle, CommentThreadLabels } from "@/registry/components/data/comment-thread";

export type PostCardVariant = "feed" | "compact" | "list" | "detail";
...
export const DEFAULT_POST_CARD_LABELS: Required<
  Omit<PostCardLabels, "formatRelativeTime" | "engagementLabels" | "commentLabels">
> = {
...
export interface PostCardProps extends PostHandlers {
  ...
  /** Forwarded to comment-thread in detail variant. */
  commentActions?: (
    comment: Comment,
    helpers: {
      currentUser?: CommentThreadCurrentUser;
  ...
  /** Override the embedded comment section in detail variant (default = <CommentThread>). */
  renderCommentSection?: (
    post: Post,
    handlers: {
      onAddComment?: PostCardProps["onAddComment"];
      onLikeComment?: PostCardProps["onLikeComment"];
      onDeleteComment?: PostCardProps["onDeleteComment"];
      onReportComment?: PostCardProps["onReportComment"];
      onLoadMoreComments?: PostCardProps["onLoadMoreComments"];
    },
  ) => ReactNode;
...
export interface PostCardHandle {
  ...
  getEngagementHandle: () => EngagementBarHandle | null;
  getThreadHandle: () => CommentThreadHandle | null;
}
```
...
export function defaultPostKebabActions(
  ...
  labels: Required<
    Pick<PostCardLabels, "bookmark" | "unbookmark" | "share" | "copyLink" | "report">
  >,
): CommentMenuItem[];
```
...
export type { MediaItem } from "@/registry/components/media/media-carousel";
export type {
  Comment,
  CommentDelta,
  CommentMenuItem,
...
```
src/registry/components/data/post-card/
├── post-card.tsx                # 1 — root dispatcher
├── parts/
...
### 1. `post-card.tsx` — root dispatcher
...
- Owns refs: `barRef: Ref<EngagementBarHandle>`, `threadRef: Ref<CommentThreadHandle>` (detail only).
...
- Body via `renderContent ?? <ExpandableText content={post.content} maxLines={bodyMaxLines}>` — `relative z-10`.
- Media block: `relative` container.
  - `renderMedia ?? <MediaCarousel items={post.media} variant="gallery" onDoubleTap={...}>`.
...
- Engagement: `renderEngagementBar ?? <EngagementBar ref={barRef} actions={engagementActionsArr} subscribe={engagementSubscribe} onSubscribeDelta={onSubscribeEngagementDelta} labels={labels.engagementLabels}>` — `relative z-10`.
- No comment section.
- The `onDoubleTap` handler passed to MediaCarousel: `() => { barRef.current?.triggerLike(); setBurstKey(k => k+1); }` — but `setBurstKey` is owned by root. Pass `triggerLikeWithBurst` callback down from root to variant.
...
- Body via `<ExpandableText content={post.content} maxLines={2}>`.
...
- Engagement: `<EngagementBar variant="compact" actions={[like, comment-only]}>`.
...
import type { Post, PostHandlers, PostCardVariant, PostCardLabels } from "../types";
...
Plus `dummyViewer` (re-exported from comment-thread's fixtures shape for consistency), `dummyCommentThread` (5 nested comments for detail demo), `createDummyEngagementSubscribe()` + `createDummyCommentSubscribe()` (synthetic delta generators).
...
4. **Detail** — full layout + embedded `<CommentThread>` with `dummyCommentThread`.
...
- `category: "data"`
- `internal: ["expandable-text", "media-carousel", "engagement-bar", "comment-thread", "video-player"]`
- `shadcn: ["avatar", "button", "card", "dropdown-menu"]`
- `notes`: "Tier-2 composite. Always-uncontrolled — `post` prop is initial state on mount; use `ref.current.reset(next)` to push updates. Auto-wires heart-burst when post has media + onLike + variant ∈ {feed, detail}. detail variant embeds <CommentThread> inline. No new shadcn primitives needed."
...
3 lines into `src/registry/manifest.ts` (after engagement-bar / comment-thread imports):
...
{ meta: postCardMeta, Demo: PostCardDemo, Usage: PostCardUsage }
```
...
```jsonc
{
  ...
  "type": "registry:block",
  "title": "Post Card",
  "description": "Tier-2 social-post composite — composes expandable-text + media-carousel + engagement-bar + comment-thread (detail) + video-player (transitive). 4 variants (feed / compact / list / detail), auto-wired heart-burst, default engagement actions + kebab builders, slot-based render escape hatches at every connective seam, optional overlay-link via getHref. No framer-motion. Sixth ship in the social-posts-system arc.",
  ...
  "files": [
    /* 11 entries: post-card.tsx + 6 parts + lib/defaults.ts + types + index */
  ]
}
```

Plus `post-card-fixtures` sibling carrying only `dummy-data.ts` with `registryDependencies: ["post-card"]`.
...
2. `pnpm lint` — clean (1 pre-existing card-tree warning OK)
3. `pnpm registry:build` — generates `post-card.json` + `post-card-fixtures.json`; spot-check `registryDependencies` lists all 5 cross-folder siblings + 4 shadcn primitives.
...
6. **Cross-folder install smoke** — `pnpm dlx shadcn add http://localhost:3000/r/post-card.json` from a tmp Next 16 app; verify all 5 siblings auto-pull.
7. **Component docs route** at `/components/post-card` returns HTTP 200.

## Risk register

| Risk | Likelihood | Impact | Mitigation |
|---|---|---|---|
| Cross-folder dependency chain (5 deep) breaks shadcn CLI install | Low | High | media-carousel → video-player + comment-thread → expandable-text + engagement-bar already work. Smoke-test step 6 covers. |
...
| Detail variant's embedded thread state drifts from external store | Medium | Medium | Same shape as comment-thread's always-uncontrolled lock; hosts sync via `getThreadHandle().reset(next)`. |
| Overlay-link inside `<article role="article">` breaks screen-reader article semantics | Low | Medium | Match news-card's pattern (article + absolute-positioned link with `aria-label`). Already validated there. |
| `<EngagementBar>` per-row ALWAYS-controlled lock from comment-thread doesn't apply here — bar is uncontrolled by default at post level | N/A | N/A | Different context: comment-thread wraps bar to coordinate per-row state with thread reducer. Post-card has no reducer above the bar — bar's hybrid mode is correct here. |
...
6. **Root dispatcher** — `post-card.tsx` + imperative handle.
