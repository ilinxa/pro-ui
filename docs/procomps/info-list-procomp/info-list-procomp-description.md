...
Every project re-implements both patterns inline. Pro-ui has no answer — `thumbnail-list` is image-led, `article-meta` is horizontal flex strip, `schedule-list` is time-anchored.
...
- **Nested / tree rows** — for nested data, use `card-tree`
...
<aside className="rounded-2xl border bg-card p-6">
  ...
  <InfoList framed={false} variant="comfortable" items={items} />
</aside>
```
...
4. **`density: "comfortable" | "compact"` vs `variant`?** **Resolved: `variant` for symmetry with news-card + event-card.** Easier to reason about in JSX.
