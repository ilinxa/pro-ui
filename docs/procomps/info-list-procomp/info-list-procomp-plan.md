# info-list — procomp plan

> Stage 2: how. The implementation contract.
>
> See [`info-list-procomp-description.md`](./info-list-procomp-description.md) for the what & why.
...
```ts
// src/registry/components/data/info-list/types.ts

import type { ComponentType, ElementType, ReactNode } from "react";

export type InfoListVariant = "comfortable" | "compact";
...
export interface InfoListLabels {
...
export const DEFAULT_INFO_LIST_LABELS: Required<InfoListLabels> = {
...
```
src/registry/components/data/info-list/
├── info-list.tsx            # 1 — root
├── parts/
...
Headless wrapping over a presentational row. Same shape as schedule-list.
...
- [ ] `pnpm build` clean — `/components/info-list` prerendered
