# progress-timeline — procomp plan

> Stage 2: how. The implementation contract.
>
> See [`progress-timeline-procomp-description.md`](./progress-timeline-procomp-description.md) for the what & why.
...
```ts
// src/registry/components/data/progress-timeline/types.ts
...
```ts
// src/registry/components/data/progress-timeline/lib/timeline-state.ts
...
export const DEFAULT_PROGRESS_TIMELINE_LABELS: Required<
  Omit<ProgressTimelineLabels, "beforeText" | "activeText" | "afterText">
> & {
  beforeText: TimelineLabelText;
  activeText: TimelineLabelText;
  afterText: TimelineLabelText;
...
```ts
// index.ts
export { default as ProgressTimeline } from "./progress-timeline";
export type {
...
```
src/registry/components/data/progress-timeline/
├── progress-timeline.tsx       # 1 — root
├── lib/
...
- `@/components/ui/progress` — already installed (event-card was first user)
- `@/lib/utils` — `cn()`
...
- [ ] `pnpm lint` clean (only pre-existing card-tree warning)
- [ ] `pnpm build` clean — `/components/progress-timeline` prerendered
...
- [ ] Helper-only import works: `import { deriveTimelineState } from "@/registry/components/data/progress-timeline"` typechecks without pulling React imports
...
5. **Internal `setInterval` for live updates** — rejected (consumer drives `now`); same call as event-card.
