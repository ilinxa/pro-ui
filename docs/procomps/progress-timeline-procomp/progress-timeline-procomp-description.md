# progress-timeline — procomp description
...
- **Internal `setInterval`** — consumer drives `now` (same pattern as event-card).
...
function LiveTimeline({ event }) {
  ...
  return <ProgressTimeline start={event.start} end={event.end} now={now} />;
}
```
...
```tsx
import { deriveTimelineState } from '@ilinxa/progress-timeline';
...
1. **`now` injection — required prop or default to `new Date()`?** **Resolved:** optional, defaults `new Date()`. Same pattern as event-card.
