# people-grid — procomp plan

> Stage 2: how. The implementation contract.
>
> See [`people-grid-procomp-description.md`](./people-grid-procomp-description.md) for the what & why.
...
```ts
// src/registry/components/data/people-grid/types.ts
...
```ts
// src/registry/components/data/people-grid/lib/get-initials.ts
...
```ts
// index.ts
export { default as PeopleGrid } from "./people-grid";
export type {
  PeopleGridItem,
  PeopleGridLabels,
  PeopleGridProps,
  PeopleGridColumns,
  PeopleGridAvatarSize,
  PeopleGridAlignment,
} from "./types";
...
```
src/registry/components/data/people-grid/
├── people-grid.tsx           # 1 — root
├── parts/
...
Headless wrapping over a presentational card. Same shape as schedule-list / info-list.
...
- [ ] `pnpm build` clean — `/components/people-grid` prerendered
...
- [ ] Helper-only import works: `import { getInitials } from "@/registry/components/data/people-grid"` typechecks without React imports
...
1. **Variant of `author-card` instead of new component** — rejected; author-card is left-aligned single-card with avatar+name+title in a horizontal row. People-grid is centered grid with vertical stack. Different shape.
