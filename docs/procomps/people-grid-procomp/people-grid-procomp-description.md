# people-grid — procomp description
...
Pro-ui has `author-card` for **single-card** author bylines (avatar left, text right, horizontal). It doesn't fit grid contexts where:
...
`people-grid` is the compound: heading + grid + per-person cards + initials fallback, in one component.
...
- **Category / department grouping** (multiple grids per page) — consumer wraps multiple `<PeopleGrid>` instances
...
```ts
import { PeopleGrid, getInitials } from "@ilinxa/people-grid";
...
1. **`headingAs` default — `h2` or `h3`?** **Resolved: `h2`.** People grids (Speakers / Team / Contributors) are typically top-level page sections, not nested cards. Differs from `info-list` (h3) and `progress-timeline` (h3) which are sidebar / sub-card patterns.
