...
- **Edit mode** — `mode='edit'` swaps the view-only renderer for a CodeMirror 6 instance. **Visual continuity with view mode is near-match** (same JetBrains Mono font, same line-height, close token colors) via a custom CodeMirror `HighlightStyle` that approximates Shiki's GitHub Light + GitHub Dark Default token palettes using dual-theme CSS variables — the same `.dark`-class swap mechanism as view mode. Pixel-perfect parity (full Shiki → CodeMirror bridge) defers to a later minor as an additive replacement of the theme file. `value` + `onChange?: (args: { value }) => void` — controlled. `readOnly?: boolean` toggle. Editor extensions opt-in via `editorExtensions?: Extension[]` (escape hatch for consumers who want autocomplete, linting, vim mode, etc.). Editor inherits all chrome (header, footer, line numbers, wrap, highlights). Tab uses CodeMirror's `indentMore` / Shift+Tab uses `indentLess` (matches VS Code). Cmd+S / Ctrl+S fires `onSave` if wired.
...
- **Soft-failure** — four modes, **all implemented as of v0.2.0** (v0.1.x documented four and shipped two; an integrator found the gap the hard way — see the v0.2.0 review):
  1. **Invalid `lang`** → falls back to `plaintext` + dev warning.
  2. **Highlighter unavailable** (view mode) → the raw code renders as **plain text** rather than an empty panel, and the body carries `data-highlight="failed"` so hosts can style or telemeter the degraded state. This is the one an embedder can trigger through their own CSP (see *Runtime requirements* below); in v0.1.x it rendered nothing at all, with nothing in the console outside dev.
  3. **CodeMirror init failure** (edit mode) → inline `role="alert"` error plus a **"Reload as view-only"** control that swaps in the view body. Labels: `editorFailed`, `reloadAsViewOnly`. In v0.1.x the mount was a floating promise — an unhandled rejection over a blank box.
  4. **Clipboard API unavailable** → copy button still renders; click falls back to `document.execCommand('copy')`; on total failure the button shows "Copy failed — select and copy manually" (icon + `role="status"` announcement).

- **Runtime requirements — WebAssembly / CSP.** The default regex engine (`oniguruma`) is **WebAssembly**. A host serving a strict `Content-Security-Policy` **must** include `'wasm-unsafe-eval'` in `script-src`, or the browser refuses the compile outright (`Wasm code generation disallowed by embedder`) and highlighting can never work — no component can override its host's CSP. Two ways out, either is fine:
  - add `'wasm-unsafe-eval'` to `script-src`; or
  - pass `regexEngine="javascript"` (v0.2.0), which uses **no wasm at all**, at the cost of slightly narrower grammar coverage.

  Highlighters are cached **per engine**, so mixing both on one page is safe. Self-hosted apps with a strict CSP are exactly the audience most likely to need this, which is why it is stated here rather than left to the bundling notes.
...
4. **Editor substrate — CodeMirror 6 or Monaco?** CodeMirror 6 is project-locked per memory ("CodeMirror stays as source-text editor"); `markdown-editor` already depends on it. Monaco is heavier (~2 MB), more featureful (full IntelliSense). **Recommendation:** **CodeMirror 6** — locked. Reuses existing project dep. Edit mode uses a custom `HighlightStyle` approximating Shiki's GitHub themes for near-match visual continuity (full Shiki→CodeMirror bridge defers to a later minor; the `@shikijs/codemirror` package referenced in earlier drafts of this doc does not exist on npm).
...
31. **Editable filename in edit mode?** IDEs let users click the filename tab to rename. **Recommendation:** **defer to a later minor**. v0.1.0 keeps `filename` display-only in all modes. Consumer wires their own rename UX outside the block (e.g., in a tab strip above). Revisit when a real consumer asks for in-block rename; lands as additive `onFilenameChange?: (args: { filename }) => void` without breaking v0.1.x consumers.
...
- **Editor substrate:** CodeMirror 6 + custom `HighlightStyle` approximating GitHub Light + GitHub Dark Default (P2 substrate-shift; locked 2026-05-10 after `@shikijs/codemirror` was found to not exist on npm). Pixel-perfect Shiki→CodeMirror bridge defers to a later minor. Language packages dynamic-import on first edit-mode use of each lang.
...
- **Filename editing:** display-only in v0.1.0; deferred to a later minor (Q31).
- **Run callback:** slot-only via `actions`; no built-in `<RunButton>` part.
- **`useCodeBlock` hook:** deferred to a later minor.
