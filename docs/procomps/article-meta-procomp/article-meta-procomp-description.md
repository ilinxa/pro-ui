# article-meta — procomp description

> Stage 1: what & why.
>
> **Migration origin:** [`docs/migrations/article-meta/`](../../migrations/article-meta/) (kasder `kas-social-front-v0` news detail page article column — `src/app/(platform)/news/[id]/page.tsx` lines 89–106)
>
> Sibling migration: [`share-bar`](../../migrations/share-bar/) — bottom-of-article share strip from the same page.
...
- **No badges or chips inside items** — keep items text-only with optional icon. Tag chips belong to `category-cloud`.
...
10. **Verification** — `pnpm tsc --noEmit` + `pnpm lint` + `pnpm build` clean (1 pre-existing card-tree warning OK; no new); SSR `HTTP 200` for `/components/article-meta` with all demo tabs rendered.

## Open questions

None. Decisions are clear from source + sibling conventions (author-card, thumbnail-list).
