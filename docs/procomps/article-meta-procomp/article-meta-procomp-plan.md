# article-meta — procomp plan

> Stage 2: how. Implementation blueprint for [`article-meta-procomp-description.md`](./article-meta-procomp-description.md).

## File map

```
src/registry/components/data/article-meta/
├── article-meta.tsx         # root, memoized
├── parts/
...
export const ARTICLE_META_DUMMY: ReadonlyArray<ArticleMetaItem> = [
...
export const ARTICLE_META_DUMMY_CENTERED: ReadonlyArray<ArticleMetaItem> = [
  { id: "author", icon: User, value: "Daniel Park" },
  { id: "date", icon: Calendar, value: "March 15, 2026" },
];

export const ARTICLE_META_DUMMY_DOCS: ReadonlyArray<ArticleMetaItem> = [
...
export const ARTICLE_META_DUMMY_VIDEO: ReadonlyArray<ArticleMetaItem> = [
...
export const ARTICLE_META_DUMMY_TEXT_ONLY: ReadonlyArray<ArticleMetaItem> = [
...
3. `pnpm build` clean — `/components/article-meta` prerendered
...
6. Registry artifacts at `public/r/article-meta.json` + `public/r/article-meta-fixtures.json`

## Risks / known unknowns

1. **Items array reference instability** — if the consumer creates `items={[...]}` inline on every render, memoization defeats. Document in guide; same caveat as `thumbnail-list`.
