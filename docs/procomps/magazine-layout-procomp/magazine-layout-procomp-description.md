# magazine-layout — procomp description

> Stage 1.
>
> **Migration origin:** [`docs/migrations/magazine-layout/`](../../migrations/magazine-layout/) — kasder `NewsMagazineGrid.tsx`. The layout assembly for the news-domain family.
...
- **Generic over item type** — `<MagazineMagazineLayout<T>>` with `renderItem(item, slot)` callback.
...
<MagazineLayout<NewsItem>
  ...
  renderItem={(item, slot) => (
    <NewsCard item={item} variant={slot} ... />
  )}
/>
```
...
- Generic typing flows through correctly (`<MagazineMagazineLayout<T>>` narrows `renderItem`'s `item` to `T`).
