...
- **Tree-structured content** — use `card-tree`.
...
<MagazineLayout<Article>
  hero={<PageHero badge="News" title="Latest" description="..." />}
  filterBar={
    <FilterBar
...
Always one of those two. The layout never asks for `featured` / `small` / `list` slots — those are news-card specifics. If your card maps differently, use a switch in `renderItem`:
...
```tsx
<MagazineLayout sidebarClassName="sticky top-16" ... />
```
...
The original's filter logic (search debounce / category chips / date range) moved entirely into `filter-bar`. The sidebar contents (popular news / categories / newsletter) became `category-cloud` + `newsletter-signup` — slot composables. The result: layout is ~120 lines + a 60-line filter hook, vs. the source's ~320-line monolith.

Originals at [`docs/migrations/magazine-layout/original/`](../../migrations/magazine-layout/original/).
