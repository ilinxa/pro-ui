...
export interface MagazineLayoutProps<T> {
  ...
  renderItem: (item: T, slot: MagazineLayoutItemSlot) => ReactNode;
...
```
src/registry/components/layout/magazine-layout/
├── magazine-layout.tsx        # 1
├── parts/
...
### 1. `magazine-layout.tsx` — root composer
