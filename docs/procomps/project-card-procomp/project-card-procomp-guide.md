# project-card — procomp guide

> Stage 3: how to use it. Authored alongside the implementation.
>
> See [`project-card-procomp-description.md`](./project-card-procomp-description.md) for *why* and [`project-card-procomp-plan.md`](./project-card-procomp-plan.md) for *how*.
...
- **Projects with a time-window state machine** — if status auto-derives from start-date / end-date / completion-percentage, you want the kind of kernel `event-card` ships. `project-card` status is editorial only (set on the data object).
- **Project detail pages** — the full case-study page with goals + gallery + meta + external link belongs to the future `project-detail-dialog-01` (composes `info-list` + `expandable-text` + `media-carousel` + `engagement-bar`).
...
The card itself is the only NEW pro-comp on this page — `page-hero`, `filter-bar`, and `magazine-layout` (with its built-in `useMagazineFilter` hook) cover the rest.
...
// Next.js
import NextLink from "next/link";
<ProjectCard ... linkComponent={NextLink} />

// Remix
import { Link as RemixLink } from "@remix-run/react";
<ProjectCard ... linkComponent={RemixLink} />

// react-router v6+
import { Link as RRLink } from "react-router-dom";
<ProjectCard ... linkComponent={RRLink} />
...
- **Sibling cards in the registry:**
  - [`event-card`](../../../src/registry/components/data/event-card/) — events with time-window state machine + capacity bar
  - [`news-card`](../../../src/registry/components/data/news-card/) — editorial content cards (5 variants, `--font-serif` titles)
- **Compose with:**
  - [`page-hero`](../../../src/registry/components/marketing/page-hero/) — page hero band
  - [`filter-bar`](../../../src/registry/components/forms/filter-bar/) — category-pill filter row
  - [`magazine-layout`](../../../src/registry/components/layout/magazine-layout/) — magazine grid + infinite scroll + `useMagazineFilter`
- **Future siblings (deferred):**
  ...
  - `project-detail-dialog-01` (data) — sequential prev/next viewer composing `info-list` + `expandable-text` + `media-carousel` + `engagement-bar`
- **Migration source:** [`docs/migrations/project-card/`](../../migrations/project-card/)
