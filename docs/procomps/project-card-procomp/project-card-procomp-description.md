# project-card — procomp description

> Stage 1: what & why. The "should we build this at all?" doc.
>
> **Migration origin:** [`docs/migrations/project-card/`](../../migrations/project-card/) (kasder `kas-social-front-v0` `ProjectCard.tsx` + `BusinessProjectsSection.tsx`)
>
> **Locked-in intake decisions:** [`source-notes.md` § Locked-in decisions](../../migrations/project-card/source-notes.md#locked-in-decisions-signed-off-2026-05-03). Extraction analysis: [`analysis.md`](../../migrations/project-card/analysis.md).
>
> Family: completes the **portfolio / case-study card pattern** alongside `news-card` (editorial) and `event-card` (registrations). Designed to compose with `page-hero` + `filter-bar` + `magazine-layout` for the public-page assembly, and with the future `bento-grid-01` (deferred) for embedded mosaic widgets. Future siblings — `bento-grid-01` (layout, generic), `project-detail-dialog-01` (data, sequential viewer composing `info-list` + `expandable-text` + `media-carousel` + `engagement-bar`).
...
- **Public status kernel** — `PROJECT_STATUS_CONFIG` constant map exported alongside `ProjectCard`. Consumers can read the same color / label / icon map for filter rows, status legends, sidebars, count summaries — without rendering the card.
...
- **Overlay-link pattern from day 1** — clickable whole-card surface that doesn't lock out nested interactives (bookmark, share, save-to-favorites, external-link). Same pattern as `event-card` + `news-card`.
...
- **No new design-system tokens** — uses existing `--primary` / `--accent` / `--muted` only. Differs from `event-card` migration which added `--warning`.
- **No new shadcn primitives** — uses existing `badge` / `button`. Differs from `event-card` which added `progress`.
...
- **Layout orchestration** — composing cards into grids, bento mosaics, infinite scroll, search/filter. Defer to existing [`magazine-layout`](../../../src/registry/components/layout/magazine-layout/) (public-page assembly) + future `bento-grid-01` (embedded-widget mosaic, deferred to its own greenfield procomp gate since it's generic — not project-specific).
- **Detail dialog / sequential viewer** — the prev/next chevron modal with goals + gallery + external-link from `BusinessProjectsSection`. Defer to future `project-detail-dialog-01` (data, deferred). It composes 4 already-shipped pro-comps + a `story-viewer`-style prev/next state machine.
...
- **Public projects page orchestrator** — composed by consumer from existing `page-hero` + `filter-bar` + `magazine-layout` + this component. Registry doesn't ship "page templates."
- **Featured layout treatment** — bigger card, separate slot above the grid (mirrors `news-card`'s `featuredItem` + `renderFeatured` integration with `magazine-layout`). The card flag is set; the layout-level treatment is the consumer's choice.
...
These are exported alongside `ProjectCard` from the package — consumers can use them WITHOUT rendering the card:
...
import {
  ...
  type ProjectCardVariant, // 'grid' | 'feature'
} from '@ilinxa/project-card';
...
The kernel is **smaller than event-card's** because there are no derivation helpers — status is editorial, and `year` is a free-form string. `PROJECT_STATUS_CONFIG` + the 4 types are the entire public surface beyond the component itself. Smaller surface = smaller breaking-change footprint long-term.
...
```tsx
import { PageHero } from '@ilinxa/page-hero';
import { FilterBar } from '@ilinxa/filter-bar';
import { MagazineLayout, useMagazineFilter } from '@ilinxa/magazine-layout';
import { ProjectCard } from '@ilinxa/project-card';
...
```tsx
import { PROJECT_STATUS_CONFIG, type ProjectStatus } from '@ilinxa/project-card';
...
- SSR `/components/project-card` returns 200 with all 5 demo tabs rendered.
...
1. **Component export name.** `ProjectCard` — matches the `<Slug><NN>` convention already established by `EventCard`, `NewsCard`, etc. **Resolved:** `ProjectCard`.
...
5. **Title font — sans or serif?** **Resolved:** sans (default `--font-sans`, Onest). Projects feel like utility/portfolio surfaces, not editorial features. Differs from `news-card` (which uses `--font-serif`), matches `event-card`.
6. **`feature` variant featured treatment** — `border-t-4` doesn't work on the full-bleed image. **Resolved (analysis):** `ring-2 ring-primary ring-inset`. Visible against image, no layout shift, mirrors `event-card`'s feed-variant treatment.
7. **`actions` slot positioning per variant.** Grid: top-right of image area when supplied (pushes category pill to bottom-right). Feature: top-right of content overlay area (pushes category pill to bottom-right). **Resolved:** documented; visual contract preserved per `event-card` precedent.
...
12. **Loading skeleton.** Should the card ship a `<ProjectCard.Skeleton variant="grid" />`? **Open** — v0.1 ships without; consumers render a `bg-muted` placeholder. v0.2 candidate if real demand surfaces.

---

**Stage 1 contract:** if you sign off on the above, the plan doc (`project-card-procomp-plan.md`) will lock down: file shape, type definitions, prop/state/effect/render flow per variant, status-config map, demo plan (5 tabs), success-gate verification steps, and known plan deviations to flag during implementation.
