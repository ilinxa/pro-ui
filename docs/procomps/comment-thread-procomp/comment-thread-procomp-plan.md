# comment-thread — procomp plan

> Stage 2: how. The implementation contract.
>
> See [`comment-thread-procomp-description.md`](./comment-thread-procomp-description.md) for the what & why.
>
...
> - [`expandable-text`](../expandable-text-procomp/) — long comment bodies
> - [`engagement-bar`](../engagement-bar-procomp/) `variant="compact"` — per-row like action
...
| Q-P6 | `Subscribe` re-subscription behaviour on identity change? | **Clean teardown + re-call** (engagement-bar carry-over). No debouncing. Hosts must memoize `subscribe` via `useCallback`. Hook uses `controlledRef` + `onSubscribeDeltaRef` mirrors so the subscription effect re-runs ONLY on `subscribe` identity change. |
...
| Q-P9 | `onLikeComment` signature? | **`(commentId, nextLiked: boolean)`.** Symmetric with `engagement-bar`'s `onToggle(next)`. Reducer flips local state first; callback fires with the new value. |
...
| Q-P15 | Description's "hybrid controlled/uncontrolled" carry-over from engagement-bar — keep or walk back? | **Walk back.** The description's pre-emptive lock was an over-eager copy from engagement-bar. For a recursive stateful tree, "echoes prop on every render = controlled" is brittle (prop identity churn from spread, deep equality cost, hidden re-syncs). **Plan locks: always uncontrolled.** `comments` prop = initial state on mount only; subsequent prop reference changes are IGNORED. Hosts wanting external state sync use the imperative handle's `reset(next: Comment[])` method or `dispatch(action: CommentLocalAction)` directly. Documented loudly in usage.tsx — non-trivial footgun for hosts coming from engagement-bar. |
...
| Q-P19 | `<ExpandableText>` `maxLines` per row? | **`maxLines = 4` in `default` variant; `maxLines = 2` in `compact` variant.** Both configurable via `bodyMaxLines?: number` on the thread root. |
...
- **Cross-folder imports declared via `registryDependencies`.** `comment-thread.tsx` and `comment-node.tsx` import `<ExpandableText>` + `<EngagementBar>` from sibling sealed folders. `registry.json` declares `registryDependencies: ["expandable-text", "engagement-bar"]` so installs auto-pull siblings. Refines the project sealed-folder rule for the second time after `media-carousel → video-player`.
...
- **`<CommentThread>` is `"use client"`** — owns thread state, imperative handle, subscription effect, replyParentId state.
...
- **Realtime contract identical to `engagement-bar`'s.** `Subscribe<TDelta>` shape; `controlledRef` + `onSubscribeDeltaRef` mirror pattern in `useCommentState`. Subscription effect re-runs only on `subscribe` identity change.
- **Refs must not be written during render.** Mirror via passive `useEffect`. (Compiler-aware lint catches it; engagement-bar set the precedent.)
...
- **Locked target convention** for `registry.json`: every file `type: "registry:component"` (no CSS in v0.1, so no `registry:file` precedent here), `target: "components/comment-thread/<sub-path>"`. Never ship `demo.tsx`, `usage.tsx`, `meta.ts`. `dummy-data.ts` ships only via `comment-thread-fixtures` sibling item.
...
```ts
// src/registry/components/data/comment-thread/hooks/use-comment-state.ts (PUBLIC EXPORT)
...
```ts
// src/registry/components/data/comment-thread/parts/comment-composer.tsx (PUBLIC EXPORT)
...
```ts
// src/registry/components/data/comment-thread/hooks/use-autosize-textarea.ts (PUBLIC EXPORT)
...
/** Coerces Date | string | number into a Date — same convention as event-card. */
export function toDate(value: Date | string | number): Date;

/**
 * Default English relative-time formatter for comments. Tighter granularity
 * than news-card's day-level formatter:
 *   < 1 min     → "Just now"
 *   < 60 min    → "5m"
 *   < 24 hours  → "2h"
 *   < 7 days    → "3d"
...
```
src/registry/components/data/comment-thread/
├── comment-thread.tsx                    # 1 — root component
├── parts/
...
(11 files ship via the registry: `comment-thread.tsx`, the 5 parts, the 2 hooks, `format-time.ts`, `types.ts`, `index.ts`. `dummy-data.ts` ships only via `comment-thread-fixtures`. `demo.tsx` / `usage.tsx` / `meta.ts` are docs-site only.)

### 1. `comment-thread.tsx` — root

- `"use client"` directive.
- `React.memo`, `forwardRef<CommentThreadHandle, CommentThreadProps>` at export.
...
- Renders:
  ...
  ```
- Tiny — but a sealed `parts/` file keeps `comment-thread.tsx` clean.
...
- **Subscription effect** runs only on `subscribe` identity. `onSubscribeDelta` and tree state are read via refs (`onDeltaRef`) / closure (dispatch is stable) so changes don't trigger re-subscribe. Same shape as engagement-bar's `use-engagement-state.ts`.
...
- Tighter granularity than `news-card`'s formatter (comments need minute / hour / day; news cards only need day-level).
...
export const DUMMY_FLAT_COMMENTS: Comment[] = [
  /* 8 flat top-level comments — varied ages (just-now / 5m / 2h / 3d / 2w / abs); some long bodies that trigger expandable-text truncation */
];
...
export const meta: ComponentMeta = {
  ...
  version: "0.1.0",
  description:
    "Recursive comment tree with composer + realtime. Composes expandable-text (long bodies) + engagement-bar compact (per-row like).",
  tags: ["social", "comments", "thread", "realtime", "composer", "recursive"],
  added: "2026-05-02",
  dependencies: {
    peer: ["lucide-react"],
    registryDeps: ["expandable-text", "engagement-bar"],
...
Already covered. Match `engagement-bar` and `media-carousel` patterns.
...
Two items: base (`comment-thread`) + fixtures (`comment-thread-fixtures`).
...
```jsonc
{
  ...
  "description": "Recursive comment tree with composer + realtime. Composes expandable-text + engagement-bar (compact).",
  ...
  "files": [
    { "path": "src/registry/components/data/comment-thread/comment-thread.tsx", "type": "registry:component", "target": "components/comment-thread/comment-thread.tsx" },
    { "path": "src/registry/components/data/comment-thread/parts/comment-node.tsx", "type": "registry:component", "target": "components/comment-thread/parts/comment-node.tsx" },
    { "path": "src/registry/components/data/comment-thread/parts/comment-composer.tsx", "type": "registry:component", "target": "components/comment-thread/parts/comment-composer.tsx" },
    { "path": "src/registry/components/data/comment-thread/parts/view-replies-link.tsx", "type": "registry:component", "target": "components/comment-thread/parts/view-replies-link.tsx" },
    { "path": "src/registry/components/data/comment-thread/parts/comment-empty-state.tsx", "type": "registry:component", "target": "components/comment-thread/parts/comment-empty-state.tsx" },
    { "path": "src/registry/components/data/comment-thread/parts/comment-kebab.tsx", "type": "registry:component", "target": "components/comment-thread/parts/comment-kebab.tsx" },
    { "path": "src/registry/components/data/comment-thread/hooks/use-autosize-textarea.ts", "type": "registry:component", "target": "components/comment-thread/hooks/use-autosize-textarea.ts" },
    { "path": "src/registry/components/data/comment-thread/hooks/use-comment-state.ts", "type": "registry:component", "target": "components/comment-thread/hooks/use-comment-state.ts" },
    { "path": "src/registry/components/data/comment-thread/lib/format-time.ts", "type": "registry:component", "target": "components/comment-thread/lib/format-time.ts" },
    { "path": "src/registry/components/data/comment-thread/types.ts", "type": "registry:component", "target": "components/comment-thread/types.ts" },
    { "path": "src/registry/components/data/comment-thread/index.ts", "type": "registry:component", "target": "components/comment-thread/index.ts" }
  ]
}
```

`comment-thread-fixtures` ships only `dummy-data.ts`, depends on `comment-thread`.

**Verify before ship:** the existing `engagement-bar.json` and `expandable-text.json` artifacts on Vercel resolve correctly when `pnpm dlx shadcn add @ilinxa/comment-thread` is called from a tmp consumer. (Cross-folder dependency precedent already exists from `media-carousel → video-player`; this is the second time. Should just work.)
...
1. **Build verification**
   ...
   - `pnpm build` — clean prerender (route `/components/comment-thread` added)
   - `pnpm registry:build` — generates `public/r/comment-thread.json` + `public/r/comment-thread-fixtures.json`. Spot-check `files[]` count = 11, all `type: "registry:component"`, all `target: "components/comment-thread/<sub>"`. Spot-check `registryDependencies` declares both `expandable-text` and `engagement-bar`.
...
6. **Smoke test from a tmp consumer (one-off)** — `pnpm dlx shadcn@latest add http://localhost:3000/r/comment-thread.json` from a tmp Next 16 app; verify (a) all 11 files land at correct targets, (b) `expandable-text` and `engagement-bar` siblings auto-pull, (c) the composed thread renders + a like flips state.
...
| Cross-folder import causes shadcn CLI to fail when installing | Low | High | `media-carousel → video-player` already proves the precedent works. Smoke-test step 6 verifies. |
| `<EngagementBar>` per-row controlled mode + thread reducer drift | Medium | Medium | Q-P12 lock makes per-row bar always-controlled. Click handler in like-action.tsx fires `onToggle` only (no local flip in controlled mode — engagement-bar's own plan §3 confirms). Single source of truth: thread reducer. If it drifts, the bug is in engagement-bar's controlled-mode short-circuit, not here. |
...
4. **Recursive node** — `parts/comment-node.tsx`. Compose `<ExpandableText>` + `<EngagementBar>` + `<CommentKebab>` + recursive `<CommentNode>` for replies. Implement `expandedToDepth` local state for view-N-replies inline expand.
...
7. **Root component** — `comment-thread.tsx` + `useImperativeHandle`.
...
13. **Draft `comment-thread-procomp-guide.md`** (Stage 3, alongside this).
