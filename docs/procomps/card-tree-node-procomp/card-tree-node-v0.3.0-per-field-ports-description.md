# `card-tree-node` v0.3.0 — Per-Field Ports + Custom Port-Type Picker Description (Stage 1)
...
> **Slug:** `card-tree-node` (additive features; sibling exports + prop widening, no breaking changes)
> **Target version:** `card-tree-node@v0.3.0` (minor bump — adds public surface)
> **Dependencies bumped:** **none anticipated** (Q1 lock (a) rcif-side `CardTreePort` subtype + Q2 lock (a) edge-with-hint rendering = zero flow-canvas changes. Q-O5 confirms this is a pure rcif version bump unless user overrides Q1 or Q2 to options that require host help).
...
`card-tree-node@v0.2.0` shipped opt-in `PortEditorStrip` with per-card + per-subcard ports. Two follow-ups carried forward from the v0.2 spotcheck:
...
│                                                                     │
│  ┌──────────────────────────────────────┐                           │
│  │ CardTreeViewer (rendered NodeRenderer│                           │
│  │                                                                  │
│  │  ┌──────────────────────────────┐                                │
...
└─────────────────────────────────────────────────────────────────────┘
  ...
  Data contract change (rcif-side ONLY; flow-canvas untouched):
    type CardTreePort = Port & { fieldKey?: string }    ← new rcif export
    ...
    flow-canvas's Port stays unchanged: a CardTreePort assigns to Port
...
2. **card-tree stays port-agnostic.** Same as v0.1/v0.2 — card-tree's editor strips ports out of its own view. v0.3 doesn't touch card-tree.
3. **flow-canvas stays portType-neutral.** `fieldKey` lives in the port record; flow-canvas treats it as opaque metadata. Edge validation (same-type-only, dir, multi) unchanged. **One potential exception:** if Q-O1 lands as "render at field row," flow-canvas may need to surface a helper for per-element handle coordinates — that's a separate v0.2.6 patch evaluated at GATE 2 plan, NOT v0.3 scope.
...
- **Port record widening (rcif-side ONLY):** new `CardTreePort = Port & { fieldKey?: string }` type exported from rcif's types.ts. flow-canvas's `Port` stays unchanged — host portability honored per CLAUDE.md mandate. Card-level ports = `fieldKey` absent (assigns to plain `Port`); field-level = `fieldKey` set to one of the card's flat-field keys (rcif narrows back to `CardTreePort` when reading).
...
- **Visual rendering of field-level ports** in CardTreeViewer — see Q-O1 for the architectural choice between "inline at field row" vs "card edge with field hint."
...
**Options:**
- (a) **rcif subtype** — `type CardTreePort = Port & { fieldKey?: string }` exported from rcif's types.ts. flow-canvas's `Port` stays untouched. rcif viewer + editor narrow to `CardTreePort` when reading; flow-canvas sees a plain `Port` (TS-structurally accepted via subtype). Single array per card. Existing card-level ports stay valid (assign as plain `Port`).
- (b) **Widen flow-canvas's `Port`** — add `fieldKey?: string` to the host type. Single source of truth, but bleeds rcif-specific knowledge into a portable host (violates CLAUDE.md portability mandate — flow-canvas should stay card-tree-agnostic).
- (c) **Side-table on CardTreeJsonNode** — add a separate `fieldPortMap?: Array<{ portId: string; fieldKey: string }>` lookup on each card. Keeps port records pristine; doubles bookkeeping surface (mutators must keep two arrays consistent; orphan-on-port-delete risk).
- (d) **Nested `ports[]` on each flat field's data** — breaks the card-tree-side contract that fields are pure data; card-tree would suddenly need to know about ports.

**Lock (proposed):** **(a) `CardTreePort = Port & { fieldKey?: string }` subtype on rcif side.** Keeps flow-canvas untouched (no v0.2.6 host patch needed for the data shape). Single array per card. Subtype narrowing is TS-native and zero-runtime-cost. (b) violates host portability. (c) doubles the bookkeeping + creates orphan-port-on-mutation footguns. (d) breaks card-tree's port-agnostic contract. **Side benefit:** Q-O5 below becomes a confirmed "no flow-canvas patch needed" — v0.3 is a pure rcif version bump.
...
**Lock (proposed):** **(a) card edge with field-hint** for v0.3. Rationale: (b) requires per-field DOM measurement + a flow-canvas helper that doesn't exist today (`getHandleCoords(fieldKey)`); too much scope for v0.3. (a) ships with zero flow-canvas changes, keeps the existing `<PortsAt>` rendering pipeline. (c) is YAGNI in v0.3 — if (b) becomes a real consumer ask, add it as `fieldPortLayout?` in v0.4 as a non-breaking widening.
...
### Q-O2 — What happens to a field-level port when its target field gets removed / renamed in card-tree?
...
**Assistant recommends:** **(a) warning, don't auto-fix.** Matches the same "consumer-policy" thinking as edge-on-port-rename (v0.2). Auto-fix is too opinionated for a primitive library; consumers can implement their own auto-rename via their `card-tree` change hook.

### Q-O3 — Per-field ports on subcards

**Question:** Subcards have their own flat fields (each subcard is itself a card-tree). Does v0.3 support per-field ports on subcard fields too?
...
### Q-O5 — Does v0.3 need a flow-canvas patch?

**Question:** Does any v0.3 work require an additive change to flow-canvas?

**Assistant analysis (post-Q1-refinement):**
- **Data shape** — Q1 lock (a) `CardTreePort` subtype keeps the type widening rcif-side. Zero flow-canvas change.
- **Visual rendering** — Q2 lock (a) edge-with-hint uses existing `<PortsAt>` pipeline. Zero flow-canvas change.

**Confirmed: zero flow-canvas changes for v0.3.** Pure rcif version bump (no Workstream A; no v0.2.6 patch).

If user overrides Q2 to (b) inline rendering, the flow-canvas work becomes a v0.2.6 patch evaluated at GATE 2 (adds a helper for per-field handle coordinates). If user overrides Q1 to (b) widen-host-Port, that's a v0.3.0 flow-canvas minor bump on the same plan as a Workstream A.
...
|---|---|---|---|
| `CardTreePort` type (rcif export) | absent | `Port & { fieldKey?: string }` | No (new export; existing `Port` consumers unaffected) |
...
| `defaultPortTypes` re-export from rcif barrel | absent | re-exports from flow-canvas for convenience | Additive |
...
- [ ] Q2 (visual rendering) — confirm (a) edge-with-hint, OR override to (b) inline + accept v0.2.6 flow-canvas patch
...
- [ ] Q-O5 (no flow-canvas patch with Q2=(a)) — confirm
