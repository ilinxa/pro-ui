# `card-tree-node` v0.2.0 — Stage 2 Plan (Port Editor)
...
> **Inputs:** Stage 1 description signed off 2026-05-17 ([card-tree-node-v0.2.0-port-editor-description.md](card-tree-node-v0.2.0-port-editor-description.md)). All 12 Qs + Q5-bis + Q-O1/2/3/4 locked.
> **What this plan covers:** the **two coordinated workstreams** that ship the port editor — `flow-canvas@v0.2.5` (1-line `defaultPortTypes` patch) AND `card-tree-node@v0.2.0` (new `<PortEditorStrip>` export + supporting modules + demo + usage). Sequenced: flow-canvas patch first, rcif minor second.
...
|---|---|---|---|---|
| **A — Built-in port type patch** | `flow-canvas` | `0.2.4 → 0.2.5` | Add `"doc"` to `defaultPortTypes` (1 line) | ~10 min |
| **B — Port editor procomp** | `card-tree-node` | `0.1.0 → 0.2.0` | New `<PortEditorStrip>` + parts + lib + demo wiring + usage + registry distribution | ~4–6 hours |
...
### F-01 — Port shape stays `Port` from `@/registry/components/data/flow-canvas/types`

No port shape extension in v0.2. The strip reads/writes exactly the `Port` shape that flow-canvas already publishes: `{ id, side, dir, type, multi?, label? }`. **No new fields, no `boundField`, no `role`.** Per-field ports (Q-O4 deferred) would later add `boundField?: string` — that's v0.3 work.
...
type PortTarget = {
  node: NodeRecord;
  cardData: CardTreeJsonNode;  // either node.data root, or a nested subcard by __rcid
...
`shortUuid` is `crypto.randomUUID().slice(0, 8)` — matches the 8-char length used by `makeEdgeId` / `makeNodeId` in [`flow-canvas/hooks/use-canvas-data.ts:34-45`](../../../src/registry/components/data/flow-canvas/hooks/use-canvas-data.ts#L34-L45). Falls back to `Math.random().toString(36).slice(2, 10)` in non-crypto environments per F-S6 below (also matches flow-canvas fallback length).
...
The row owns its own inline-edit state (which field is being edited) via local `useState` — these are uncontrolled edits committed on blur, matching card-tree's field-edit pattern. The strip itself is uncontrolled-by-data; rows are uncontrolled-by-field-state. Two-level model is intentional and matches card-tree precedent.
...
When `editable={false}` is passed, the strip renders read-only port summary rows (no add button, no row edit affordances, no remove buttons) — analogous to card-tree's `editable={false}` mode. Consumer can hide the strip entirely by simply not mounting it.

### F-09 — Cross-procomp imports use RELATIVE paths

Per F-S1 lock (rcif v0.1.0). All cross-procomp imports in shipped source use relative paths — same-category siblings (card-tree, flow-canvas) are subject to shadcn's path-rewriter substituting the current slug for the target slug. Specifically:

**From flow-canvas:**
- `import type { Port, PortType, CanvasData, NodeRecord } from "../flow-canvas/types"` ✓ relative
- `import { updateNodeData } from "../flow-canvas/lib/update-node-data"` ✓ relative
- `import { defaultPortTypes } from "../flow-canvas/registries/port-type-registry"` ✓ relative

**From card-tree:**
- `import type { CardTreeJsonNode } from "../card-tree/types"` ✓ relative (referenced by `PortTarget.cardData` in F-02; also reused throughout the strip for type narrowing)
...
### F-10 — Demo wires the strip above `<CardTree>` per Q1 lock
...
Strip operates on `canvas` directly (not on `editingTree`) — port mutations bypass card-tree's data path entirely, so rcif's `stripFlowCanvasFields` / `mergeFlowCanvasFields` helpers are unchanged.
...
All target convention: `target: "components/card-tree-node/<sub-path>"`, type `registry:component`. Demo / usage / meta stay docs-site-only (per the rule). Fixtures sibling item (`card-tree-node-fixtures`) needs no changes — `dummy-data.ts` already carries ports in the fixture.
...
- shadcn deps: **was empty in v0.1; v0.2 adds six primitives** — `popover` (add-popover + tooltips on warnings), `select` (type + side + dir pickers), `checkbox` ([✓in][✓out] in add-popover), `input` (id + label inline edits), `tooltip` (rename-warning + orphan-doc tooltip), `label` (form field labels). Final array: `["popover", "select", "checkbox", "input", "tooltip", "label"]`. All six get auto-installed via the registry pipeline when consumer runs `pnpm dlx shadcn@latest add @ilinxa/card-tree-node`.
- internal deps: `["card-tree", "flow-canvas"]` (unchanged — same as v0.1)
- npm deps: unchanged (Port shape already imported via flow-canvas; no new third-party packages)
...
### Workstream A — `flow-canvas@v0.2.5`

| File | Change |
|---|---|
| `src/registry/components/data/flow-canvas/registries/port-type-registry.ts` | Add `{ id: "doc", color: "var(--chart-3)", label: "Doc" }` to `defaultPortTypes` array — single line in the array literal |
| `src/registry/components/data/flow-canvas/meta.ts` | Bump `version: "0.2.5"`, `updatedAt: "2026-05-17"`; add `"doc"` to the features list (no API change, but useful for discoverability) |
| `public/r/flow-canvas.json` | Regen via `pnpm registry:build` |
...
### Workstream B — `card-tree-node@v0.2.0`
...
| `demo.tsx` | Mount `<PortEditorStrip>` above `<CardTree>` in the dialog (per F-10) |
...
| File | Change |
|---|---|
| `registry.json` | Add 5 new files to `card-tree-node` base item, all `type: "registry:component"` (per F-12) |
| `public/r/card-tree-node.json` | Regen via `pnpm registry:build` |

---

## 5. Implementation sequencing

**A1 — flow-canvas@v0.2.5 patch** (single commit):
...
6. Commit: `fix(flow-canvas): v0.2.5 — add "doc" built-in port type to defaultPortTypes`

**A2 — flow-canvas tracking** (single commit; lands AFTER B5 in the same overall push batch — both A2 and B5 commits go to origin together so deployed Vercel artifacts are consistent):
...
4. tsc clean
5. Commit: `feat(card-tree-node): v0.2.0 B1 — scaffold port editor types + lib helpers`
...
5. Commit: `feat(card-tree-node): v0.2.0 B2 — PortEditorStrip + Row + AddPopover`

**B3 — Demo + usage + barrel + guide** (single commit):
1. Modify `demo.tsx` (mount strip above CardTree)
...
5. Author new §"Port editor" section in `docs/procomps/card-tree-node-procomp/card-tree-node-procomp-guide.md` (PV5 — guide-section authoring lives here, NOT B5)
...
9. Commit: `feat(card-tree-node): v0.2.0 B3 — demo + usage + barrel + meta + guide`

**B4 — Registry + smoke** (single commit):
1. Modify `registry.json` (add 5 files to rcif base item)
2. `pnpm registry:build`
3. Smoke harness path-b run — F-S5 lock; install rcif into `e:/tmp/ilinxa-smoke-consumer/` from local-served `public/r/card-tree-node.json`, run consumer-side `pnpm tsc --noEmit`
4. Fix any F-S surfacing (relative-import fixes per F-S1 if rewriter trips)
5. Commit: `feat(card-tree-node): v0.2.0 B4 — registry distribution + smoke run`

**B5 — GATE 3 spot-check + tracking** (single commit; guide moved to B3 per PV5):
1. Author `docs/procomps/card-tree-node-procomp/reviews/2026-05-17-v0.2.0-spotcheck.md`
...
4. Author `.claude/decisions/2026-05-17-card-tree-node-v0.2.0-port-editor.md`
5. STATUS.md row + Recent activity + Last-updated
6. component-versions.md table row + highlights
7. Commit: `docs(tracking,card-tree-node): v0.2.0 SHIPPED — STATUS + decision + spot-check + component-versions`
...
| `findPortTarget` walker fails on card-tree-internal `__rcid` formats vs consumer-defined formats | Low | Walker uses `===` on `__rcid` string only; format-agnostic (card-tree auto-attaches but doesn't normalize) |
