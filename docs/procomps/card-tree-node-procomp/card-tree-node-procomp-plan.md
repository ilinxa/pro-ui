...
> **Inputs:** Stage 1 description signed off 2026-05-16 ([card-tree-node-procomp-description.md](card-tree-node-procomp-description.md), commit `4a9b5a3`). All 10 Qs locked. flow-canvas currently at **v0.2.0** (commits `a6b3295`..`6587ef6`, shipped 2026-05-16) — the `onEditRequest` API has NOT yet landed; per Q5 it ships as `v0.2.1` patch, sequenced before card-tree-node@v0.1.0.
> **What this plan covers:** the **two coordinated workstreams** that together unlock card-tree content inside flow-canvas nodes — `flow-canvas@v0.2.1` (small additive API patch) AND `card-tree-node@v0.1.0` (new procomp). The two ship sequentially in close succession; v0.2.1 first, card-tree-node second.
...
`card-tree-node@v0.1.0` is the canonical implementation of the popup-edit renderer convention locked in `flow-canvas@v0.2.0` perf description Q33. Architecture: a read-only `CardTreeViewer` renderer paints inside each flow-canvas node (~150–180 LoC; title + first 3 flat fields + nested-card outlines with their own ports + selectability + click affordances at root and subcard levels); clicking fires `ctx.onEditRequest?.(subPath?: string)` which bubbles up through `FlowCanvasProps.onEditRequest?: (nodeId, subPath?) => void`; the consumer's dialog mounts the full `<CardTree editable={true}>` editor with the same `CardTreeJsonNode` data (no transformation layer); card-tree's `onChange` writes back to canvas state live (Q2 lock — every keystroke; card-tree owns undo). At most ONE card-tree editor instance mounted at any time, regardless of node count.

The `subPath?: string` argument (Q1 lock — subcards with their own ports + selectability) carries the clicked subcard's `__rcid` so the consumer can pre-focus the dialog via `CardTreeHandle.focusCard(subPath)` (imperative ref method).
...
|---|---|---|---|---|
| **A — Host API patch** | `flow-canvas` | `0.2.0 → 0.2.1` | Add `onEditRequest` (both surfaces) + optional `updateNodeData` helper export | ~1 hour |
| **B — Viewer procomp** | `card-tree-node` (new) | `0.1.0 first ship` | New sealed folder + viewer renderer + demo + usage + registry distribution | ~4–6 hours |
...
**Reality (verified):** [card-tree's `CardTreeJsonNode`](../../../src/registry/components/data/card-tree/types.ts) uses `__rcid?: string` (UUID-like) as the canonical card identifier — auto-attached by card-tree on parse. There is no path-like addressing surface in card-tree's public API; `CardTreeHandle.focusCard(id: string)` takes the `__rcid` directly.

**Lock:** the `subPath` argument is the clicked subcard's `__rcid` value (a string UUID). The viewer reads `subcard.__rcid` from the rendered subcard's data and fires `ctx.onEditRequest?.(subcard.__rcid)`. Consumers pass this string verbatim to `CardTreeHandle.focusCard(subPath)` via ref. **The `subPath` type stays opaque (`string`) at the flow-canvas API level** — flow-canvas remains card-tree-unaware; the string semantic ("it's an rcid") is card-tree-node's contract.

### F-02 — Consumer dialog wiring uses imperative `focusCard(id)`, not a hypothetical prop

**Description's wording (§5.3):** sample code uses `<CardTree initialFocusPath={editingSubPath} ... />` — a fictional prop.

**Reality (verified):** [`CardTreeProps`](../../../src/registry/components/data/card-tree/types.ts) has no `initialFocusPath` (or `initialFocusCardId` or similar). Focusing a specific card is via [`CardTreeHandle.focusCard(id: string)`](../../../src/registry/components/data/card-tree/types.ts) — imperative method on the ref.

**Lock:** the consumer-side dialog wiring uses `useRef<CardTreeHandle>()` + a `useEffect` that calls `cardTreeRef.current?.focusCard(subPath)` when `subPath` is set. The procomp guide (Stage 3) ships this corrected pattern. **No PR against card-tree needed** — the imperative path works today; adding an `initialFocusCardId?: string` prop to card-tree is a v0.2 polish if the imperative-via-ref ergonomics bite consumers.

### F-03 — Subcard-click-to-edit gracefully degrades when `__rcid` is missing

**Issue:** `__rcid` is OPTIONAL on `CardTreeJsonNode` (`__rcid?: string`). card-tree auto-attaches IDs on parse, but a freshly-constructed `CardTreeJsonNode` (e.g. consumer-defined fixture data) may not carry `__rcid` until it's been through `<CardTree>` once.
...
- Clicking the subcard bubbles to root (`ctx.onEditRequest?.()` with no subPath) — opens the dialog at root; the user navigates within card-tree's tree to find the subcard.

Dev-mode `console.warn` when a viewer encounters a subcard without `__rcid`: *"CardTreeViewer: subcard without __rcid; click-to-focus-this-subcard disabled. Pass the data through `<CardTree>` once or call `card-tree`'s ID-attach helper to fix."*

### F-04 — Subcard enumeration: card-tree uses open-shape, not a fixed `children[]` array

**Issue:** [`CardTreeJsonNode`](../../../src/registry/components/data/card-tree/types.ts) is open-shape (`[key: string]: unknown`). Nested cards live under arbitrary keys with their own `__rcid` — there is no canonical `children[]` array (despite the description's §2 ASCII diagram hinting at one).
...
.react-flow__node                  ← xyflow wrapper (xyflow makes this positioned via transforms)
  └─ NodeShell <div>               ← MUST be position: relative (it already is — v0.2.0 line in node-shell.tsx)
     └─ CardTreeViewer outer <div> ← MUST be position: relative (locked in §5.2 code sketch via `className="relative ..."`)
...
1. **Port IDs unique within the entire node** (flow-canvas's locked Q9 — port-walker assumes uniqueness; collisions return the first match). Dev-mode `console.warn` if a duplicate port ID is detected during viewer mount (cheap one-pass scan).
2. **`CardTreeViewer` outer `<div>` MUST carry `relative` className** — preserved in code reviews. Comment in `card-tree-viewer.tsx` makes it explicit.
...
### F-06 — Editor mode + card-tree version requirement

**Lock:** the dialog mounts `<CardTree editable={true}>` (per Q2's live-save lock — card-tree's v0.2+ editor surface). The procomp declares `card-tree@^0.4.0` as its `meta.ts dependencies.internal` baseline. card-tree is currently at v0.4.1 beta — the requirement is already satisfied. The v0.4 `validators` / `maxUndoDepth` / `onUndo` / `onRedo` props are NOT passed by the procomp guide's default wiring (consumer can pass them through if they want); v0.1 procomp keeps the consumer-side dialog example minimal (~15 lines).
...
**v0.2 escape hatch:** `cardTreeViewerOptions?.editTrigger?: "click" | "doubleClick"` if a consumer surfaces with a real conflict (e.g. they wire their own single-click handler on the canvas and want card-tree edits gated to double-click). Out of v0.1 scope.
...
|---|---|---|---|
| **F-V1** | 🔸 Med | §5.2 (CardTreeViewer impl notes) | The "outer button-role surface" with subcard inner clicks doesn't compose — clicking a subcard inside a button would fire both. Lock: renderer uses a `<div role="group">` at root + per-subcard `<button>` elements + a root `<button>` for the title strip area. NOT one outer button. |
| **F-V2** | 🔸 Med | §6 step 10 | The smoke harness rule for v0.1.0 first ships (per readiness-review rule) applies to **card-tree-node@v0.1.0** — this is a new component. Smoke harness path-b MUST run pre-push. Plan locks this as a required step (NOT skippable like in flow-canvas@v0.2.0's case). |
| **F-V3** | 🔹 Low | §10 Impl-time | `dependencies.internal` in `card-tree-node/meta.ts` declares `["card-tree", "flow-canvas"]`. The `validate-meta-deps` script will verify both cross-registry imports resolve; precedent: `json-form@v0.1.0` was the first procomp to use `dependencies.internal`. The script's "internal-registry slug" detector handles the `@/registry/components/<cat>/<slug>` pattern in source imports. Verify behavior at impl time. |
| **F-V4** | 🔸 Med | §4.3.1 (NEW — `flow-canvas.tsx` wiring) | Plan §4.3 mentioned threading `onEditRequest` through canvas-context "terse" — but the actual wiring needs to land in [`flow-canvas.tsx`](../../../src/registry/components/data/flow-canvas/flow-canvas.tsx) (lines 34–53), which is where the canvas-context `useMemo` is built. Added a dedicated §4.3.1 sub-step documenting the exact edit. The Canvas component itself does NOT need to know about `onEditRequest` (it's context-only, like `renderers`). |
| **F-V5** | 🔸 Med | §4.3.2 (NEW — consumer callback ref-mirror) | The existing pattern in [`use-canvas-data.ts`](../../../src/registry/components/data/flow-canvas/hooks/use-canvas-data.ts) ref-mirrors all consumer callbacks (`onChangeRef`, `onBeforeConnectRef`, etc.) so the canvas's internal callbacks stay stable even when consumers pass fresh function identities each render. `onEditRequest` flows through canvas-context (not `useCanvasData`), so it bypasses this defense — every render where the consumer passes a fresh `onEditRequest` would invalidate the context memo + cause ALL `NodeAdapter`s to re-render. Lock: ref-mirror `onEditRequest` inside `flow-canvas.tsx`; pass a STABLE wrapped callback through to context. Matches the existing project posture (ref-mirror consumer callbacks defensively; don't trust consumer `useCallback`). |
| **F-V6** | 🚫 Blocker | §5.2 (renderer typing) + §5.7 (new `CardTreeCanvasNode` intersection type) + §5.8 (barrel re-export) | The plan originally typed the renderer as `NodeRenderer<CardTreeJsonNode>`. `NodeRenderer<TData extends NodeData = NodeData>` ([flow-canvas/types.ts:60](../../../src/registry/components/data/flow-canvas/types.ts)) constrains `TData` to extend `NodeData` ([flow-canvas/types.ts:19](../../../src/registry/components/data/flow-canvas/types.ts)), which requires `__type: string`. `CardTreeJsonNode` ([card-tree/types.ts:49](../../../src/registry/components/data/card-tree/types.ts)) has only optional `__rcid?`/`__rcorder?`/`__rcmeta?` + index signature — `__type` is NOT required. tsc would error: *"Property '__type' is missing in type 'CardTreeJsonNode' but required in type 'NodeData'."* Established precedent in [`parts/custom-json-node.tsx:8`](../../../src/registry/components/data/flow-canvas/parts/custom-json-node.tsx) uses intersection: `type CustomJsonData = NodeData & { _label?: string }`. **Lock:** define `type CardTreeCanvasNode = NodeData & CardTreeJsonNode` in `card-tree-node/types.ts`; type the renderer as `NodeRenderer<CardTreeCanvasNode>`. Renderer body can then read `data.ports`, `data.__rcid`, etc. directly without the `(data as NodeData)` cast. Subcards stay typed as `CardTreeJsonNode` (legit — they're inner tree nodes, not flow-canvas nodes, and may not have `__type`); the `(card as NodeData).ports` cast at the subcard boundary is structurally sound (NodeData → CardTreeJsonNode via the index signature) and stays. Re-export `CardTreeCanvasNode` from `index.ts` so consumers can type their `CanvasData.nodes[].data` correctly. |
...
| ID | Severity | Resolved in | One-line summary |
|---|---|---|---|
| **G1** | 🔸 Med | §3 F-05 (extended) + §9 risks | F-05 originally locked "port-walker recursion already supports subcard ports — no new plumbing." TRUE for connection routing; FALSE for visual positioning. xyflow `<Handle>` is `position: absolute` — anchors to nearest positioned ancestor. Subcard handles render at subcard locations ONLY IF `position: relative` is preserved at every DOM level (NodeShell, CardTreeViewer outer, SubcardBlock). Future contributor removing `relative` from any level → silent visual regression. Lock: explicit code comments at each level + spot-check review verifies. |
| **G2** | 🔹 Low | (impl-time TODO, not plan update) | Demo fixture spec is a stub — concrete `CardTreeJsonNode` data with `__rcid`s pre-attached needs to be written at impl time. Cite flow-canvas's existing `dummy-data.ts` pattern. |
| **G3** | 🔹 Low | §9 risks | Rapid open/close per-mount cost — `key={editingNodeId}` forces `<CardTree>` remount per click; Plate re-initializes each open. v0.2 polish path: file a card-tree v0.5 PR for `CardTreeHandle.setTree(tree)` if rapid-switching becomes real pain. |
...
| **G5** | 🔹 Low | Appendix A | Subcards are NOT drag-extractable in v0.1 — they're part of one card-tree tree per Q1 lock. Consumer expecting flow-canvas's `data-draggable-subobject` pattern on a subcard will find it doesn't work. v0.2 candidate. |

---

## 4. Workstream A — `flow-canvas@v0.2.1` patch — file-by-file plan
...
**File:** [`src/registry/components/data/flow-canvas/types.ts`](../../../src/registry/components/data/flow-canvas/types.ts)
...
**File:** [`src/registry/components/data/flow-canvas/parts/node-adapter.tsx`](../../../src/registry/components/data/flow-canvas/parts/node-adapter.tsx)
...
**File:** [`src/registry/components/data/flow-canvas/registries/canvas-context.tsx`](../../../src/registry/components/data/flow-canvas/registries/canvas-context.tsx)
...
### 4.3.1 `flow-canvas.tsx` — root component wiring (F-V4 lock)

**File:** [`src/registry/components/data/flow-canvas/flow-canvas.tsx`](../../../src/registry/components/data/flow-canvas/flow-canvas.tsx)
...
**File (new):** `src/registry/components/data/flow-canvas/lib/update-node-data.ts`
 ...
 * don't reinvent it. See the card-tree-node procomp guide for the canonical
...
**File:** [`src/registry/components/data/flow-canvas/index.ts`](../../../src/registry/components/data/flow-canvas/index.ts) — add re-export `export { updateNodeData } from "./lib/update-node-data";`

**Locked per Q10 recommendation:** ship the helper with flow-canvas@v0.2.1 (alongside the `onEditRequest` API that creates the need). Pairs the helper with the broader convention; future heavy-content adapters reuse it.

### 4.5 `meta.ts` + `registry.json`

- `src/registry/components/data/flow-canvas/meta.ts` — `version: "0.2.0" → "0.2.1"`, `updatedAt` to the patch date.
- `registry.json` — `flow-canvas` base item gains `lib/update-node-data.ts` as `type: "registry:component"`. Other files unchanged. `pnpm registry:build` regenerates the artifact.
...
## 5. Workstream B — `card-tree-node@v0.1.0` — file-by-file plan

New sealed folder at `src/registry/components/data/card-tree-node/`. Scaffolded via `pnpm new:component data/card-tree-node` after GATE 2 sign-off, then implementations follow.

### 5.1 Sealed folder shape (target)

```
src/registry/components/data/card-tree-node/
├── card-tree-node.tsx          (~30 LoC barrel-host — re-exports + maybe a wrapper component, TBD impl-time)
├── types.ts                       (~40 LoC — re-exports + CardTreeViewerOptions stub for v0.2)
├── index.ts                       (~25 LoC — public barrel)
├── meta.ts                        (per scaffolder + filled-in)
├── parts/
│   ├── card-tree-viewer.tsx       (~120 LoC — the renderer's <div> tree; the actual NodeRenderer object)
...
├── dummy-data.ts                  (~80 LoC — 3-5 fixture nodes with non-trivial CardTreeJsonNode trees + ports)
├── demo.tsx                       (~120 LoC — working FlowCanvas + Dialog + CardTree wiring)
└── usage.tsx                      (~80 LoC — consumer-facing docs surface)
```

Total estimated ~700 LoC across 13 files. Comparable shape to `code-block` (42 files for a much more complex component) and `kanban-board` (similar registry pattern). Smaller than `card-tree` itself (~51 files at v0.4.1).

### 5.2 `parts/card-tree-viewer.tsx` — the renderer

The exported `NodeRenderer<CardTreeJsonNode>`:
...
import { PortsAt } from "@/registry/components/data/flow-canvas";
...
function CardTreeViewerImpl({
  ...
  const ports = data.ports;  // root-level ports — typed via CardTreeCanvasNode (F-V6 lock)
...
// (which requires `__type: string`). CardTreeJsonNode alone does NOT satisfy that
// constraint — see §5.7 + §3.5 F-V6. CardTreeCanvasNode = NodeData & CardTreeJsonNode
// is the type that flows into flow-canvas's renderer registry.
...
import { PortsAt } from "@/registry/components/data/flow-canvas";
import type { NodeData } from "@/registry/components/data/flow-canvas";
...
function isCardLike(value: unknown): value is CardTreeJsonNode {
  ...
  // card-tree metadata fields OR carries its own ports[].
  return (
...
export type { CardTreeJsonNode } from "@/registry/components/data/card-tree";
 ...
 * with card-tree's open-shape `CardTreeJsonNode` (`__rcid?` / `__rcorder?` /
...
export type { CardTreeJsonNode } from "@/registry/components/data/card-tree";
export type { CardTreeCanvasNode } from "./types";  // F-V6 lock — canvas-node form

// Optional helper re-export (sourced from flow-canvas@v0.2.1's lib;
// re-exported here for ergonomic consumer DX — one import for the whole flow).
export { updateNodeData } from "@/registry/components/data/flow-canvas";
```

Note the `updateNodeData` re-export — convenience for consumers who write the card-tree-node dialog wiring (Q10 recommendation: helper homes in flow-canvas@v0.2.1; card-tree-node re-exports for ergonomic convenience).
...
export const meta: ComponentMeta = {
  ...
  category: "data",
  description:
    "Read-only CardTreeViewer renderer for flow-canvas + consumer-owned-dialog pattern for editing card-tree content inside canvas nodes. Canonical implementation of the popup-edit renderer convention from flow-canvas@v0.2.0 perf description Q33.",
  context:
  ...
  features: [
    "CardTreeViewer NodeRenderer<CardTreeJsonNode> — drop-in for flow-canvas's renderer registry",
    ...
    "n8n-style multi-select supported via flow-canvas's existing canvas-level marquee + shift-click (bulk-edit-via-dialog deferred to v0.2)",
    "Consumer-owned dialog pattern (no shipped dialog chrome) — documented in procomp guide",
  ],
  tags: [
    "card-tree-node", "flow-canvas", "card-tree", "popup-edit",
  ...
  dependencies: {
    ...
    npm: {},                     // no new npm deps
    internal: ["card-tree", "flow-canvas"],   // both cross-registry deps
  },
...
- **demo.tsx** — single-page demo: a flow-canvas with 3 card-tree nodes + 1 custom-json node (for renderer-mixed proof). Dialog wires the imperative `CardTreeHandle.focusCard(subPath)` per F-02 lock. Click → opens dialog → edits → live-saves via `onChange`. Includes a "Select multiple nodes" hint (marquee select supported per Q3).
...
// 3 card-tree nodes + 1 custom-json node demonstrating the renderer-mixed pattern.
// Each card-tree node carries a non-trivial tree with __rcids pre-attached (so
// subcard-click-to-focus works out of the box per F-03 lock).

export const initialData: CanvasData = {
  version: 1,
  nodes: [
    /* Parent card-tree with 2 subcards, ports at root + each subcard */
    /* ... */
  ],
...
```jsonc
{
  ...
  "description": "Read-only CardTreeViewer renderer for flow-canvas nodes; consumer-owned-dialog pattern for editing card-tree content inside canvas nodes.",
  "author": "ilinxa",
  "categories": ["data", "flow", "rich-content"],
  "registryDependencies": ["dialog", "@ilinxa/card-tree", "@ilinxa/flow-canvas"],
  "dependencies": [],
  "files": [
    /* every shipped .tsx/.ts file in the sealed folder, target: "components/card-tree-node/<sub-path>" */
...
},
{
  ...
  "description": "Card Tree Node bundle including the dummy-data fixtures used by the docs-site demo.",
  "author": "ilinxa",
  "categories": ["data", "flow", "rich-content", "fixtures"],
  "registryDependencies": ["@ilinxa/card-tree-node"],
  "files": [
    { "path": "src/registry/components/data/card-tree-node/dummy-data.ts", "type": "registry:component", "target": "components/card-tree-node/dummy-data.ts" }
  ]
}
```
...
3-line edit to [`src/registry/manifest.ts`](../../../src/registry/manifest.ts) — the scaffolder prints the exact lines after `pnpm new:component data/card-tree-node`.
...
1. **Workstream A — Commit A1:** flow-canvas@v0.2.1 — types.ts + canvas-context.tsx + node-adapter.tsx + lib/update-node-data.ts (new). `pnpm tsc --noEmit && pnpm lint && pnpm validate:meta-deps` clean before commit.
2. **Workstream A — Commit A2:** flow-canvas@v0.2.1 — meta.ts bump + registry.json update (lib/update-node-data.ts as `registry:component`) + `pnpm registry:build` regenerated artifacts. Spot-check the regenerated `public/r/flow-canvas.json` carries the new file.
3. **Workstream A — Commit A3:** flow-canvas@v0.2.1 — STATUS.md `Last updated` lead update + decision file at `.claude/decisions/<date>-flow-canvas-v0.2.1-on-edit-request.md` + `docs/component-versions.md` row + Highlights bullet. No GATE 3 review (patch-bump exemption).
4. **Workstream A — push to origin** (3 commits land together; Vercel auto-deploys; `@ilinxa/flow-canvas@v0.2.1` becomes available to consumers).
5. **Workstream B — pnpm new:component data/card-tree-node** — scaffold the sealed folder.
6. **Workstream B — Commit B1:** implement parts/ + lib/ files (card-tree-viewer.tsx, subcard-block.tsx, flat-field-strip.tsx, enumerate-subcards.ts, derive-title.ts, derive-flat-fields.ts, format-value.ts).
7. **Workstream B — Commit B2:** types.ts + index.ts + meta.ts + the 3-line manifest.ts registration.
8. **Workstream B — Commit B3:** dummy-data.ts + demo.tsx + usage.tsx — verify docs render at `/components/card-tree-node`.
9. **Workstream B — Commit B4:** registry.json — add both items (base + fixtures) + `pnpm registry:build` regen.
10. **Workstream B — Run F-V2 smoke harness path-b** (required per readiness-review rule for v0.1.0 first ships). Smoke against locally-served `public/r/card-tree-node.json`; consumer-side `pnpm tsc --noEmit` clean post-install. Fix any consumer-side findings as additional commits.
11. **Workstream B — Author Stage 3 procomp guide** (`card-tree-node-procomp-guide.md`) — same time as B3 typically, but can land in a separate B5 commit.
12. **Workstream B — GATE 3 spot-check review** at `docs/procomps/card-tree-node-procomp/reviews/<date>-v0.1.0-spotcheck.md`. **Rotating dimension: Public API** (subPath signature, viewer composition, the imperative-ref pattern in the consumer wiring all qualify as load-bearing API axes; verdict must be `Pass` or `Pass with follow-ups`).
13. **Workstream B — Commit B5:** STATUS.md update + `.claude/decisions/<date>-card-tree-node-v0.1.0-first-ship.md`. Push to origin.

**Each commit's verification:** `pnpm tsc --noEmit && pnpm lint && pnpm validate:meta-deps` clean.

**Estimated wall-clock (no measurement-driven blocks here, unlike flow-canvas perf):** Workstream A ~1–2 hours. Workstream B ~4–6 hours including smoke + spot-check + decision file. Both workstreams in one session is plausible if uninterrupted.

---

## 7. Public API matrix (cumulative across both workstreams)

### Workstream A — `flow-canvas@v0.2.1`
...
### Workstream B — `card-tree-node@v0.1.0`

| Surface | v0.1.0 |
|---|---|
| `cardTreeViewerRenderer: NodeRenderer<CardTreeJsonNode>` | new named export — drop-in for flow-canvas's `renderers` prop |
| `CardTreeJsonNode` (re-export from card-tree) | convenience re-export |
| `updateNodeData` (re-export from flow-canvas@v0.2.1) | convenience re-export |
...
- **Two consumers register the viewer with `type: "card-tree"`**: last-wins per flow-canvas's renderer registry; dev warning fires.
- **A subcard's port has the same ID as a root port**: violates flow-canvas's port-uniqueness rule (Q9 of v0.1 description); `findPortInTree` returns the first match (root); silent edge mis-routing. **Mitigation: dev-mode warning on viewer mount** when duplicate port IDs detected (cheap one-pass scan).
- **`CardTreeJsonNode` with no recognizable fields** (e.g. only `__rcid`): viewer shows the F-07 neutral placeholder "Untitled card-tree" + click bubbles to root edit; no subcards / no flat fields / no port handles render (since `data.ports` is undefined).
- **Multi-select with 5 card-tree nodes, then click one to edit**: `onEditRequest(clickedNodeId)` fires for ONE nodeId; other 4 stay selected for bulk move/delete/duplicate (Q3 lock).
- **Drag a card-tree node**: xyflow's drag handler suppresses the mouseup-click that would fire `onEditRequest`. No dialog open on drag-end.
- **Consumer wires two parallel dialogs** (anti-pattern; warned in guide): two card-tree editors mount; performance degrades (multi-instance editor cost); functional correctness preserved (each dialog edits its own node).

---

## 9. Risks (this plan's scope; description's full risk list at Stage 1 §10)

- **F-V1 nested-button trap evaded by careful composition.** If a future contributor refactors `CardTreeViewer` to wrap the whole thing in `<button>`, the nested-button violation returns + clicks fire double. Mitigation: comment in `card-tree-viewer.tsx` lock the `<div role="group">` outer pattern; spot-check review covers it.
- **F-04 `isCardLike` heuristic over-triggers on non-card objects.** A consumer-defined CardTreeJsonNode might have non-card object values that incidentally carry `ports` or `__rcorder`. The viewer would render them as subcard blocks. Mitigation: the heuristic is conservative (requires AT LEAST ONE card-tree meta field OR `ports`); v0.2 tightens via a canonical predicate from card-tree if it ships one. Doc this in the procomp guide so consumers know the rule.
...
- **card-tree v0.4.x to v0.5 migration during this work.** Unlikely (card-tree is at v0.4.1 beta, stable for now), but if card-tree minors during the card-tree-node build, the `CardTreeJsonNode` shape OR `CardTreeHandle.focusCard` API could shift. Mitigation: pin card-tree version in `dependencies.internal` requirement (`^0.4.0` per the description's Q9 lock); CI smoke catches regressions.
- **Imperative `focusCard(id)` via ref ergonomics (F-02 follow-up).** Consumers may find the `useRef + useEffect` pattern awkward vs a hypothetical prop. Mitigation: the procomp guide ships the canonical wiring as a copy-pasteable example. If three consumers express friction, file a PR against card-tree to add an `initialFocusCardId?: string` prop in card-tree v0.5.
...
- **G1 position-relative chain regression** (added 2026-05-16) — see F-05 lock above. If `relative` is removed from any DOM tree level (NodeShell, CardTreeViewer outer, SubcardBlock), subcard handles silently fly to a wrong positioned ancestor. Mitigation: explicit code comments at each level + the GATE 3 spot-check's rotating dim (Public API) should target visual port positioning at the subcard level.
- **G3 rapid open/close per-mount cost** (added 2026-05-16) — `key={editingNodeId}` forces `<CardTree>` remount on every node-click. Plate re-initializes (TTI cost) each open. Users clicking through 5 nodes in quick succession experience perceptible delay. Mitigation: this is the price of the "at most one editor instance mounted" property — alternative (persistent editor + imperative tree-switching) would require a `CardTreeHandle.setTree(tree)` API that doesn't exist in card-tree v0.4.1. Document in the procomp guide; v0.2 perf polish path: file a card-tree v0.5 PR for `setTree` if rapid-switching surfaces as a real consumer pain.
- **G4 Radix Dialog `forceMount` defeats the single-instance property** (added 2026-05-16) — the "at most one card-tree editor mounted" claim holds because shadcn's `<Dialog>` (Radix) unmounts content when `open=false`. A consumer wrapping with `<Dialog.Portal forceMount>` keeps content (and thus `<CardTree>`) mounted always → defeats the perf property. Mitigation: document `forceMount` as a consumer footgun in the procomp guide; not enforceable in code (consumer's dialog, consumer's choice).

---

## 10. Plan-stage TODOs (impl-time items + F-V findings + minor reminders)

- **Impl-time** (F-V2 lock) — smoke harness path-b MUST run before push for card-tree-node@v0.1.0. Use the harness at `e:/tmp/ilinxa-smoke-consumer/` (per project_smoke_harness memory). Run against locally-served `public/r/card-tree-node.json`; consumer-side `pnpm tsc --noEmit` clean post-install is the bar.
- **Impl-time** (F-V3 lock) — `dependencies.internal: ["card-tree", "flow-canvas"]` in `meta.ts`; verify `validate:meta-deps` accepts both. The `flow-canvas` dep is for the relative TS imports (`@/registry/components/data/flow-canvas`); the script's "internal-registry slug" detector should recognize it via the existing pattern.
- **Impl-time** — when scaffolding the procomp, do NOT re-export `enumerate-subcards.ts` / `derive-*.ts` / `format-value.ts` from `index.ts`. They're internal helpers; only the renderer + type re-exports + helper re-exports go in the barrel. **Why kept private (F-rev-3 lock):** `enumerateSubcards`'s signature depends on the `isCardLike` heuristic (F-04), which is explicitly marked for tightening in v0.2 if card-tree ships a canonical "is-card" predicate. Exporting now then changing the signature later = breaking change for consumers writing custom card-tree renderers on top of this procomp. **Revisit trigger:** v0.2 when (a) the configurable options surface lands AND (b) the `isCardLike` predicate stabilizes, OR a real consumer asks for it.
- **Impl-time** (F-V6 lock) — the renderer is typed `NodeRenderer<CardTreeCanvasNode>` where `CardTreeCanvasNode = NodeData & CardTreeJsonNode` (defined in `types.ts` per §5.7). Re-exported from `index.ts` so consumers can type `CanvasData.nodes[].data` correctly. The renderer body reads `data.ports` directly (no `(data as NodeData)` cast). Subcards stay typed as `CardTreeJsonNode` and use the existing `(card as NodeData).ports` cast at the subcard boundary.
...
- **Impl-time** — Stage 3 procomp guide (`card-tree-node-procomp-guide.md`) sections to include: 1. Architecture, 2. Quick start (consumer wiring), 3. The dialog pattern (canonical example), 4. subPath model (`__rcid`-based), 5. Multi-select support (Q3 lock note), 6. Subcard limits (NESTED_DEPTH=1; MAX_FLAT_FIELDS=3), 7. Footguns (nested-button trap, port-uniqueness, missing-__rcid behavior), 8. Migration (none for v0.1; for v0.2 the configurable-knobs path).
- **Impl-time** — when authoring the `updateNodeData` helper, type it strictly: `nextData: NodeData` (not `CardTreeJsonNode`) — the helper is generic across all renderer types, not card-tree-specific. The procomp guide's example casts at the call site (`updateNodeData(prev, editingNodeId, next as NodeData)`).
- **Impl-time** — the flow-canvas@v0.2.1 patch's `decision file` should reference the card-tree-node procomp as the convention's canonical consumer (so future readers can trace why the API was added).
- **Decided at GATE 2; impl-time verify** — Q10 helper home is `flow-canvas@v0.2.1` (Q10 recommendation accepted by Workstream A inclusion). The card-tree-node `index.ts` re-exports for ergonomic convenience.
...
1. **Workstream A** — flow-canvas@v0.2.1 patch shipped (3 commits A1/A2/A3 + push). `meta.ts` at `0.2.1`. `public/r/flow-canvas.json` carries `lib/update-node-data.ts`. `flow-canvas@v0.2.1` is consumable via `pnpm dlx shadcn add @ilinxa/flow-canvas` post-Vercel-deploy.
...
4. **F-V1 nested-button avoided** — `CardTreeViewer` outer element is `<div role="group">`; per-subcard `<button>`; title strip `<button>`. NOT a button-of-buttons.
5. **F-V2 smoke harness path-b** runs clean: `pnpm dlx shadcn add @ilinxa/card-tree-node` succeeds; consumer-side `pnpm tsc --noEmit` clean post-install.
5a. **F-V6 renderer typing honored** — `types.ts` defines `export type CardTreeCanvasNode = NodeData & CardTreeJsonNode`; `index.ts` re-exports it; the renderer is `NodeRenderer<CardTreeCanvasNode>`; `data.ports` is accessed directly (no `(data as NodeData)` cast inside the renderer body).
6. **`meta.ts` declares** `version: "0.1.0"`, `status: "alpha"`, `dependencies.internal: ["card-tree", "flow-canvas"]`.
7. **`registry.json` includes** both `card-tree-node` (base) and `card-tree-node-fixtures` items.
8. **`pnpm tsc --noEmit && pnpm lint && pnpm validate:meta-deps` clean** (43/43 once this ships — flow-canvas@v0.2.1's `update-node-data.ts` doesn't trigger a count change).
...
- **Configurable viewer options** (`CardTreeViewerOptions`) — Q6 lock; v0.2 scope. v0.1 stub-types only; no factory function, no `cardTreeViewer(options)` API.
...
- **`<CardTree>` permissions / validators / undo passthrough from the demo's default wiring** — consumers can pass these props through but the v0.1 demo doesn't exercise them.
...
- **Sub-object drag-extract of subcards** (G5 fold-in, 2026-05-16) — flow-canvas has a v0.1 feature (`data-draggable-subobject={path}` + `emitSubObjectDrag` helper) that lets users drag a sub-piece OUT of a node to spawn it as a new flow-canvas node. **v0.1 subcards are NOT drag-extractable** — subcards are part of one card-tree tree per Q1's content-edit-routes-through-popup lock; extracting them would mean splitting the tree across multiple flow-canvas nodes, which Q1 explicitly rejected. Consumer expecting flow-canvas's familiar drag-extract pattern on a card-tree subcard will find it doesn't work. Document this gap in the procomp guide. **v0.2 candidate** if a consumer signals — would need a careful UX shape (split-tree vs copy-tree-content-as-new-node) that doesn't violate the single-tree contract.
...
- flow-canvas v0.2.0 plan: [`../flow-canvas-procomp/flow-canvas-v0.2.0-procomp-plan.md`](../flow-canvas-procomp/flow-canvas-v0.2.0-procomp-plan.md) (signed off 2026-05-16)
- flow-canvas v0.2.0 perf description: [`../flow-canvas-procomp/flow-canvas-v0.2.0-perf-description.md`](../flow-canvas-procomp/flow-canvas-v0.2.0-perf-description.md) (Q33 = popup-edit convention; Q35 = parallel-track confirmation)
- flow-canvas v0.2.0 spot-check review: [`../flow-canvas-procomp/reviews/2026-05-16-v0.2.0-spotcheck.md`](../flow-canvas-procomp/reviews/2026-05-16-v0.2.0-spotcheck.md)
- card-tree description: [`../card-tree-procomp/card-tree-procomp-description.md`](../card-tree-procomp/card-tree-procomp-description.md)
- card-tree source: [`../../../src/registry/components/data/card-tree/`](../../../src/registry/components/data/card-tree/) (v0.4.1 beta — verified)
- flow-canvas source: [`../../../src/registry/components/data/flow-canvas/`](../../../src/registry/components/data/flow-canvas/) (v0.2.0 — verified)
- port-walker: [`../../../src/registry/components/data/flow-canvas/lib/port-walker.ts`](../../../src/registry/components/data/flow-canvas/lib/port-walker.ts) (F-05 confirmation)
