# `card-tree-node` v0.2.0 — Port Editor Description (Stage 1)
...
> **Slug:** `card-tree-node` (additive feature; sibling export, no breaking changes)
> **Target version:** `card-tree-node@v0.2.0` (minor bump — adds public surface)
> **Dependencies bumped:** `flow-canvas@v0.2.5` (1-line patch — adds built-in `"doc"` port type to `defaultPortTypes`; no runtime contract changes)
> **Parallel track to:** v0.1.0 (shipped 2026-05-16); v0.2.0 builds ON v0.1.0's `CardTreeViewer` + `onEditRequest` plumbing.

This is the description doc. Job: pin down the architecture for putting **port editing UI** into the card-tree-node dialog flow, surface decisions, earn sign-off before any planning or code.

---

## 1. Problem

`card-tree-node@v0.1.0` lets consumers put card-tree content into flow-canvas nodes and edit that content through a popup dialog. **Ports are static** — set in the fixture at construction time, never editable through the UI. Adding, removing, or retyping a port today requires editing source code.

The flow-canvas port primitive already supports everything needed:
...
| **"doc" port type — bottom-only, links to doc files** | ❌ this doc (1-line flow-canvas patch + editor-side enforcement) |
...
│                                                                     │
│  ┌──────────────┐  ┌──────────────┐                                 │
│  │CardTreeViewer│  │CardTreeViewer│  ... × N         (unchanged)    │
│  │  + ports     │  │  + ports     │                                 │
│  └──────┬───────┘  └──────────────┘                                 │
...
└─────────┼───────────────────────────────────────────────────────────┘
  ...
  │   │   targets the same subPath/nodeId as CardTree           │    │
  ...
  │   └─────────────────────────────────────────────────────────┘    │
  │                                                                  │
  │   <CardTree editable> ... </CardTree>     (unchanged from v0.1)  │
  └──────────────────────────────────────────────────────────────────┘

  Data contract: ports[] live on the same CardTreeJsonNode at the same
  __rcid level the user clicked. PortEditorStrip and CardTree operate on
...
2. **card-tree stays port-agnostic.** No card-tree changes. The strip reads/writes the `ports` array directly on the node data; card-tree's `stripFlowCanvasFields` continues to hide ports from its own editor.
3. **flow-canvas minimally affected.** One narrow addition: `"doc"` built-in type. **No runtime contract change** — doc-port side enforcement is editor-side only (PortEditorStrip disables non-bottom options in the picker; flow-canvas stays neutral about it).
...
- New built-in `"doc"` port type in flow-canvas `defaultPortTypes` (one-line addition to `port-type-registry.ts`)
- `"doc"`-typed ports: side forced to `"bottom"` in PortEditorStrip's picker (other options disabled with tooltip). **No runtime enforcement** — flow-canvas doesn't validate this; only the editor does.
- Per-card port management (ports on the targeted card/subcard's data root)
- Live save (every change flows immediately, matching v0.1's card-tree pattern)
...
- **Consumer-registered custom port types** in PortEditorStrip's picker — deferred to v0.3 (requires either a shared context or a `getPortTypes(canvas)` helper from flow-canvas).
- **Runtime port validation** (e.g. `validatePort(port)` hook in flow-canvas) — deferred indefinitely; editor-side enforcement is sufficient for v0.2.
...
- **Port-from-existing-card-tree-meta sync** (using `__rcmeta` to drive port defaults) — deferred indefinitely; unclear use case.
...
**Problem:** Strip and CardTree share dialog real estate; visual order matters.

**Options:**
- (a) Above CardTree — port edits feel "metadata-first," card content secondary
- (b) Below CardTree — content first, ports as configuration tail
- (c) Inside a Tabs control — "Content" / "Ports" tabs

**Lock:** **(a) above CardTree.** Port edits are typically rare; surfacing them at top makes the affordance discoverable without burying it under card-tree's scroll. Tabs (c) add a click for the common (content-edit) case. Consumer can override by ignoring the recommended slot and placing the strip wherever they want — `<PortEditorStrip>` doesn't impose its own dialog chrome.
...
**Lock:** **2 ports — at CREATE time only.** No new `dir` value in flow-canvas — keeps runtime model unchanged. The "+ Add port" popover offers `[✓in] [✓out]` checkboxes. Both checked → creates two port rows (one `dir:"in"`, one `dir:"out"`) sharing type/side/multi/label, with ids `{base}-in` and `{base}-out`.
...
- **Side: editor-side forced to `"bottom"`.** PortEditorStrip's side picker disables left/right/top when the row's type is `"doc"`. flow-canvas runtime does NOT enforce this — if a consumer programmatically constructs a `{type:"doc", side:"left"}` port outside the editor, flow-canvas renders it without warning. The editor is the gate.
...
User can edit the id inline; uniqueness validated within the node (cross-card unique not required — flow-canvas already qualifies by `nodeId:portId`).
...
**Lock:** v0.2 PortEditorStrip uses the 5 v0.1 built-ins + the new `"doc"` type. **No `portTypes` prop.** Consumer-registered custom types support is v0.3 work and needs proper shared-context plumbing (or a `getPortTypes(canvas)` helper from flow-canvas) to avoid the strip's picker silently diverging from the canvas's runtime validator.
...
**Lock:** **Live save** (matches v0.1 card-tree's pattern). Every change calls `onChange(updatedCanvas)`. No commit/cancel button. Consistent with the existing rcif demo's live-save UX.
...
No validation prevents save — strip operates in best-effort mode. Hard validation lives in flow-canvas's `isValidConnection`.

### Q8 — Permissions

**Lock:** PortEditorStrip accepts an optional `permissions?` prop that mirrors card-tree's predicate API shape:
```ts
...
```
Default: everything allowed when `editable={true}` (consistent with card-tree's default).

### Q9 — Strip lifecycle (mount + node switch)

**Lock:** PortEditorStrip is always mounted alongside CardTree inside the dialog — same lifecycle. If the dialog is closed, both unmount. If editing is null, both are absent.

**Uncontrolled by design.** The strip operates on `canvas.data` (passed via prop) and has no local state to corrupt across node switches. On `nodeId` or `subPath` prop change, the strip re-reads `ports[]` from the new node's data and re-renders. **No `key={nodeId}` remount needed** (unlike the v0.1 `<CardTree>` mount pattern, which is uncontrolled-by-defaultValue and DOES need the remount).
...
**Lock:** rcif `<PortEditorStrip>` imports from `@/registry/components/data/flow-canvas/types` (Port, PortType, CanvasData) and `@/registry/components/data/flow-canvas/lib/update-node-data` (the helper). **F-S1 lock applies:** use RELATIVE imports (`../flow-canvas/types`, `../flow-canvas/lib/update-node-data`) in shipped source per the json-form v0.1.4 + rcif v0.1.0 precedent.

### Q11 — Visual design

**Lock:** strip matches card-tree's existing affordance style for visual continuity. Specifically the "+ Add port" button mirrors card-tree's `+ FIELD` / `+ BLOCK` pattern at [`parts/predefined-add-menu.tsx:56-59`](../../src/registry/components/data/card-tree/parts/predefined-add-menu.tsx#L56-L59):
...
Port type pickers show the type's color swatch alongside the label, with colors rendered via design tokens (`var(--chart-*)` / `var(--primary)` / etc., never raw hex), consistent with flow-canvas's port handle rendering. One row per port, no nesting. Compact spacing — strip should not dominate dialog height.
...
- **Q-O3 (flow-canvas version bump):** **(b) v0.2.5 patch.** Closed by V6 earlier; no runtime contract growth so semver patch is correct.
- **Q-O4 (per-field ports in v0.2):** **(a) defer to v0.3** with explicit "next-up after v0.2" priority. Rationale: keeps v0.2 scope tight + shippable; per-field UX deserves its own design pass (right-click affordance vs. inline toggle vs. promote-from-card-tree all merit exploration). User's "for fields and cards" original ask is honored as a near-term roadmap commitment, not a v0.2 cut.
...
- Not changing card-tree.
- Not changing flow-canvas's port runtime model (only `defaultPortTypes` array gains one entry).
...
| **V4** | Visual design cites the exact card-tree `+ FIELD` Tailwind classes (line-anchored to `predefined-add-menu.tsx:56-59`). | Q11 lock |
...
| **V6** | Dropped proposed `validatePort` runtime hook in flow-canvas. v0.2 flow-canvas changes reduce to 1-line `defaultPortTypes` addition. Bump goes from `v0.3.0` minor → `v0.2.5` patch. Answers Q-O3. | Header line 6; §2 property #3; Q4 lock (side enforcement editor-only); §3 in-scope (note); §3 out-of-scope (runtime validation deferred); Q-O3 marked closed |

Summary of net effect: v0.2 ships as PortEditorStrip + 1-line flow-canvas patch + zero runtime contract changes. Cleaner scope.
