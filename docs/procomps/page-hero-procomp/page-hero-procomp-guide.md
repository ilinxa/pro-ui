...
Pro-ui's `--font-heading` token resolves to Onest (the existing sans). `font-display` Tailwind utility maps there. If you want serif headlines (matching news-card), wrap your title in `<span className="font-serif">...</span>` inside `titleSlot`, OR globally swap `--font-heading: var(--font-playfair-display)` in your app.
...
Originals at [`docs/migrations/page-hero/original/`](../../migrations/page-hero/original/).
