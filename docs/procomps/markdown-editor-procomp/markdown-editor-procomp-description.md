...
- **Pure controlled `value` / `onChange`** — host owns the markdown string. Same pattern as `properties-form`, `filter-panel`, `entity-picker`.
...
With this sign-off, **all five Tier 1 procomp descriptions are locked** (`properties-form`, `detail-panel`, `filter-panel`, `entity-picker`, `markdown-editor`). The `force-graph` (Tier 2) description is now unblocked per the [system §9 authoring order](../../systems/graph-system/graph-system-description.md#L437).
