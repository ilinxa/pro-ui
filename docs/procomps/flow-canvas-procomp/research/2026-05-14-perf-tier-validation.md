# flow-canvas — performance tier validation
...
> **Project tip at time of research:** `832e4f0` (flow-canvas at v0.1.3, post perf+paint patches)

---

## TL;DR

1. flow-canvas is already heavily tuned. Most "obvious" xyflow perf levers (module-scope `nodeTypes`/`edgeTypes`, `React.memo` on every node + edge + shell, ref-mirrored callbacks, empty-dep `useCallback` on `isValidConnection`) are already in place. Confirmed by inspection of the source.
...
3. There IS still measurable headroom in flow-canvas — but **it's smaller than the four-tier comparison initially suggested**. Several proposed Tier 2 levers (quadtree port hit-test, edge memoization, event throttling) were either already done or addressed problems that don't exist in this codebase.
...
| Lever | Source | Status |
|---|---|---|
| Module-scope `NODE_TYPES` / `EDGE_TYPES` | [canvas.tsx:31-32](../../../../src/registry/components/data/flow-canvas/parts/canvas.tsx) | ✅ in place, comment cites the skill |
| `React.memo` on `NodeAdapter` | [node-adapter.tsx:62](../../../../src/registry/components/data/flow-canvas/parts/node-adapter.tsx) | ✅ |
| `React.memo` on `NodeShell` | [node-shell.tsx:54](../../../../src/registry/components/data/flow-canvas/parts/node-shell.tsx) | ✅ |
| `React.memo` on `CustomJsonNode` + collapsed-mode skip of `JSON.stringify`/`<pre>`/shadow | [custom-json-node.tsx:58,26-55](../../../../src/registry/components/data/flow-canvas/parts/custom-json-node.tsx) | ✅ |
| `React.memo` on `DefaultEdge` | [default-edge.tsx:73](../../../../src/registry/components/data/flow-canvas/parts/default-edge.tsx) | ✅ |
| `isValidConnection` with empty deps + ref-based reads | [canvas.tsx:139-179](../../../../src/registry/components/data/flow-canvas/parts/canvas.tsx) | ✅ v0.1.2 perf patch |
| Ref-mirrored callbacks in `useCanvasData` | [use-canvas-data.ts:177-209](../../../../src/registry/components/data/flow-canvas/hooks/use-canvas-data.ts) | ✅ |
| `defaultViewport` to preserve initial zoom and not defeat culling | [canvas.tsx:117-121](../../../../src/registry/components/data/flow-canvas/parts/canvas.tsx) | ✅ |
| `onlyRenderVisibleElements` flipped on | [canvas.tsx:78,242](../../../../src/registry/components/data/flow-canvas/parts/canvas.tsx) | ⚠ **off by default** — consumer opt-in |
...
| `useShallow` / `createWithEqualityFn` on `useStore` selectors | [default-edge.tsx:38-43](../../../../src/registry/components/data/flow-canvas/parts/default-edge.tsx) reads `useStore` without shallow | ❌ not applied |
| Batched `fireOnChange` (drag-end only, not per tick) | [use-canvas-data.ts:222-238](../../../../src/registry/components/data/flow-canvas/hooks/use-canvas-data.ts) fires on every change | ❌ fires per tick |

**flow-canvas's own procomp doc commits to 200 nodes as the v0.1 success ceiling.** That ceiling reflects the realistic interactive sweet spot for the current node shape (custom-JSON nodes + rich nested data).
...
### Tier 4 — Beyond xyflow  (3-4 weeks, sibling procomp, not flow-canvas)
...
1. Add `src/app/(devtools)/flow-canvas-stress/page.tsx` (or similar) using `makeStressData(N)` from [dummy-data.ts](../../../../src/registry/components/data/flow-canvas/dummy-data.ts).
...
If/when flow-canvas v0.2.0 lands the popup-edit constraint, **card-tree** is NOT a viable in-canvas node (5,302 LOC, per-instance `DndContext` + `useReducer` + ~12 hooks + permissions + undo + search). The right pattern is an **adapter** that registers a lean preview-renderer with flow-canvas's renderer registry, and opens the FULL card-tree in a dialog on click. Details to be captured in a separate research doc when that decision is made.
