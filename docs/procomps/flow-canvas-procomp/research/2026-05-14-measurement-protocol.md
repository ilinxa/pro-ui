# flow-canvas — perf measurement protocol

> **Status:** authoritative procedure for every FPS measurement that informs v0.2.x / v0.3.0 tier decisions.
> **Date:** 2026-05-14
> **Locked by:** [v0.2.0 perf description](../flow-canvas-v0.2.0-perf-description.md) Q27 (multi-machine recording) + §3.2 pre-work.
...
|---|---|---|---|---|
| Light (`makeStressData`) | [dummy-data.ts:125-178](../../../../src/registry/components/data/flow-canvas/dummy-data.ts) | `custom-json` (built-in fallback) | 2 (`in`, `out`) | ~2 per node (next-but-one + next-row) |
...
Heavy fixture intentionally avoids `ProjectCard` and `card-tree` (per Q28 — cross-component coupling distorts measurements).
...
Each measurement run produces ONE file under `docs/procomps/flow-canvas-procomp/research/`:
