# `flow-canvas` — v0.2.0 Performance Plan (Stage 2, perf scope)
...
**Renderer-author API:** `RenderContext.isSelected` remains ([types.ts:54](../../../src/registry/components/data/flow-canvas/types.ts#L54)). Renderers that want their OWN selection-conditional content keep using it — that's a read-only flag, no re-render coupling beyond what the renderer itself opts into.
...
**Adapter change:** [`NodeAdapter` at node-adapter.tsx](../../../src/registry/components/data/flow-canvas/parts/node-adapter.tsx) currently passes `selected` to `<NodeShell>`. Stops passing it. `RenderContext.isSelected` continues to be populated from xyflow's `selected` prop for the renderer's own use.
...
| **F-V3** | 🔸 Med | §10 Impl-time, §11 #7 | `flow-canvas.css` ships as `type: "registry:file"`, NOT `registry:component` — verified precedent: `engagement-bar/parts/engagement-heart-burst.css` already follows this. The literal CLAUDE.md "every file registry:component" wording overstates; .css is the exception. `lib/shallow.ts` stays as `registry:component`. |
...
**File:** [src/registry/components/data/flow-canvas/parts/canvas.tsx](../../../src/registry/components/data/flow-canvas/parts/canvas.tsx)
...
**File:** [src/registry/components/data/flow-canvas/hooks/use-canvas-data.ts](../../../src/registry/components/data/flow-canvas/hooks/use-canvas-data.ts)
...
**Wiring:** [canvas.tsx](../../../src/registry/components/data/flow-canvas/parts/canvas.tsx) `<ReactFlow>` gains `onNodeDragStart={canvas.onNodeDragStart}` and `onNodeDragStop={canvas.onNodeDragStop}` (these are existing xyflow props, currently unset).

**Hook return shape:** `UseCanvasDataResult` ([use-canvas-data.ts:105-136](../../../src/registry/components/data/flow-canvas/hooks/use-canvas-data.ts#L105-L136)) gains two new fields:
...
Type extension only — `UseCanvasDataResult` is exported but not part of `FlowCanvasProps`, so no public-API surface change. The hook itself is internal-only ([not re-exported from index.ts](../../../src/registry/components/data/flow-canvas/index.ts)).
...
**File:** [src/registry/components/data/flow-canvas/parts/default-edge.tsx](../../../src/registry/components/data/flow-canvas/parts/default-edge.tsx)
...
**Anti-regression check:** verify edges still re-stroke when port `type` changes — open the demo at `/components/flow-canvas`, drag a node whose source port type changes via consumer-side edit, confirm edge color updates.

### 5.3 Selection ring: decouple from React-prop re-render

Per F-02 resolution. Three coupled edits:

**File 1:** [node-shell.tsx:14-52](../../../src/registry/components/data/flow-canvas/parts/node-shell.tsx#L14-L52) — drop `isSelected` prop.
  ...
  return (
    <div
      ...
      className={cn(
        ...
        // (see sealed-folder flow-canvas.css). Focus-visible kept here
...
**File 2:** new sealed-folder `src/registry/components/data/flow-canvas/flow-canvas.css` — CSS rule keyed on xyflow's class, two levels deep to match v0.1.x ring placement.

```css
/* flow-canvas selection ring — driven by xyflow's wrapper class.
...
**Import:** [canvas.tsx](../../../src/registry/components/data/flow-canvas/parts/canvas.tsx) gets a side-effect import at the top — `import "../flow-canvas.css";` — which Turbopack bundles + injects automatically; consumers receive the file on install + the same relative import resolves post-install.
...
- **(c) LOCKED** — sealed-folder `flow-canvas.css` side-effect imported by `canvas.tsx`. Registry-portable; consumers get it on install; F-cross-07 lint won't flag it (the validator tracks `npm` deps, not `.css` siblings).

**File 3:** [node-adapter.tsx](../../../src/registry/components/data/flow-canvas/parts/node-adapter.tsx) — stop passing `selected` to `<NodeShell>`. `RenderContext.isSelected` continues to be populated for the renderer's own use (read-only flag).

### 5.4 Renderer-author rule doc (description §4.2 Change #3)

**File:** [docs/procomps/flow-canvas-procomp/flow-canvas-procomp-guide.md](flow-canvas-procomp-guide.md) — adds a new top-level section **"Performance & scale"** with two subsections:
...
| `flow-canvas.css` | n/a | new file (sealed-folder) | None — auto-imported by canvas.tsx |
...
- **Impl-time** — when scaffolding `lib/shallow.ts`, verify the `flow-canvas` `index.ts` does NOT re-export it (internal helper only). The component's barrel exposes public types only.
- **Impl-time** (F-V3 lock, 2026-05-16) — `registry.json` for `flow-canvas` base item needs two new entries in its `files` array: `lib/shallow.ts` as **`type: "registry:component"`** (matches the locked CLAUDE.md convention for .ts files), and `flow-canvas.css` as **`type: "registry:file"`** (CSS exception — verified precedent: `engagement-bar/parts/engagement-heart-burst.css` already ships as `registry:file`, NOT `registry:component`. The literal "every file registry:component" wording in CLAUDE.md overstates; .css is the exception). Both use the locked target convention (`target: "components/flow-canvas/<sub-path>"`).
...
7. `registry.json` includes `lib/shallow.ts` as `registry:component` AND `flow-canvas.css` as `registry:file` (F-V3 lock — CSS is the registry:component exception per engagement-heart-burst.css precedent).
...
- **card-tree-node system** — parallel track (Q35); its own description doc, its own plan doc.
...
> ### `flow-canvas` v0.2.0 — Tier 1 + Tier 2 perf bundle (2026-MM-DD)
>
...
>
> **Internal:** `DefaultEdge`'s xyflow store selector now uses a narrow port comparator (avoids re-rendering on unrelated store updates). Selection ring **visual** now driven by xyflow's `.react-flow__node.selected` CSS class — the ring applies as a pure browser-side effect regardless of React reconciliation timing (no React-render savings on selection; the win is visual resilience + a cleaner `<NodeShell>` contract). New sealed-folder `flow-canvas.css` auto-imported by `canvas.tsx`.
>
> **No breaking type changes.** All v0.1.x types (`Port`, `NodeRecord`, `EdgeRecord`, etc.) unchanged.
>
> **Measured (per [protocol](docs/procomps/flow-canvas-procomp/research/2026-05-14-measurement-protocol.md), single dev machine, hardware logged in [research/](docs/procomps/flow-canvas-procomp/research/)):** light fixture at N=2000 min ≥50 FPS; heavy fixture at N=1000 min ≥50 FPS. (Numbers filled in at impl time from `<date>-tier2-postship.md`.)

---

## Appendix C — Cross-references

- v0.2.0 description (signed off): [flow-canvas-v0.2.0-perf-description.md](flow-canvas-v0.2.0-perf-description.md)
- v0.1.x description / plan / guide: [flow-canvas-procomp-description.md](flow-canvas-procomp-description.md), [flow-canvas-procomp-plan.md](flow-canvas-procomp-plan.md), [flow-canvas-procomp-guide.md](flow-canvas-procomp-guide.md)
