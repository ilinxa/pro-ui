# `flow-canvas` — Pro-component Plan (Stage 2)

> **Stage:** 2 of 3 · **Status:** Draft — awaiting sign-off
> **Slug:** `flow-canvas` · **Category:** `data`
> **Inputs:** description signed off ([flow-canvas-procomp-description.md](flow-canvas-procomp-description.md)). All twenty-four description-stage decisions (Q1–Q24) are inherited as fixed inputs.
...
Sealed-folder structure per [docs/component-guide.md §5](../component-guide.md#5-anatomy-of-a-component-folder), under `src/registry/components/data/flow-canvas/`:

```
flow-canvas/
├── flow-canvas.tsx        # Root export — <FlowCanvas {...props} />
├── parts/
...
- **`flow-canvas.tsx`** — composes Context provider + `<Canvas>`. The only file consumers reach directly. `'use client'`.
...
**Total file count:** 4 root + 14 parts + 7 hooks + 4 registries + 5 lib + 7 fixed (`types`, `dummy-data`, `demo`, `usage`, `meta`, `index`, plus `flow-canvas.tsx`) ≈ **35 files**. Comparable to `kanban-board` (~25 files) and `data-table` (~30 files); justified by the three-registry plus drop-pipeline plus sub-object-extract surface area.
...
- **Renderer registry** (renderer-registry pattern from `kanban-board` and `workspace`) — proven; default for v0.1.
...
|---|---|---|---|
| **M1** | Skeleton — `<FlowCanvas>` renders, custom-json fallback, save via `exportRef`, dark-aware gradient background. Empty-canvas demo. | xyflow boots in our shell; CSS imports right; gradient theme works light + dark; export round-trips. | `flow-canvas.tsx`, `parts/canvas.tsx`, `parts/background.tsx`, `parts/controls.tsx`, `parts/custom-json-node.tsx`, `parts/node-adapter.tsx` (minimal), `hooks/use-canvas-data.ts`, `hooks/use-export.ts`, `registries/*` (skeleton), `types.ts`, `dummy-data.ts`, `demo.tsx` (1 demo), `meta.ts`, `index.ts`. |
...
| **M9** | Demos + usage docs — five demos mirroring §5 of description; full `usage.tsx` + procomp-guide.md. | Docs site shows all five demos; `usage.tsx` covers register-renderer / typed-port / theme / perf recipes. | `demo.tsx`, `usage.tsx`, `flow-canvas-procomp-guide.md`. |
...
Before scaffolding (`pnpm new:component data/flow-canvas`):
...
- [ ] **User explicitly says "plan approved" (or equivalent)** — this unlocks scaffolding (`pnpm new:component data/flow-canvas`) and code.
